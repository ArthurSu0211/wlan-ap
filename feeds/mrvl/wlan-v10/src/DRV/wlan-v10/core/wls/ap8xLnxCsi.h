...
int hal_processCsiWls(unsigned int *fftBuffer,
		      hal_wls_packet_params_t * packetparams,
		      hal_wls_aoa_processing_input_params_t * inputVals,
		      int *resArray);
void programRegister(unsigned int *bufferMemoryPtr, int bufferNum);
void hal_rx_csi_processing_buf(struct net_device *netdev,
			       evt_prdcsi_t * prdcsi_data);
void hal_rx_csi_aoa_processing(struct net_device *netdev,
			       evt_prdcsi_t * prdcsi_data,
			       int *CSI_Processing_Results_Array);

SINT32 hal_wls_getCalVal(UINT32 SignalBW, UINT32 DeviceBW, int chanOffset5G,
			 UINT8 isSc5);
