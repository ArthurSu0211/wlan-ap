		...
		if (channel) {
			...
			ch_load = wl_ch_load(vmacSta_p,
					     ktime_us_delta(ktime_get_real(),
							    vmacSta_p->
							    acs_scantime),
					     vmacSta_p->acs_chload, 100);
			vmacSta_p->acs_db[vmacSta_p->ChanIdx].channel = channel;
#ifdef SOC_W906X
			vmacSta_p->acs_db[vmacSta_p->ChanIdx].ch_load = ch_load;
			vmacSta_p->autochannel[vmacSta_p->ChanIdx] =
				ch_load * 100 + bbu_cnt;
#else /* SOC_W906X */
			vmacSta_p->autochannel[vmacSta_p->ChanIdx] =
				t_cnt * 100 + bbu_cnt;
...
//        printk( "finished scanning channel %d, traffic cnt %d, bbu cnt %d\n", channel, (int)t_cnt, (int)bbu_cnt);
		...
		extern void macMgmtMlme_IncrBonlyStnCnt(vmacApInfo_t *
							vmacSta_p,
							UINT8 option) {
			MIB_802DOT11 *mib;
			if (vmacSta_p->master)
...
//FIX NOW ????  def FIX_LATER /* Remove for Sfly, need to inform firmware of protection mode change. */
		...
		extern void macMgmtMlme_DecrBonlyStnCnt(vmacApInfo_t *
							vmacSta_p,
							UINT8 option) {
			MIB_802DOT11 *mib;
			if (vmacSta_p->master)
...
#ifdef CONFIG_IEEE80211W
		int parsing_rsn_ie(UINT8 * pData,
				   IEEEtypes_RSN_IE_WPA2_t * pRSNE,
...
#endif
		{
			...
			*mfpc = *mfpr = 0;
			...
			memcpy(pRSNE->GrpKeyCipher, ptr,
			       sizeof(pRSNE->GrpKeyCipher));
			...
			left -= sizeof(pRSNE->PwsKeyCipherList) *
				pRSNE->PwsKeyCnt[0];
			...
			memcpy(pRSNE->AuthKeyList, ptr,
			       sizeof(pRSNE->AuthKeyList) *
			       pRSNE->AuthKeyCnt[0]);
			ptr += sizeof(pRSNE->AuthKeyList) *
				pRSNE->AuthKeyCnt[0];
			left -= sizeof(pRSNE->AuthKeyList) *
				pRSNE->AuthKeyCnt[0];
			...
			*mfpr = (pRSNE->RsnCap[0] & 0x40) ? 1 : 0;
			...
			if (left <
			    pRSNE->PMKIDCnt[0] * sizeof(pRSNE->PMKIDList)) {
				return -1;
			}
			...
			if (pRSNE->PMKIDCnt[0] > 0) {
				memcpy(pRSNE->PMKIDList, ptr,
				       sizeof(pRSNE->PMKIDList));
	...
	/** RSN CAP **/
		...
		void SAQueryMgmt_TimeoutHdlr(UINT8 * data) {
			...
			if (StaInfo_p->sa_query_count++ > 5) {
				...
				macMgmtMlme_SAQuery(vmacSta_p, &StaInfo_p->Addr,
						    NULL, 0);
		...
		static BOOLEAN macMgmtMlme_WPA2PMF_Verify(vmacApInfo_t *
							  vmacSta_p,
							  struct sk_buff
							  *txSkb_p,
							  extStaDb_StaInfo_t *
			...
			*reconfig_rsn_ie = FALSE;
			...
			ret = parsing_rsn_ie((UINT8 *) RsnIEWPA2_p, &RsnIEWPA2,
					     &mfpc, &mfpr);
...
#define OUT_OF_BOUNDARDY_MESSAGE(x, y) printk("%s[line %d] out of boundary (%d %d)\n", __FUNCTION__, __LINE__, x, y)
		...
		static BOOLEAN macMgmtMlme_AssocReAssocIESanityCheck(UINT8 *
								     buf,
								     SINT32 len)
		{
			SINT32 VarLen = 0;
		...
		extern int wlFwSetOfdma_Mode(struct net_device *netdev,
					     UINT8 option, UINT8 ru_mode,
					     UINT32 max_delay, U32 max_sta);
		...
		void macMgmtMlme_AssocReAssocReqHandler(vmacApInfo_t *
							vmacSta_p,
							macmgmtQ_MgmtMsg3_t *
							MgmtMsg_p,
							UINT32 msgSize,
...
#ifdef IEEE80211H
			IEEEtypes_PowerCapabilityElement_t *PowerCapability_p =
				NULL;
			IEEEtypes_SupportedChannelElement_t *SupportedChanne_p =
				NULL;
#endif /* IEEE80211H */
			...
			UINT32 vhtcap = 0;
			UINT8 *endofmessagelocation =
				(UINT8 *) MgmtMsg_p + msgSize;
#ifdef SOC_W906X
			...
			if (((vmacSta_p->Mib802dot11->WPA2AuthSuites->
			      AuthSuites[3]) == 0x12)) {
//              extra_size += (sizeof(StaInfo_p->AP_DHIEBuf) + sizeof(StaInfo_p->EXT_RsnIE));
			...
			WLDBG_INFO(DBG_LEVEL_7, "%sassoc rxed",
				   flag ? "re" : "");
			...
			if (vmacSta_p->probeRspBody.basic_ies) {
				...
				vmacSta_p->macCapInfo.ShortPreamble =
					((IEEEtypes_ProbeRsp_t *) vmacSta_p->
					 probeRspBody.basic_ies)->CapInfo.
					ShortPreamble;
			}
			MgmtRsp_p->Body.AssocRsp.CapInfo =
				vmacSta_p->macCapInfo;
			MgmtRsp_p->Body.AssocRsp.SuppRates.ElementId =
				SUPPORTED_RATES;
			MgmtRsp_p->Body.AssocRsp.SuppRates.Len =
				SuppRateSet_p->Len;
			memcpy((char *)&MgmtRsp_p->Body.AssocRsp.SuppRates.
			       Rates, (char *)&SuppRateSet_p->Rates,
			       SuppRateSet_p->Len);
			...
			if (ExtSuppRateSet_p->Len > 0) {
				TxExtRates_p =
					(IEEEtypes_ExtSuppRatesElement_t
					 *) ((UINT8 *) & MgmtRsp_p->Body.
					     AssocRsp.SuppRates +
					     sizeof(IEEEtypes_InfoElementHdr_t)
					     + SuppRateSet_p->Len);
				TxExtRates_p->ElementId =
					ExtSuppRateSet_p->ElementId;
			...
			if ((StaInfo_p =
			    ...
			    NULL || !(StaInfo_p->State == AUTHENTICATED ||
			...
			StaInfo_p->ListenInterval =
				ENDIAN_SWAP16(MgmtMsg_p->Body.AssocRqst.
					      ListenInterval);
#endif
#ifdef MRVL_WSC			//MRVL_WSC_IE
			memset(&StaInfo_p->WscIEBuf, 0,
...
//      cleanupAmpduTx(vmacSta_p,(UINT8 *)&StaInfo_p->Addr);
			...
			if (flag) {
				...
				VariableElements_p =
					(UINT8 *) & MgmtMsg_p->Body.AssocRqst.
					SsId + sizeof(IEEEtypes_MacAddr_t);
			...
			IELen = VarLen;
			capInfo_p =
				(UINT8 *) & MgmtMsg_p->Body.AssocRqst.CapInfo;
			listenInterval_p =
				(UINT8 *) & MgmtMsg_p->Body.AssocRqst.
				ListenInterval;
#endif
			...
			while (VarLen > 0) {
				...
				case PROPRIETARY_IE:
#ifdef APCFGUR
					if (isUrClientIE(VariableElements_p) ==
					    TRUE) {
						StaInfo_p->UR = 1;
					}
#endif
					if (isMcIdIE(VariableElements_p) ==
					    TRUE) {
						PeerInfo.MrvlSta = 1;
						StaInfo_p->IsStaMSTA = 1;
					}
					if ((isM_RptrIdIE(VariableElements_p) ==
					     TRUE)) {
					...
					if (!memcmp
					    (&
					     ((WSC_ProbeRespIE_t *)
					      VariableElements_p)->OUI, WSC_OUI,
					     4)) {
					...
					if (!memcmp
						...
						/** 3x3 configuration **/
								...
								if (MCSCapEnable) {	/*Enabled through mcscap debug cmd */
									if ((MCSCap < 8) && (tmp_p->SupportedMCSset[0] != 0)) {
										shiftno = (MCSCap % 8) + 1;
										mcscapmask
											=
											0xff
								...
								PeerInfo.
									...
									=
									ENDIAN_SWAP32
									((tmp_p->SupportedMCSset[0] | (tmp_p->SupportedMCSset[1] << 8) | (tmp_p->SupportedMCSset[2] << 16) | (tmp_p->SupportedMCSset[3] << 24)));
							} else if (!
								   ((*
								     (mib->
								      mib_rxAntenna)
						...
						/** 1x1 configuration **/
								...
								PeerInfo.
									...
									=
									ENDIAN_SWAP32
									((tmp_p->SupportedMCSset[0] | (tmp_p->SupportedMCSset[1] << 8)));
							}
			...
			/** FOR I_COMP ONLY, not use anymore **/
					...
					if (!memcmp
					    (&
					     ((WSM_QoS_Cap_Elem_t *)
					      VariableElements_p)->OUI, WiFiOUI,
					     3)) {
				...
				case HT:
					...
					/** 1x1 configuration **/
						...
						if (PeerInfo.HTRateBitMap ==
						    0x00)
							PeerInfo.HTRateBitMap =
								0xFF;

						memcpy(&(StaInfo_p->HtElem),
						       tmp_p,
						       sizeof
						       (IEEEtypes_HT_Element_t));
						if ((*
				...
				case VHT_CAP:
					if (*
						...
						StaInfo_p->vhtCap.
							...
							ENDIAN_SWAP32(tmp_p->
								      SupportedTxMcsSet);
						memcpy((UINT8 *) & PeerInfo.
						       vht_cap,
						       (UINT8 *) & tmp_p->cap,
						       sizeof
						       (IEEEtypes_VHT_Cap_Info_t));
						if (tmp_p->SupportedRxMcsSet ==
						    0xffff)
						...
						case HE_CAPABILITIES_IE:
							if (*
								...
								if (ie_length <=
								    ...
								    (PeerInfo.
								     he_cap))
									memcpy((void *)&PeerInfo.he_cap, (void *)tmp_p, ie_length);
						...
						case HE_OPERATION_IE:
							if (*
								...
								if (ie_length <=
								    ...
								    (PeerInfo.
								     he_op))
									memcpy((void *)&PeerInfo.he_op, (void *)tmp_p, ie_length);
							}
							he_op_present = 1;
							break;
						default:
			...
			if ((mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS) &&
			    !(mib->
			      multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS)) {
			...
			if (IE199Present) {
				...
				if ((PeerInfo.vht_MaxRxMcs & 0xc000) != 0xc000)
					vht_peer_RxNss = 8;
				else if ((PeerInfo.vht_MaxRxMcs & 0x3000) !=
					 0x3000)
					vht_peer_RxNss = 7;
				else if ((PeerInfo.vht_MaxRxMcs & 0xc00) !=
					 0xc00)
					vht_peer_RxNss = 6;
				else if ((PeerInfo.vht_MaxRxMcs & 0x300) !=
					 0x300)
					vht_peer_RxNss = 5;
				...
				if (IE192Present) {
					...
					case 1:
						...
						if (vht_center_freq1_IE192 == 0) {
							...
							UINT8 diff;
							if (vht_center_freq1_IE192 > vht_center_freq0_IE192) {
			...
			if (!StaInfo_p->IsStaMSTA &&
			    mib_PrivacyTable_p->PrivInvoked) {
			...
			if (!allow_ht_tkip &&
			    ((HTpresent) &&
			     ((*(vmacSta_p->Mib802dot11->mib_wpaWpa2Mode) &
			       0x0F) == 1))) {
				MgmtRsp_p->Body.AssocRsp.StatusCode =
			...
			if (!gRatePresent &&
			     ...
			     *(mib->mib_ApMode) == AP_MODE_5GHZ_NandACand11AX ||
			     *(mib->mib_ApMode) == AP_MODE_5GHZ_11AX_ONLY))
#else
			...
			if (mib_PrivacyTable_p->RSNEnabled) {
				...
				memcpy(pStaMsg->staMACAddr,
				       MgmtMsg_p->Hdr.SrcAddr, 6);
				...
				if (mib_RSNConfigWPA2_p->WPA2OnlyEnabled) {
					if (RsnIEWPA2_p != NULL) {
						...
						ret = parsing_rsn_ie((UINT8 *)
								     ...
								     &RsnIEWPA2);
#endif
						if (RsnIEWPA2.AuthKeyList[3] ==
						    0x08 ||
						    RsnIEWPA2.AuthKeyList1[3] ==
						    0x08) {
							if (mfpc == 0 &&
							    mfpr == 0) {
								MgmtRsp_p->Body.
				...
				if (MDIE_p != NULL) {
					if ((MDIE_p->Len + 2) >
					    MAX_SIZE_MDIE_BUF)
						OUT_OF_BOUNDARDY_MESSAGE((MDIE_p->Len + 2), MAX_SIZE_MDIE_BUF);
					else
						memcpy(StaInfo_p->
						       keyMgmtStateInfo.
						       mdie_buf, MDIE_p,
			...
			if (vmacSta_p->macCapInfo.Rrm) {
				struct IEEEtypes_RM_Enable_Capabilities_Element_t RRM_ie;
				UINT8 rrm_ie_len;
	...
	/********************************************/
			MgmtRsp_p->Body.AssocRsp.AId =
				ENDIAN_SWAP16(Aid | AID_PREFIX);
			MgmtRsp_p->Body.AssocRsp.StatusCode =
...
#ifdef SOC_W906X
			free_any_pending_ampdu_pck(vmacSta_p->dev,
						   StaInfo_p->StnId);
			for (i = 0; i < MAX_UP; i++)
	...
	/** Add Additional HT **/
			...
			if (*(vmacSta_p->ShadowMib802dot11->mib_HT40MIntoler)) {
				extern void Coexist_RearmTimer(vmacApInfo_t *
							       vmacSta_p);
				extern void Check20_40_Channel_switch(int
								      option,
								      int
								      *mode);
...
/** we are at 20MHz mode **/
		...
		/** Add Additional HT IE61 **/
			...
			if (*(vmacSta_p->Mib802dot11->mib_ApMode) &
				...
				if (he_cap_present) {
					...
					StaInfo_p->operating_mode.chbw =
						PeerInfo.vht_RxChannelWidth;
					StaInfo_p->operating_mode.ulmu_disable =
						0;
				}
			}
#endif /* SOC_W906X */
			...
			memcpy(&StaInfo_p->PeerInfo, &PeerInfo,
			       sizeof(PeerInfo_t));
#endif //CB_SUPPORT
			...
			if (mib_PrivacyTable_p->WAPIEnabled)
				macMgmtMlme_WAPI_event(vmacSta_p->dev,
						       IWEVASSOCREQIE, 0x00F1,
						       &MgmtMsg_p->Hdr.SrcAddr,
						       &MgmtMsg_p->Hdr.DestAddr,
						       (char *)WAPI_IE_p);
#endif
			...
			WLSYSLOG(vmacSta_p->dev, WLSYSLOG_CLASS_ALL,
				 ...
				 MgmtMsg_p->Hdr.SrcAddr[0],
				 MgmtMsg_p->Hdr.SrcAddr[1],
				 MgmtMsg_p->Hdr.SrcAddr[2],
				 MgmtMsg_p->Hdr.SrcAddr[3],
				 MgmtMsg_p->Hdr.SrcAddr[4],
				 MgmtMsg_p->Hdr.SrcAddr[5], StaInfo_p->StnId);
			...
			if (((vmacSta_p->Mib802dot11->WPA2AuthSuites->
				...
				if (StaInfo_p->EXT_RsnIE[0] == RSN_IEWPA2 &&
					...
					IE_p = skb_put(txSkb_p,
						       StaInfo_p->EXT_RsnIE[1] +
						       2);
					if (IE_p != NULL)
						memcpy(IE_p,
						       &StaInfo_p->EXT_RsnIE[0],
						       StaInfo_p->EXT_RsnIE[1] +
						       2);
				}
			}
			...
			if (wfa_11ax_pf && vmacSta_p->master) {
				if (vmacSta_p->master->dl_ofdma_para.
				    postpone_time) {
					wlFwSetOfdma_Mode(vmacSta_p->master->
							  dev, 0,
							  vmacSta_p->master->
							  ...
							  dl_ofdma_para.
							  max_sta);
					vmacSta_p->master->dl_ofdma_para.
						started = 0;
				}
			}
...
#ifdef MRVL_80211R
			if ((flag) &&
			    (StaInfo_p->keyMgmtStateInfo.pending_assoc))
...
#if defined(SOC_W906X) && defined(CFG80211)
				mwl_send_vendor_assoc_notification(vmacSta_p->
								   dev,
								   (uint8_t *) &
								   MgmtMsg_p->
								   Hdr.SrcAddr,
			...
			if (!gRatePresent) {
				if (ChangeAssocParam == TRUE)
					macMgmtMlme_IncrBonlyStnCnt(vmacSta_p,
								    0);
			...
			if (StaInfo_p->StnId < sta_num) {
				...
				if (wlpptr->wlpd_p->
				    scheHistogram[StaInfo_p->StnId] != NULL)
					memset(wlpptr->wlpd_p->
					       scheHistogram[StaInfo_p->StnId],
					       0, sizeof(WLAN_SCHEDULER_HIST));
				else {
					if ((sch_histo_p =
					     (WLAN_SCHEDULER_HIST *)
					     wl_kmalloc_autogfp(sizeof
...
#else
			if ((StaInfo_p->StnId > 0) &&
			    (StaInfo_p->StnId <= sta_num)) {
				if ((txrate_histo_p =
				     (WLAN_TX_RATE_HIST *)
				     wl_kmalloc_autogfp(sizeof
							(WLAN_TX_RATE_HIST))) !=
			...
			if (ofdma_autogrp && StaInfo_p) {
				if ((is_he_capable_sta(StaInfo_p)) ||	/* add HE STA into list */
					...
					MUAddStaToMUStaList(vmacSta_p,
							    StaInfo_p);

				if (ofdma_autogrp == 2) {
					int num =
						auto_group_ofdma_mu(vmacSta_p);
...
*****************************************************************************/
...
#endif
		{
			...
			MgmtMsg_p =
				(macmgmtQ_MgmtMsg2_t *) ((UINT8 *) txSkb_p->
							 data);
			...
			if ((StaInfo_p =
			     extStaDb_GetStaInfo(vmacSta_p, Addr,
						 STADB_UPDATE_AGINGTIME)) !=
			    NULL) {
				WLSNDEVT(vmacSta_p->dev, IWEVEXPIRED, Addr,
					 NULL);
				...
				if ((mib->
				    ...
				    (mib->
				     ...
				     MAP_ATTRIBUTE_BACKHAUL_BSS)) {
#if defined(SOC_W906X) && defined(CFG80211)
					sinfo = wl_kzalloc(sizeof
							   (struct
							    station_info),
							   GFP_ATOMIC);
					if (sinfo)
						fillStaInfo(vmacSta_p->dev,
							    sinfo, StaInfo_p);
#endif /* SOC_W906X */
					...
					disassoc_event_p->tx_packets =
						(sinfo) ? sinfo->
						tx_packets : StaInfo_p->
						tx_packets;
					disassoc_event_p->rx_packets =
						(sinfo) ? sinfo->
						rx_packets : StaInfo_p->
						rx_packets;
					disassoc_event_p->tx_bytes =
						(sinfo) ? sinfo->
						tx_bytes : StaInfo_p->tx_bytes;
					disassoc_event_p->rx_bytes =
						(sinfo) ? sinfo->
						rx_bytes : StaInfo_p->rx_bytes;
		...
		void macMgmtMlme_DeauthenticateMsg(vmacApInfo_t * vmacSta_p,
			...
			if (*(mib->QoSOptImpl) &&
			    extStaDb_GetQoSOptn(vmacSta_p,
						(IEEEtypes_MacAddr_t *)
						MgmtMsg_p->Hdr.SrcAddr)) {
				ClearQoSDB((IEEEtypes_MacAddr_t *) MgmtMsg_p->
					   Hdr.SrcAddr);
			}
#endif
			...
			if (StaInfo_p->State == ASSOCIATED) {
				...
				UpdateAssocStnData(StaInfo_p->Aid,
						   StaInfo_p->ApMode);
				...
				if (StaInfo_p->ClientMode == BONLY_MODE)
					macMgmtMlme_DecrBonlyStnCnt(vmacSta_p,
								    0);

				if (!StaInfo_p->CapInfo.ShortPreamble)
					macMgmtMlme_DecrBarkerPreambleStnCnt
						(vmacSta_p);
			...
			if ((mib->multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS)
				...
				memcpy(disassoc_event_p->bssid,
				       vmacSta_p->macBssId,
				       sizeof(IEEEtypes_MacAddr_t));
				memcpy(disassoc_event_p->sta_mac,
				       StaInfo_p->Addr,
				       sizeof(IEEEtypes_MacAddr_t));
			...
			wlFwSetNewStn(vmacSta_p->dev,
				      ...
				      StaInfoDbActionRemoveEntry, NULL, 0, 0,
				      0);
			/* Send event to user space */
			WLSNDEVT(vmacSta_p->dev, IWEVEXPIRED,
				 &MgmtMsg_p->Hdr.SrcAddr, NULL);
#ifdef CFG80211
#ifdef CFG80211_COMPATIABLE
			mwl_cfg80211_disassoc_event(vmacSta_p->dev,
...
*****************************************************************************/
		void macMgmtMlme_DisassociateCmd(vmacApInfo_t * vmacSta_p,
			...
			if (*(mib->QoSOptImpl) &&
			    extStaDb_GetQoSOptn(vmacSta_p,
						(IEEEtypes_MacAddr_t *)
						DisassocCmd_p->PeerStaAddr)) {
				ClearQoSDB((IEEEtypes_MacAddr_t *)
					   DisassocCmd_p->PeerStaAddr);
			}
#endif
#ifdef AP_MAC_LINUX
...
#ifdef MULTI_AP_SUPPORT
		void macMgmtMlme_SendDisassociateMsg4MAP(vmacApInfo_t *
							 vmacSta_p,
							 IEEEtypes_MacAddr_t *
							 Addr, UINT16 StnId,
							 UINT16 Reason) {
...
#ifdef SOC_W906X
				skb_put(txSkb_p,
					sizeof(MgmtMsg_p->Body.DisAssoc.
					       ReasonCode));
#endif
				MgmtMsg_p->Body.DisAssoc.ReasonCode = Reason;
				if (txMgmtMsg(vmacSta_p->dev, txSkb_p) !=
		...
		void pending_assoc_timeout_handler(void *ctx) {
			...
			if (sta_info->keyMgmtStateInfo.pending_assoc) {
				...
				if (sta_info->keyMgmtStateInfo.assoc) {
					wl_free_skb(sta_info->keyMgmtStateInfo.
						    assoc);
		...
		void pending_assoc_timer_add(extStaDb_StaInfo_t * sta_info,
			...
			TimerFireIn(&sta_info->mgtAssoc.timer, 1,
				    &pending_assoc_timeout_handler,
				    (UINT8 *) sta_info, ticks);
		}
		...
		int macMgmtMlme_SendAuthenticateMsg(vmacApInfo_t * vmacAP_p,
			...
			memcpy((UINT8 *) & MgmtMsg_p->Body.Auth.ChallengeText,
			       optie, optie_len);
...
*
*****************************************************************************/
		extern void macMgmtMlme_DisassociateMsg(vmacApInfo_t *
							vmacSta_p,
							macmgmtQ_MgmtMsg3_t *
							MgmtMsg_p,
							UINT32 msgSize) {
...
#ifndef SOC_W906X
			wlFwSetNewStn(vmacSta_p->dev,
				      (u_int8_t *) & StaInfo_p->Addr,
				      StaInfo_p->Aid, StaInfo_p->StnId, 2, NULL,
				      0, 0, 0);
#endif
			...
			if (wfa_11ax_pf) {
				...
				if ((vmacSta_master_p->dl_ofdma_para.max_sta >
				     0) &&
					...
					if (!vmacSta_master_p->dl_ofdma_para.
					    sta_cnt) {
						vmacSta_master_p->dl_ofdma_para.
							option = 0;
						vmacSta_master_p->dl_ofdma_para.
...
#ifdef CCK_DESENSE
			{
				...
				cck_desense_ctrl(wlpptr->master,
						 CCK_DES_DISASSOC);
			}
#endif /* CCK_DESENSE */
...
#ifdef CONFIG_IEEE80211W
		extern void
			*probeRequest_ParseAttribWithinFrame(macmgmtQ_MgmtMsg3_t
							     * mgtFrame_p,
							     UINT8 * data_p,
							     UINT8 attrib) {
		...
		void macMgmtMlme_UpdateProbeRspInfo(vmacApInfo_t * vmacSta_p,
			...
			probersp_p =
				(IEEEtypes_ProbeRsp_t *) vmacSta_p->
				probeRspBody.basic_ies;

			memcpy((void *)&probersp_p->BcnInterval,
			       (void *)&bcn_interval, sizeof(bcn_interval));
			memcpy((void *)&probersp_p->CapInfo, (void *)&capinfo,
			       sizeof(capinfo));
		}

		void macMgmtMlme_UpdateProbeRspExtraIes(vmacApInfo_t *
							vmacSta_p,
							UINT8 * probeRspIe,
							UINT16 length) {
			if (!length)
		...
		void macMgmtMlme_UpdateProbeRspBasicIes(vmacApInfo_t *
							vmacSta_p, UINT8 * iep,
							UINT16 length) {
			IEEEtypes_ProbeRsp_t *probersp_p;
			...
			memcpy((void *)((UINT8 *) & probersp_p->SsId +
					vmacSta_p->probeRspBody.basic_len),
			       (void *)iep, length);
			vmacSta_p->probeRspBody.basic_len += length;
		}
		...
		void macMgmtMlme_UpdateProbeRspCsaIes(vmacApInfo_t * vmacSta_p,
			...
			if (length < MAX_BEACON_SIZE) {
				memcpy(vmacSta_p->probeRspBody.csa_ies, iep,
				       length);
...
#define IW_CUSTOM_MAX2 512
...
#endif
			{
				...
				u32 idvBSSmap =
					get_individual_bss(wlpptr->wlpd_p);
				u32 ToIfmap = (1 << vmacSta_p->VMacEntry.macId);
				u32 nonTxBSSmap =
					get_mbssid_profile(wlpptr->wlpd_p, 2);
			...
			if (isSsIdMatch
				...
				if (*(mib->mib_broadcastssid) == TRUE) {
					if ((MgmtMsg_p->Body.ProbeRqst.SsId.
					     Len != 0) &&
					    (*(mib->mib_mbssid) == 0)) {
				...
				if (vmacSta_p->WPSOn) {
					...
					macmgmtQ_MgmtMsg3_t *Msg_p =
						(macmgmtQ_MgmtMsg3_t *) (buf +
									 strlen
									 (tag));
					mwl_send_vendor_wps_req_event
						(vmacSta_p->dev,
						 ((UINT8 *) Msg_p) + 2,
						 Msg_p->Hdr.FrmBodyLen, 0);
#endif /* CFG80211_COMPATIABLE */
#endif /* CFG80211 */
				}
			} else
			...
			ssid_p = (IEEEtypes_SsIdElement_t *) (vmacSta_p->
							      probeRspBody.
							      basic_ies +
							      len_before_ssid);
...
//if (vmacSta_p->VMacEntry.macId == wlpptr->wlpd_p->mbssSet[0].primbss) {
			if (*(mib->mib_broadcastssid) &&
				...
				memcpy((void *)&MgmtRsp_p->Body.ProbeRsp +
				       len_before_ssid,
				       (void *)vmacSta_p->probeRspBody.
				       basic_ies + len_before_ssid,
				       out_ssid_len);
...
//}
			...
			memcpy(&MgmtRsp_p->Body.ProbeRsp.TimeStamp[0],
			       &bsstsf64, sizeof(bsstsf64));
					...
					if (wlpptr->vdev[i]) {
						...
						wlpptr1 =
							NETDEV_PRIV_P(struct
								      wlprivate,
								      dev);
						if (wlpptr1->vmacSta_p->
						    VMacEntry.modeOfService ==
						    VMAC_MODE_AP)
							if (isMacAccessList
							    (wlpptr1->vmacSta_p,
							     &(MgmtMsg_p->Hdr.
...
/* send wapi event to user space: AP/STA MAC, type, WIE (WAPI IE) etc. */
		void macMgmtMlme_WAPI_event(struct net_device *dev,
					    int event_type, u16 auth_type,
					    IEEEtypes_MacAddr_t * sta_addr,
					    IEEEtypes_MacAddr_t * ap_addr,
					    char *info) {
			...
			*(UINT32 *) & (asso_mt_info.gsn[12]) =
			...
			if (info) {
				IEEEtypes_WAPI_IE_t *p =
					(IEEEtypes_WAPI_IE_t *) info;
				extra_len = p->Len + 2;
...
#define MAX_TWT_FRAME_DEBUG_COUNTERS  12
		u16 debug_twt_frame_errors[MAX_TWT_FRAME_DEBUG_COUNTERS] =
			{ 0 };
		...
		void ProcessTWTsetup(vmacApInfo_t * vmacSta_p,
			...
			struct net_device *netdev =
				(wlpptr->master) ? wlpptr->master : wlpptr->
				netDev;
			...
			ptwt = (IEEEtypes_TWT_Element_t *) & (MgmtMsg_p->Body.
							      Action.Data.
							      TwtElement);
			...
			if (ptwt->ctrl.NegoType & 0x2) {
				...
				if (pIdvl->ReqType.SetupCmd <
				    TWT_SETUP_CMD_GROUPING) {

					flowid = pIdvl->ReqType.
						FlowID_or_Bcast_Recom;

					if (flowid == 0) {
						u32 idx = 1;
						u8 map = pStaInfo->TwtAgree.
							ActFlowID;

						for (idx = 1;
						     idx <
						     ITWT_AGREEMENT_PER_STA;
						     idx++) {
							if (!(map & (1 << idx)))
								break;
						}
				...
				MgmtRsp_p =
					(macmgmtQ_MgmtMsg2_t *) txSkb_p->data;
				ptwtResp =
					(IEEEtypes_TWT_Element_t *) &
					(MgmtRsp_p->Body.Action.Data.
					 TwtElement);
...
//twt
		void ProcessTWTteardown(vmacApInfo_t * vmacSta_p,
			...
			if (!wlFwTwtParam
			    (netdev, WL_RESET, MgmtMsg_p->Hdr.SrcAddr, flowid,
			     NULL)) {
				...
				if (pStaInfo) {
					pStaInfo->TwtAgree.ActFlowID &=
						(~(1 << flowid));
					memset(&pStaInfo->TwtAgree.twtE[flowid],
					       0,
					       sizeof(IEEEtypes_TWT_Element_t));
				}
...
*/
		...
		UINT8 get_ampdu_rx_disable_flag(vmacApInfo_t * vmacSta_p,
			...
			BOOLEAN is_except =
				_is_mac_exist(MgmtMsg_p->Hdr.SrcAddr,
				...
				case ADDTS_REQ:
					{
						...
						if (pStaInfo) {
							...
							Status = ProcessDELTSRequest(vmacSta_p, (IEEEtypes_MacAddr_t *) MgmtMsg_p->Hdr.SrcAddr, pWfaAddTsReqFrm->TSpec.ts_info.tsid);
							//Process ADDTS Request. Return a ADDTS Response.
							Status = ProcessADDTSRequest_WFA(vmacSta_p, (IEEEtypes_WFA_ADDTS_Req_t *) & MgmtMsg_p->Body, &MgmtMsg_p->Hdr.SrcAddr, pStaInfo->Aid, &TspecDBindex, pStaInfo->ClientMode, &MediumTime);
					...
					if (pStaInfo) {
						...
						if (pAddTsReqFrm->TSpec.ts_info.
							...
							if (Status ==
							    IEEEtypes_STATUS_SUCCESS)
							{
								if (pAddTsReqFrm->TSpec.ts_info.direction != DOWNLINK) {
									pAddTsRspFrm->
										TSpec.
										med_time
										=
					...
					/** Reset the current queue **/
							...
							for (i = 0;
								...
								if (baRxInfo >
								    ...
								    [i] != NULL)
									wl_free_skb
										((struct sk_buff *)baRxInfo->pFrame[tid][i]);
								baRxInfo->
									pFrame
									[tid][i]
									= NULL;
					...
					/** Reset the current queue **/
							for (i = 0;
								...
								if (wlpptr->
								    ...
								    [i] != NULL)
									wl_free_skb
										((struct sk_buff *)wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].pFrame[tid][i]);
								wlpptr->wlpd_p->
									AmpduPckReorder
									[pStaInfo->
									 Aid].
...
/** either stream 0 or 1 is equal **/
		...
		int macMgmtMlme_RrmAct(vmacApInfo_t * vmacSta_p,
			...
			if (MgmtMsg_p->Body.Action.Category !=
			    AC_RADIO_MEASUREMENT || !*(mib->mib_rrm))
				return FALSE;
			...
			if (mib->mib_conf_capab->rrm) {
				...
				if ((msgbuf =
				     (macmgmtQ_MgmtMsg3_t *)
				     wl_kmalloc(MgmtMsg_p->Hdr.FrmBodyLen + 2,
						GFP_ATOMIC)) == NULL) {
			...
			case AF_RM_NEIGHBOR_REQUEST:
						...
						case SSID:
							{
								...
								UINT8 scannedSSID[IEEEtypes_SSID_SIZE];
								...
								memset(&scannedSSID[0], 0, IEEEtypes_SSID_SIZE);
								memcpy(&scannedSSID[0], &ssid->SsId[0], ssid->Len);
				...
				if (nb_num > 0) {
					NeighborRpt_Element =
						(struct
						 IEEEtypes_Neighbor_Report_Element_t
						 *)&MgmtResp_p->Body.Action.
						Data.NeighborReqportElemes[0];
			...
			case AF_RM_MEASUREMENT_REPORT:
				{
					...
					while (msg_len <
						...
						printk("msg_len (%d,%d) type:%d\n", msg_len, MgmtMsg_p->Hdr.FrmBodyLen, ie_p->Type);
		...
		int macMgmtMlme_WNMAct(vmacApInfo_t * vmacSta_p,
			...
			MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
			struct IEEEtypes_Neighbor_Report_Element_t
				__maybe_unused *nlistbyssid = NULL;
			...
			case AF_WNM_BTM_QUERY:
				{
					struct IEEEtypes_BSS_TM_Query_t
						*bss_tm_query_p = NULL;
					struct IEEEtypes_BSS_TM_Request_t
						BSS_TM_Req;
					...
					if (!nb_num) {
						nb_num = MSAN_get_neighbor_bySSID(&vmacSta_p->macSsId, &nlistbyssid);
						if (!nb_num) {
					...
					BSS_TM_Req.Abridged = 1;
					BSS_TM_Req.DisassocImm = 1;
					BSS_TM_Req.BSSTermiInc = 0;
					BSS_TM_Req.ESSDisassocImm = 0;
			...
			case AF_WNM_BTM_RESPONSE:
				{
					IEEEtypes_BSS_TM_Resp_t *bss_tm_resp_p =
						NULL;
					static const char *tag = "1905_BTM";
					unsigned char buf[IW_CUSTOM_MAX] =
						{ 0 };
					union iwreq_data wreq;
					UINT16 msg_len = 0;
					...
					msg_len =
						sizeof(struct
						       IEEEtypes_BSS_TM_Resp_t);
					printk("Receive BTM_RESPONSE, Status Code:%d, ", bss_tm_resp_p->StatusCode);
...
*
*****************************************************************************/
		extern void macMgmtMlme_ReassociateRsp(macmgmtQ_MgmtMsg_t *
						       MgmtMsg_p) {
		}
...
*****************************************************************************/
		extern void macMgmtMlme_StartCmd(vmacApInfo_t * vmacSta_p,
			...
			if (StartCmd_p->SsParamSet.CfParamSet.ElementId ==
			    CF_PARAM_SET &&
...
*****************************************************************************/
		...
		UINT32 GetHighestRateIndex(vmacApInfo_t * vmacSta_p,
			...
			*gRatePresent = FALSE;
			...
			for (i = 0; i < SuppRates->Len && SuppRates->Rates[i];
			     i++) {
				if ((Index =
				     hw_GetPhyRateIndex(vmacSta_p,
							(SuppRates->
		...
		void macMgmtMlme_StepUpRate(UINT16 Aid, UINT8 LastPktRate) {
			...
			if (LastPktRate < (AssocTable[Aid].SuppRates.Len - 1)) {
				AssocTable[Aid].RateToBeUsedForTx =
					LastPktRate + 1;
			}
#endif
			...
			if ((StaInfo_p =
				...
				if (rsnSta) {
					memcpy(StaInfo_p->keyMgmtStateInfo.
					       RsnIEBuf, rsnIE, rsnIE[1] + 2);
				}
				cleanupAmpduTx(vmacAP_p,
					       (UINT8 *) & StaInfo_p->Addr);
				if (((PeerInfo->
				      LegacyRateBitMap & 0xFFFFFFF0) == 0) ||
				    (*(mib->mib_ApMode) == AP_MODE_B_ONLY))
					StaInfo_p->ClientMode = BONLY_MODE;
		...
		void macMgmtMlme_del_sta_entry(vmacApInfo_t * vmacSta_p,
			...
			if (*(mib->QoSOptImpl) &&
			    ...
			    wlFwSetAPUpdateTim(vmacSta_p->dev, StaInfo_p->Aid,
					       RESETBIT);
			    FreeAid(vmacSta_p, StaInfo_p->Aid);
			    ResetAid(vmacSta_p, StaInfo_p->StnId,
				     StaInfo_p->Aid); StaInfo_p->Aid = 0;
			    if (StaInfo_p->State == ASSOCIATED) {
#ifdef ENABLE_RATE_ADAPTATION_BASEBAND_REMOVE
			    UpdateAssocStnData(StaInfo_p->Aid,
					       StaInfo_p->ApMode);
			    ...
			    StaInfo_p->State = UNAUTHENTICATED;
			    FreeStnId(vmacSta_p, StaInfo_p->StnId);
			    extStaDb_DelSta(vmacSta_p, staMac,
					    STADB_DONT_UPDATE_AGINGTIME);
			    wlFwSetNewStn(vmacSta_p->dev, (u_int8_t *) staMac,
					  StaInfo_p->Aid, StaInfo_p->StnId,
					  StaInfoDbActionRemoveEntry, NULL, 0,
					  0, 0); return;}
...
///////////////////////////////////////////////////////////////////////////////
			    void macMgmtMlme_UpdatePwrMode(vmacApInfo_t *
							   vmacSta_p,
							   struct
							   ieee80211_frame
							   *Hdr_p,
...
*
**************************************************************************/
			    extern void syncSrv_ScanCmd(vmacApInfo_t *
							vmacSta_p,
							IEEEtypes_ScanCmd_t *
							ScanCmd_p) {

			    /* provide list of channel to ScanParams */
			    memcpy((UINT8 *) & vmacSta_p->ScanParams,
				   (UINT8 *) ScanCmd_p,
...
//#ifdef COEXIST_20_40_SUPPORT
			    ...
			    TimerFireIn(&vmacSta_p->scaningTimer, 1,
					&syncSrv_ScanActTimeOut,
					(unsigned char *)vmacSta_p, SCAN_TIME);}
			    void Disable_ScanTimerProcess(vmacApInfo_t *
							  vmacSta_p) {
...
**************************************************************************/
			    ...
			    extern void *syncSrv_ParseAttrib(macmgmtQ_MgmtMsg_t
							     * mgtFrame_p,
							     UINT8 attrib,
							     UINT16 len) {
			    void *data_p; UINT32 lenPacket; UINT32 lenOffset;
...
#ifdef CONFIG_IEEE80211W
#ifdef SOC_W906X
			    extern int wlxmit(struct net_device *netdev,
					      struct sk_buff *skb, UINT8 type,
					      extStaDb_StaInfo_t * pStaInfo,
			    ...
			    MgmtMsg_p->Body.SAQuery_Req.Category =
			    WLAN_ACTION_SA_QUERY;
			    MgmtMsg_p->Body.SAQuery_Req.Action =
			    WLAN_SA_QUERY_REQUEST;
			    memcpy(MgmtMsg_p->Body.SAQuery_Req.trans_id,
				...
				(vmacSta_p->dev, txSkb_p,
				 IEEE_TYPE_MANAGEMENT | (key << 7) | (1 << 6),
				 NULL, 0, FALSE, 0))
#else
			    ...
			    void macMgmtMlme_SAQuery_Rsp(vmacApInfo_t *
							 vmacSta_p,
							 IEEEtypes_MacAddr_t *
							 Addr,
							 IEEEtypes_MacAddr_t *
			    ...
			    macmgmtQ_MgmtMsg2_t * MgmtMsg_p;
			    BOOLEAN sendResult = FALSE;
			    struct sk_buff * txSkb_p; WLDBG_ENTER(DBG_LEVEL_7);
			    if ((txSkb_p =
				 mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, Addr,
						    SrcAddr,
						    sizeof
			    ...
			    return;
			    MgmtMsg_p = (macmgmtQ_MgmtMsg2_t *) txSkb_p->data;
			    MgmtMsg_p->Body.SAQuery_Rsp.Category =
			    WLAN_ACTION_SA_QUERY;
			    MgmtMsg_p->Body.SAQuery_Rsp.Action =
			    WLAN_SA_QUERY_RESPONSE;
			    memcpy(MgmtMsg_p->Body.SAQuery_Rsp.trans_id,
				   trans_id, WLAN_SA_QUERY_TR_ID_LEN);
			    if (vmacSta_p->VMacEntry.modeOfService ==
				VMAC_MODE_CLNT_INFRA) {
			    UINT8 * pbssid = NULL;
...
#ifdef IEEE80211H
#if 0
			    static WL_STATUS macMgmtMlme_asso_ind(vmacApInfo_t *
								  vmacSta_p,
								  IEEEtypes_MacAddr_t
								  * addr) {
			    ...
			    if ((toSmeMsg =
				 (smeQ_MgmtMsg_t *)
				 wl_kmalloc_autogfp(sizeof(smeQ_MgmtMsg_t))) ==
				NULL) {
			    WLDBG_INFO(DBG_LEVEL_7,
			    ...
			    WLDBG_EXIT(DBG_LEVEL_7); return TRUE;}
#endif //0
/******************************************************************************
*
...
*****************************************************************************/
			    ...
			    void macMgmtMlme_MRequestReq(vmacApInfo_t *
							 vmacSta_p,
							 IEEEtypes_MRequestCmd_t
							 * MrequestCmd_p) {
			    MIB_802DOT11 * mib = vmacSta_p->Mib802dot11;
			    macmgmtQ_MgmtMsg2_t * MgmtMsg_p;
			    extStaDb_StaInfo_t * StaInfo_p;
			    ...
			    if ((*(mib->mib_ApMode) != AP_MODE_AandG) &&
				(*(mib->mib_ApMode) != AP_MODE_A_ONLY)) {
			    WLDBG_INFO(DBG_LEVEL_7, "macMgmtMlme_MRequestReq: no need to sent in current AP mode(%d)\n", *(mib->mib_ApMode)); return;	/* No need for those stations not in A mode */
			    }

			    if (!IS_BROADCAST
				((UINT8 *) MrequestCmd_p->PeerStaAddr)) {
			    if ((StaInfo_p =
				 extStaDb_GetStaInfo(vmacSta_p,
						     &MrequestCmd_p->
						     PeerStaAddr,
			    ...
			    MgmtMsg_p->Body.Action.DialogToken =
			    MrequestCmd_p->DiaglogToken; for (loop = 0;
							      loop <
							      MrequestCmd_p->
							      MeasureItems;
							      loop++) {
			    ...
			    MrequestCmd_p->MeasureReqSet[loop].MeasurementToken;
			    MgmtMsg_p->Body.Action.Data.
			    ...
			    MrequestCmd_p->MeasureReqSet[loop].Request;
			    len +=
			    sizeof(IEEEtypes_MeasurementRequestElement_t);}
...
//void macMgmtMlme_MReportReq(vmacApInfo_t *vmacSta_p,IEEEtypes_MReportCmd_t *MreportCmd_p)
			    void macMgmtMlme_MReportReq(struct net_device
							*staDev,
							UINT8 * macStaAddr,
							IEEEtypes_MReportCmd_t *
							MreportCmd_p) {
			    macmgmtQ_MgmtMsg2_t * MgmtMsg_p;
			    IEEEtypes_MacAddr_t SrcMacAddr;
#ifdef AP_MAC_LINUX
			    ...
			    UINT32 loop;
			    WLDBG_ENTER(DBG_LEVEL_7);
			    memcpy(SrcMacAddr, macStaAddr,
				   sizeof(IEEEtypes_MacAddr_t)); for (loop = 0;
								      loop <
								      MreportCmd_p->
								      ReportItems;
								      loop++) {
...
#ifdef WMON
case TYPE_REP_APS:
ielen[loop] = 3 + 11 + strlen(MreportCmd_p->MeasureRepSet[loop].Report.data.APS) + 1; break; case TYPE_REP_DFS:
ielen[loop] = 3 + 11 + strlen(MreportCmd_p->MeasureRepSet[loop].Report.data.DFS) + 1; break; case TYPE_REP_PSE:
ielen[loop] = 3 + 11 + strlen(MreportCmd_p->MeasureRepSet[loop].Report.data.PSE) + 1; break; case TYPE_REP_RSS:
ielen[loop] = 3 + 11 + 1; break; case TYPE_REP_NOI:
ielen[loop] = 3 + 11 + 8; break; case TYPE_REP_FCS:
case TYPE_REP_VRX:
			    ielen[loop] = 3 + 11 + 4; break;
#endif
default:
			    ielen[loop] = 0;}
			    totalLen += ielen[loop];}
			    ...
			    IE_p =
			    MgmtMsg_p->Body.Action.Data.MeasurementReport;
			    for (loop = 0; loop < MreportCmd_p->ReportItems;
				 loop++) {
			    UINT32 subLen = 0; IE_p->ElementId = MEASUREMENT_REP; IE_p->MeasurementToken = MreportCmd_p->MeasureRepSet[loop].MeasurementToken; IE_p->Mode = MreportCmd_p->MeasureRepSet[loop].Mode; IE_p->Type = MreportCmd_p->MeasureRepSet[loop].Type; if ((MreportCmd_p->MeasureRepSet[loop].Mode.Incapable) ||	/* problem to produce report result */
																																	     (MreportCmd_p->MeasureRepSet[loop].Mode.Late) || (MreportCmd_p->MeasureRepSet[loop].Mode.Refused))
			    IE_p->Len = 3;
			    else
			    {
			    memcpy(&IE_p->Report.DefReport,
				   &MreportCmd_p->MeasureRepSet[loop].Report,
				   sizeof(IEEEtypes_MeasurementRep_t));}
			    /* this IE's length */
			    IE_p->Len = ielen[loop]; subLen = ielen[loop] + 2;
			    /* point to starting address of next IE */
			    IE_p =
			    (IEEEtypes_MeasurementReportElement_t *) ((UINT8 *)
								      IE_p +
								      subLen);
			    len += subLen;}

			    txSkb_p->len =
			    sizeof(struct ieee80211_frame) + len + 3;
			    //MgmtMsg_p->Hdr.FrmBodyLen = len + 3; /* length of Measurement IEs plus 3(Category+action+DialogToken)*/
			    WLDBG_INFO(DBG_LEVEL_7,
			    ...
			    ++BcnRptToken; if (BcnRptToken == 0)
			    BcnRptToken = 1; return BcnRptToken;}

			    void macMgmtMlme_RmBeaconRequest(struct net_device
							     *netdev,
							     UINT8 * stamac,
							     UINT8 * bssid,
							     UINT8 RegDomain,
			    ...
			    RequestElement; IEEEtypes_BeaconRequest_t * bcn_req;
			    UINT8 * opt; UINT16 len = 0; struct sk_buff * skb;
			    WLDBG_ENTER(DBG_LEVEL_7);
			    skb =
			    mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION,
					       (IEEEtypes_MacAddr_t *) stamac,
...
case 2:
			    ...
			    opt[len + 1] = 3;
			    opt[len + 2] = 12;
			    opt[len + 3] = 1; opt[len + 4] = 6;
				/***** AP Channel Report End *******/
				/***** AP Channel Report Start *******/
			    opt[len + 5] = 51;
			    opt[len + 6] = 3;
			    opt[len + 7] = 1;
			    opt[len + 8] = 36; opt[len + 9] = 48;
				/***** AP Channel Report End *******/
			    len += 10;}
case 1:
case 4:
			    /* Request */
			    opt[len] = 10; opt[len + 1] = 5;	/* Length */
			    opt[len + 2] = 0;	/* SSID */
			    opt[len + 3] = 48;	/* RSN */
...
len += 7; break; default:
			    ...
			    void getVhtBwFreqInfo(UINT16 FreqBand, UINT16 Chan,
						  UINT16 Chan2,
						  UINT16 ChnlWidth,
						  UINT8 radiomode,
						  UINT8 * ch_width,
			    ...
			    *center_freq0 =
			    GetCenterFreq(Chan, CH_80_MHz_WIDTH);
			    ...
			    *center_freq1 = 0;
			    printk
			    ("getVhtFreqInfo::: 1 center_freq0=%d center_freq1=%d\n",
			     *center_freq0, *center_freq1);}
			    if (*ch_width == 2)	//160MHz 
			    {
			    *center_freq0 =
			    GetCenterFreq(Chan, CH_160_MHz_WIDTH);
			    *center_freq1 = 0;}
#endif
			    }
			    else
			    ...
			    * ch_width = *ch_width ? 1 : 0;	//new ch_width = 1 for all 80, 160 and 80+80
...
#ifdef SOC_W906X
			    void macMgmtMlme_ChannelSwitchReq(vmacApInfo_t *
							      vmacSta_p,
							      Dfs_ChanSwitchReq_t
							      * pChanSwitch) {
			    MIB_802DOT11 * mib = vmacSta_p->ShadowMib802dot11;
			    ...
			    pChanSwitch->ChannelSwitchCmd.ChannelSwitchCount;
			    ChannelSwitchAnnouncementElement_p->Channel =
			    pChanSwitch->ChannelSwitchCmd.ChannelNumber;
			    if (*(mib->mib_ApMode) &
				(AP_MODE_N_ONLY | AP_MODE_11AC | AP_MODE_11AX))
			    {
			    pIE =
			    (IEEEtypes_SecondaryChannelOffsetElement_t
			     *) (ChannelSwitchAnnouncementElement_p + 1);
			    ...
			    **/
			    ...
			    pIE->Len = 1; if (ChnlWidth < CH_40_MHz_WIDTH)
			    pIE->Offset = 0;
			    else
			    pIE->Offset =
			    ...
			    if (ChnlWidth > CH_40_MHz_WIDTH) {
			    pWideIE =
			    (IEEEtypes_WideBWCS_Element_t *) (pIE + 1);
				/** VHT: Wide Bandwidth Channel Switch Element [5]
			        ...
			        **/
			    pWideIE->ElementId = WIDE_BW_CHAN_SWITCH;
			    pWideIE->Len = 3;
			    getVhtBwFreqInfo(PhyDSSSTable->Chanflag.FreqBand,
					     pChanSwitch->chInfo.channel,
					     pChanSwitch->chInfo.channel2,
					     PhyDSSSTable->Chanflag.ChnlWidth,
					     pChanSwitch->chInfo.chanflag.
					     radiomode, &pWideIE->ch_width,
					     &pWideIE->center_freq0,
					     &pWideIE->center_freq1);}
			    else
			    skb_trim(txSkb_p,
				     txSkb_p->len -
				     sizeof(IEEEtypes_WideBWCS_Element_t));}
			    ...
			    {
			    skb_trim(txSkb_p, txSkb_p->len - extraLen);}
			    ...
			    vmacSta_p->ShadowMib802dot11->SpectrumMagament;
			    ...
			    extern BOOLEAN UpdateSecondChannelInMIB(vmacApInfo_t
								    * vmacSta_p,
								    UINT32
								    channel);
				...
				**/
			    ...
			    static void macMgmtMlme_TPCReport(vmacApInfo_t *
							      vmacSta_p,
							      IEEEtypes_MacAddr_t
							      * Addr,
							      UINT8 DialogToken,
...
#endif
			    {
			    IEEEtypes_TPCRepElement_t * TPCRepElement_p; MgmtMsg_p->Body.Action.Category = SPECTRUM_MANAGEMENT; MgmtMsg_p->Body.Action.Action = TPC_REPORT; MgmtMsg_p->Body.Action.DialogToken = DialogToken; TPCRepElement_p = (IEEEtypes_TPCRepElement_t *) & MgmtMsg_p->Body.Action.Data.TPCReport; TPCRepElement_p->ElementId = TPC_REP; TPCRepElement_p->Len = 2; TPCRepElement_p->TxPwr = BcnTxPwr;	/* approx. value as the one with beacon frame */
			    ...
			    void msgMrequestIndPack(IEEEtypes_MRequestInd_t *
						    MrequestInd_p,
						    dot11MgtFrame_t *
						    mgtFrame_p) {
			    UINT32 lenBody; UINT32 loop_d, loop_s; UINT32 ieNum;
			    /* cast it because the sourceInsight has problem to decode the structure definition
			     * and make the code more readable
			     */
			    IEEEtypes_ActionField_t * Action_p =
			    &mgtFrame_p->Body.Action;
			    IEEEtypes_MeasurementRequestElement_t *
			    MeasurementRequest_p =
			    ...
			    memcpy(MrequestInd_p->PeerStaAddr,
				   mgtFrame_p->Hdr.SrcAddr,
				   sizeof(IEEEtypes_MacAddr_t));
			    ...
			    /* pack it */
			    MrequestInd_p->MeasureReqSet[loop_d].
			    MeasurementToken =
			    ...
			    static void msgMreportIndPack(IEEEtypes_MReportInd_t
							  * MReportInd_p,
							  macmgmtQ_MgmtMsg_t *
							  MgmtMsg_p) {
			    UINT32 idx = 0;
			    ...
			    /*peerstaAddr is the the station the frame coming from */
			    memcpy(MReportInd_p->PeerStaAddr,
				   MgmtMsg_p->Hdr.SrcAddr,
				   sizeof(IEEEtypes_MacAddr_t));
			    /* take the dialog token
			     * the dialog token is the token associated with the action frame
			    ...
			    /* a simple semantic check */
			    if (MeasurementReport_p->ElementId !=
				MEASUREMENT_REP) {
			    ...
			    MReportInd_p->ReportItems = idx; return;}
			     ...
			     */
			    /*peerstaAddr is the the station the frame coming from */
			    memcpy(ChannelSwitchInd_p->PeerStaAddr,
				   MgmtMsg_p->Hdr.SrcAddr,
				   sizeof(IEEEtypes_MacAddr_t));
			    ChannelSwitchInd_p->Mode = ChannelSwitch_p->Mode;
			    ChannelSwitchInd_p->ChannelNumber =
			    ChannelSwitch_p->Channel;
			    ChannelSwitchInd_p->ChannelSwitchCount =
			    ChannelSwitch_p->Count; return;}

			    extern BOOLEAN macMgmtMlme_80211hAct(vmacApInfo_t *
								 vmacSta_p,
								 macmgmtQ_MgmtMsg3_t
								 * MgmtMsg_p) {
			    MIB_802DOT11 * mib = vmacSta_p->Mib802dot11;
			    ...
			    if (((*(mib->mib_ApMode) != AP_MODE_A_ONLY) &&
				 (*(mib->mib_ApMode) != AP_MODE_AandG)) ||
				(mib_StaCfg_p->SpectrumManagementRequired !=
				 TRUE)) {
			    WLDBG_INFO(DBG_LEVEL_7,
...
case TPC_REQUEST:
			    ...
			    macMgmtMlme_TPCReport(vmacSta_p,
						  ...
						  MgmtMsg_p->Body.Action.
						  DialogToken,
						  0 /*RxBufPtr->u.RxSign.RSSI */
						  );
			    }
			    /* no msg have to be sent to SME
			     * have to free the buffer
			     */
			    wl_kfree((UINT8 *) toSmeMsg); return TRUE; break;
			    /* start TPC protocol and processed in MLME */
case TPC_REPORT:
			    /*
			     * Call TPC protocol sub-function here
...
toSmeMsg->MsgType = SMC_NOTIFY_CHANNELSWITCH_IND; msgChannelswitchIndPack((IEEEtypes_ChannelSwitchInd_t *) & toSmeMsg->Msg.MrequestInd, (macmgmtQ_MgmtMsg_t *) MgmtMsg_p); break; default:
			    break;}
			    toSmeMsg->vmacSta_p = vmacSta_p;
			    smeQ_MgmtWriteNoBlock(toSmeMsg);
			    wl_kfree((UINT8 *) toSmeMsg);
...
#ifdef COEXIST_20_40_SUPPORT
			    extern BOOLEAN
			    macMgmtMlme_80211PublicAction(vmacApInfo_t *
							  vmacSta_p,
							  macmgmtQ_MgmtMsg3_t *
							  MgmtMsg_p) {
			    ...
			    if (CoexistAction->Coexist_Report.Coexist.
				FortyMhz_Intorant == 1 ||
			    ...
			    for (i = 0;
				 i < CoexistAction->Intolerant_Report.Len;
				 i++) {

			    if (CoexistAction->Intolerant_Report.ChanList[i] !=
				0) {
			    extern int Check_40MHz_Affected_range(vmacApInfo_t
								  *, int, int);
			    if (Check_40MHz_Affected_range
				(vmacSta_p,
				 CoexistAction->Intolerant_Report.ChanList[i],
				 0))
			    Handle20_40_Channel_switch(vmacSta_p, 0); break;}
			    ...
			    UINT32 hw_GetPhyRateIndex(vmacApInfo_t * vmacSta_p,
						      UINT32 Rate) {
			    MIB_802DOT11 * mib = vmacSta_p->Mib802dot11;
			    UINT32 i; UINT8 MaxRate;
			    if (Is5GBand(*(mib->mib_ApMode))) {
			    MaxRate = IEEEtypes_MAX_DATA_RATES_A;}
			    else
			    MaxRate = IEEEtypes_MAX_DATA_RATES_G;
...
/* RTS protection check */
			    ...
			    extern UINT32 wlFwSetProtectMode(struct net_device
							     *netdev,
							     UINT32 action,
							     UINT32 * mode);
			    struct wlprivate * wlpptr; int entries, ivap;
			    long rxbps, txbps; int entries_total = 0;
			    int do_check = 0; u32 mode;
			    wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
			    if (protect_dynamic) {
			    ...
			    wlpptr_v =
			    NETDEV_PRIV_P(struct wlprivate, wlpptr->vdev[ivap]);
			    if (wlpptr_v->vmacSta_p->OpMode == WL_OP_MODE_VSTA)
			    do_check = 1;}
			    else {
			    for (ivap = 0; ivap < wlpptr->wlpd_p->NumOfAPs;
				 ivap++) {
			    if (wlpptr->vdev[ivap] != NULL &&
				(wlpptr->vdev[ivap]->flags & IFF_RUNNING)) {
			    wlpptr_v =
			    ...
			    wlFwSetProtectMode(wlpptr->netDev,
					       HostCmd_ACT_GEN_SET, &mode);}
			    else
			    if ((txbps >= protect_tx_rate_thres) &&
				(rxbps >= protect_rx_rate_thres)) {
			    /* protect rts for TCP */
			    ...
			    TimerRearm(&vmacSta_p->monTimer,
				       MONITER_PERIOD_1SEC);}

#ifdef SOC_W906X
			    typedef struct _mbssid_ie_db {
			    u8 mbssid_index;	//value in mbssid-index
			    u8 rsvd[3]; u16 ietable[256][4];	//lookup table for the mbssid_ie entry        
			    u8 iebuf[260]; struct _mbssid_ie_db * link;	//point to next mbssid with same mbssid-index 
			    struct _mbssid_ie_db * next;	//point to next mbssid ie
			    } PACK_END mbssid_ie_db_t; typedef struct _bcn_ie_db {
			    u8 * ie[256];
			    struct _bcn_ie_db * next;} PACK_END bcn_ie_db_t;
//retrieve mbssie-index from mbssid IE  
			    int getMBSSID_index(IEEEtypes_Mbssid_Element_t *
						pmb) {
			    IEEEtypes_NonTransmitted_BSSID_Profile_t * pNonTxPf;
			    int idx = (int)(-1); int len; u8 * subie;
			    ...
			    goto exit; subie = (u8 *) & pNonTxPf->ntBssidCap;
			    len = pNonTxPf->Len; while (len > 0) {

			    if (subie[0] == MBSSID_INDEX) {	//found 
			    return (((IEEEtypes_mbssid_idx_t *) subie)->
				    BssidIdx);}

			    len -= (2 + subie[1]); subie += (2 + subie[1]);	//point to next subIE. 
			    }
...
//3. parse the node and do subelement index in table
			    u8 extract_mbssid_ies(void *attrib_p, int len,
						  mbssid_ie_db_t ** phead) {
			    IEEEtypes_Mbssid_Element_t * pmb; mbssid_ie_db_t * entry = NULL; int idx; u8 num = 0;	//num of mbssid IE with different BSSID-index
			    u32 i;
			    ...
			    goto nextie; if (pmb->NonTxPf.subElementId != NONTRANSMITTED_BSSID_PROFILE_SUBELM_ID)	//step to next ie 
			    goto nextie;
			    if ((entry =
				 (mbssid_ie_db_t *)
			    ...
			    wl_kfree(entry); entry = NULL; goto exit;}
			    ...
			    mbssid_ie_db_t * temp = pnode; while (temp->next)
			    temp = temp->next; temp->link = entry;	//chain the same mbssid together by 'link'
			    }
			    else {
			    pnode->next = entry;	//next another mbssid
			    pnode = pnode->next; num++;}
			    }
...
nextie:
			    len -= (pmb->Len + 2); attrib_p += (pmb->Len + 2);	//step to next IE 
			    } while (len > 0);
			    //hash all IEs within the non-transmitted profile for facilitating to rebuild legacy bcn frame later.
			    pnode = (*phead); for (i = 0; i < num; i++) {
			    pnode_s = pnode; while (pnode) {
			    ...
			    u16 len1 = 3;	//
			    u32 k; if (subie[0] != NONTX_BSSID_CAP) {
			    printk("invalid first non-transmitted IE:%u\n",
				   subie[0]); pnode = pnode->next; break;}

			    while (length > len1) {

			    for (k = 0;
				 k < 4 && pnode->ietable[subie[0]][k] != 0;
				 k++); if (k < 4) {
			    ...
			    len1 += (subie[1] + 2); subie += (subie[1] + 2);	//move to next subie
			    }
...
#define BEACON_MAX_MBSSIE_CNT 32
			    ...
			    UINT8 decomposeMBSSID(struct net_device * dev,
						  struct sk_buff * skb,
						  struct sk_buff_head *
						  skbList) {
				...
				IEEE_MSG_PROBE_RSP)
			    goto exit; wlanMsg_p->Hdr.FrmBodyLen = skb->len;
			    if ((attrib_p =
				 syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *)
						     wlanMsg_p, EXT_CAP_IE,
						     len)) != NULL) {
			    ...
			    if ((attrib_p =
				 syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *)
						     wlanMsg_p, MULTI_BSSID,
						     len)) != NULL) {
			    //mbssid_ie_db_t  *pentry;
			    int len2 =
			    (skb->len - (u32) ((u8 *) attrib_p - skb->data));
			    //printk("MULTI_BSSID: len2:%u\n", len2);
			    //mwl_hex_dump(attrib_p, len2);
			    if ((num =
				 extract_mbssid_ies(attrib_p, len2,
						    &phead)) == 0) {
			    ...
			    pmgmtf_src =
			    (macmgmtQ_MgmtMsg_t *) ((UINT8 *) skb->data - 2);
			    pmgmtf =
			    (macmgmtQ_MgmtMsg_t *) ((UINT8 *) skb2->data - 2);
			    pmgmtf->Hdr.FrmBodyLen = skb->len;
			    len = skb2->len + 2;
			    len1 =
			    ((u8 *) & (pmgmtf->Body.Bcn.SsId) - (u8 *) pmgmtf);
			    ...
			    pnode = pnode->link;} while (pnode);
			    pnode = pnode_s;
			    //move src to next ie
			    len1 += (2 + iesrc[1]); iesrc += (2 + iesrc[1]);	//step to next ie in iesrc
			    }

			    cnt++; while (pnode_s) {
			    pnode_s = pnode->link; wl_kfree(pnode);	//free node memory allocated within extract_mbssid_ies
			    }
...
exit:
			    ...
			    extern void RxBeacon(vmacApInfo_t * vmacSta_p,
						 void *BssData_p, UINT16 len,
						 UINT32 rssi) {
			    ...
			    if ((attrib_p =
				 syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *)
						     ...
						     EXT_SUPPORTED_RATES,
						     len)) != NULL) {
			    if (((IEEEtypes_SuppRatesElement_t *) attrib_p)->
				Len > 0) {
			    }
			    else
			    BAPCount++;}
			    ...
			    ERPCount++; BStnAroundCnt = BSTATION_AGECOUNT;}
			    }
#ifdef NPROTECTION
			    if ((attrib_p =
				 syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *)
			    ...
			    if ((attrib_p =
				 syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *)
						     BssData_p, ADD_HT,
						     len)) != NULL) {
			    if (((IEEEtypes_Add_HT_Element_t *) attrib_p)->
				OpMode.NonHTStaPresent == 1)
			    wlpptr->wlpd_p->legAPCount = 1;}
			    else {
			    wlpptr->wlpd_p->legAPCount = 1;}
#endif
			    ...
			    Scanbeacon = 1; if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ && (vmacSta_p->busyScanning ||	/* Recv beacon during coex scan */
													  PhyDSSSTable->
													  Chanflag.
													  ChnlWidth
													  ==
...
/** 40MHz channel capable AP found **/
			    ...
			    HTCap = 1;
			    AddhtSetIE_p =
			    (IEEEtypes_Add_HT_Element_t *) attrib_p2;
			    if (AddhtSetIE_p->AddChan.STAChannelWidth) {
			    AddHtChannelOffset =
			    AddhtSetIE_p->AddChan.ExtChanOffset;}
			    }
			    }
			    }
			...
			only check for primary channel **/
			    ...
			    if (Check_40MHz_Affected_range
				(vmacSta_p, dsPSetIE_p->CurrentChan, 0)) {
			    Handle20_40_Channel_switch(vmacSta_p, 1);
			    ...
			    Handle20_40_Channel_switch(vmacSta_p, 1);
			    printk(KERN_ERR
				   "%s: Curr %dch, Recv %d ch beacon. Swith to 20MHz\n",
				   __func__, PhyDSSSTable->CurrChan,
...
/* DisableMacMgmtTimers - disable timers for removing module. */
			    extern void Disable_stationAgingTimer(vmacApInfo_t *
								  vmacSta_p);
			    extern void Disable_GrpKeyTimer(vmacApInfo_t *
							    vmacSta_p);
			    void DisableMacMgmtTimers(vmacApInfo_t *
						      vmacSta_p) {
			    ...
			    RateToRateBitMapConversion_t
			    IEEEToMrvlRateBitMapConversionTbl[] = {
...
#define ENDOFTBL 0xFF
			    ...
			    ENDOFTBL, ENDOFTBL},};
			    void IEEEToMrvlRateBitMapConversion(UINT8
								SupportedIEEERate,
								UINT32 *
			    ...
			    while (IEEEToMrvlRateBitMapConversionTbl[i].
				   IEEERate != ENDOFTBL) {
			    if ((IEEEToMrvlRateBitMapConversionTbl[i].
				 IEEERate == SupportedIEEERate)) {
			    *pMrvlLegacySupportedRateBitMap =
			    *pMrvlLegacySupportedRateBitMap |
			    ...
			    static UINT32 GetLegacyRateBitMap(vmacApInfo_t *
							      vmacSta_p,
							      IEEEtypes_SuppRatesElement_t
							      * SuppRates,
							      IEEEtypes_ExtSuppRatesElement_t
			    ...
			    UINT8 Rates[512] = {
			    0}; UINT32 SupportedLegacyIEEERateBitMap = 0;
			    /* Get legacy rates */
			    for (i = 0; i < SuppRates->Len; i++) {
...
#ifdef BRS_SUPPORT
			    int isClientRateMatchAP(vmacApInfo_t * vmacSta_p,
						    UINT8 * Rates) {
			    UINT32 i, j, findRate; UINT32 rateMask;
			    MIB_802DOT11 * mib = vmacSta_p->Mib802dot11;
			    ...
			    *(mib->BssBasicRateMask) | *(mib->
			    ...
			    vmacSta_p->nClients++; wlpptr->wlpd_p->nClients++;
			    ...
			    vmacSta_p->NonGFSta++; wlpptr->wlpd_p->NonGFSta++;}
			    }
			    else {
			    ...
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
			    vmacSta_p->numClients = 0; vmacSta_p->nClients = 0;
			    vmacSta_p->legClients = 0;
			    vmacSta_p->n40MClients = 0;
			    vmacSta_p->n20MClients = 0;
			    vmacSta_p->gaClients = 0; vmacSta_p->bClients = 0;
			    vmacSta_p->NonGFSta = 0;
			    ...
			    MIB_802DOT11 * mib = vmacSta_p->ShadowMib802dot11;
			    MIB_PHY_DSSS_TABLE * PhyDSSSTable =
			    mib->PhyDSSSTable; UINT8 BcnAddHtAddChannel = 0;
			    if (*
				(vmacSta_p->ShadowMib802dot11->
				 mib_HT40MIntoler)) {
			    if (*(vmacSta_p->Mib802dot11->mib_ApMode) ==
		...
		/** switch back to 40MHz here **/
			    ...
			    TimerRearm(&vmacSta_p->CoexistTimer,
					 ...
					 mib_Channel_Width_Trigger_Scan_Interval)
				       *
				       (*
					(mib->
					 mib_Channel_Transition_Delay_Factor)) *
				       10);}
			    ...
			    TimerFireIn(&vmacSta_p->CoexistTimer, 1,
					Coexist_TimeoutHdlr,
					(unsigned char *)vmacSta_p,
					*(mib->
					  mib_Channel_Width_Trigger_Scan_Interval)
			    ...
			    void Disable_StartCoexisTimer(vmacApInfo_t *
							  vmacSta_p) {
			    TimerRemove(&vmacSta_p->CoexistTimer);}

			    int Check_40MHz_Affected_range(vmacApInfo_t *
							   vmacSta_p,
							   int curchannel,
							   int extchan) {
			    ...
			    void Handle20_40_Channel_switch(vmacApInfo_t *
							    vmacSta_p,
							    UINT8 Scanning) {
			    MIB_802DOT11 * mib = vmacSta_p->ShadowMib802dot11;
			    ...
			    extern int wlFwSet11N_20_40_Switch(struct net_device *netdev, UINT8 mode); extern BOOLEAN macMgmtMlme_SendNotifyChannelWidthManagementAction(vmacApInfo_t * vmacSta_p, UINT8 mode); if (((*(mib->USER_ChnlWidth)) & 0x0f) == 0x00) {
/** we are already at 20MHz mode **/
		...
		/** Start timer to switch back to 40MHz **/
			    if (vmacSta_p->n40MClients)
						/** start timer only if there is 11n 40M sta **/
			    StartCoexistTimer(vmacSta_p); if (Scanning)
			    ...
			    *(mib->USER_ChnlWidth) = 0x0;
			    ...
			    if (wlpptr->wlpd_p->n20MClients &&
				((*(mib->USER_ChnlWidth) & 0x0f) == 1)) {
			    BcnAddHtOpMode = 0x02;}
			    else
			    {
				...
				((!BcnAddHtOpMode) || (BcnAddHtOpMode == 2))) {
			    TxGf = 1; BcnAddHtOpMode &= ~0x0004;	/* bit2 is non-GF present */
			    }
			    else
			    ...
			    {
			    BcnAddHtOpMode |= 0x0004; wlFwSetHTGF(vmacSta_p->dev, TxGf);	/*Bug 46196: Added to handle disabled GF fw update */
			    }
			    }

			    if (wlpptr->wlpd_p->BcnAddHtOpMode !=
				BcnAddHtOpMode) {
			    ...
			    void checkLegDevOutBSS(vmacApInfo_t * vmacSta_p) {
			    struct wlprivate * wlpptr = NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev); UINT8 HWIndex = vmacSta_p->VMacEntry.phyHwMacIndx; struct net_device * dev; struct wlprivate * wlpptr1; int i = 0; if ((legacyAPCount[HWIndex] != wlpptr->wlpd_p->legAPCount) || (counterHtProt[HWIndex]++ % 10 == 0)) {	/* 30s check */
				...
				wlpptr1->vmacSta_p->VMacEntry.modeOfService ==
				VMAC_MODE_AP)
			    if (dev->flags & IFF_RUNNING)
			    HandleNProtectionMode(wlpptr1->vmacSta_p);}
			    i++;}
			    }
			    ...
			    macmgmtQ_MgmtMsg2_t * MgmtMsg_p;
			    struct sk_buff * txSkb_p; IEEEtypes_SM_PwrCtl_t * p;
			    UINT16 FrameLen =
			    sizeof(IEEEtypes_Category_t) +
			    sizeof(IEEEtypes_ActionFieldType_t) +
			    ...
			    (vmacApInfo_t * vmacSta_p, UINT8 mode) {
			    macmgmtQ_MgmtMsg2_t * MgmtMsg_p;
			    struct sk_buff * txSkb_p; IEEEtypes_BWCtl_t * p;
			    ...
			    (macmgmtQ_MgmtMsg2_t *) ((UINT8 *) txSkb_p->data);
			    MgmtMsg_p->Body.Act.Category = HT_CATEGORY;
			    MgmtMsg_p->Body.Act.Action =
			    ACTION_NOTIFYCHANNELWIDTH;
...
#ifdef INTOLERANT40
			    BOOLEAN
			    macMgmtMlme_SendInfoExchHtManagementAction(struct
								       net_device
								       * dev,
								       IEEEtypes_MacAddr_t
								       * Addr) {
			    macmgmtQ_MgmtMsg2_t * MgmtMsg_p;
			    struct sk_buff * txSkb_p; IEEEtypes_InfoExch_t * p;
			    UINT16 FrameLen =
			    sizeof(IEEEtypes_Category_t) +
			    sizeof(IEEEtypes_ActionFieldType_t) +
			    ...
			    BOOLEAN
			    macMgmtMlme_SendBeaconReqMeasureReqAction(struct
								      net_device
								      * dev,
								      IEEEtypes_MacAddr_t
								      * Addr) {
			    extern UINT8 getRegulatoryClass(vmacApInfo_t *
							    vmacSta_p);
			    macmgmtQ_MgmtMsg2_t * MgmtMsg_p;
			    struct sk_buff * txSkb_p;
			    struct wlprivate * wlpptr =
			    ...
			    sizeof(IEEEtypes_MeasurementRequestEl_t) - (34 -
									strlen(&
									       (mib->
										StationConfig->
										DesiredSsId
			    ...
			    MgmtMsg_p =
			    (macmgmtQ_MgmtMsg2_t *) ((UINT8 *) txSkb_p->data);
			    MgmtMsg_p->Body.Act.Category =
			    RADIO_MEASURE_CATEGOTY;
			    ...
			    p =
			    &MgmtMsg_p->Body.Act.Field.Field5.Data.
			    MeasurementRequestEl;
			    p->ElementId = MEASUREMENT_REQ; p->Token = 0;
			    p->Mode.Enable = 1; p->Mode.Request = 1;
			    p->Mode.Report = 0; p->Type = TYPE_REQ_BCN;
			    p->Request.RegClass = getRegulatoryClass(vmacSta_p);
			    memcpy(p->Request.BSSID, &vmacSta_p->macStaAddr,
				   sizeof(IEEEtypes_MacAddr_t));
			    p->Request.ChanNum = 255;
			    p->Request.RandInterval = 0; p->Request.Mode = 0;
			    p->Request.Duration = 10;
			    p->Request.ReportCondi = 254;
			    p->Request.Threshold_offset = 1;
			    memcpy(p->Request.SSID,
				   &(mib->StationConfig->DesiredSsId[0]),
				   strlen(&
					  (mib->StationConfig->
					   DesiredSsId[0])));
			    p->Len =
			    3 + sizeof(IEEEtypes_MeasurementReqBcn_t) - (34 -
									 strlen
									 (&
									  (mib->
									   StationConfig->
			    ...
			    BOOLEAN
			    macMgmtMlme_SendExtChanSwitchAnnounceAction(struct
									net_device
									* dev,
									IEEEtypes_MacAddr_t
									*
			    ...
			    return FALSE; MgmtMsg_p = (macmgmtQ_MgmtMsg2_t *) ((UINT8 *) txSkb_p->data); MgmtMsg_p->Body.Act.Category = SPECTRUM_MANAGE_CATEGOTY; MgmtMsg_p->Body.Act.Action = ACTION_EXTCHANSWTANNO; p = &MgmtMsg_p->Body.Act.Field.ExtendChanSwitchAnnounceEl; p->ElementId = 60;	// Temp
			    p->ChanSwitchMode = 1;
...
#ifdef SOC_W906X
			    BOOLEAN UpdateSecondChannelInMIB(vmacApInfo_t *
							     vmacSta_p,
							     UINT32 channel) {
...
/* This function updates the shadow MIB wth the given channel*/
			    BOOLEAN UpdateCurrentChannelInMIB(vmacApInfo_t *
							      vmacSta_p,
							      UINT32 channel) {
#ifndef SOC_W906X
			    extern BOOLEAN force_5G_channel;
...
case 3:
case 4:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 5:
...
case 10:
			    ...
			    if (*mib_extSubCh_p == 2)
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 11:
case 12:
case 13:
case 14:
			    PhyDSSSTable->Chanflag.ExtChnlOffset =
			    EXT_CH_BELOW_CTRL_CH; break;
			    /* for 5G */
case 36:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 40:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 44:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 48:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 52:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 56:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 60:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 64:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 68:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 72:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 76:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 80:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 84:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 88:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 92:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 96:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 100:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 104:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 108:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 112:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 116:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 120:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 124:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 128:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 132:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 136:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 140:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 144:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 149:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 153:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 157:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 161:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 165:
			    ...
			    EXT_CH_BELOW_CTRL_CH;}
break; case 169:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 173:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 177:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 181:
PhyDSSSTable->Chanflag.ExtChnlOffset = NO_EXT_CHANNEL; break; case 184:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 188:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 192:
PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 196:
			    PhyDSSSTable->Chanflag.ExtChnlOffset =
			    EXT_CH_BELOW_CTRL_CH; break;}
			    }
#ifndef SOC_W906X
			    ...
			    void ApplyCSAChannel(struct net_device *netdev,
						 UINT32 channel) {
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, netdev);
			    vmacApInfo_t * syscfg =
...
#ifdef MRVL_DFS
			    void macMgmtMlme_StartRadarDetection(struct
								 net_device
								 *dev,
								 UINT8
								 detectionMode)
			    ...
			    void macMgmtMlme_StopRadarDetection(struct
								net_device *dev,
								UINT8
								detectionMode) {
			    wlFwSetRadarDetection(dev,
			    ...
			    void macMgmtMlme_StartAuxRadarDetection(struct
								    net_device
								    *dev,
								    UINT8
								    detectionMode)
			    ...
			    void macMgmtMlme_StopAuxRadarDetection(struct
								   net_device
								   *dev,
								   UINT8
								   detectionMode)
			    ...
			    void macMgmtMlme_SendChannelSwitchCmd(struct
								  net_device
								  *dev,
								  Dfs_ChanSwitchReq_t
								  *
								  pChannelSwitchCmd)
			    {
			    int i; struct wlprivate * wlpptr = NULL;
			    ...
			    void macMgmtMlme_GetActiveVAPs(struct net_device
							   *dev,
							   UINT8 * vaplist,
							   UINT8 * vapcount_p) {
			    UINT8 j = 0; UINT8 count = 0;
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, dev);
			    ...
			    *vapcount_p = count;}
			    ...
			    void macMgmtMlme_Reset(struct net_device *dev,
						   UINT8 * vaplist,
						   UINT8 * vapcount_p) {
			    ...
			    void macMgmtMlme_MBSS_Reset(struct net_device
							*netdev,
							UINT8 * vaplist,
							UINT8 vapcount) {
			    int i = 0;
			    ...
			    void macMgmtMlme_SwitchChannel(struct net_device
							   *dev, UINT8 channel,
							   UINT8 channel2,
							   CHNL_FLAGS *
							   chanFlag_p) {
			    ...
			    extern DFS_STATUS dfs_set_op_ch(struct net_device
							    *dev,
							    UINT16 channel);
			    dfs_set_op_ch(dev, channel); return;}
#endif /* CONCURRENT_DFS_SUPPORT */
			    ...
			    if (wlchannelSet
				(dev, channel, channel2, PhyDSSSTable->Chanflag,
				 0))
#else
			    if (wlchannelSet
			    ...
			    BOOLEAN macMgmtMlme_DfsEnabled(struct net_device
							   *dev) {
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, dev);
			    vmacApInfo_t * syscfg =
			    ...
			    void macMgmtMlme_StopDataTraffic(struct net_device
							     *dev) {
			    struct wlprivate * wlpptr = NULL;
			    DfsAp * pdfsApMain = NULL; if (!dev)
			    return;
			    wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
			    ...
			    dfsTrace(dev, 0xe, 0x1, NULL,
				     (0x10000000 |
				      ((dev->
					flags & IFF_RUNNING) << 4) | (pdfsApMain
								      ? 1 :
								      0)));
			    if (dev->flags & IFF_RUNNING) {
			    ...
			    void macMgmtMlme_RestartDataTraffic(struct
								net_device *dev)
			    {
			    ...
			    netif_carrier_on(dev); netif_wake_queue(dev);
			    dev->flags |= IFF_RUNNING;}
			    ...
			    UINT8 macMgmtMlme_Get40MHzExtChannelOffset(UINT8
								       channel)
			    {
...
case 3:
case 4:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 5:
case 6:
case 7:
case 8:
case 9:
...
case 13:
case 14:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 36:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 40:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 44:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 48:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 52:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 56:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 60:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 64:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 68:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 72:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 76:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 80:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 84:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 88:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 92:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 96:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 100:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 104:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 108:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 112:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 116:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 120:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 124:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 128:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 132:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 136:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 140:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 144:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 149:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 153:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 157:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 161:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 165:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 169:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 173:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 177:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 181:
extChnlOffset = NO_EXT_CHANNEL; break; case 184:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 188:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; case 192:
extChnlOffset = EXT_CH_ABOVE_CTRL_CH; break; case 196:
extChnlOffset = EXT_CH_BELOW_CTRL_CH; break; default:
...
#ifdef INTOLERANT40
			    static void HT40MIntolerantHandler(vmacApInfo_t *
							       vmacSta_p,
							       UINT8 soon) {
			    extern void
			    extStaDb_SendBeaconReqMeasureReqAction(vmacApInfo_t
								   * vmac_p);
			    MIB_802DOT11 * mib = vmacSta_p->Mib802dot11;
			    MIB_802DOT11 * mibShadow =
			    vmacSta_p->ShadowMib802dot11; UINT8 Addr[] = {
			    0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
			    ...
			    (vmacSta_p->dev, (IEEEtypes_MacAddr_t *) & Addr[0]);
			    *(mibShadow->mib_FortyMIntolerant) = 1;
			    mibShadow->PhyDSSSTable->Chanflag.ChnlWidth =
			    CH_20_MHz_WIDTH;
			    /* Start 30 min timer */
			    sMonitorcnt30min = 1; wlResetTask(vmacSta_p->dev);
			    //printk("to 20\n");
			    }
			    }
			    else
			    ...
			    *(mibShadow->mib_FortyMIntolerant) = 0;
			    ...
			    if (*(mib->USER_ChnlWidth) == CH_20_MHz_WIDTH) {
			    /* Close 30 min timeer */
			    sMonitorcnt30min = 0;}
			    //sHt30minStaIntolerant = 0;
			    }

			    void RecHTIntolerant(vmacApInfo_t * vmacSta_p,
						 UINT8 enable) {
			    MIB_802DOT11 * mib = vmacSta_p->Mib802dot11;
			    //printk("Rec:HT %d\n", enable);
			    if (enable) {
			    ...
			    {
			    sMonitorcnt30min = 1; sHt30minStaIntolerant = 0;}
			    }
#endif //#ifdef INTOLERANT40

			    void FixedRateCtl(extStaDb_StaInfo_t * pStaInfo,
					      PeerInfo_t * PeerInfo,
					      MIB_802DOT11 * mib) {
			    if ((pStaInfo == NULL) ||
...
case AP_MODE_B_ONLY:
PeerInfo->StaMode = AP_MODE_B_ONLY; PeerInfo->IeeeRate = (UINT8) * (mib->mib_txDataRate); break; case AP_MODE_G_ONLY:
PeerInfo->StaMode = AP_MODE_G_ONLY; PeerInfo->IeeeRate = (UINT8) * (mib->mib_txDataRateG); break; case AP_MODE_A_ONLY:
PeerInfo->StaMode = AP_MODE_A_ONLY; PeerInfo->IeeeRate = (UINT8) * (mib->mib_txDataRateA); break; case AP_MODE_MIXED:
			    if (pStaInfo->ClientMode == BONLY_MODE)
			    {
...
break; case AP_MODE_N_ONLY:
			    if (*(mib->mib_FixedRateTxType) == 1)
			    PeerInfo->StaMode = AP_MODE_N_ONLY;
			    else
PeerInfo->StaMode = AP_MODE_G_ONLY; PeerInfo->IeeeRate = (UINT8) * (mib->mib_txDataRateN); break; case AP_MODE_BandN:
			    if (pStaInfo->ClientMode == NONLY_MODE)
			    {
			    if (*(mib->mib_FixedRateTxType) == 1)
			    PeerInfo->StaMode = AP_MODE_N_ONLY;
...
break; default:
			    ...
			    UINT8 macMgmtMlme_Get80MHzPrimaryChannelOffset(UINT8
									   channel)
			    {
			    UINT8 act_primary = ACT_PRIMARY_CHAN_0;
			    switch (channel) {
case 36:
act_primary = ACT_PRIMARY_CHAN_0; break; case 40:
act_primary = ACT_PRIMARY_CHAN_1; break; case 44:
act_primary = ACT_PRIMARY_CHAN_2; break; case 48:
act_primary = ACT_PRIMARY_CHAN_3; break; case 52:
act_primary = ACT_PRIMARY_CHAN_0; break; case 56:
act_primary = ACT_PRIMARY_CHAN_1; break; case 60:
act_primary = ACT_PRIMARY_CHAN_2; break; case 64:
act_primary = ACT_PRIMARY_CHAN_3; break; case 68:
act_primary = ACT_PRIMARY_CHAN_0; break; case 72:
act_primary = ACT_PRIMARY_CHAN_1; break; case 76:
act_primary = ACT_PRIMARY_CHAN_2; break; case 80:
act_primary = ACT_PRIMARY_CHAN_3; break; case 84:
act_primary = ACT_PRIMARY_CHAN_0; break; case 88:
act_primary = ACT_PRIMARY_CHAN_1; break; case 92:
act_primary = ACT_PRIMARY_CHAN_2; break; case 96:
act_primary = ACT_PRIMARY_CHAN_3; break; case 100:
act_primary = ACT_PRIMARY_CHAN_0; break; case 104:
act_primary = ACT_PRIMARY_CHAN_1; break; case 108:
act_primary = ACT_PRIMARY_CHAN_2; break; case 112:
act_primary = ACT_PRIMARY_CHAN_3; break; case 116:
act_primary = ACT_PRIMARY_CHAN_0; break; case 120:
act_primary = ACT_PRIMARY_CHAN_1; break; case 124:
act_primary = ACT_PRIMARY_CHAN_2; break; case 128:
act_primary = ACT_PRIMARY_CHAN_3; break; case 132:
act_primary = ACT_PRIMARY_CHAN_0; break; case 136:
act_primary = ACT_PRIMARY_CHAN_1; break; case 140:
act_primary = ACT_PRIMARY_CHAN_2; break; case 144:
act_primary = ACT_PRIMARY_CHAN_3; break; case 149:
act_primary = ACT_PRIMARY_CHAN_0; break; case 153:
act_primary = ACT_PRIMARY_CHAN_1; break; case 157:
act_primary = ACT_PRIMARY_CHAN_2; break; case 161:
act_primary = ACT_PRIMARY_CHAN_3; break; case 165:
act_primary = ACT_PRIMARY_CHAN_0; break; case 169:
act_primary = ACT_PRIMARY_CHAN_1; break; case 173:
act_primary = ACT_PRIMARY_CHAN_2; break; case 177:
			    act_primary = ACT_PRIMARY_CHAN_3; break;}

			    return act_primary;}

			    UINT8
			    macMgmtMlme_Get160MHzPrimaryChannelOffset(UINT8
								      channel) {
			    UINT8 act_primary = ACT_PRIMARY_CHAN_0;
			    switch (channel) {
...
act_primary = ACT_PRIMARY_CHAN_2; break; case 48:
act_primary = ACT_PRIMARY_CHAN_3; break; case 52:
act_primary = ACT_PRIMARY_CHAN_4; break; case 56:
act_primary = ACT_PRIMARY_CHAN_5; break; case 60:
act_primary = ACT_PRIMARY_CHAN_6; break; case 64:
			    act_primary = ACT_PRIMARY_CHAN_7; break;
...
act_primary = ACT_PRIMARY_CHAN_2; break; case 80:
act_primary = ACT_PRIMARY_CHAN_3; break; case 84:
act_primary = ACT_PRIMARY_CHAN_4; break; case 88:
act_primary = ACT_PRIMARY_CHAN_5; break; case 92:
act_primary = ACT_PRIMARY_CHAN_6; break; case 96:
			    act_primary = ACT_PRIMARY_CHAN_7; break;
...
act_primary = ACT_PRIMARY_CHAN_2; break; case 112:
act_primary = ACT_PRIMARY_CHAN_3; break; case 116:
act_primary = ACT_PRIMARY_CHAN_4; break; case 120:
act_primary = ACT_PRIMARY_CHAN_5; break; case 124:
act_primary = ACT_PRIMARY_CHAN_6; break; case 128:
			    act_primary = ACT_PRIMARY_CHAN_7; break;
...
act_primary = ACT_PRIMARY_CHAN_2; break; case 161:
act_primary = ACT_PRIMARY_CHAN_3; break; case 165:
act_primary = ACT_PRIMARY_CHAN_4; break; case 169:
act_primary = ACT_PRIMARY_CHAN_5; break; case 173:
act_primary = ACT_PRIMARY_CHAN_6; break; case 177:
			    act_primary = ACT_PRIMARY_CHAN_7; break;}
...
/*Function to check if station is in MUSetList*/
			    int MUchecksta_SetList(vmacApInfo_t * vmac_p,
						   extStaDb_StaInfo_t *
						   pStaInfo) {
			    struct wlprivate * wlpptr = NETDEV_PRIV_P(struct wlprivate, vmac_p->dev); muset_t * tempnode; int i; tempnode = (muset_t *) wlpptr->wlpd_p->MUSetList.tail;	//start with first item added into list, new node is added to head
			    while (tempnode != NULL) {
#ifdef SOC_W906X
			    for (i = 0; i < 4; i++) {
#else
			    ...
			    if (memcmp
				(tempnode->StaInfo[i]->Addr, pStaInfo->Addr,
				 6) == 0)
			    return 1;}
			    }
...
/*Function to check if station is in MUStaList*/
			    int MUchecksta_StaList(vmacApInfo_t * vmac_p,
						   extStaDb_StaInfo_t *
						   pStaInfo) {
			    MUCapStaNode_t * tempnode = NULL; u8 index; index = is_he_capable_sta(pStaInfo) * 4 + pStaInfo->vht_RxChannelWidth; tempnode = (MUCapStaNode_t *) vmac_p->MUStaList[index].tail;	//start with first item added into list, new node is added to head
			    while (tempnode != NULL) {
...
/*Function to check whether a MU set index is already used. If the index is used, return 1 else return 0*/
			    ...
			    return 1; headnode = headnode->nxt;}
			    return 0;}

/*Function to insert a MU set node into a MUSetList*/
#ifdef SOC_W906X
			    int MUinsert_node(vmacApInfo_t * vmac_p,
					      extStaDb_StaInfo_t * MUUsr[],
					      int index)
#else
			    int MUinsert_node(vmacApInfo_t * vmac_p,
			    ...
			    UINT8 i;
			    tempnode =
			    (muset_t *) wl_kmalloc(sizeof(muset_t), GFP_ATOMIC);
			    if (tempnode == NULL)
			    return 0; memset(tempnode, 0, sizeof(muset_t));
#ifndef SOC_W906X
			    MUUsr[0] = pStaInfo0;
			    MUUsr[1] = pStaInfo1; MUUsr[2] = pStaInfo2;
#endif
...
*/
			    ...
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
			    UINT16 stnid[MU_MAX_USERS]; UINT8 i;
			    UINT8 myGid = 0; unsigned long dbflags;
			    vmacApInfo_t * vmacSta_master_p;
#ifndef SOC_W906X
...
#ifdef SOC_W906X
			    SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->MUSetListLock,
					      MUSetflags);
#endif
			    ...
			    pStaInfo[1] = pStaInfo1; pStaInfo[2] = pStaInfo2;
#endif
			    SPIN_LOCK_IRQSAVE(&vmacSta_master_p->StaCtl->dbLock,
					      dbflags);
...
#else
			    if (MUinsert_node
				(vmac_p, pStaInfo0, pStaInfo1, pStaInfo2,
				 i) == 0)
#endif
			    {
	...
	/** check available GID **/
			    ...
			    SendGroupIDMgmtframe(vmac_p, pStaInfo[i]->Addr,
						 myGid, i);
			    else
			    ...
			    {
#ifdef SOC_W906X
			    printk("Setting %s MU Set GID=%d Index=%d Stnid=",
				   (cmd_option == 2) ? "HE" : "VHT", myGid,
				   myGid - 1); for (i = 0;
						    (i < MU_MAX_USERS) &&
						    (stnid[i] != 0xffff); i++)
			    printk("%d ", stnid[i]); printk("\n");
#else
			    printk
			    ("FW Set MU Set GID=%d Index=%d Stnid=%d %d %d\n",
			     myGid, myGid - 1, stnid[0], stnid[1], stnid[2]);
#endif
			    for (i = 0; i < MU_MAX_USERS; i++) {
			    if (pStaInfo[i] != NULL) {
			    pStaInfo[i]->mu_sta = 1;
			    pStaInfo[i]->mu_index = myGid - 1;}
			    }
			    ...
			    muset_node = pStaInfo[i]->MUset; break;}
			    }
#ifdef SOC_W906X
...
*/
			    BOOLEAN MUAddStaToMUStaList(vmacApInfo_t * vmac_p,
							extStaDb_StaInfo_t *
							StaInfo_p) {
			    MUCapStaNode_t * item_p = NULL; ListItem * tmp;
			    vmacApInfo_t * vmacSta_master_p;
			    unsigned long dbflags; unsigned long MUlistflags;
#ifndef SOC_W906X
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
			    unsigned long MUSetflags;
			    ...
			    SPIN_UNLOCK_IRQRESTORE(&vmac_p->MUStaListLock,
						   MUlistflags); return FALSE;}

			    /*Get a new node to fill info and put into list */
			    tmp =
			    ListGetItem(&vmacSta_master_p->StaCtl->
					FreeMUStaList); if (tmp) {
...
*/
			    ...
			    //ListItem *tmp;
			    vmacApInfo_t * vmacSta_master_p;
			    extStaDb_StaInfo_t * StaInfo_p; UINT8 index;
#ifndef SOC_W906X
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
#else
...
#endif
			    item_p = MUStaNode_p; StaInfo_p = item_p->StaInfo_p;
			    ...
			    ListPutItem(&vmacSta_master_p->StaCtl->
					FreeMUStaList,
...
*/
			    ...
			    return TRUE;
			    pStaInfo->mu_sta = 0;
			    pStaInfo->mu_index = 0; pStaInfo->MUset = NULL;
...
*/
			    ...
			    vmacApInfo_t * vmac_p =
			    (vmacApInfo_t *) wlpptr->vmacSta_p;
			    muset_t * muset_p = NULL; UINT32 free_MUUsr_cnt = 0;
			    MUCapStaNode_t * item_p, *free_sta_p;
			    free_MUUsr_cnt =
			    MUStaList->cnt - MUStaList->taken_musta_cnt;
			    if (free_MUUsr_cnt) {
			    ...
			    free_sta_p = NULL; item_p = (MUCapStaNode_t *) vmac_p->MUStaList[index].tail;	//get first item added to list from tail        
			    while (item_p != NULL) {
			    if (!item_p->MUSet_created_flag) {
			    free_sta_p = item_p; break;}
			    ...
			    while (muset_p != NULL) {
			    if (muset_p->cnt == 3)
			    break; if (muset_p->prv == NULL)
			    break; muset_p = muset_p->prv;}

			    /*Decide whether to break group */
			    if (free_sta_p != NULL) {
#ifdef SOC_W906X
			    if (((muset_p->antcnt == 8) && (muset_p->cnt == 2))
				|| ((muset_p->cnt == 2) &&
				    ((free_sta_p->StaInfo_p->vht_peer_RxNss +
				      muset_p->antcnt) > 8))) {
			    //((muset_p->antcnt==2) &&(muset_p->cnt==2) && (free_sta_p->StaInfo_p->vht_peer_RxNss==2))){
#else
			    if (((muset_p->antcnt == 3) && (muset_p->cnt == 2))
				|| ((muset_p->antcnt == 2) &&
				    (muset_p->cnt == 2) &&
				    (free_sta_p->StaInfo_p->vht_peer_RxNss ==
				     2))) {
#endif
			    *MUUsr_total = 0; return NULL;}
			    else {
			    return muset_p;}
			    }
			    }
				...
				((wlpptr->wlpd_p->MUSetList.total_sta % 2) !=
				 0)) {
			    muset_p =
			    (muset_t *) wlpptr->wlpd_p->MUSetList.tail;
			    ...
			    if (((muset_p->cnt + free_MUUsr_cnt) % 2) == 0) {
			    return muset_p;}
			    else {
			    *MUUsr_total = 2;}
			    }
...
*/
			    ...
			    *MUUsr_total = 0; return NULL;}
			     ...
			     */
			    if (wlpptr->wlpd_p->MUSetList.total_sta &&
				((wlpptr->wlpd_p->MUSetList.total_sta % 3) !=
				 0)) {
			    muset_p =
			    (muset_t *) wlpptr->wlpd_p->MUSetList.tail;
			    ...
			    if (((muset_p->cnt + free_MUUsr_cnt) % 3) == 0) {
			    return muset_p;}
			    else {
...
*/
			    ...
			    muset_t * muset_p = NULL; UINT32 free_MUUsr_cnt = 0;
			    MUCapStaNode_t * item_p, *free_sta_p;
			    u32 mu_max_user; switch (wlpptr->devid) {
case SC4:
mu_max_user = 3; break; case SC5:
mu_max_user = 8; break; case SCBT:
mu_max_user = 4; break; default:
			    mu_max_user = 3;}
			    ...
			    *MUUsr_total = 0; return NULL;}
			    ...
			    if ((muset_p != NULL) &&
				(muset_p->cnt == (n_user - 1))) {
			    if (((muset_p->cnt + free_MUUsr_cnt) % n_user) == 0)
			    return muset_p;
			    else {
			    if ((free_MUUsr_cnt % n_user) == 0)
			    * MUUsr_total = n_user;
			    else
			    *MUUsr_total = n_user - 1;}
			    }
			    else {
			    if ((free_MUUsr_cnt % n_user) == 0)
			    * MUUsr_total = n_user;
			    else {
			    if (free_MUUsr_cnt > (n_user + 1))
...
/*Function to form MU Set automatically*/
			    ...
			    vmacApInfo_t * vmacSta_master_p;
			    MUCapStaNode_t * item_p; muset_t * del_muset[8] = {
			    NULL}; UINT8 del_muset_cnt = 0;
			    ...
			    MUPrefer2Usr(netdev, &vmac_p->MUStaList[i],
					 &MUUsr_total, i);
			    else {
			    if ((i < 4) && (vmac_p->MUSet_Prefer_UsrCnt > 4))
			    del_muset[del_muset_cnt] =
			    MUPreferNUsr(netdev, &vmac_p->MUStaList[i],
					 &MUUsr_total, i, 4);
			    else
			    del_muset[del_muset_cnt] =
			    MUPreferNUsr(netdev, &vmac_p->MUStaList[i],
					 &MUUsr_total, i,
					 vmac_p->MUSet_Prefer_UsrCnt);}
			    ...
			    MUUsr[k] = NULL; item_p = (MUCapStaNode_t *) vmac_p->MUStaList[i].tail;	//get first item added to list from tail
			    while ((item_p != NULL) &&
				   (item_p->StaInfo_p != NULL)) {
#ifdef SOC_W906X
			    if (!item_p->MUSet_created_flag &&
				...
				(item_p->StaInfo_p->rx_packets == 0)) {
			    // Postpone the MU grouping after ampdu==1 && rx_cnt > 0
			    // Ref: cl#47513/47514                                  
			    item_p = item_p->prv; continue;}
#endif
			    if (MUUsr_cnt == 0) {
			    nooftxantenna += item_p->StaInfo_p->vht_peer_RxNss; MUUsr[MUUsr_cnt] = item_p->StaInfo_p;	//save pointer to sta for MU grouping decision later
			    MUUsr_cnt++;}
			    else {
#ifdef SOC_W906X
			    if ((nooftxantenna +
				 item_p->StaInfo_p->vht_peer_RxNss) <= 8) {
#else
			    if ((nooftxantenna +
				 item_p->StaInfo_p->vht_peer_RxNss) <= 3) {
#endif
			    nooftxantenna += item_p->StaInfo_p->vht_peer_RxNss;
			    MUUsr[MUUsr_cnt] = item_p->StaInfo_p; MUUsr_cnt++;}
			    }
			    }
#ifdef SOC_W906X
			    if (nooftxantenna >= 8)
			    ...
			    if (nooftxantenna >= 3)
#endif
			    break; if (MUUsr_cnt >= MUUsr_total)
			    break; item_p = item_p->prv;}
#ifdef SOC_W906X
			    SPIN_UNLOCK_IRQRESTORE(&vmac_p->MUStaListLock,
						   MUlistflags);
#endif
...
#endif
			    vmac_p->MUStaList[i].taken_musta_cnt += MUUsr_cnt;
			    /*Mark stations used to create MU group */
			    cnt = MUUsr_cnt; while (cnt) {
			    if (MUUsr[cnt - 1] != NULL) {
			    if (MUUsr[cnt - 1]->MUStaListNode != NULL)
			    MUUsr[cnt - 1]->MUStaListNode->MUSet_created_flag =
			    1;}
...
/*Function to form MU Set manually*/
			    ...
			    unsigned long MUSetflags;
#endif
			    UINT8 nooftxantenna = 0; if (vmac_p->master)
			    vmacSta_master_p = vmac_p->master;
			    else
			    vmacSta_master_p = vmac_p;
			    if (!vmacSta_master_p->StaCtl->Initialized) {
			    ...
			    nooftxantenna += MUUsr[i]->vht_peer_RxNss;}
			    }
			    ...
			    printk
			    ("FAIL to create MU group, no. of usr must be >=2 && total txantenna <=3\n");}
			    else
			    printk
			    ("FAIL to create MU group, no. of available usr < 2\n");
			    //SPIN_UNLOCK_IRQRESTORE(&vmacSta_master_p->StaCtl->dbLock, dbflags);
#ifndef SOC_W906X
			    SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->
						   MUSetListLock, MUSetflags);
...
* IMPORTANT: Make sure caller to this function has locked MUStaListLock and MUSetflags
*/
			    BOOLEAN MUDel_MUSet(vmacApInfo_t * vmac_p,
						muset_t * muset_p,
						UINT8 option) {
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
			    muset_t * delnode; UINT8 i;
			    //extStaDb_StaInfo_t *StaInfo_p;
#ifdef SOC_W906X
			    unsigned long MUSetflags; if (muset_p != NULL) {
			    ...
			    MURemStaFromMUSet(vmac_p, muset_p->StaInfo[i],
					      MUSta_NO_BLOCK);
			    else
			    MURemStaFromMUSet(vmac_p, muset_p->StaInfo[i], MUSta_BLOCK); muset_p->StaInfo[i] = NULL;	//set to NULL only at the end after all been reset
			    }
			    }

			    delnode =
			    (muset_t *) ListRmvItem((List *) & wlpptr->wlpd_p->
						    MUSetList.head,
						    (ListItem *) muset_p);
			    if (option & MUSet_BLOCK)
			    SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->
			    ...
			    vmacApInfo_t * vmacSta_master_p; if (vmac_p->master)
			    vmacSta_master_p = vmac_p->master;
			    else
			    vmacSta_master_p = vmac_p;
			    /*Remove stations that are tied to this set */
			    for (i = 0; i < 3; i++) {
			    if (muset_p->StaInfo[i] != NULL) {
			    if ((option & MUSet_NO_GID_FRAME) == 0) {
			    BUG_ON(SPIN_LOCK_IS_LOCKED_BY_SAME_CORE
				   (&vmacSta_master_p->StaCtl->dbLock));
			    SendGroupIDMgmtframe(vmac_p,
						 muset_p->StaInfo[i]->Addr, 0,
						 0);}

			    MURemStaFromMUSet(vmac_p, muset_p->StaInfo[i]); muset_p->StaInfo[i] = NULL;	//set to NULL only at the end after all been reset
			    }
			    }
			    ...
			    if (wlpptr->wlpd_p->MUSetList.total_sta >=
				delnode->cnt)
			    wlpptr->wlpd_p->MUSetList.total_sta -= delnode->cnt;
			    wl_kfree(delnode); return TRUE;}
			    }
			    else {
...
/*Function to delete a MU set by using index*/
			    void MUDel_MUSetIndex(vmacApInfo_t * vmac_p,
						  UINT8 muset_index) {
			    struct wlprivate * wlpptr =
			    NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
			    muset_t * tempnode, *delnode, *foundnode = NULL;
			    UINT8 i; unsigned long MUSetflags;
#ifndef SOC_W906X
			    unsigned long MUlistflags;
#endif
			    ...
			    if (tempnode->StaInfo[i] != NULL) {
#ifdef SOC_W906X
			    MURemStaFromMUSet(vmac_p, tempnode->StaInfo[i],
					      MUSta_BLOCK);
#else
			    BUG_ON(SPIN_LOCK_IS_LOCKED_BY_SAME_CORE
				   (&vmacSta_master_p->StaCtl->dbLock));
			    SendGroupIDMgmtframe(vmac_p,
						 tempnode->StaInfo[i]->Addr, 0,
						 0);
			    MURemStaFromMUSet(vmac_p, tempnode->StaInfo[i]);
#endif
...
/*Function to display all MU capable stations, whether MU Set is created or not,  in MUStaList*/
			    ...
			    MUCapStaNode_t * item_p = NULL; UINT8 i, bw[4] = {
			    20, 40, 80, 160};
			    extStaDb_StaInfo_t * pStaInfo;
			    unsigned long MUlistflags;
			    /* To avoid SMP deadlocks be sure the locking order is:
			    ...
			    printk
			    ("MUStaList: %s, %dMHz, total cnt:%d, isgrouped cnt:%d\n\n",
			     (item_p->MUStaList_idx > 3) ? "HE" : "VHT",
			     bw[item_p->MUStaList_idx % 4],
			     vmac_p->MUStaList[i].cnt,
			     vmac_p->MUStaList[i].taken_musta_cnt);
			    while (item_p != NULL) {
			    if (item_p->StaInfo_p != NULL) {
			    UINT32 * capval;
			    pStaInfo = item_p->StaInfo_p;
			    capval = (UINT32 *) & pStaInfo->vhtCap.cap;
			    printk("Mac address = %s \n",
				   mac_display(&pStaInfo->Addr[0]));
			    printk("Vhtcap=%x ", *capval);
			    printk
			    ("MUBeamformeeCapable=%x VHTBW=%dMHz RxNss=%x\n",
			     pStaInfo->vhtCap.cap.MUBeamformeeCapable,
			     bw[pStaInfo->vht_RxChannelWidth],
			     pStaInfo->vht_peer_RxNss);
			    printk
			    ("staindx=%d mu_sta_flag=%d mu_created=%d, mu_index=%d\n",
			     pStaInfo->StnId, pStaInfo->mu_sta,
			     item_p->MUSet_created_flag, pStaInfo->mu_index);
			    printk
			    ("---------------------------------------------------\n\n");}
			    item_p = item_p->prv;}
...
/*Function to display all MU sets that have been created*/
			    ...
			    struct wlprivate * wlpptr = NETDEV_PRIV_P(struct wlprivate, vmac_p->dev); muset_t * muset; UINT8 i; unsigned long MUSetflags; SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->MUSetListLock, MUSetflags); muset = (muset_t *) (wlpptr->wlpd_p->MUSetList.tail);	//start with first item added into list, new node is added to head
			    ...
			    printk("MU set index: %d, GID:%d\n", muset->index,
				   (muset->index + 1)); for (i = 0;
							     i < MU_MAX_USERS;
							     i++) {
			    if (muset->StaInfo[i] != NULL) {
			    printk("sta %d:", i);
			    printk(" Mac address = %s \n",
				   mac_display(&muset->StaInfo[i]->Addr[0]));}
...
#ifdef AP_STEERING_SUPPORT
			    void macMgmtMlme_AssocDenied(IEEEtypes_StatusCode_t
							 statuscode) {
			    ...
			    extern int wlFwGetStaStats(struct net_device
						       *netdev, int staId,
						       SMAC_STA_STATISTICS_st *
						       sta_stats);
			    void fillStaInfo(struct net_device *netdev,
					     struct station_info *pSinfo,
					     struct extStaDb_StaInfo_t
					     *pStaInfo) {
			    SMAC_STA_STATISTICS_st StaStatsTbl;
			    dbRateInfo_t RateInfo; s8 rssi;
			    if (!netdev || !pSinfo || !pStaInfo)
			    return;
			    rssi =
			    pStaInfo->RSSI ? -(pStaInfo->RSSI) : -(pStaInfo->
								   assocRSSI);
			    pSinfo->signal = rssi; pSinfo->signal_avg = rssi;
			    ...
			    memcpy(&RateInfo,
				   &(pStaInfo->rx_info_aux.rate_info),
				   sizeof(dbRateInfo_t));
			    ...
			    u32 len = 0;
			    htCap =
			    (struct IEEEtypes_HT_Element_t *)pSinfo->
			    assoc_req_ies;
			    len += sizeof(IEEEtypes_HT_Element_t);
			    vhtCap =
			    (struct IEEEtypes_VhtCap_t *)(pSinfo->
							  assoc_req_ies + len);
			    len += sizeof(IEEEtypes_VhtCap_t);
			    heCap =
			    (struct HE_Capabilities_IE_t *)(pSinfo->
							    assoc_req_ies +
							    len);
			    ...
			    void fillApCapInfo(struct net_device *netdev,
					       int mode, u8 * pApCap,
					       u32 * pLen) {
			    ...
			    extern UINT16 Build_IE_191(vmacApInfo_t * vmacSta_p,
						       UINT8 * IE_p);
			    extern UINT16 Build_IE_HE_CAP(vmacApInfo_t *
							  vmacSta_p,
							  UINT8 * IE_p);
...
case 0:
			    //HT
AddHT_IE(vmacSta_p, (IEEEtypes_HT_Element_t *) pApCap); *pLen = sizeof(IEEEtypes_HT_Element_t); break; case 1:
			    //VHT
Build_IE_191(vmacSta_p, pApCap); *pLen = sizeof(IEEEtypes_VhtCap_t); break; case 2:
			    //HE
Build_IE_HE_CAP(vmacSta_p, pApCap); *pLen = sizeof(HE_Capabilities_IE_t); break; default:
			    break;}
			    }

			    void fillApRadioInfo(struct net_device *netdev,
						 struct
						 ap_radio_basic_capa_rpt_t
						 *pRpt) {
			    extern void getOpClass(op_class_info_t * pOpClass);
			    struct wlprivate * wlpriv =
			    NETDEV_PRIV_P(struct wlprivate, netdev);
			    vmacApInfo_t * vmacSta_p = wlpriv->vmacSta_p;
			    op_class_info_t * pOpc = NULL; u8 i = 0;
			    if (!netdev || !pRpt)
			    return;
			    memset(pRpt, 0,
				   sizeof(struct ap_radio_basic_capa_rpt_t));
			    pRpt->maxNumBss = wlpriv->wlpd_p->NumOfAPs;
			    memcpy(&pRpt->ruid, &vmacSta_p->VMacEntry.vmacAddr,
				   IEEEtypes_ADDRESS_SIZE);
			    pOpc =
			    wl_kmalloc(sizeof(op_class_info_t), GFP_ATOMIC);
			    if (!pOpc)
			    return; memset(pOpc, 0, sizeof(op_class_info_t));
			    getOpClass(pOpc); for (i = 0;
						   i < pOpc->op_class_nums;
						   i++) {
			    ...
			    pRpt->num_of_operating_class =
			    pOpc->op_class_nums - i;
			    memcpy(&pRpt->operatingClass,
				   &pOpc->op_class_tab[i],
				   sizeof(op_class_tab_t) *
				   (pOpc->op_class_nums - i));}
