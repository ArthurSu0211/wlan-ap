 ...
 *=============================================================================
*/
#define NBREPORT_SEPARATE_SCAN_INTERVEL     (300 * TIMER_1MS)
#define NBREPORT_SCAN_INTERVEL              (600)
...
static UINT32 L_offchan_scan_time = 60;	// default is 60ms
		  ...
		  struct neighbor_list_entrie_t *nlist)
{
	...
	if ((nlist->time_stamp - nlist_entry[nlist_idx].time_stamp) <
...
nlist_scan(struct net_device *netdev)
{
	...
	offchan.dwell_time = L_offchan_scan_time;
...
nlist_scan(struct net_device *netdev)
{
	...
	if (smeSendScanRequest
	    (vmacEntry_p->phyHwMacIndx, SCAN_PASSIVE, 3, L_offchan_scan_time,
	     &bcAddr1[0], &ieBuf[0], ieBufLen) == MLME_SUCCESS) {
...
neighbor_report_scan_cb(UINT8 * data)
{
	...
	if (ret == -EPERM) {
		...
		TimerFireInByJiffies(&priv->vmacSta_p->RRM_ScanTimer, 1,
				     ...
				     100 * NBREPORT_SEPARATE_SCAN_INTERVEL);
	} else if (ret == -EBUSY) {
		/* scan busy, wait 300 + offchan_time ms to scan */
		TimerFireInByJiffies(&priv->vmacSta_p->RRM_ScanTimer, 1,
				     &neighbor_report_scan_cb, (UINT8 *) netdev,
				     L_offchan_scan_time +
				     NBREPORT_SEPARATE_SCAN_INTERVEL);
	} else if (priv->vmacSta_p->SepareChanIdx == 0) {
		/* after 60 sec to scan */
		TimerFireIn(&priv->vmacSta_p->RRM_ScanTimer, 1,
			    &neighbor_report_scan_cb, (UINT8 *) netdev,
			    NBREPORT_SCAN_INTERVEL);
	} else {
		/* after 300 ms + offchan_time to scan */
		TimerFireInByJiffies(&priv->vmacSta_p->RRM_ScanTimer, 1,
				     &neighbor_report_scan_cb, (UINT8 *) netdev,
				     L_offchan_scan_time +
				     NBREPORT_SEPARATE_SCAN_INTERVEL);
		      ...
		      RssiPathInfo_t * prssiPaths, UINT8 scan_path)
{
	...
	nf = (SINT32) wl_util_get_nf(netdev, &wlpptr->wlpd_p->NF_path, NULL);
	/* nf is signed value */

	if (nf != 0 && (rssi_avg - nf) <= 0) {
		/* weak signal */
		return;
	}
...
MSAN_update_neighbor_list(struct net_device *netdev)
{
	...
	if (vmacSta_p->SepareChanIdx == 0) {
		...
		/* after 60 sec to scan */
		TimerFireIn(&vmacSta_p->RRM_ScanTimer, 1,
			    &neighbor_report_scan_cb, (UINT8 *) mnetdev,
			    NBREPORT_SCAN_INTERVEL);
	} else {
		/* after 300 ms to scan */
...
void
MSAN_set_offchan_time(UINT32 time_val)
{
	L_offchan_scan_time = time_val;
}

UINT32
MSAN_get_offchan_time(void)
{
	return L_offchan_scan_time;
}
...
Enable_MSAN_timer(struct net_device *netdev)
{
	...
	TimerFireInByJiffies(&vmacSta_p->RRM_ScanTimer, 1,
			     &neighbor_report_scan_cb, (UINT8 *) netdev,
			     NBREPORT_SEPARATE_SCAN_INTERVEL * 30);
