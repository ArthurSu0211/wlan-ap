...
void
set_dfs_status(DfsAp * dfs_ap, DFS_STATE state)
{
...
BOOLEAN
cac_not_required(DfsChanInfo * chan_info)
{
	return chan_info->no_cac;
}

BOOLEAN
do_csa(DfsChanInfo * chan_info)
{
	return chan_info->do_csa;
}

BOOLEAN
have_dfs_scnr(struct wlprivate * wlpptr)
{
...
**************************************************************************/
MhsmEvent_t const *
DfsAp_top(DfsAp * me, MhsmEvent_t * msg)
{
...
**************************************************************************/
MhsmEvent_t const *
Dfs_Ap_Handle(DfsAp * me, MhsmEvent_t * msg)
{
...
**************************************************************************/
MhsmEvent_t const *
Dfs_Init_Handle(DfsAp * me, MhsmEvent_t * msg)
{
	...
	case MHSM_ENTER:
		...
		dfsTrace(dev, DFS_STATE_INIT, (MHSM_ENTER & 0xf),
			 &dfsDesc_p->currChanInfo, 0);
		return 0;
	case CHANNEL_CHANGE_EVT:
		{
			DfsChanInfo *chanInfo_p =
				(DfsChanInfo *) dfsMsg_p->mgtMsg;
			PRINT1(INFO,
			       "Dfs_Init_Handle:: event-> CHANNEL_CHANGE_EVT\n");

			if (chanInfo_p == NULL) {
				dfsTrace(dev, DFS_STATE_INIT,
					 CHANNEL_CHANGE_EVT,
					 &dfsDesc_p->currChanInfo, 0);
				return 0;
			}
			memcpy(&dfsDesc_p->currChanInfo, chanInfo_p,
			       sizeof(DfsChanInfo));
			/* First check if the given channel is within DFS range */
#ifdef CONCURRENT_DFS_SUPPORT
			if (DfsEnabledChannel
			    (me->pNetDev, &dfsDesc_p->currChanInfo) &&
			    !cac_not_required(&dfsDesc_p->currChanInfo)) {
#else
			if (DfsEnabledChannel
			    (me->pNetDev, &dfsDesc_p->currChanInfo)) {
#endif
				...
				dfsTrace(dev, DFS_STATE_INIT,
					 CHANNEL_CHANGE_EVT,
					 &dfsDesc_p->currChanInfo, 0x10000000);
...
#ifdef SOC_W906X
				mib->PhyDSSSTable->Chanflag.isDfsChan =
					dfsDesc_p->currChanInfo.chanflag.
					isDfsChan;
				mib->PhyDSSSTable->Chanflag.isDfsChan2 =
					dfsDesc_p->currChanInfo.chanflag.
					isDfsChan2;
#endif
				macMgmtMlme_StartRadarDetection(dev,
								DFS_QUIET_MODE);
				...
				dfsTrace(dev, DFS_STATE_INIT,
					 CHANNEL_CHANGE_EVT,
					 &dfsDesc_p->currChanInfo, 0x11000000);

				WLDBG_INFO(DBG_LEVEL_1,
					   "Dfs_Init_Handle  Become Operational \n");
				/* Switch off radar detection */
				macMgmtMlme_StopRadarDetection(dev,
							       DFS_NORMAL_MODE);
				/* Become operational */
				mhsm_transition(&me->super,
						&me->Dfs_Operational);
			}
			return 0;
		}
...
UINT8
useEtsiTimerValue(CHNL_FLAGS * pChanflag, UINT8 channel, UINT8 channel2)
{
#if 0
	if ((((pChanflag->ChnlWidth == CH_160_MHz_WIDTH) ||
	      (pChanflag->ChnlWidth == CH_AUTO_WIDTH)) && (channel >= 100 &&
							   channel <= 128)) ||
	    ((pChanflag->ChnlWidth == CH_80_MHz_WIDTH) &&
	     (channel >= 116 && channel <= 128)) ||
	    ((pChanflag->ChnlWidth == CH_40_MHz_WIDTH) &&
	     (channel >= 116 && channel <= 128)) ||
	    ((pChanflag->ChnlWidth == CH_20_MHz_WIDTH) &&
	     (channel >= 120 && channel <= 128)))
		return TRUE;
#else
	if (pChanflag->radiomode == RADIO_MODE_80p80) {
		if ((channel >= 116 && channel <= 128) ||
		    (channel2 >= 116 && channel2 <= 128))
			return TRUE;
	} else if (((pChanflag->ChnlWidth == CH_160_MHz_WIDTH) ||
		    (pChanflag->ChnlWidth == CH_AUTO_WIDTH)) && (channel >= 100
								 && channel <=
								 128))
		return TRUE;
	else if ((pChanflag->ChnlWidth == CH_80_MHz_WIDTH) &&
		 (channel >= 116 && channel <= 128))
		return TRUE;
	else if ((pChanflag->ChnlWidth == CH_40_MHz_WIDTH) &&
		 (channel >= 116 && channel <= 128))
		return TRUE;
	else if ((pChanflag->ChnlWidth == CH_20_MHz_WIDTH) &&
		 (channel >= 120 && channel <= 128))
		return TRUE;
#endif
	return FALSE;
...
UINT16
getCacTimerValue(UINT16 regionCode, UINT16 cacTimeout, UINT16 etsiTimeout,
		 CHNL_FLAGS * pChanflag, UINT8 channel, UINT8 channel2)
{
	if (regionCode == DOMAIN_CODE_ETSI) {
...
**************************************************************************/
MhsmEvent_t const *
Dfs_Scan_Handle(DfsAp * me, MhsmEvent_t * msg)
{
	UINT8 channel;
	DfsApMsg *dfsMsg_p = NULL;
	DfsApDesc *dfsDesc_p = NULL;
	...
	case MHSM_ENTER:
		...
		dfsTrace(dev, DFS_STATE_SCAN, (MHSM_ENTER & 0xf),
			 &dfsDesc_p->currChanInfo, 0x10000000);
		return 0;
	case RADAR_EVT:
		...
		if (!dfs_probability) {
			if (dfs_monitor)
				return 0;
#ifdef CONCURRENT_DFS_SUPPORT
			if (UpdateNOL(dfsDesc_p, 0) == DFS_FAILURE)
#else
			if (UpdateNOL(dfsDesc_p) == DFS_FAILURE)
#endif /* CONCURRENT_DFS_SUPPORT */
			{
				dfsTrace(dev, DFS_STATE_SCAN, RADAR_EVT,
					 &dfsDesc_p->currChanInfo, 0x11000000);
				return 0;
			}
		}
		...
		if (have_dfs_scnr(wlpptr)) {
			dfsTrace(dev, DFS_STATE_SCAN, RADAR_EVT,
				 &dfsDesc_p->currChanInfo, 0x20000000);
			macMgmtMlme_StopRadarDetection(dev, DFS_NORMAL_MODE);
#ifdef CONCURRENT_DFS_SUPPORT
			me->scnr_ctl_evt(dev, ScnrCtl_Radar_Detected,
					 DFS_STATE_SCAN, 0);
#else
			me->scnr_send_evt(dev, ScnrCtl_Radar_Detected,
					  DFS_STATE_SCAN);
#endif /* CONCURRENT_DFS_SUPPORT */
...
#ifdef CONCURRENT_DFS_SUPPORT
			channel =
				DfsDecideNewTargetChannel(dev, dfsDesc_p, FALSE,
							  FALSE);
#else
			channel =
				DfsDecideNewTargetChannel(dev, dfsDesc_p,
							  FALSE);
#endif /* CONCURRENT_DFS_SUPPORT */
			if (channel == 0) {
				PRINT1(INFO,
				       "No target channel found to switch to\n");
				dfsTrace(dev, DFS_STATE_SCAN, RADAR_EVT,
					 &dfsDesc_p->currChanInfo, 0x12000000);
				return 0;
			}
		}

		dfsTrace(dev, DFS_STATE_SCAN, RADAR_EVT,
			 &dfsDesc_p->currChanInfo, 0x13000000 | dfs_monitor);

#ifdef 	BARBADOS_DFS_TEST
		if (!dfs_monitor)
			macMgmtMlme_SwitchChannel(dev,
		...
		if (DfsEnabledChannel(me->pNetDev, &dfsDesc_p->currChanInfo)) {
			...
			mib_CACTimeOut =
				getCacTimerValue(*(mib->mib_regionCode),
						 *(mib->mib_CACTimeOut),
			...
			dfsTrace(dev, DFS_STATE_SCAN, RADAR_EVT,
				 &dfsDesc_p->currChanInfo, 0x15000000);

			mib->PhyDSSSTable->Chanflag.isDfsChan =
				dfsDesc_p->currChanInfo.chanflag.isDfsChan;
			mib->PhyDSSSTable->Chanflag.isDfsChan2 =
				dfsDesc_p->currChanInfo.chanflag.isDfsChan2;
			...
			/* Switch off radar detection */
			dfsTrace(dev, DFS_STATE_SCAN, RADAR_EVT,
				 &dfsDesc_p->currChanInfo, 0x16000000);
			macMgmtMlme_StopRadarDetection(dev, DFS_NORMAL_MODE);
			/* Restart data traffic */
			macMgmtMlme_RestartDataTraffic(dev);
			macMgmtMlme_Reset(dev, dfsDesc_p->vaplist,
					  &dfsDesc_p->vapcount);
	...
	case CAC_EXPIRY_EVT:
		...
		if (have_dfs_scnr(wlpptr)) {
#ifdef CONCURRENT_DFS_SUPPORT
			me->scnr_ctl_evt(dev, ScnrCtl_CAC_Done, DFS_STATE_SCAN,
					 0);
#else
			me->scnr_send_evt(dev, ScnrCtl_CAC_Done,
					  DFS_STATE_SCAN);
#endif /* CONCURRENT_DFS_SUPPORT */
		}
#endif
		dfsTrace(dev, DFS_STATE_SCAN, CAC_EXPIRY_EVT,
			 &dfsDesc_p->currChanInfo, 0);
		/* Restart data traffic */
		macMgmtMlme_RestartDataTraffic(dev);
	...
	case WL_RESET_EVT:
		...
		dfsTrace(dev, DFS_STATE_SCAN, WL_RESET_EVT,
			 &dfsDesc_p->currChanInfo,
			 0x10000000 | dfsDesc_p->cac_complete);

		if (dfsDesc_p->cac_complete > 0) {
			dfsDesc_p->currState = DFS_STATE_OPERATIONAL;
			macMgmtMlme_MBSS_Reset(dev, dfsDesc_p->vaplist,
					       (dfsDesc_p->vapcount <
						bss_num) ? dfsDesc_p->
					       vapcount : bss_num);
	...
	case CHANNEL_CHANGE_EVT:
		{
			...
			PRINT1(INFO,
			       "Dfs_Scan_Handle:: event-> CHANNEL_CHANGE_EVT\n");
			...
			if (DfsEnabledChannel
				...
				if (dfs_test) {
					if (!dfs_probability) {
						/* Restart data traffic */
						dfsTrace(dev, DFS_STATE_SCAN,
							 CHANNEL_CHANGE_EVT,
							 &dfsDesc_p->
							 currChanInfo,
							 0x10000000);
						macMgmtMlme_RestartDataTraffic
							(dev);
						macMgmtMlme_Reset(dev,
								  dfsDesc_p->
								  vaplist,
				...
				if (have_dfs_scnr(wlpptr)) {
					...
					if (cac_not_required(chanInfo_p)) {
						...
						macMgmtMlme_StartRadarDetection
							(dev, DFS_QUIET_MODE);
					}
				}
#endif
...
**************************************************************************/
MhsmEvent_t const *
Dfs_Operational_Handle(DfsAp * me, MhsmEvent_t * msg)
{
	UINT8 channel;
	...
	switch (msg->event) {
	case MHSM_ENTER:
		dfsTrace(dev, DFS_STATE_OPERATIONAL, (MHSM_ENTER & 0xf),
			 &dfsDesc_p->currChanInfo, 0);
		dfsDesc_p->currState = DFS_STATE_OPERATIONAL;
#ifdef RADAR_SCANNER_SUPPORT
		//Pete, to notify app ch become Operational
#ifdef CONCURRENT_DFS_SUPPORT
		me->scnr_ctl_evt(dev, ScnrCtl_Chan_Operational,
				 DFS_STATE_OPERATIONAL, 0);
#else
		me->scnr_send_evt(dev, ScnrCtl_Chan_Operational,
	...
	case CHANNEL_CHANGE_EVT:
		PRINT1(INFO,
		       "Dfs_Operational_Handle:: event-> POST_CHANNEL_CHANGE_EVT\n");
		chanInfo_p = (DfsChanInfo *) dfsMsg->mgtMsg;
		if (chanInfo_p == NULL) {
			PRINT1(INFO, "NULL ChanInfo received\n");
			dfsTrace(dev, DFS_STATE_OPERATIONAL, CHANNEL_CHANGE_EVT,
				 &dfsDesc_p->currChanInfo, 0x10000000);
			return 0;
		}
		...
		if (DfsEnabledChannel(me->pNetDev, chanInfo_p)) {
			if (dfs_test) {
				if (!dfs_probability) {
					memcpy(&dfsDesc_p->currChanInfo,
					       chanInfo_p, sizeof(DfsChanInfo));
					dfsTrace(dev, DFS_STATE_OPERATIONAL,
						 CHANNEL_CHANGE_EVT,
						 &dfsDesc_p->currChanInfo,
						 0x11000000);
					return 0;
			...
			if (have_dfs_scnr(wlpptr)) {	//Pete, how to know Channel change evt is from app?
				...
				if (do_csa(chanInfo_p)) {
					...
					chanSwitch.ChannelSwitchCmd.Mode =
						DFS_DEFAULT_CSAMODE;
					chanSwitch.ChannelSwitchCmd.
						ChannelNumber =
						chanInfo_p->channel;
					chanSwitch.ChannelSwitchCmd.
						ChannelSwitchCount =
						DFS_DEFAULT_COUNTDOWN_NUMBER;
					chanSwitch.chInfo =
						dfsDesc_p->currChanInfo;
			...
			dfsTrace(dev, DFS_STATE_OPERATIONAL, CHANNEL_CHANGE_EVT,
				 &dfsDesc_p->currChanInfo, 0x13000000);
			...
			if (have_dfs_scnr(wlpptr)) {
				dfsTrace(dev, DFS_STATE_OPERATIONAL,
					 CHANNEL_CHANGE_EVT,
					 &dfsDesc_p->currChanInfo, 0x23000000);
				macMgmtMlme_SwitchChannel(dev,
							  chanInfo_p->channel,
							  chanInfo_p->channel2,
							  &dfsDesc_p->
	...
	case RADAR_EVT:
		PRINT1(INFO,
		       "Dfs_Operational_Handle:: event-> RADAR_EVT on channel %d\n",
		...
		if (DfsEnabledChannel(me->pNetDev, chanInfo_p)) {
...
#ifdef CONCURRENT_DFS_SUPPORT
				if (dfsDesc_p->currChanInfo.chanflag.
				    ChnlWidth == CH_160_MHz_WIDTH) {
					if (UpdateNOL(dfsDesc_p, DFS_MAIN) ==
					    DFS_FAILURE) {
						dfsTrace(dev,
							 DFS_STATE_OPERATIONAL,
							 RADAR_EVT,
...
#endif /* CONCURRENT_DFS_SUPPORT */
				dfsTrace(dev, DFS_STATE_OPERATIONAL, RADAR_EVT,
					 &dfsDesc_p->currChanInfo, 0x14000000);
			}
#else
			if (!dfs_test_mode) {
...
#ifdef CONCURRENT_DFS_SUPPORT
			PRINT1(INFO,
			       "[1]New Target channel is : chan=%d chan2=%d isDfsChan=%d isDfsChan2=%d\n",
			       dfsDesc_p->currChanInfo.channel,
...
#endif
			dfsTrace(dev, DFS_STATE_OPERATIONAL, RADAR_EVT,
				 &dfsDesc_p->currChanInfo, 0x18000000);
			/* Insert Channel Switch Announcement IE in the beacon/probe-response
			 * and initiate countdown process */
			...
			/* Switch off radar detection */
			dfsTrace(dev, DFS_STATE_OPERATIONAL, RADAR_EVT,
				 &dfsDesc_p->currChanInfo, 0x19000000);
			macMgmtMlme_StopRadarDetection(dev, DFS_NORMAL_MODE);
		}
		return 0;
#ifdef RADAR_SCANNER_SUPPORT
	case WL_RESET_EVT:
		//Pete, to be reviewed. 
		dfsTrace(dev, DFS_STATE_OPERATIONAL, WL_RESET_EVT,
			 &dfsDesc_p->currChanInfo, 0x20000000);
		mib_op->PhyDSSSTable->Chanflag.isDfsChan =
			dfsDesc_p->currChanInfo.chanflag.isDfsChan;
		mib_op->PhyDSSSTable->Chanflag.isDfsChan2 =
			dfsDesc_p->currChanInfo.chanflag.isDfsChan2;

		if (mib->PhyDSSSTable->Chanflag.isDfsChan ||
		    mib->PhyDSSSTable->Chanflag.isDfsChan2) {
...
**************************************************************************/
MhsmEvent_t const *
Dfs_Csa_Handle(DfsAp * me, MhsmEvent_t * msg)
{
	DfsApMsg *dfsMsg = NULL;
	DfsApDesc *dfsDesc_p = NULL;
	...
	case MHSM_ENTER:
		...
		dfsTrace(dev, DFS_STATE_CSA, (MHSM_ENTER & 0xf),
			 &dfsDesc_p->currChanInfo, 0);

		dfsDesc_p->currState = DFS_STATE_CSA;
		/* Start DFS CSA timer */
		TimerFireIn(&dfsDesc_p->CSATimer, 1,
			    &CSATimeoutHandler, (unsigned char *)me,
			    dfsDesc_p->CSATimeOut);
		return 0;
	case CHANNEL_CHANGE_EVT:
		{
			if (!dfsMsg) {
				PRINT1(INFO,
				       "CSA::CHANNEL_CHANGE_EVT NULL dfsMsg\n");
				dfsTrace(dev, DFS_STATE_CSA, CHANNEL_CHANGE_EVT,
					 &dfsDesc_p->currChanInfo, 0x10000000);
				return 0;
			}
			chanInfo_p = (DfsChanInfo *) dfsMsg->mgtMsg;
			...
			if (DfsEnabledChannel
				...
				mib_CACTimeOut =
					getCacTimerValue(*(mib->mib_regionCode),
							 ...
							 *(mib->
							 ...
							 pChanflag,
							 dfsDesc_p->
							 currChanInfo.channel,
							 dfsDesc_p->
							 currChanInfo.channel2);
				...
				mib->PhyDSSSTable->Chanflag.isDfsChan2 =
					chanInfo_p->chanflag.isDfsChan2;
	...
	case CSA_EXPIRY_EVT:
		...
		dfsTrace(dev, DFS_STATE_CSA, CSA_EXPIRY_EVT,
			 &dfsDesc_p->currChanInfo, 0x10000000);
		/* Restart data traffic */
		macMgmtMlme_RestartDataTraffic(dev);
		macMgmtMlme_Reset(dev, dfsDesc_p->vaplist,
...
#ifdef RADAR_SCANNER_SUPPORT
	case WL_RESET_EVT:
		dfsTrace(dev, DFS_STATE_CSA, WL_RESET_EVT,
			 &dfsDesc_p->currChanInfo, 0x20000000);
...
**************************************************************************/
void
DFSApCtor(struct net_device *pNetDev, DfsAp * me)
{
...
DFS_STATUS
DfsEnabledChannel(struct net_device *pNetDev, DfsChanInfo * chanInfo_p)
{
	...
	if (chanInfo_p == NULL) {
		PRINT1(INFO,
		       "NULL ChanInfo passed to DfsEnabledChannel function\n");
		return DFS_FAILURE;
	}
	...
	for (i = 0;
	     i < domainGetSizeOfdfsEnabledChannels() / sizeof(DFS_CHANNEL_LIST);
	     i++) {
...
#ifdef CONCURRENT_DFS_SUPPORT
			if (chanInfo_p->channel2 &&
			    (chanInfo_p->chanflag.radiomode == RADIO_MODE_80p80)
			    && !chanInfo_p->chanflag.isDfsChan2) {
				if (chanInfo_p->channel2 ==
				    dfsEnabledChannels[i].dfschannelEntry[j]) {
					chanInfo_p->chanflag.isDfsChan2 = TRUE;
				}
			}
#endif
			if (chanInfo_p->channel &&
			    !chanInfo_p->chanflag.isDfsChan &&
			    (chanInfo_p->channel ==
			     dfsEnabledChannels[i].dfschannelEntry[j])) {
	...
	if (chanInfo_p->chanflag.radiomode == RADIO_MODE_80p80) {
		...
		for (i = 0;
			...
			if (chanInfo_p->channel &&
				...
				if (channel_exists
				    (chanInfo_p->channel,
				     GrpChList80Mhz[i].channelEntry, 4)) {
					//Is this DFS group?
					for (j = 0; j < DFS_MAX_CHANNELS; j++) {
						channel =
							dfsEnabledChannels
...
#endif
	if (chanInfo_p->chanflag.ChnlWidth == CH_160_MHz_WIDTH ||
		    chanInfo_p->chanflag.ChnlWidth == CH_AUTO_WIDTH) {
		for (i = 0;
		     i <
			...
			if (channel_exists
			    (chanInfo_p->channel,
			     GrpChList160Mhz[i].channelEntry, 8)) {
				//Is this DFS group? 
				for (j = 0; j < DFS_MAX_CHANNELS; j++) {
					channel =
						dfsEnabledChannels
		...
		if (channel_exists
		    (chanInfo_p->channel, GrpChList40Mhz[i].channelEntry, 2)) {
			//Is this DFS group?
			for (j = 0; j < DFS_MAX_CHANNELS; j++) {
				channel =
					dfsEnabledChannels[dfsEnabledEntry].
					dfschannelEntry[j];
				if (channel_exists
				    (channel, GrpChList40Mhz[i].channelEntry,
				     2))
					return DFS_SUCCESS;
			}
		}
...
DFS_STATUS
DfsAddToNOL(DfsApDesc * dfsDesc_p, UINT8 channel, CHNL_FLAGS chanflag,
	    unsigned long occurance)
{
	...
	if ((nocListItem_p =
	     wl_kmalloc(sizeof(NOCListItem), GFP_ATOMIC)) == NULL) {
...
DFS_STATUS
DfsRemoveFromNOL(DfsApDesc * dfsDesc_p)
{
...
next:
	...
	if (nocListItem_p) {
		timeoutPeriod =
			dfsDesc_p->NOCTimeOut -
			((jiffies - nocListItem_p->occurance) / 10);
		if (timeoutPeriod <= 0)
			TimerFireIn(&dfsDesc_p->NOCTimer, 1, &NOCTimeoutHandler,
				    (unsigned char *)dfsDesc_p, 1);
		else
			TimerFireIn(&dfsDesc_p->NOCTimer, 1, &NOCTimeoutHandler,
				    (unsigned char *)dfsDesc_p, timeoutPeriod);
		/* Put it back */
		ListPutItemFILO(&dfsDesc_p->NOCList,
				(ListItem *) nocListItem_p);
	}
	return DFS_SUCCESS;
}
...
#ifdef CONCURRENT_DFS_SUPPORT
DFS_STATUS
UpdateNOL(DfsApDesc * dfsDesc_p, UINT8 IsFromAux)
#else
DFS_STATUS
UpdateNOL(DfsApDesc * dfsDesc_p)
#endif				/* CONCURRENT_DFS_SUPPORT */
{
	UINT8 domainCode;
	int i = 0;
...
#ifdef CONCURRENT_DFS_SUPPORT
	if (IsFromAux) {
		if (dfsDesc_p->currChanInfo.chanflag.radiomode ==
		    RADIO_MODE_80p80) {
	...
	if (bw == CH_40_MHz_WIDTH) {
		...
		for (i = 0; i < 4; i++) {
			if (DfsDetected80MhzGrpChan[i] != 0)
				DfsAddToNOL(dfsDesc_p,
					    DfsDetected80MhzGrpChan[i], cf,
					    occurance);
			else
				PRINT1(INFO,
				       "ERROR: BW 80Mhz channel is ZERO \n");
		}
		...
		for (i = 0; i < 8; i++) {
			if (DfsDetected160MhzGrpChan[i] != 0) {
				if (channel_exists
				    (DfsDetected160MhzGrpChan[i],
				     dfsEnabledChannels[domainInd].
				     dfschannelEntry, DFS_MAX_CHANNELS))
					DfsAddToNOL(dfsDesc_p,
						    DfsDetected160MhzGrpChan[i],
						    cf, occurance);
			} else {
				PRINT1(INFO,
				       "ERROR: BW 160Mhz channel is ZERO \n");
			}
		}
	} else {
...
void
GetDfs80MhzGrpChan(UINT8 domainInd, UINT8 channel, UINT8 * GrpChan)
{
...
void
DfsGetNOLChannelList(List * nocList_p, UINT8 * channel)
{
...
void
Get80MhzNOLchanList(UINT8 domainInd)
{
	UINT8 *ptr1, *ptr2;
...
void
Get160MhzNOLchanList(UINT8 domainInd)
{
	UINT8 *ptr1, *ptr2;
...
#ifdef CONCURRENT_DFS_SUPPORT
void
prepareNewTargetChannels(DfsApDesc * dfsDesc_p, u8 channel, U8 IsFromAux)
{
	...
	if (dfsDesc_p->currChanInfo.chanflag.ChnlWidth == CH_160_MHz_WIDTH) {
		...
		dfsDesc_p->currChanInfo.channel = channel;
		dfsDesc_p->currChanInfo.channel2 =
			GetSecondChannel(channel, CH_160_MHz_WIDTH);
...
UINT8
DfsDecideNewTargetChannel(struct net_device *dev, DfsApDesc * dfsDesc_p,
...
#endif				/* CONCURRENT_DFS_SUPPORT */
{
	...
	} else if (dfsDesc_p->currChanInfo.chanflag.radiomode ==
		   RADIO_MODE_80p80) {
	...
	if ((bw == CH_80_MHz_WIDTH) ||
	    (bw == CH_40_MHz_WIDTH) || (bw == CH_20_MHz_WIDTH)) {

		if (bw == CH_80_MHz_WIDTH)
			GetDfs80MhzGrpChan(domainInd, ch,
					   DfsDetected80MhzGrpChan);

		for (j = 0; j < DFS_MAX_CHANNELS; j++) {
			if (dfsEnabledChannels[domainInd].dfschannelEntry[j] ==
			    0)
				continue;
			if (dfsEnabledChannels[domainInd].dfschannelEntry[j] ==
			    ch)
			...
			if ((dfsDesc_p->currChanInfo.chanflag.radiomode ==
			     RADIO_MODE_7x7p1x1) ||
			    (dfsDesc_p->currChanInfo.chanflag.radiomode ==
			     RADIO_MODE_80p80)) {
				if (IsTheSameGroup
				    (ch,
				     dfsEnabledChannels[domainInd].
				     dfschannelEntry[j], bw)) {
					continue;
				}
				if (IsTheSameGroup
				    (ch_another_path,
			...
			if (bw == CH_40_MHz_WIDTH) {
				...
				extChan =
					...
					(domainInd, extChanOffset, testchannel);
				if (!extChan)
					continue;
				if (DfsFindInNOL
				    (&dfsDesc_p->NOCList,
				     testchannel) == DFS_SUCCESS)
					continue;
				if (IsTestChannelHiddenInNOL_40MHz
				    (dfsDesc_p, testchannel) == DFS_FAILURE)
				...
				if (!dfs_probability)
					if (channel_exists
					    (testchannel,
					     DfsDetected80MhzGrpChan, 4))
						continue;
				if (DfsFindInNOL
				    (&dfsDesc_p->NOCList,
				     testchannel) == DFS_SUCCESS)
					continue;
				if (IsTestChannelHiddenInNOL_80MHz
				    (dfsDesc_p, testchannel) == DFS_FAILURE)
					chanList[count++] = testchannel;
			} else if (bw == CH_20_MHz_WIDTH) {
				if (DfsFindInNOL
				    (&dfsDesc_p->NOCList,
				     testchannel) == DFS_FAILURE)
					chanList[count++] = testchannel;
			} else
				PRINT1(INFO, "unknown BW \n");
		}
		...
		UINT8 *pChanList = GetRegionChanList(domainInd_IEEERegion);
		...
		for (j = 0; j < IEEE_80211_MAX_NUMBER_OF_CHANNELS; j++) {
			...
			if (dfsDesc_p->currChanInfo.chanflag.radiomode ==
			    RADIO_MODE_80p80) {
				if (IsTheSameGroup
				    (ch, testchannel, CH_160_MHz_WIDTH)) {
					continue;
				}
				if (IsTheSameGroup
				    (ch_another_path, testchannel,
				     CH_160_MHz_WIDTH)) {
					continue;
				}
			}
			...
			if (Is160MzChannel(testchannel, domainInd_IEEERegion) ==
			    FALSE)
				continue;
			if (!dfs_probability) {
				if (!bChkDFSgroup)
					if (channel_exists
					    (testchannel,
					     DfsDetected160MhzGrpChan, 8))
						continue;
			}
			if (DfsFindInNOL(&dfsDesc_p->NOCList, testchannel) ==
			    DFS_SUCCESS)
				continue;
			if (IsTestChannelHiddenInNOL_160MHz
			    (dfsDesc_p, testchannel) == DFS_FAILURE)
				chanList[count++] = testchannel;
		}
	}
	if (!count) {
		...
		if (bw == CH_80_MHz_WIDTH) {
			retChan =
				FindFallbackChannel(fallbackCnt,
						    FallbackChannelList,
						    CH_80_MHz_WIDTH);
...
DFS_STATUS
IsTestChannelHiddenInNOL_40MHz(DfsApDesc * dfsDesc_p, UINT8 testchannel)
{
	UINT8 i, j;
	...
	for (j = 0; j < 2; j++) {
		if (DfsFindInNOL
		    (&dfsDesc_p->NOCList,
		     GrpChList40Mhz[i].channelEntry[j]) == DFS_SUCCESS)
			break;
	}
	if (j != 2)
		return DFS_SUCCESS;
...
DFS_STATUS
IsTestChannelHiddenInNOL_80MHz(DfsApDesc * dfsDesc_p, UINT8 testchannel)
{
	UINT8 i, j;
	...
	for (j = 0; j < 4; j++) {
		if (DfsFindInNOL
		    (&dfsDesc_p->NOCList,
		     GrpChList80Mhz[i].channelEntry[j]) == DFS_SUCCESS)
			break;
	}
	if (j != 4)
		return DFS_SUCCESS;
...
DFS_STATUS
IsTestChannelHiddenInNOL_160MHz(DfsApDesc * dfsDesc_p, UINT8 testchannel)
{
	UINT8 i, j;
	...
	for (j = 0; j < 8; j++) {
		if (DfsFindInNOL
		    (&dfsDesc_p->NOCList,
		     GrpChList160Mhz[i].channelEntry[j]) == DFS_SUCCESS)
			break;
	}
	if (j != 8)
		return DFS_SUCCESS;
...
DFS_STATUS
DfsFindInNOL(List * nocList_p, UINT8 channel)
{
	NOCListItem *ptr = (NOCListItem *) nocList_p->head;
...
void
DfsPrintNOLChannelDetails(DfsAp * me, char *NOLPrintStr_p, int maxLength)
{
	int len = 0;
	int copylen = 0;
	NOCListItem *nocListItem_p;
	...
	while (nocListItem_p) {
		len += sprintf(buf + len,
			       "Channel:%d\t BW:%d MHz\tAge:%lu seconds\n",
			       nocListItem_p->channel, nocListItem_p->BW,
			       (jiffies - nocListItem_p->occurance) / 100);
...
DFS_STATUS
DfsPresentInNOL(struct net_device * dev, UINT8 channel)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, dev);
	...
	if (PhyDSSSTable->Chanflag.ChnlWidth == CH_40_MHz_WIDTH) {
		if (IsTestchannel40MzChannel(channel, domainInd_IEEERegion) ==
		    TRUE)
			return (IsTestChannelHiddenInNOL_40MHz
				(&me->dfsApDesc, channel));
		else
			return DFS_FAILURE;
		...
		if (IsTestchannel80MzChannel(channel, domainInd_IEEERegion) ==
			...
			if (IsTestchannel40MzChannel
			    (channel, domainInd_IEEERegion) == TRUE)
				return (IsTestChannelHiddenInNOL_40MHz
					(&me->dfsApDesc, channel));
			else
				return DFS_FAILURE;
		} else
			return (IsTestChannelHiddenInNOL_80MHz
				(&me->dfsApDesc, channel));
	} else if ((PhyDSSSTable->Chanflag.ChnlWidth == CH_AUTO_WIDTH) ||
		...
		if (Is160MzChannel(channel, domainInd_IEEERegion) == FALSE) {
			...
			return (IsTestChannelHiddenInNOL_160MHz
				(&me->dfsApDesc, channel));
	} else {
...
DFS_STATUS
DfsSetCACTimeOut(DfsAp * me, UINT16 timeout)
{
	if (!me)
		return DFS_FAILURE;
...
DFS_STATUS
DfsSetNOCTimeOut(DfsAp * me, UINT16 timeout)
{
	if (!me)
		return DFS_FAILURE;
...
UINT8
DFSGetExtensionChannelOfDFSChan(UINT8 domain, UINT8 extChnlOffset,
				UINT8 channel)
{
	SINT8 extChanInd = 0;
	UINT8 extChan = 0;
	...
	for (j = 0; j < DFS_MAX_CHANNELS; j++) {
		if (extChanInd && (j + extChanInd >= 0) &&
		    ((j + extChanInd) < DFS_MAX_CHANNELS)) {
			if (dfsEnabledChannels[domain].dfschannelEntry[j] ==
			    channel) {
				extChan =
					dfsEnabledChannels[domain].
					dfschannelEntry[j + extChanInd];
				return extChan;
			}
		}
	}
...
UINT8
DFSGetCurrentRadarDetectionMode(DfsAp * me, UINT8 channel, UINT8 secChan,
				CHNL_FLAGS chanFlag)
{
...
DFSApReset(DfsAp * me)
{
	...
	wl_kfree(nocListItem_p);
	while ((nocListItem_p =
		(NOCListItem *) ListGetItem(&dfsDesc_p->NOCList)) != NULL) {
		wl_kfree(nocListItem_p);
	}
	memset(dfsDesc_p, 0, sizeof(DfsApDesc));
}
...
FireCACTimer(DfsAp * me)
{
	...
	*(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->config.rxEnable) = 1;
	...
	TimerFireIn(&dfsDesc_p->CACTimer, 1,
		    &CACTimeoutHandler, (unsigned char *)me,
		    dfsDesc_p->CACTimeOut);
...
#ifdef CONCURRENT_DFS_SUPPORT
void
AuxCACTimeoutHandler(void *data_p)
{
	DfsAp *me;
	struct net_device *dev;
	struct wlprivate *wlpptr = NULL;
...
FireAuxCACTimer(DfsAp * me)
{
	...
	TimerFireIn(&dfsDesc_p->CtlCACTimer, 1, &AuxCACTimeoutHandler,
