...
UINT32 ClientModeDataCount[NUM_OF_WLMACS] = { 0, 0, 0 };	//To keep track active tx/ rx data traffic within g_CheckStatusTime interval
...
extern void AccumulateScanResults(vmacStaInfo_t * vStaInfo_p, void *BssData_p,
				  UINT8 * rfHdr_p);
...
extern int wlFwGetNoiseLevel(struct net_device *netdev, UINT16 action,
			     UINT8 * pNoise);
extern IEEEtypes_InfoElementHdr_t *smeParseIeType(UINT8 ieType, UINT8 * ieBuf_p,
						  UINT16 ieBufLen);
...
extern UINT16 AddExtended_Cap_IE(vmacApInfo_t * vmacSta_p,
...
*/
UINT32 g_CheckStatusTime[NUM_OF_WLMACS] = { 30, 30, 30 };

UINT32 g_PrbeReqCheckTheshold[NUM_OF_WLMACS] = { 4, 4, 4 };
...
**************************************************************************/
static void
syncSrv_SndScanCnfm(vmacStaInfo_t * vStaInfo_p,
		    UINT16 scanResult,
		    UINT8 numSet,
		    UINT16 bufSize, IEEEtypes_BssDesc_t * BssDescSet_p)
{
	mlmeApiSndScanNotificationOnly(vStaInfo_p,
				       scanResult,
				       numSet, bufSize, (UINT8 *) BssDescSet_p);
}
...
**************************************************************************/
static void
syncSrv_SndJoinCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 joinResult)
{
...
**************************************************************************/
static void
syncSrv_SndStartCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 startResult)
{
	IEEEtypes_StartCfrm_t StartCfrm;

	StartCfrm.Result = startResult;
	mlmeApiSndNotification(vStaInfo_p, (UINT8 *) & StartCfrm,
			       MlmeStart_Cnfm);
}
...
**************************************************************************/
static void
syncSrv_SndResetCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 resetResult)
{
	IEEEtypes_ResetCfrm_t ResetCfrm;

	ResetCfrm.Result = resetResult;
	mlmeApiSndNotification(vStaInfo_p, (UINT8 *) & ResetCfrm,
			       MlmeReset_Cnfm);
}
...
**************************************************************************/
extern void
syncSrv_UpdateJoinStatus(vmacStaInfo_t * vStaInfo_p, UINT16 status)
{
	UINT16 joinResult;
...
**************************************************************************/
extern void
syncSrv_SndLinkLostInd(vmacStaInfo_t * vStaInfo_p)
{
	...
	if (vStaInfo_p == NULL) {
		...
		if ((vStaInfo_tmp_p =
		     (vmacStaInfo_t *) vmacGetVMacStaInfo(clientVMacId)) ==
		    NULL) {
			return;
		}
	} else {
...
**************************************************************************/
extern SINT32
syncSrvSta_ScanDwellTimeOut(UINT8 * data)
{
...
#ifdef ETH_DEBUG
	eprintf("syncSrvSta_ScanDwellTimeOut \n");
#endif
	...
	mlmeApiStartScanTimer(vStaInfo_p, vStaInfo_p->scanTime_tick,
...
**************************************************************************/
extern SINT32
syncSrvSta_ScanActTimeOut(UINT8 * data)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) data;
...
**************************************************************************/
extern SINT32
syncSrv_JoinActTimeOut(UINT8 * data)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) data;
...
**************************************************************************/
extern SINT32
syncSrv_ReJoinActTimeOut(UINT8 * data)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) data;
...
**************************************************************************/
extern void
syncSrv_SetAdhocWatchTimer(vmacStaInfo_t * vStaInfo_p)
{
...
**************************************************************************/
extern BOOLEAN
syncSrv_IsLinkConnected(UINT8 * data_p)
{
...
**************************************************************************/
extern BOOLEAN
syncSrv_IsIbssMode(UINT8 * data_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) data_p;
...
**************************************************************************/
extern SINT32
syncSrv_AdhocActTimeOut(UINT8 * data)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) data;
	...
	if (backOffCnt == -1) {
		...
		backOffCnt =
			(mlmeApiGenRandomUINT32(vStaInfo_p, seed) % 31) + 15;
	}

	if (vStaInfo_p->macMgmtMain_State == STATE_JOINED ||
...
**************************************************************************/
extern void
syncSrv_StatusCheckTimeOut(UINT8 * data)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) data;
	UINT16 macId;
	UINT8 macIndex;
	...
	if (vStaInfo_p->AssociatedFlag) {
		...
		if (vStaInfo_p->mib_WB_p->opMode) {
			...
			if (ClientModeDataCount[macIndex] == 0) {
...
#ifndef SC_PALLADIUM
				if (vStaInfo_p->urProbeRspMissedCnt >=
					...
					if (!vStaInfo_p->g_rcvdProbeRsp) {	/* added probe req link check */
						...
						cleanupAmpduTx(vmacSta_p,
							       ...
							       BssId[0]);
#endif
						mlmeApiEventNotification
							(vStaInfo_p, Tbcn,
							 &vStaInfo_p->
							 macMgmtMlme_ThisStaData.
							 BssId[0], 0);
						mlmeApiUpdateLinkStatus
							(vStaInfo_p, LINK_DOWN);
						vStaInfo_p->AssociatedFlag = 0;
#ifdef SOC_W8964
						vStaInfo_p->in_obss_scan =
							FALSE;
#endif
						vStaInfo_p->staSecurityMibs.
							mib_PrivacyTable_p->
							RSNLinkStatus = 0;
						syncSrv_SndLinkLostInd
							(vStaInfo_p);
				...
				if ((*(vmacSta_p->Mib802dot11->mib_wpaWpa2Mode)
				     > 3) && (!vStaInfo_p->rxBcnCnt)) {
					mlmeApiUpdateLinkStatus(vStaInfo_p,
								LINK_DOWN);
					vStaInfo_p->AssociatedFlag = 0;
				}
#endif
			...
			if ((vStaInfo_p->linkQuality =
			     (vStaInfo_p->rxBcnPeriod /
			      g_CheckStatusTime[macIndex]) *
			     vStaInfo_p->rxBcnCnt) > 100) {
				vStaInfo_p->linkQuality = 100;
			}
		}
		...
		eprintf("rxBcn Count = %d: Period = %d: Quality = %d \n",
			vStaInfo_p->rxBcnCnt, vStaInfo_p->rxBcnPeriod,
			vStaInfo_p->linkQuality);
#endif /* ETH_DEBUG */
		vStaInfo_p->rxBcnCnt = 0;
	} else if ((vStaInfo_p->macMgmtMain_State == STATE_IDLE) &&
		   !vStaInfo_p->mib_WB_p->opMode) {
		syncSrv_SndProbeReq(vStaInfo_p, FALSE);
		if (!vStaInfo_p->keyMgmtInfoSta_p->sta_MIC_Error.disableStaAsso) {
			if (vStaInfo_p->staProbeReqCntIdle++ >
			    IDLE_PROBEREQ_CNT) {
...
**************************************************************************/
extern void
syncSrv_KeepAliveTimeOut(UINT8 * data)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) data;
	UINT16 macId;
	UINT8 macIndex;
	...
	if (vStaInfo_p->AssociatedFlag) {
		if (vStaInfo_p->isParentSession)
			if (mlmeApiIsTrunkIdActive
			    (vStaInfo_p, vmacEntry_p->trunkId))
				/* For checking status when Associated */
				if (!vStaInfo_p->mib_WB_p->opMode) {
					for (ii = 0; ii < 3; ii++) {
						if (SUCCESS ==
						    mlmeApiSendNullDataPktUr
						    (vStaInfo_p,
						     &vStaInfo_p->
						     macMgmtMlme_ThisStaData.
						     BssId,
...
**************************************************************************/
extern void *
syncSrvSta_ParseAttrib(dot11MgtFrame_t * mgtFrame_p, UINT8 attrib)
{
	void *data_p;
	UINT32 lenPacket;
	UINT32 lenOffset;
...
**************************************************************************/
extern BOOLEAN
syncSrv_ValidatePeerSetting(vmacStaInfo_t * vStaInfo_p,
			    dot11MgtFrame_t * MgmtMsg_p)
{
	...
	memcpy((void *)&profileCapInfo,
	       (const void *)&vStaInfo_p->bssDescProfile_p->Cap,
	       sizeof(UINT16));
	if ((*(UINT16 *) & MgmtMsg_p->Body.ProbeRsp.
	     CapInfo & MLME_CAPINFO_VALIDATION_MASK) !=
	...
	/* Validate RF Channel */
	if ((attrib_p =
	     syncSrvSta_ParseAttrib(MgmtMsg_p, DS_PARAM_SET)) != NULL) {
		if (((IEEEtypes_DsParamSet_t *) attrib_p)->CurrentChan !=
		    vStaInfo_p->bssDescProfile_p->PhyParamSet.DsParamSet.
		    CurrentChan) {
			return FALSE;
		}
	}
	...
	if ((attrib_p =
	     syncSrvSta_ParseAttrib(MgmtMsg_p, SUPPORTED_RATES)) != NULL) {
		for (i = 0;
		     i < ((IEEEtypes_SuppRatesElement_t *) attrib_p)->Len;
		     i++) {
			if (((IEEEtypes_SuppRatesElement_t *) attrib_p)->
			    Rates[i] > IEEEtypes_BASIC_RATE_FLAG) {
				if (peerBasicRatesLen <
				    IEEEtypes_MAX_DATA_RATES_G) {
					peerBasicRatesBuf[peerBasicRatesLen] =
	...
	if ((attrib_p =
	     syncSrvSta_ParseAttrib(MgmtMsg_p, EXT_SUPPORTED_RATES)) != NULL) {
		for (i = 0;
		     i < ((IEEEtypes_SuppRatesElement_t *) attrib_p)->Len;
		     i++) {
			if (((IEEEtypes_SuppRatesElement_t *) attrib_p)->
	...
	for (i = 0; i < IEEEtypes_MAX_DATA_RATES_G; i++) {
		if (vStaInfo_p->bssDescProfile_p->DataRates[i] >
		    IEEEtypes_BASIC_RATE_FLAG) {
			profileBasicRatesBuf[profileBasicRatesLen] =
				vStaInfo_p->bssDescProfile_p->DataRates[i];
...
**************************************************************************/
extern void
syncSrv_ProbeRspRcvd(vmacStaInfo_t * vStaInfo_p,
		     dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	UINT8 ssidLen;
...
//    UINT32                 capInfo;
	...
	if (vStaInfo_p->g_rcvdProbeRsp == 0) {
		ssidLen =
			util_CopyList(tmpBufSSID,
				      vStaInfo_p->staSystemMibs.mib_StaCfg_p->
				      DesiredSsId, IEEEtypes_SSID_SIZE);
		if (ssidLen > 0) {
			...
			if (memcmp
			    (&MgmtMsg_p->Hdr.BssId[0],
			     &vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0],
			     sizeof(IEEEtypes_MacAddr_t))) {
				return;
			}
			if (syncSrv_ValidatePeerSetting(vStaInfo_p, MgmtMsg_p)
			    == FALSE) {
				return;
			}
		...
		if (vStaInfo_p->AssociatedFlag) {
			/* Check ERP */
			if ((attrib_p =
			     syncSrvSta_ParseAttrib(MgmtMsg_p,
						    ERP_INFO)) != NULL) {
				vStaInfo_p->Station_p->ChipCtrl.Protection4g =
					((IEEEtypes_ERPInfoElement_t *)
					 attrib_p)->ERPInfo.UseProtection;
			} else {
				vStaInfo_p->Station_p->ChipCtrl.Protection4g =
					0;
			}
...
**************************************************************************/
static SINT32
syncSrv_SndProbeReq(vmacStaInfo_t * vStaInfo_p, BOOLEAN isScan)
{
	UINT32 lenSsId;
	IEEEtypes_MacAddr_t destAddr;
	IEEEtypes_MacAddr_t bssIdAddr;
	dot11MgtFrame_t *probeMsg_p;
	UINT8 rateSupport[IEEEtypes_MAX_DATA_RATES_G];
	UINT8 rateExt[IEEEtypes_MAX_DATA_RATES_G];
	...
	struct net_device *clientDev_p, *staDev =
		(struct net_device *)vmacEntry_p->privInfo_p;
	struct wlprivate *priv_p, *stapriv = NULL;
	vmacApInfo_t *vmacSta_p;
	IEEEtypes_InfoElementHdr_t *ieHdr_p;
	MIB_802DOT11 *mib;
	int currRate = 0, rateid;
	int rateMask, basic_rateMask;
	...
	if ((probeMsg_p =
	     mlmeApiAllocMgtMsg(vmacEntry_p->phyHwMacIndx)) == NULL) {
		return MLME_FAILURE;
	}
	/* Fill out the MAC header */
	if (!isScan &&
	    !is_zero_ether_addr(&vStaInfo_p->macMgmtMlme_ThisStaData.
				BssId[0])) {
		memcpy(&destAddr, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0],
		       sizeof(IEEEtypes_MacAddr_t));
		memcpy(&bssIdAddr,
		       &vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0],
		       sizeof(IEEEtypes_MacAddr_t));
		...
		memcpy(&bssIdAddr, &vStaInfo_p->ScanParams.BssId,
		       sizeof(IEEEtypes_MacAddr_t));
	}
#ifdef ETH_DEBUG
	eprintf("syncSrv_SndProbeReq for SSID = %s \n",
		vStaInfo_p->ScanParams.SsId);
#endif
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p,
				  probeMsg_p,
				  &destAddr, IEEE_MSG_PROBE_RQST, &bssIdAddr);
	probeMsg_p->Hdr.FrmBodyLen = 0;
	/* Add SSID Attrib */
	if (!isScan) {
		lenSsId =
			strlen(vStaInfo_p->staSystemMibs.mib_StaCfg_p->
			       DesiredSsId);
		syncSrv_AddAttrib(probeMsg_p, SSID,
				  vStaInfo_p->staSystemMibs.mib_StaCfg_p->
				  DesiredSsId, lenSsId);
	} else {
		lenSsId = strlen(vStaInfo_p->ScanParams.SsId);
		syncSrv_AddAttrib(probeMsg_p,
				  SSID, vStaInfo_p->ScanParams.SsId, lenSsId);
	}

#ifdef SOC_W906X
	if (vStaInfo_p->ChanIdx >=
	    (IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A))
		return MLME_FAILURE;

	if (vStaInfo_p->ScanParams.ChanList[vStaInfo_p->ChanIdx] <= 14) {
		if (Is5GBand(*(mib->mib_ApMode))) {
			...
			case AP_MODE_5GHZ_11AX_ONLY:
				*(mib->mib_ApMode) = AP_MODE_2_4GHZ_11AX_MIXED;
				break;
			...
			*(mib->NotBssBasicRateMask) =
				MRVL_NOTBSSBASICRATEMASK_BGN;
			*(mib->mib_shortSlotTime) = TRUE;
		}
	} else if (!Is5GBand(*(mib->mib_ApMode))) {
		switch (*(mib->mib_ApMode)) {
		case AP_MODE_B_ONLY:
	...
	basic_rateMask = *(mib->BssBasicRateMask);
	rateMask = *(mib->BssBasicRateMask) | *(mib->NotBssBasicRateMask);

	for (currRate = 0, rateid = 0; currRate < 14; currRate++) {
		if (rateMask & 0x01) {
			if (mib->StationConfig->OpRateSet[currRate] != 0) {
				...
				if (basic_rateMask & 0x01)
					rateSupport[rateid] |= 0x80;
		...
		if (supportedRateLen == 8) {
			currRate++;
			break;
		}
	}

	for (rateid = 0; currRate < 14; currRate++) {
		if (rateMask & 0x01) {
			if (mib->StationConfig->OpRateSet[currRate] != 0) {
				rateExt[rateid] =
					mib->StationConfig->
	...
	if (supportedRateLen) {
		syncSrv_AddAttrib(probeMsg_p,
				  SUPPORTED_RATES,
				  &rateSupport[0], supportedRateLen);
	}
#ifdef MRVL_WPS_CLIENT
	if (stapriv->wpsProbeRequestIeLen > 0) {
		UINT8 *pos =
			(UINT8 *) & probeMsg_p->Body +
			probeMsg_p->Hdr.FrmBodyLen;
		memcpy(pos, stapriv->wpsProbeRequestIe,
		       stapriv->wpsProbeRequestIeLen);
		probeMsg_p->Hdr.FrmBodyLen += stapriv->wpsProbeRequestIeLen;
	}
#endif

#ifdef MBO_SUPPORT
	if (stapriv->mboProbeRequestIeLen > 0) {
		UINT8 *pos =
			(UINT8 *) & probeMsg_p->Body +
			probeMsg_p->Hdr.FrmBodyLen;
		memcpy(pos, stapriv->mboProbeRequestIe,
		       stapriv->mboProbeRequestIeLen);
		probeMsg_p->Hdr.FrmBodyLen += stapriv->mboProbeRequestIeLen;
	}

	if ((mib->mib_mbo_wnm) || (mib->Interworking))
		extendedCap_needAdd = 1;
#endif /* MBO_SUPPORT */

#ifdef AP_STEERING_SUPPORT
	if (*(mib->mib_btm_enabled))
		extendedCap_needAdd = 1;
	...
	if (extendedCap_needAdd) {
		...
		pExtCap =
			wl_kmalloc(sizeof(IEEEtypes_Extended_Cap_Element_t),
				   GFP_ATOMIC);
		if (pExtCap) {
			AddExtended_Cap_IE(vmacSta_p, pExtCap);
			syncSrv_AddAttrib(probeMsg_p, EXT_CAP_IE,
					  (UINT8 *) & pExtCap->ExtCap,
					  sizeof
					  (IEEEtypes_Extended_Cap_Element_t) -
					  2);
			wl_kfree(pExtCap);
	...
	if ((*(mib->mib_ApMode) & AP_MODE_N_ONLY)
	    ...
	    || (*(mib->mib_ApMode) & AP_MODE_11AX)) {
		probeMsg_p->Hdr.FrmBodyLen +=
			AddHT_IE(priv_p->vmacSta_p,
				 (IEEEtypes_HT_Element_t *) ((UINT8 *) &
							     probeMsg_p->Body +
							     probeMsg_p->Hdr.
							     FrmBodyLen));
	}
	...
	if (*(mib->mib_ApMode) & AP_MODE_11AX) {
		probeMsg_p->Hdr.FrmBodyLen +=
			Build_IE_HE_CAP(priv_p->vmacSta_p,
					(UINT8 *) & probeMsg_p->Body +
					probeMsg_p->Hdr.FrmBodyLen);
	}
#endif
	...
	if (*(vmacSta_p->Mib802dot11->mib_STAMacCloneEnable) == 2) {
		...
		UINT8 mrvl_id[] = "\x00\x50\x43\x03\xE0\0x00";

		/* Pack in NXP IE. */
		ieHdr_p =
			(IEEEtypes_InfoElementHdr_t *) &
			(p[probeMsg_p->Hdr.FrmBodyLen]);
		ieHdr_p->ElementId = PROPRIETARY_IE;
		ieHdr_p->Len = sizeof(mrvl_id);
		probeMsg_p->Hdr.FrmBodyLen +=
			sizeof(IEEEtypes_InfoElementHdr_t);
		if (probeMsg_p->Hdr.FrmBodyLen + ieHdr_p->Len < sizeof(*p))
			memcpy((void *)&(p[probeMsg_p->Hdr.FrmBodyLen]),
			       mrvl_id, ieHdr_p->Len);
		else
			return MLME_FAILURE;
...
**************************************************************************/
extern void
syncSrv_ProbeReqRcvd(vmacStaInfo_t * vStaInfo_p,
		     dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	UINT8 ssidLen = MgmtMsg_p->Body.ProbeRqst.SsId.Len;

	if (ssidLen &&
	    (*(UINT8 *) & MgmtMsg_p->Body.ProbeRqst.SsId.SsId[0] != 0x0)) {
...
**************************************************************************/
static void
syncSrv_UpdateAdhocTxInfo(vmacStaInfo_t * vStaInfo_p,
			  ...
			  txBcnInfo_t * MgtTxInfo)
{
   ...
   /*-------------------------------------------------------------*/
	...
	ssId_p->Len = util_CopyList(ssId_p->SsId,
				    StartCmd_p->SsId, IEEEtypes_SSID_SIZE);
   ...
   /*----------------------------------------------------------*/
	...
	nextElement_p += sizeof(IEEEtypes_ElementId_t) +
		sizeof(IEEEtypes_Len_t) + suppRates_p->Len;
   ...
   /*-----------------------------------------*/
	...
	if (StartCmd_p->BssType == BSS_INFRASTRUCTURE) {
		...
		ssParamSet_p->IbssParamSet =
			StartCmd_p->SsParamSet.IbssParamSet;
   ...
   /*-----------------------------------------------------------*/
	if (vStaInfo_p->macMgt_StaMode == CLIENT_MODE_G) {
		...
		ExtsuppRates_p =
			(IEEEtypes_ExtSuppRatesElement_t *) nextElement_p;
		ExtsuppRates_p->ElementId = EXT_SUPPORTED_RATES;

		ExtsuppRates_p->Len = util_CopyList(ExtsuppRates_p->Rates,
						    &StartCmd_p->
						    OpRateSet[MAX_B_DATA_RATES],
						    IEEEtypes_MAX_DATA_RATES_G -
						    MAX_B_DATA_RATES);

		nextElement_p += sizeof(IEEEtypes_ElementId_t) +
			sizeof(IEEEtypes_Len_t) + ExtsuppRates_p->Len;
	}
   ...
   /*-------------------------------------------------------------*/
	MgtTxInfo->RetryCnt =
		vStaInfo_p->staSystemMibs.mib_OpData_p->ShortRetryLim;
	MgtTxInfo->Status = 0;
   ...
   /*----------------------------------------------------------------*/
	bcnBuf_p->Hdr.FrmBodyLen = (UINT32) tim_p -
		(UINT32) bcnBuf_p - sizeof(IEEEtypes_MgmtHdr_t);
   ...
   /*--------------------------------------------------------------*/
	MgtTxInfo->CfOffset = (UINT32) ssParamSet_p -
		(UINT32) bcnBuf_p -
		sizeof(bcnBuf_p->Hdr.FrmBodyLen) - sizeof(bcnBuf_p->Hdr.Rsrvd);
   ...
   /*--------------------------------------------------------------*/
	MgtTxInfo->ProbeRespLen = bcnBuf_p->Hdr.FrmBodyLen +
		...
		sizeof(bcnBuf_p->Hdr.Rsrvd) + sizeof(UINT32);
   ...
   /*--------------------------------------------------------------*/
	if (IEEE_MSG_PROBE_RSP == packType) {
		if (vStaInfo_p->staSystemMibs.mib_OpData_p->RtsThresh <
		    (MgtTxInfo->ProbeRespLen))
...
**************************************************************************/
static void
syncSrv_ScanPrep(vmacStaInfo_t * vStaInfo_p)
{
	UINT32 i;
	...
	memset(&vStaInfo_p->scanTableResult_p->BssSourceAddr,
	       0, sizeof(IEEEtypes_MacAddr_t) * IEEEtypes_MAX_BSS_DESCRIPTS);

	for (i = 0; i <= IEEEtypes_MAX_BSS_DESCRIPTS - 1; i++) {
		vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[i].
			NumReadings = 0;
		vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[i].
			RSSI = 0;
		vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[i].
	...
	vStaInfo_p->ScanResults_p =
		&vStaInfo_p->scanTableResult_p->ScanResults[0];
	vStaInfo_p->scanTableResult_p->ScanResultsMap_p[0] =
		vStaInfo_p->ScanResults_p;
...
extern void
syncSrvSta_ScanCmd(vmacStaInfo_t * vStaInfo_p, macMgmtQ_ScanCmd_t * ScanCmd_p)
{
	UINT32 i = 0;
	UINT32 ssIdLen;
   ...
   /*--------------------------------------------------------------------*/
	if (ScanCmd_p->ScanSrc == SCAN_CMD) {
		memcpy(&vStaInfo_p->ScanParams, &ScanCmd_p->Cmd,
   ...
   /*-----------------------------------------*/
	if (vStaInfo_p->ScanParams.ChanList[0] == 0) {
		...
		syncSrv_SndScanCnfm(vStaInfo_p,
				    SCAN_RESULT_INVALID_PARAMETERS, 0, 0, NULL);
		return;
	}
   ...
   /*--------------------------------------------------------*/
	...
	/* given list.                                            */
   /*--------------------------------------------------------*/
	while (i < (IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A) &&
	       moreChannels) {
	...
	if (vStaInfo_p->NumScanChannels > 1) {
		/* syncSrvSta_SetNextChannel() will set channel again, ignore here! */
		mlmeApiSetRfChannel(vStaInfo_p,
				    vStaInfo_p->ScanParams.ChanList[0], 1,
				    TRUE);
	}
   ...
   /*-----------------------------------------------------------*/
	...
	/* Get and start a scan timer with duration of the maximum channel time */
#ifdef MLME_SEPERATE_SCAN_TIMER
	mlmeApiStartScanTimer(vStaInfo_p, time_scan_start,
			      &syncSrvSta_ScanDwellTimeOut);
#else
	mlmeApiStartTimer(vStaInfo_p,
			  (UINT8 *) & vStaInfo_p->scaningTimer,
			  &syncSrvSta_ScanDwellTimeOut, time_scan_start);
#endif /* MLME_SEPERATE_SCAN_TIMER */
...
**************************************************************************/
extern void
syncSrv_JoinCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_JoinCmd_t * JoinCmd_p)
{
	vmacEntry_t *vmacEntry_p;
	UINT32 length;
	...
	vStaInfo_p->JoinChannel =
		JoinCmd_p->BssDesc.PhyParamSet.DsParamSet.CurrentChan;
	...
	rateLen =
		util_ListLen(&JoinCmd_p->BssDesc.DataRates[0],
			     IEEEtypes_MAX_DATA_RATES_G);
	for (i = 0; i < rateLen; i++) {
		...
		if (parseRate <= 0xb || parseRate == 0x16) {
			if (bRateCnt < MAX_B_DATA_RATES) {
				...
				vStaInfo_p->bOpRateSet[bRateCnt] =
					JoinCmd_p->BssDesc.DataRates[i];
				bRateCnt++;
			}
		} else {
			/* Assume it's 11g rates */
			if (gRateCnt < MAX_G_DATA_RATES) {
				vStaInfo_p->gOpRateSet[gRateCnt] =
					JoinCmd_p->BssDesc.DataRates[i];
				gRateCnt++;
			}
		}
	...
	/********************************************
		...
		*********************************************/
		mlmeApiCreateSupportedRateElement((IEEEtypes_SuppRatesElement_t
						   *)
						  mlmeApiGetClientModeSupportedRates
						  (vmacEntry_p->phyHwMacIndx),
						  (IEEEtypes_ExtSuppRatesElement_t
						   *)
						  mlmeApiGetClientModeExtSupportedRates
						  (vmacEntry_p->phyHwMacIndx),
						  &(vStaInfo_p->bOpRateSet[0]),
						  &(vStaInfo_p->gOpRateSet[0]),
						  vmacEntry_p->phyHwMacIndx);

		mlmeApiSetRfChannel(vStaInfo_p, vStaInfo_p->JoinChannel, 1,
				    FALSE);
	...
	if (JoinCmd_p->BssDesc.Cap.Ibss) {
		...
		memset(&StartCmd, 0, sizeof(IEEEtypes_StartCmd_t));
		...
		memcpy(&(StartCmd.SsId),
		       JoinCmd_p->BssDesc.SsId, sizeof(IEEEtypes_SsId_t));
...
**************************************************************************/
extern void
syncSrv_StartCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_StartCmd_t * StartCmd_p)
{
	vmacEntry_t *vmacEntry_p;
	UINT8 newSsIdLen;
	...
	memset(vStaInfo_p->gOpRateSet, 0, MAX_G_DATA_RATES);
	rateLen =
		util_ListLen(&StartCmd_p->OpRateSet[0],
			     IEEEtypes_MAX_DATA_RATES_G);
	for (i = 0; i < rateLen; i++) {
   ...
   *********************************************/
   ...
   /*------------------------------------------------------------------*/
	if (vStaInfo_p->macMgmtMain_State == STATE_SCANNING) {
		syncSrv_SndStartCnfm(vStaInfo_p,
		...
		if (newSsIdLen == vStaInfo_p->macMgmtMlme_ThisStaData.BssSsIdLen
		    && !memcmp(vStaInfo_p->macMgmtMlme_ThisStaData.BssSsId,
			       StartCmd_p->SsId,
			       vStaInfo_p->macMgmtMlme_ThisStaData.
			       BssSsIdLen)) {
			syncSrv_SndStartCnfm(vStaInfo_p,
   ...
   /*--------------------------------------------------------------------*/
	...
	vStaInfo_p->macMgmtMlme_ThisStaData.BssSsIdLen =
		util_CopyList(vStaInfo_p->macMgmtMlme_ThisStaData.BssSsId,
			      StartCmd_p->SsId, IEEEtypes_SSID_SIZE);
   ...
   /*-------------------------------------------*/
	if (StartCmd_p->PhyParamSet.DsParamSet.ElementId == DS_PARAM_SET) {
		mlmeApiSetRfChannel(vStaInfo_p,
				    StartCmd_p->PhyParamSet.DsParamSet.
				    CurrentChan, 1, TRUE);
	}
   ...
   /*--------------------------------------------------------------*/
	...
	mlmeApiSetTrunkIdActive(vStaInfo_p, vmacEntry_p->trunkId, TRUE,
				STA_TRUNK_TYPE);
...
**************************************************************************/
extern void
syncSrv_ResetCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_ResetCmd_t * ResetCmd_p)
{
	vmacEntry_t *vmacEntry_p;
...
**************************************************************************/
static void
syncSrv_RestorePreScanParameters(vmacStaInfo_t * vStaInfo_p)
{
	vmacEntry_t *vmacEntry_p;
	...
	case STATE_IBSS_STARTED:
		mlmeApiSetBssidToMac(vStaInfo_p,
				     vStaInfo_p->macMgmtMlme_ThisStaData.BssId);
		...
		if (vStaInfo_p->AssociatedFlag &&
		    (vStaInfo_p->macMgmtMlme_ThisStaData.BssType ==
		     BSS_INFRASTRUCTURE)) {
...
**************************************************************************/
extern void
syncSrvSta_SetNextChannel(vmacStaInfo_t * vStaInfo_p)
{
...
#endif
	{
		...
		syncSrv_SndScanCnfm(vStaInfo_p,
				    ...
				    (IEEEtypes_BssDesc_t *) & vStaInfo_p->
				    scanTableResult_p->ScanResults);
		return;
	}
#ifdef SC_PALLADIUM
	vStaInfo_p->ChanIdx++;
...
#ifndef SC_PALLADIUM
	mlmeApiSetRfChannel(vStaInfo_p,
			    vStaInfo_p->ScanParams.ChanList[vStaInfo_p->
							    ChanIdx], 1, TRUE);
#endif
...
#ifdef DFS_PASSIVE_SCAN
		if (!
		    (((vStaInfo_p->ScanParams.ChanList[vStaInfo_p->ChanIdx] >=
		       52) &&
		      (vStaInfo_p->ScanParams.ChanList[vStaInfo_p->ChanIdx] <=
		       64)) ||
		     ((vStaInfo_p->ScanParams.ChanList[vStaInfo_p->ChanIdx] >=
...
**************************************************************************/
void
syncSrv_SetInitValues(vmacStaInfo_t * vStaInfo_p)
{
	/* Remove associated AP from UR station database, intially added for
	   pseudo ether port support. */
	mlmeApiDelStaDbEntry(vStaInfo_p,
			     (UINT8 *) & vStaInfo_p->macMgmtMlme_ThisStaData.
			     BssId);

	memset(&vStaInfo_p->macMgmtMlme_ThisStaData.BssId,
	       0, sizeof(IEEEtypes_MacAddr_t));
	memset(&(vStaInfo_p->macMgmtMlme_ThisStaData.UsedRateSet),
	       0, sizeof(IEEEtypes_SuppRatesElement_t));
	memset(&(vStaInfo_p->macMgmtMlme_ThisStaData.OpRateSet),
	       0, sizeof(IEEEtypes_SuppRatesElement_t));
	memset(&(vStaInfo_p->macMgmtMlme_ThisStaData.CapInfo),
	       0, sizeof(IEEEtypes_CapInfo_t));
	vStaInfo_p->macMgmtMlme_ThisStaData.CapInfo.CfPollable =
		vStaInfo_p->staSystemMibs.mib_StaCfg_p->CfPollable;
	vStaInfo_p->macMgmtMlme_ThisStaData.UsedRateSet.ElementId =
		SUPPORTED_RATES;
	vStaInfo_p->macMgmtMlme_ThisStaData.OpRateSet.ElementId =
		SUPPORTED_RATES;
	vStaInfo_p->macMgmtMlme_ThisStaData.OpRateSet.Len =
		util_CopyList(vStaInfo_p->macMgmtMlme_ThisStaData.OpRateSet.
			      Rates,
...
**************************************************************************/
static void
syncSrv_adhocCheckDoRescan(vmacStaInfo_t * vStaInfo_p,
			   dot11MgtFrame_t * MgmtMsg_p)
{
	UINT32 seed;
	...
	if (!memcmp
	    (&(vStaInfo_p->macMgmtMlme_ThisStaData.BssSsId),
	     MgmtMsg_p->Body.Bcn.SsId.SsId, MgmtMsg_p->Body.Bcn.SsId.Len)
...
**************************************************************************/
static void
syncSrv_SetBeaconTime(vmacStaInfo_t * vStaInfo_p, IEEEtypes_Bcn_t * Bcn_p)
{
	UINT64 intervals;
	UINT64 *bcnTime_p;
	UINT64 nextBcnTime;
...
extern void *
syncSrv_ParseAttribWithinFrame(dot11MgtFrame_t * mgtFrame_p, UINT8 * data_p,
			       UINT8 attrib)
{
	...
	lenOffset +=
		(unsigned long)data_p - (unsigned long)(&(mgtFrame_p->Body))
		+ sizeof(IEEEtypes_MgmtHdr_t);
...
**************************************************************************/
extern UINT32
syncSrv_AddAttrib(dot11MgtFrame_t * mgtMsg_p,
		  UINT8 attribType, UINT8 * attribData, UINT8 attribLen)
{
	UINT32 curMsgLen;
	UINT8 *curData_p;
	UINT8 lenAdded = 0;
...
**************************************************************************/
extern void
AccumulateScanResults(vmacStaInfo_t * vStaInfo_p,
		      void *BssData_p, UINT8 * rfHdr_p)
{
	UINT32 i = 0;
	UINT32 length;
	...
	while (notFound && i < IEEEtypes_MAX_BSS_DESCRIPTS) {
		if (memcmp
		    (((scanDescptHdr_t *) vStaInfo_p->scanTableResult_p->
		      ScanResultsMap_p[i])->bssId, emptyAddr,
		     sizeof(IEEEtypes_MacAddr_t)) ||
		    ((scanDescptHdr_t *) vStaInfo_p->scanTableResult_p->
		     ScanResultsMap_p[i])->length > 0) {
			/* Not Empty so let's see if Bssid Matched */
			if (!memcmp
			    ((((scanDescptHdr_t *) vStaInfo_p->
			       scanTableResult_p->ScanResultsMap_p[i])->bssId),
			     ((dot11MgtFrame_t *) BssData_p)->Hdr.BssId,
			     sizeof(IEEEtypes_MacAddr_t))) {
				/* Bssid Matched so just update the signal quality info */
				notFound = FALSE;
				vStaInfo_p->scanTableResult_p->
					macMgmtMlme_SigQltyResults[i].
					NumReadings++;

				if (((WLAN_RX_INFO *) rfHdr_p)->RSSI <
				    vStaInfo_p->scanTableResult_p->
				    macMgmtMlme_SigQltyResults[i].RSSI) {
					vStaInfo_p->scanTableResult_p->
						macMgmtMlme_SigQltyResults[i].
						RSSI =
						mlmeApiHalRssiDbmGet(vStaInfo_p,
								     ((WLAN_RX_INFO *) rfHdr_p)->RSSI, ((WLAN_RX_INFO *) rfHdr_p)->Sq1);

					((scanDescptHdr_t *) vStaInfo_p->
					 scanTableResult_p->
					 ScanResultsMap_p[i])->rssi =
		vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[i].
		RSSI;
				}
				vStaInfo_p->scanTableResult_p->
					macMgmtMlme_SigQltyResults[i].SigQual1 =
				 ...
				 */
				if ((i < IEEEtypes_MAX_BSS_DESCRIPTS) &&
				    (i < vStaInfo_p->NumDescripts)) {
					continue;
				}
				local_RSSI =
					mlmeApiHalRssiDbmGet(vStaInfo_p,
							     ...
							     ((WLAN_RX_INFO *)
							      rfHdr_p)->Sq1);
				scanDesctLen =
					((dot11MgtFrame_t *) BssData_p)->Hdr.
					FrmBodyLen -
					(sizeof(IEEEtypes_MgmtHdr_t));
				if ((attrib_p =
				     syncSrvSta_ParseAttrib(BssData_p,
							    DS_PARAM_SET)) ==
				    NULL) {
					scanDesctLen +=
						sizeof(IEEEtypes_DsParamSet_t);
				...
				if ((i >= IEEEtypes_MAX_BSS_DESCRIPTS) ||
					...
					length = 0;
					for (j = 0;
					     j < vStaInfo_p->NumDescripts;
					     j++) {
						if (vStaInfo_p->
						    scanTableResult_p->
						    macMgmtMlme_SigQltyResults
						    [j].RSSI > local_RSSI) {
							length +=
								((scanDescptHdr_t *) vStaInfo_p->scanTableResult_p->ScanResultsMap_p[j])->length;
						}
					}
		...
		/********************************************************** 
				...
				while ((i >= IEEEtypes_MAX_BSS_DESCRIPTS ||
					...
					if (local_RSSI <
					    vStaInfo_p->scanTableResult_p->
					    macMgmtMlme_SigQltyResults[weakest].
					    RSSI) {
		...
		memcpy(&vStaInfo_p->scanTableResult_p->BssSourceAddr[i],
		       ...
		       sizeof(IEEEtypes_MacAddr_t));
		vStaInfo_p->NumDescripts++;
		length = ((dot11MgtFrame_t *) BssData_p)->Hdr.FrmBodyLen -
			(sizeof(IEEEtypes_MgmtHdr_t));
		...
		if ((attrib_p =
		     syncSrvSta_ParseAttrib(BssData_p, DS_PARAM_SET)) == NULL) {
			addChannelIE = TRUE;
			IElength += sizeof(IEEEtypes_DsParamSet_t);
		} else if (mlmeApiGetRfChannel(vStaInfo_p) !=
			   ((IEEEtypes_DsParamSet_t *) attrib_p)->CurrentChan) {
			vStaInfo_p->NumDescripts--;
			return;
		}

		/* Make sure there is enought memory */
		if ((vStaInfo_p->ScanResultsLen + IElength + sizeof(IElength)) >
		    MAX_SCAN_BUF_SIZE) {
			vStaInfo_p->NumDescripts--;
			return;
		}
		/* Fill Scan Result */
		vStaInfo_p->scanTableResult_p->ScanResultsMap_p[i] =
			vStaInfo_p->ScanResults_p;
		memcpy(vStaInfo_p->ScanResults_p, (const void *)&IElength,
		       sizeof(IElength));
		vStaInfo_p->ScanResultsLen += sizeof(IElength);
		vStaInfo_p->ScanResults_p =
			(void *)((UINT8 *) vStaInfo_p->ScanResults_p +
				 sizeof(IElength));
		/*Pack in the BssId */
		...
		local_RSSI =
			mlmeApiHalRssiDbmGet(vStaInfo_p,
					     ((WLAN_RX_INFO *) rfHdr_p)->RSSI,
					     ((WLAN_RX_INFO *) rfHdr_p)->Sq1);
		memcpy((void *)(vStaInfo_p->ScanResults_p), &local_RSSI,
		       sizeof(UINT8));
		...
		memcpy(vStaInfo_p->ScanResults_p,
		       (const void *)&MgmtMsg_p->Body.Bcn.TimeStamp, length);
		memcpy(vStaInfo_p->ScanResults_p,
		       (const void *)&((dot11MgtFrame_t *) BssData_p)->Body.Bcn.
		       TimeStamp, length);
		vStaInfo_p->ScanResultsLen += length;
		vStaInfo_p->ScanResults_p =
			(void *)((UINT8 *) vStaInfo_p->ScanResults_p + length);
		/* Find out if we have channel IE */
		if (addChannelIE) {
		...
		if (i < (IEEEtypes_MAX_BSS_DESCRIPTS - 1))
			vStaInfo_p->scanTableResult_p->ScanResultsMap_p[i + 1] =
				vStaInfo_p->ScanResults_p;
		...
		vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[i].
			...
			mlmeApiHalRssiDbmGet(vStaInfo_p,
					     ...
					     ((WLAN_RX_INFO *) rfHdr_p)->Sq1);
		vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[i].
			SigQual1 = ((WLAN_RX_INFO *) rfHdr_p)->Sq1;
	}			/* while */
...
**************************************************************************/
extern void
syncSrv_BncRecvAssociatedHandler(vmacStaInfo_t * vStaInfo_p,
				 dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	void *attrib_p;
#ifdef STA_QOS
...
//  UINT32                 capInfo;
	...
	/* Validate RF Channel  */
	if (stapriv->vmacSta_p->Mib802dot11->PhyDSSSTable->Chanflag.FreqBand ==
	    FREQ_BAND_2DOT4GHZ) {
		if ((attrib_p =
		     syncSrvSta_ParseAttrib(MgmtMsg_p, DS_PARAM_SET)) != NULL) {
			if (((IEEEtypes_DsParamSet_t *) attrib_p)->
			    CurrentChan !=
			    vStaInfo_p->bssDescProfile_p->PhyParamSet.
			    DsParamSet.CurrentChan) {
				return;
			}
		}
	}
	/* Get current level, noise, and quality. */
	vStaInfo_p->linkInfo.wStats.qual.level =
		((WLAN_RX_INFO *) rfHdr_p)->RSSI;

	if (NoiseSampleBeaconCount++ == NOISE_SAMPLE_BEACON_COUNT) {
		NoiseSampleBeaconCount = 0;
		//wlFwGetNoiseLevel (staDev, 0, &noise);
		vStaInfo_p->linkInfo.wStats.qual.noise =
			(vStaInfo_p->linkInfo.wStats.qual.noise + noise) / 2;
	}

	vStaInfo_p->linkInfo.wStats.qual.qual =
		vStaInfo_p->linkInfo.wStats.qual.noise -
		vStaInfo_p->linkInfo.wStats.qual.level;

	/* Detecting b/g network */
	/* Calculate average level, noise and quality. */
	vStaInfo_p->linkInfo.avg_qual.level =
		(vStaInfo_p->linkInfo.avg_qual.level +
		 vStaInfo_p->linkInfo.wStats.qual.level) / 2;
	vStaInfo_p->linkInfo.avg_qual.noise =
		(vStaInfo_p->linkInfo.avg_qual.noise +
		 vStaInfo_p->linkInfo.wStats.qual.noise) / 2;
	vStaInfo_p->linkInfo.avg_qual.qual =
		(vStaInfo_p->linkInfo.avg_qual.qual +
		 vStaInfo_p->linkInfo.wStats.qual.qual) / 2;
	/* Keep track of maximum level, noise and quality. */
	/* Measurements in -dBm. */
	if ((vStaInfo_p->linkInfo.wStats.qual.level <
	     vStaInfo_p->linkInfo.max_qual.level) ||
	    (vStaInfo_p->linkInfo.max_qual.level == 0)) {
		vStaInfo_p->linkInfo.max_qual.level =
			vStaInfo_p->linkInfo.wStats.qual.level;
	}
	if ((vStaInfo_p->linkInfo.wStats.qual.noise <
	     vStaInfo_p->linkInfo.max_qual.noise) ||
	    (vStaInfo_p->linkInfo.max_qual.noise == 0)) {
		vStaInfo_p->linkInfo.max_qual.noise =
			vStaInfo_p->linkInfo.wStats.qual.noise;
	}
	if (vStaInfo_p->linkInfo.wStats.qual.qual >
	    vStaInfo_p->linkInfo.max_qual.qual) {
		vStaInfo_p->linkInfo.max_qual.qual =
			vStaInfo_p->linkInfo.wStats.qual.qual;
	}
	...
	if ((attrib_p =
	     syncSrvSta_ParseAttrib(MgmtMsg_p, SUPPORTED_RATES)) != NULL) {
		vStaInfo_p->macMgmtMlme_ThisStaData.AP_RateLen =
			((IEEEtypes_SuppRatesElement_t *) attrib_p)->Len;
	}
	if ((attrib_p =
	     syncSrvSta_ParseAttrib(MgmtMsg_p, EXT_SUPPORTED_RATES)) != NULL) {
		vStaInfo_p->macMgmtMlme_ThisStaData.AP_gRateLen =
			((IEEEtypes_SuppRatesElement_t *) attrib_p)->Len;
	}
#ifdef STA_QOS
#ifdef QOS_WSM_FEATURE
		...
		if (!memcmp(pWME_info_elem->OUI.OUI, WiFiOUI, 3)) {
			...
			if (pWME_info_elem->OUI.Type == 2) {
				...
				if (pWME_info_elem->OUI.Subtype == 0) {
					...
					if (pWME_info_elem->QoS_info.
					    EDCA_param_set_update_cnt !=
					    Qos_Stn_Data[0].QoS_Info.
					    EDCA_param_set_update_cnt) {
				...
				if (pWME_info_elem->OUI.Subtype == 1) {
					...
					QoS_UpdateStnEDCAParameters
						(pWME_param_elem);
					memcpy(&(Qos_Stn_Data[0].QoS_Info),
					       &(pWME_param_elem->QoS_info),
					       sizeof(QoS_Info_t));
				}
			}
	...
	if (vStaInfo_p->macMgmtMlme_ThisStaData.BssType == BSS_INDEPENDENT) {
		...
		if ((attrib_p =
		     syncSrvSta_ParseAttrib(MgmtMsg_p, ERP_INFO)) != NULL) {
			vStaInfo_p->Station_p->ChipCtrl.Protection4g =
				((IEEEtypes_ERPInfoElement_t *) attrib_p)->
				ERPInfo.UseProtection;
			if (!(*mib->mib_forceProtectiondisable)) {
				if (vStaInfo_p->Station_p->ChipCtrl.
				    Protection4g)
					protection4g = 0x02;
				else
					protection4g = 0;
				if (stapriv->master)
					wlFwSetGProt(stapriv->master,
						     protection4g);
			}
		}
		...
		if ((attrib_p = syncSrvSta_ParseAttrib(MgmtMsg_p, HT)) != NULL) {
			if ((*mib->mib_HtGreenField) &&
			    ((IEEEtypes_HT_Element_t *) attrib_p)->
			    HTCapabilitiesInfo.GreenField) {
				if ((attrib_p =
				     syncSrvSta_ParseAttrib(MgmtMsg_p,
							    ADD_HT)) != NULL) {
					if ((vStaInfo_p->Station_p->ChipCtrl.
					     ...
					     ((IEEEtypes_Add_HT_Element_t *)
					      attrib_p)->OpMode.NonGFStaPresent)
					    || (!vStaInfo_p->Station_p->
						ChipCtrl.GreenFieldSet)) {
						vStaInfo_p->Station_p->ChipCtrl.
							GreenField =
							!(((IEEEtypes_Add_HT_Element_t *) attrib_p)->OpMode.NonGFStaPresent);
						if (stapriv->master) {
							vStaInfo_p->Station_p->
								ChipCtrl.
								GreenFieldSet =
		...
		if (vStaInfo_p->staSystemMibs.mib_StaCfg_p->sta11hMode) {
			 ...
			 */
			if ((attrib_p =
			     syncSrvSta_ParseAttrib(MgmtMsg_p, CSA)) != NULL) {
				UINT8 channel =
				...
				UINT16 beaconInterval =
					MgmtMsg_p->Body.Bcn.BcnInterval;
				UINT16 tickCount =
					beaconInterval ? (beaconInterval / 100 *
							  count) : count;
...
**************************************************************************/
extern SINT32
syncSrv_LinkLostHandler(vmacStaInfo_t * vStaInfo_p)
{
	vmacEntry_t *vmacEntry_p;
	...
	if (vStaInfo_p->AssociatedFlag) {
		...
		if (vStaInfo_p->isParentSession) {
			...
			mlmeApiSetTrunkIdActive(vStaInfo_p,
						vmacEntry_p->trunkId, FALSE,
						STA_TRUNK_TYPE);
		}
	}
	vStaInfo_p->macMgmtMain_State = STATE_IDLE;
	/* Remove associated AP from UR station database, intially added for
	   pseudo ether port support. */
	mlmeApiFreePeerStationStaInfoAndAid(&vStaInfo_p->
					    macMgmtMlme_ThisStaData.BssId,
					    vmacEntry_p);
	return MLME_SUCCESS;
}
...
**************************************************************************/
extern void
syncSrv_ScanFilter(vmacStaInfo_t * vStaInfo_p,
		   dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	UINT8 ssidLen;
	UINT8 tmpBufSSID[IEEEtypes_SSID_SIZE];
		...
		if (vStaInfo_p->scanFilterMap & MLME_SSID_FILTER) {
			...
			if ((ssidLen != MgmtMsg_p->Body.ProbeRsp.SsId.Len)
			    || memcmp(MgmtMsg_p->Body.ProbeRsp.SsId.SsId,
				      vStaInfo_p->ScanParams.SsId, ssidLen)) {
				return;
			}
		}
		/* BSS Type Filter */
		if (vStaInfo_p->scanFilterMap & MLME_BSS_TYPE_FILTER) {
			if ((vStaInfo_p->ScanParams.BssType ==
			     BSS_INFRASTRUCTURE) &&
			    !MgmtMsg_p->Body.ProbeRsp.CapInfo.Ess) {
				return;
			} else if ((vStaInfo_p->ScanParams.BssType ==
				    BSS_INDEPENDENT) &&
		...
		if (vmacSta_p->gUserInitScan != TRUE) {
			...
			wpa2IE_p =
				...
				smeParseIeType(RSN_IEWPA2,
					       (UINT8 *) & MgmtMsg_p->Body.
					       ProbeRsp.SsId, parseLen);
			wpaIE_p =
				linkMgtParseWpaIe((UINT8 *) & MgmtMsg_p->Body.
						  ProbeRsp.SsId, parseLen);
			if (wpa2IE_p || wpaIE_p) {
				if (!vStaInfo_p->staSecurityMibs.
				    mib_PrivacyTable_p->RSNEnabled) {
					if (vStaInfo_p->staSystemMibs.
					    mib_StaCfg_p->wpawpa2Mode != 16)
						/* AP requires WPA/WPA2 - station WPA/WPA2 not enabled. */
						return;
				} else if (vStaInfo_p->staSecurityMibs.
					   mib_RSNConfigWPA2_p->
					   WPA2OnlyEnabled) {
					if (!wpa2IE_p) {
				...
				if ((vStaInfo_p->staSecurityMibs.
				     mib_PrivacyTable_p->
				     PrivInvoked ^ MgmtMsg_p->Body.ProbeRsp.
				     CapInfo.Privacy)) {
					return;
				}
			}
	...
	SPIN_LOCK_IRQSAVE(&vStaInfo_p->ScanResultsLock,
			  vStaInfo_p->ScanResultsFlags);
	AccumulateScanResults(vStaInfo_p, MgmtMsg_p, rfHdr_p);
	SPIN_UNLOCK_IRQRESTORE(&vStaInfo_p->ScanResultsLock,
			       vStaInfo_p->ScanResultsFlags);
}
...
**************************************************************************/
extern SINT32
syncSrv_SetStatusTimer(vmacStaInfo_t * vStaInfo_p, UINT8 status)
{
	UINT8 macIndex;
...
**************************************************************************/
extern SINT32
syncSrv_SetKeepAliveTimer(vmacStaInfo_t * vStaInfo_p, UINT8 status)
{
	if (!g_KeepAliveEnable)
		return MLME_SUCCESS;
	...
	if (status == 1) {
		mlmeApiStartTimer(vStaInfo_p,
				  ...
				  &syncSrv_KeepAliveTimeOut, g_KeepAliveTime);
	} else if (status == 0) {
		mlmeApiStopTimer(vStaInfo_p,
				 (UINT8 *) & vStaInfo_p->keepaliveTimer);
	}
...
**************************************************************************/
static UINT32
syncSrv_IsWpaMode(dot11MgtFrame_t * mgtFrame_p)
{
	void *data_p;
	UINT32 lenPacket;
	UINT32 lenOffset;
	...
	while (lenOffset < lenPacket) {
		if (*(IEEEtypes_ElementId_t *) data_p == RSN_IE) {
			...
			count++;
			memcpy((UINT8 *) & wpaInfo, (UINT8 *) (data_p + 2),
			       sizeof(UINT32));
			...
			if ((cipher[0] == 0x00) && (cipher[1] == 0x0f) &&
			    (cipher[2] == 0xac))
				return 2;
		}
		data_p += (2 + *((UINT8 *) (data_p + 1)));
...
extern void
SyncSrvSta_AccumulateSiteSurveyResults(vmacStaInfo_t * vStaInfo_p,
				       void *BssData_p, UINT8 * rfHdr_p)
{
	UINT32 i;
	BOOLEAN notFound = TRUE;
	...
	if ((attrib_p =
	     syncSrvSta_ParseAttrib((dot11MgtFrame_t *) BssData_p,
				    DS_PARAM_SET)) != NULL) {
		if (mlmeApiGetRfChannel(vStaInfo_p) !=
		    ((IEEEtypes_DsParamSet_t *) attrib_p)->CurrentChan) {
			return;
		}
	}
	for (i = 0; (i < SITE_SURVEY_ENTRY_MAX) && notFound; i++) {
		if (!vStaInfo_p->sSurveyTable_p->siteSurveyInfo[i].dirty) {
			bncEntry_p =
				&vStaInfo_p->sSurveyTable_p->siteSurveyInfo[i];
			memset(bncEntry_p, 0, sizeof(API_SURVEY_ENTRY));
			notFound = FALSE;
		} else if (!memcmp
			   (vStaInfo_p->sSurveyTable_p->siteSurveyInfo[i].BssId,
			    ((dot11MgtFrame_t *) BssData_p)->Hdr.BssId, 6)) {
			/* Record RSSI */
			vStaInfo_p->sSurveyTable_p->siteSurveyInfo[i].RSSI =
				((WLAN_RX_INFO *) rfHdr_p)->RSSI;
			return;
		}
	}
	...
	/* Record BSSID */
	memcpy(bncEntry_p->BssId, ((dot11MgtFrame_t *) BssData_p)->Hdr.BssId,
	       6);
	/* Record SSID */
	memcpy(bncEntry_p->SsId,
	       ((dot11MgtFrame_t *) BssData_p)->Body.Bcn.SsId.SsId,
	       ((dot11MgtFrame_t *) BssData_p)->Body.Bcn.SsId.Len);
	/* Record Channel */
	if ((attrib_p =
	     syncSrvSta_ParseAttrib((dot11MgtFrame_t *) BssData_p,
				    DS_PARAM_SET)) != NULL) {
	...
	/* Record IBSS bit */
	bncEntry_p->IBSS =
		((dot11MgtFrame_t *) BssData_p)->Body.Bcn.CapInfo.Ibss;
	/* Record WEP enable option */
	bncEntry_p->wepEnabled =
		((dot11MgtFrame_t *) BssData_p)->Body.Bcn.CapInfo.Privacy;
	/* Record WPA enable option */
	bncEntry_p->wpaEnabled =
		syncSrv_IsWpaMode((dot11MgtFrame_t *) BssData_p);
	/* Set dirty bit */
	bncEntry_p->dirty = 1;
}
...
**************************************************************************/
extern SINT32
syncSrv_CmdExceptionHandler(vmacStaInfo_t * vStaInfo_p)
{
	if (vStaInfo_p->cmdHistory & cmdScan) {
		syncSrv_SndScanCnfm(vStaInfo_p,
				    SCAN_RESULT_UNEXPECTED_ERROR, 0, 0, NULL);
	}
...
**************************************************************************/
extern void
syncSrv_PrbeRspNoBssHandler(vmacStaInfo_t * vStaInfo_p,
			    dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	/* Check BSSID */
	if (memcmp(&MgmtMsg_p->Hdr.BssId[0],
...
station11hTimerCB(void *data_p)
{
	...
	mlmeApiSetRfChannel(vStaInfo_p, vStaInfo_p->station11hChannel, 1,
			    FALSE);
	vStaInfo_p->JoinChannel = vStaInfo_p->station11hChannel;
	macMgmtMlme_RestartDataTraffic(staDev);
#ifdef WMON
	sprintf(g_wmon_DFSLog + strlen(g_wmon_DFSLog),
		"DFS:: Switched Channel to %d\n", vStaInfo_p->JoinChannel);
#endif //WMON
}
