...
/*============================================================================= */
static extStaDb_Status_e LocateAddr(vmacApInfo_t * vmacSta_p,
				    IEEEtypes_MacAddr_t * Addr_p,
				    ExtStaInfoItem_t ** Item_p, UINT32 * Idx_p);
...
extern void ProcessKeyMgmtData(vmacApInfo_t * vmacSta_p, void *,
			       IEEEtypes_MacAddr_t *, MhsmEvent_t *);
void extStaDb_ProcessAgeTick(UINT8 * data);
void ethStaDb_ProcessAgeTick(UINT8 * data);

int send_11n_aggregation_skb(struct net_device *netdev,
...
extern void ClientStatistics(vmacApInfo_t * vmacSta_p,
...
extern void ap8xLnxStat_clients_WQadd(vmacApInfo_t * vmac_p,
				      IEEEtypes_MacAddr_t * addr_p, UINT8 Hdlr);
...
static extStaDb_StaInfo_t *
extStaDb_GetCachedStaInfo(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p)
{
	if (!vmac_p->lru_stadb)
		goto not_found;
...
static void
extStaDb_DelCachedStaInfo(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p)
{
	if (!vmac_p->lru_stadb)
		return;
...
static void
extStaDb_UpdateCachedStaInfo(vmacApInfo_t * vmac_p,
...
*/
extern WL_STATUS
extStaDb_Init(vmacApInfo_t * vmacSta_p, UINT16 MaxStns)
{
	...
	if (vmacSta_p->StaCtl == NULL) {
		vmacSta_p->StaCtl =
			(struct STADB_CTL *)
			wl_kmalloc_autogfp(sizeof(struct STADB_CTL));
		if (vmacSta_p->StaCtl == NULL) {
	...
	vmacSta_p->StaCtl->aging_time_in_minutes =
		3 * 60 / AGING_TIMER_VALUE_IN_SECONDS;

	/*pre-allocate sta datebase */
	if (vmacSta_p->StaCtl->ExtStaInfoDb == NULL) {
		vmacSta_p->StaCtl->ExtStaInfoDb =
			(ExtStaInfoItem_t *) wl_kmalloc_autogfp(MaxStns *
								sizeof
								(ExtStaInfoItem_t));
		if (vmacSta_p->StaCtl->ExtStaInfoDb == NULL) {
			printk("fail to alloc memory\n");
		...
		memset(vmacSta_p->StaCtl->ExtStaInfoDb, 0,
		       MaxStns * sizeof(ExtStaInfoItem_t));
	}

	/*Allocate memory for station MU capabilities */
	if (vmacSta_p->StaCtl->MUStaDb == NULL) {
		vmacSta_p->StaCtl->MUStaDb =
			(MUCapStaNode_t *) wl_kmalloc_autogfp(MaxStns *
							      sizeof
							      (MUCapStaNode_t));
		if (vmacSta_p->StaCtl->MUStaDb == NULL) {
		...
		memset(vmacSta_p->StaCtl->MUStaDb, 0,
		       MaxStns * sizeof(MUCapStaNode_t));
	}
	...
	for (i = 0; i < MaxStns; i++) {
		...
		vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.State =
			UNAUTHENTICATED;
		vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.PwrMode =
			PWR_MODE_ACTIVE;
...
#endif
		memset(&vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.Addr, 0,
		       sizeof(IEEEtypes_MacAddr_t));
		vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.pn =
			wl_kmalloc(sizeof(rx_pn_info_t), GFP_KERNEL);
		if (vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.pn == NULL) {
			printk("fail to alloc memory for station db pn entry\n");
			return OS_FAIL;
		}
		TimerInit(&vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.
			  keyMgmtHskHsm.keyTimer);
		TimerInit(&vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.mgtAssoc.
			  timer);
		TimerInit(&vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.
			  mgtAuthReq.timer);
		TimerInit(&vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.
		...
		TimerInit(&vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.
			  SA_Query_Timer);
#endif
		ListPutItem(&vmacSta_p->StaCtl->FreeStaList,
			    (ListItem *) (vmacSta_p->StaCtl->ExtStaInfoDb + i));

		/*Init MU db list */
		vmacSta_p->StaCtl->MUStaDb[i].nxt = NULL;
		vmacSta_p->StaCtl->MUStaDb[i].prv = NULL;
		vmacSta_p->StaCtl->MUStaDb[i].StaInfo_p = NULL;
		ListPutItem(&vmacSta_p->StaCtl->FreeMUStaList,
			    (ListItem *) (vmacSta_p->StaCtl->MUStaDb + i));
	}
...
extern void
extStaDb_Cleanup(vmacApInfo_t * vmacSta_p)
{
	...
	if (vmacSta_p->StaCtl->ExtStaInfoDb) {
		...
		for (i = 0; i < sta_num; i++) {
			if (vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.pn)
				wl_kfree(vmacSta_p->StaCtl->ExtStaInfoDb[i].
					 StaInfo.pn);
		}
...
*         ADD_SUCCESS          
*/
extern extStaDb_Status_e
extStaDb_AddSta(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * StaInfo_p)
{
	...
	/*------------------------------------------------------------------*/
	if ((result =
	     LocateAddr(vmacSta_p, &(StaInfo_p->Addr), &item_p,
			&idx)) != LOCATE_FAILURE) {
	...
	if (tmp) {
		rx_pn_info_t *tmpPn;
		ExtStaInfoItem_t *search_p =
			vmacSta_p->StaCtl->ExtStaInfoDb_p[idx];
		...
		memset(&item_p->StaInfo.HtElem, 0,
		       sizeof(IEEEtypes_HT_Element_t));
		memset(&item_p->StaInfo.AddHtElme, 0,
		       sizeof(IEEEtypes_Add_HT_Element_t));

		item_p->StaInfo.MUStaListNode = NULL;
		item_p->StaInfo.MUset = NULL;
#ifdef MRVL_80211R
		memset(&item_p->StaInfo.keyMgmtStateInfo.RsnIEBuf, 0,
		       MAX_SIZE_RSN_IE_BUF);
		memset(&item_p->StaInfo.keyMgmtStateInfo.mdie_buf, 0,
		       MAX_SIZE_MDIE_BUF);
		memset(&item_p->StaInfo.keyMgmtStateInfo.ftie_buf, 0,
		       MAX_SIZE_FTIE_BUF);
		...
		memset(&item_p->StaInfo.DeFragBufInfo, 0,
		       sizeof(DeFragBufInfo_t));
...
void
extStaDb_ProcessKeepAliveTimer(UINT8 * data)
{
...
void
Disable_extStaDb_ProcessKeepAliveTimer(vmacApInfo_t * vmacSta_p)
{
	TimerRemove(&vmacSta_p->KeepAliveTimer);
}

extern void
extStaDb_ProcessKeepAliveTimerInit(vmacApInfo_t * vmacSta_p)
{
	TimerInit(&vmacSta_p->KeepAliveTimer);
	TimerFireIn(&vmacSta_p->KeepAliveTimer, 1,
		    &extStaDb_ProcessKeepAliveTimer, (unsigned char *)vmacSta_p,
		    AGING_TIMER_VALUE_IN_SECONDS);
}

void
stationAging(UINT8 * data)
{
	vmacApInfo_t *vmacSta_p = (vmacApInfo_t *) data;
...
void
Disable_stationAgingTimer(vmacApInfo_t * vmacSta_p)
{
	TimerRemove(&vmacSta_p->AgingTimer);
}

extern void
extStaDb_AgingTimerInit(vmacApInfo_t * vmacSta_p)
{
	/** Start aging timer */
	TimerInit(&vmacSta_p->AgingTimer);
	TimerFireIn(&vmacSta_p->AgingTimer, 1, &stationAging,
		    (unsigned char *)vmacSta_p,
		    AGING_TIMER_VALUE_IN_SECONDS * 10);
}
...
#endif
#ifdef SOC_W906X
extern int StaKeepAliveCheck(vmacApInfo_t * vmacSta_p,
			     extStaDb_StaInfo_t * StaInfo_p, int cmd);
#define STA_KEEP_ALIVE_CHECK_THRES	6	// 6 x 10sec
#endif /* SOC_W906X */
void
extStaDb_ProcessAgeTick(UINT8 * data)
{
	vmacApInfo_t *vmacSta_p = (vmacApInfo_t *) data;
#ifdef SOC_W906X
#ifdef MBSS
	...
	while (Curr_p != NULL) {
...
#endif
		{
...
#endif
			{
				...
				if (Item_p->StaInfo.TimeStamp <=
				    STA_KEEP_ALIVE_CHECK_THRES) {
					int ts = Item_p->StaInfo.TimeStamp;
					SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->
							       StaCtl->dbLock,
							       dbflags);
					StaKeepAliveCheck(vmacSta_p, &Item_p->StaInfo, 1);	//send tx test pkt
					SPIN_LOCK_IRQSAVE(&vmacSta_p->StaCtl->
							  dbLock, dbflags);
					Item_p->StaInfo.TimeStamp = ts;
				} else if (Item_p->StaInfo.TimeStamp ==
					   vmacSta_p->StaCtl->
					   aging_time_in_minutes - 1) {
			...
			if ((StaInfo_p =
				...
				SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->
						       dbLock, dbflags);
				return;
			}
#ifdef CLIENT_SUPPORT
...
#endif
			{
				...
				SPIN_LOCK_IRQSAVE(&vmacSta_p->StaCtl->dbLock,
						  dbflags);

				if (sta_alive) {
					Item_p->StaInfo.TimeStamp =
						vmacSta_p->StaCtl->
						aging_time_in_minutes;
					Curr_p = Curr_p->nxt;
					continue;
				}
#endif /* SOC_W906X */
				...
				urHash_RemoveSta(vmacSta_p,
						 &(Item_p->StaInfo.Addr), 0);
#endif
			...
			SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock,
					       dbflags);
#ifdef APEVT_STA_ASSOC_SUPPORT

			apEvtRun(APEVT_STA_ASSOC, (uint8 *) & StaInfo.Addr,
				 APEVT_STA_DEAUTHENTICATED);
#endif
			extStaDb_DelSta(vmacSta_p, &(Item_p->StaInfo.Addr),
...
#ifdef STADB_IN_CACHE
			{
				...
				wlpptr = NETDEV_PRIV_P(struct wlprivate,
						       vmacSta_p->dev);
				for (i = 0; wlpptr->vdev[i] && i <= bss_num;
				     i++) {
					wlpptr1 =
						NETDEV_PRIV_P(struct wlprivate,
							      wlpptr->vdev[i]);
					extStaDb_DelCachedStaInfo(wlpptr1->
								  vmacSta_p,
								  &(Item_p->
								    StaInfo.
								    Addr));
			...
			wlFwSetNewStn(vmacSta_p->dev,
				      (u_int8_t *) & (Item_p->StaInfo.Addr), 0,
				      0, StaInfoDbActionRemoveEntry, NULL, 0, 0,
				      0);
...
void
extStaDb_RemoveAllStns(vmacApInfo_t * vmac_p, UINT16 Reason)
{
		...
		Item_p = Curr_p;
		{
			...
			if ((StaInfo_p =
			     extStaDb_GetStaInfo(vmac_p,
						 &(Item_p->StaInfo.Addr),
						 STADB_NO_BLOCK)) == NULL ||
			    memcmp(&vmac_p->macBssId, &StaInfo_p->Bssid,
				   sizeof(IEEEtypes_MacAddr_t))) {
				...
				if (removeall) {
					SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->
							       StaCtl->dbLock,
							       dbflags);
					extStaDb_DelSta(vmac_p,
							&(Item_p->StaInfo.Addr),
							STADB_DONT_UPDATE_AGINGTIME);
					SPIN_LOCK_IRQSAVE(&vmacSta_p->StaCtl->
							  dbLock, dbflags);
					Curr_p = (ExtStaInfoItem_t
						  *) (vmacSta_p->StaCtl->
						      StaList.head);
					continue;
				}
			...
			if (StaInfo_p->State == ASSOCIATED) {
				SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->
						       dbLock, dbflags);
#ifdef SOC_W906X
				macMgmtMlme_SendDeauthenticateMsg(vmac_p,
								  &Item_p->
								  StaInfo.Addr,
								  Item_p->
								  StaInfo.StnId,
	...
	while (Curr_p != NULL) {
		Item_p = Curr_p;
		{
			...
			if ((StaInfo_p =
				...
				extStaDb_DelSta(vmac_p, &(Item_p->StaInfo.Addr),
						STADB_DONT_UPDATE_AGINGTIME);
				wlFwSetNewStn(vmac_p->dev,
					      (u_int8_t *) & (Item_p->StaInfo.
							      Addr), 0, 0,
					      StaInfoDbActionRemoveEntry, NULL,
					      0, 0, 0);
				SPIN_LOCK_IRQSAVE(&vmacSta_p->StaCtl->dbLock,
						  dbflags);
			}
...
*/
extern extStaDb_Status_e
extStaDb_DelSta(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p, int option)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	...
	SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->
			  txRateHistoLock[item_p->StaInfo.StnId - 1],
			  txRateHistoflags);

	if (wlpptr->wlpd_p->txRateHistogram[item_p->StaInfo.StnId - 1] != NULL) {
		wl_kfree(wlpptr->wlpd_p->
			 txRateHistogram[item_p->StaInfo.StnId - 1]);
		wlpptr->wlpd_p->txRateHistogram[item_p->StaInfo.StnId - 1] =
			NULL;
	}
	...
	if (wlpptr->wlpd_p->txRateHistogram[item_p->StaInfo.StnId] != NULL) {
		wl_kfree(wlpptr->wlpd_p->
			 txRateHistogram[item_p->StaInfo.StnId]);
		wlpptr->wlpd_p->txRateHistogram[item_p->StaInfo.StnId] = NULL;
	}

	if ((item_p->StaInfo.StnId < sta_num) &&
	    (wlpptr->wlpd_p->scheHistogram[item_p->StaInfo.StnId] != NULL)) {
		memset(wlpptr->wlpd_p->scheHistogram[item_p->StaInfo.StnId], 0,
	...
	if (item_p->StaInfo.StnId < sta_num)
		for (tid = 0; tid < MAX_UP; tid++)
			TimerRemove(&wlpptr->wlpd_p->
				    AmpduPckReorder[item_p->StaInfo.StnId].
				    timer[tid]);
#endif //SOC_W906X
	ListPutItem(&vmacSta_p->StaCtl->FreeStaList,
		    ListRmvItem(&vmacSta_p->StaCtl->StaList,
				(ListItem *) item_p));
	/*-------------------------------------------------------*/
	/* Finished - give back the semaphore and return status. */
	/*-------------------------------------------------------*/
...
extStaDb_GetStaInfo(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		    int option)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
...
#ifdef STADB_IN_CACHE
	if ((option & STADB_FIND_IN_CACHE) &&
	    (StaInfo_p = extStaDb_GetCachedStaInfo(vmac_p, Addr_p))) {
		if (need_lock)
			SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock,
					       dbflags);
	...
	/*------------------------------------------------------------------*/
	if ((result =
	     LocateAddr(vmacSta_p, Addr_p, &item_p, &idx)) != LOCATE_SUCCESS) {
		if (need_lock)
			SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock,
					       dbflags);
		return NULL;
	...
	if ((option != STADB_DONT_UPDATE_AGINGTIME) &&
	...
	if (StaInfo_p->Client)
		...
		if (!memcmp
		    (&vmac_p->macBssId, &StaInfo_p->Bssid,
		     sizeof(IEEEtypes_MacAddr_t)))
			goto stadb_found;
		else
			return NULL;
...
*
*****************************************************************************/
static extStaDb_Status_e
LocateAddr(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
	   ExtStaInfoItem_t ** Item_pp, UINT32 * Idx_p)
{
	UINT32 key;
	vmacApInfo_t *vmacSta_p;
	...
	*Item_pp = vmacSta_p->StaCtl->ExtStaInfoDb_p[*Idx_p];
	if (*Item_pp == NULL) {
		...
		if (!memcmp((*Item_pp)->StaInfo.Addr,
			    Addr_p, 6 * sizeof(IEEEtypes_Addr_t))) {
...
*/
extern UINT16
extStaDb_entries(vmacApInfo_t * vmac_p, UINT8 flag)
{
	ListItem *search;
...
#endif
		if ((search1->StaInfo.AP == FALSE) &&
			    ((search1->StaInfo.State == AUTHENTICATED)
				     || (search1->StaInfo.State ==
						 ASSOCIATED))) {
			if (!memcmp
			    (&vmac_p->macBssId, &search1->StaInfo.Bssid,
			     sizeof(IEEEtypes_MacAddr_t)))
				StaList_entries++;
#ifdef NPROTECTION
...
extern UINT16
extStaDb_entries2(vmacApInfo_t * vmac_p, UINT8 flag)
{
	ListItem *search;
	ExtStaInfoItem_t *search1;
	UINT16 StaList_entries = 0;
	...
	while (search) {
		search1 = (ExtStaInfoItem_t *) search;
		if (1
		    /**(search1->StaInfo.AP == FALSE) && ((search1->StaInfo.State == AUTHENTICATED) || (search1->StaInfo.State == ASSOCIATED))**/
		    ) {
			if (1
...
*/
extern UINT16
extStaDb_list(vmacApInfo_t * vmac_p, UINT8 * buf, UINT8 get)
{
	int j;
	ListItem *search;
	ExtStaInfoItem_t *search1;
...
extern UINT16
extStaDb_list2(vmacApInfo_t * vmac_p, UINT8 * buf, UINT8 get)
{
	int j;
	ListItem *search;
	ExtStaInfoItem_t *search1;
...
extern UINT16
extStaDb_AggrCk(vmacApInfo_t * vmac_p)
{
	ListItem *search;
	ExtStaInfoItem_t *search1;
	vmacApInfo_t *vmacSta_p;
	...
	while (search) {
		...
		if ((pAggr11n->type & WL_WLAN_TYPE_AMSDU) &&
		    (*(search1->StaInfo.mib_p->pMib_11nAggrMode) &
		     WL_MODE_AMSDU_TX_MASK)) {
			if (pAggr11n->threshold &&
			    (pAggr11n->txcnt > pAggr11n->threshold)) {
...
extern UINT16
extStaDb_AggrFrameCk(vmacApInfo_t * vmac_p, int force)
{
	int retval = 0;
	ListItem *search;
	ExtStaInfoItem_t *search1;
	...
	while (search) {
		...
		{
			SPIN_LOCK_IRQSAVE(&search1->StaInfo.aggr11n.Lock,
					  flags);
			{
#ifdef WDS_FEATURE
				struct net_device *dev =
					(struct net_device *)(search1->StaInfo.
							      wdsInfo);
				if (dev)
					retval = send_11n_aggregation_skb(dev,
									  &search1->
									  StaInfo,
									  force);
				else
#endif
					retval = send_11n_aggregation_skb
						(search1->StaInfo.dev,
						 &search1->StaInfo, force);
			}
			SPIN_UNLOCK_IRQRESTORE(&search1->StaInfo.aggr11n.Lock,
					       flags);
		}
		search = search->nxt;
	}
...
extern extStaDb_Status_e
extStaDb_RemoveStaNSendDeauthMsg(vmacApInfo_t * vmac_p,
				 IEEEtypes_MacAddr_t * Addr_p)
{
	extStaDb_StaInfo_t *StaInfo_p;
	vmacApInfo_t *vmacSta_p;
	...
	//SPIN_LOCK_IRQSAVE(&vmacSta_p->StaCtl->dbLock, dbflags);
	if ((StaInfo_p =
	     extStaDb_GetStaInfo(vmac_p, Addr_p,
...
#ifdef SOC_W906X
		macMgmtMlme_SendDeauthenticateMsg(vmac_p, Addr_p,
						  StaInfo_p->StnId,
						  IEEEtypes_REASON_DISASSOC_INACTIVE,
						  TRUE);
#else
...
//      SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock, dbflags);    
	...
	FreeStnId(vmac_p, StaInfo_p->StnId);
	wlFwSetNewStn(vmac_p->dev, (u_int8_t *) Addr_p, 0, 0,
...
*/
extern extStaDb_Status_e
extStaDb_RemoveSta(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p)
{
	extStaDb_StaInfo_t *StaInfo_p;
	vmacApInfo_t *vmacSta_p;
	unsigned long dbflags;
	...
	SPIN_LOCK_IRQSAVE(&vmacSta_p->StaCtl->dbLock, dbflags);
	if ((StaInfo_p =
	     extStaDb_GetStaInfo(vmac_p, Addr_p, STADB_NO_BLOCK)) == NULL) {
		SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock, dbflags);
		/* Station not known, do nothing */
		return LOCATE_FAILURE;
	}
...
UINT8
GetAssociateMacAddress(vmacApInfo_t * vmac_p, unsigned char *MacAddress)
{
	ListItem *search;
	ExtStaInfoItem_t *search1;
	vmacApInfo_t *vmacSta_p;
...
void
extStaDb_SetNewState4AllSta(vmacApInfo_t * vmac_p, extStaDb_State_e NewState)
{
	vmacApInfo_t *vmacSta_p;
	extStaDb_StaInfo_t *StaInfo_p;
	ExtStaInfoItem_t *Curr_p, *Item_p;
...
void
extStaDb_SendGrpKeyMsgToAllSta(vmacApInfo_t * vmac_p)
{
	MhsmEvent_t msg;
	vmacApInfo_t *vmacSta_p;
	extStaDb_StaInfo_t *StaInfo_p;
	...
	while (Curr_p != NULL) {
		...
		if ((StaInfo_p =
			...
			if (StaInfo_p->State == ASSOCIATED) {
				...
				if (StaInfo_p->keyMgmtHskHsm.super.pCurrent !=
				    NULL) {
...
extern extStaDb_Status_e
extStaDb_SetRSNDataTrafficEnabled(vmacApInfo_t * vmac_p,
...
extStaDb_Status_e
extStaDb_SetRSNPwkAndDataTraffic(vmacApInfo_t * vmac_p,
				 IEEEtypes_MacAddr_t * Addr_p,
				 ...
				 UINT32 * pRxMICKey)
{
	...
	memcpy(item_p->StaInfo.keyMgmtStateInfo.PairwiseTempKey1, pEncryptKey,
	       TK_SIZE_MAX);
	memcpy(item_p->StaInfo.keyMgmtStateInfo.RSNPwkTxMICKey, pTxMICKey, 8);
	memcpy(item_p->StaInfo.keyMgmtStateInfo.RSNPwkRxMICKey, pRxMICKey, 8);
	/*-------------------------------------------------------*/
...
extern extStaDb_Status_e
extStaDb_SetRSNPwk(vmacApInfo_t * vmac_p,
		   IEEEtypes_MacAddr_t * Addr_p,
...
extern extStaDb_Status_e
extStaDb_SetRSNPmk(vmacApInfo_t * vmac_p,
...
extern extStaDb_Status_e
extStaDb_GetRSN_IE(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		   UINT8 * RsnIE_p)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
...
#ifdef MRVL_80211R
extern extStaDb_Status_e
extStaDb_Get_11r_IEs(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		     UINT8 * iebuf, UINT16 * len, UINT8 * reassoc)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	UINT32 idx;
	...
	/* Get the value. */
	/*------------------------*/
	if (item_p->StaInfo.keyMgmtStateInfo.mdie_buf[0] == MD_IE &&
	    item_p->StaInfo.keyMgmtStateInfo.mdie_buf[1] == 3) {
		memcpy((UINT8 *) iebuf,
		       item_p->StaInfo.keyMgmtStateInfo.mdie_buf,
		       2 + *(item_p->StaInfo.keyMgmtStateInfo.mdie_buf + 1));
		*len = (iebuf[1] + 2);
	} else {
		*len = 0;
		SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock, dbflags);
	...
	if (item_p->StaInfo.keyMgmtStateInfo.ftie_buf[0] == FT_IE &&
	    item_p->StaInfo.keyMgmtStateInfo.ftie_buf[1] != 0) {
		memcpy((UINT8 *) & iebuf[*len],
		       item_p->StaInfo.keyMgmtStateInfo.ftie_buf,
		       2 + *(item_p->StaInfo.keyMgmtStateInfo.ftie_buf + 1));
...
extern extStaDb_Status_e
extStaDb_GetWSC_IE(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		   UINT8 * WscIE_p)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
...
extern extStaDb_Status_e
extStaDb_SetRSN_IE(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		   IEEEtypes_RSN_IE_t * RsnIE_p)
{
	...
	if ((RsnIELen + 2) <= MAX_SIZE_RSN_IE_BUF) {
		memset(item_p->StaInfo.keyMgmtStateInfo.RsnIEBuf, 0,
		       MAX_SIZE_RSN_IE_BUF);
		memcpy((UINT8 *) item_p->StaInfo.keyMgmtStateInfo.RsnIEBuf,
		       RsnIE_p, RsnIELen + 2);
	} else {
		SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock, dbflags);
...
extern extStaDb_Status_e
extStaDb_GetPairwiseTSC(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
...
extern extStaDb_Status_e
extStaDb_SetPairwiseTSC(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
...
extStaDb_Status_e
extStaDb_GetStaInfoAndTxKeys(vmacApInfo_t * vmac_p,
			     IEEEtypes_MacAddr_t * Addr_p,
			     extStaDb_StaInfo_t * StaInfo_p,
			     UINT32 AgingTimeMode)
{
	...
	if (LocateAddr(vmacSta_p, Addr_p, &item_p, &idx) == LOCATE_SUCCESS) {
		...
		memcpy(StaInfo_p->keyMgmtStateInfo.PairwiseTempKey1,
		       item_p->StaInfo.keyMgmtStateInfo.PairwiseTempKey1,
		       TK_SIZE_MAX);
		...
		memcpy(StaInfo_p->keyMgmtStateInfo.RsnIEBuf,
		       item_p->StaInfo.keyMgmtStateInfo.RsnIEBuf,
		       MAX_SIZE_RSN_IE_BUF);
...
extStaDb_Status_e
extStaDb_GetRSNPwk(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		   UINT8 * pEncryptKey, UINT32 * pTxMICKey, UINT32 * pRxMICKey)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	...
	memcpy(pEncryptKey, item_p->StaInfo.keyMgmtStateInfo.PairwiseTempKey1,
	       TK_SIZE_MAX);
	memcpy(pTxMICKey, item_p->StaInfo.keyMgmtStateInfo.RSNPwkTxMICKey, 8);
	memcpy(pRxMICKey, item_p->StaInfo.keyMgmtStateInfo.RSNPwkRxMICKey, 8);
	/*-------------------------------------------------------*/
...
extStaDb_Status_e
extStaDb_GetStaInfoAndRxKeys(vmacApInfo_t * vmac_p,
			     IEEEtypes_MacAddr_t * Addr_p,
			     extStaDb_StaInfo_t * StaInfo_p,
			     UINT32 AgingTimeMode)
{
...
extStaDb_Status_e
extStaDb_GetKeyMgmtInfo(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
...
extStaDb_Status_e
extStaDb_SetKeyMgmtInfo(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
...
UINT8
extStaDb_GetQoSOptn(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	UINT32 idx;
...
extern extStaDb_Status_e
extStaDb_SetQoSOptn(vmacApInfo_t * vmac_p,
		    IEEEtypes_MacAddr_t * Addr_p, UINT8 QosOptn)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	UINT32 idx;
...
extern extStaDb_Status_e
extStaDb_SetQosInfo(vmacApInfo_t * vmac_p,
		    IEEEtypes_MacAddr_t * Addr_p, QoS_WmeInfo_Info_t * pQosinfo)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	UINT32 idx;
...
extern extStaDb_Status_e
extStaDb_GetWMM_DeliveryEnableInfo(vmacApInfo_t * vmac_p,
				   IEEEtypes_MacAddr_t * Addr_p,
				   UINT8 Priority, UINT8 * Wmm_DeliveryInfo,
				   UINT8 * UpdateTim)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	...
	*UpdateTim = 0;
	...
	if ((item_p->StaInfo.Qosinfo.Uapsd_ac_be &&
	     item_p->StaInfo.Qosinfo.Uapsd_ac_bk &&
	     item_p->StaInfo.Qosinfo.Uapsd_ac_vi &&
	     item_p->StaInfo.Qosinfo.Uapsd_ac_vo) || *Wmm_DeliveryInfo == 0)
		*UpdateTim = 1;
...
extern UINT8
extStaDb_Check_Uapsd_Capability(vmacApInfo_t * vmac_p,
				IEEEtypes_MacAddr_t * Addr_p)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	...
	if ((item_p->StaInfo.Qosinfo.Uapsd_ac_be ||
	     item_p->StaInfo.Qosinfo.Uapsd_ac_bk ||
	     item_p->StaInfo.Qosinfo.Uapsd_ac_vi ||
	     item_p->StaInfo.Qosinfo.Uapsd_ac_vo)) {
		SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock, dbflags);
		return 1;
	}
...
extern UINT8
extStaDb_Check_ALL_AC_DeliveryEnableInfo(vmacApInfo_t * vmac_p,
					 IEEEtypes_MacAddr_t * Addr_p)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
...
extStaDb_Status_e
extStaDb_SetPhase1Key(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		      UINT16 * Phase1Key, PacketType_e mode, UINT32 RxIV32)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	UINT32 idx;
	...
	if (mode == Tx)
		memcpy(item_p->StaInfo.keyMgmtStateInfo.Phase1KeyTx, Phase1Key,
		       10);
	//(*my_memcpyFp)(item_p->StaInfo.keyMgmtStateInfo.Phase1KeyTx, Phase1Key, 10);
	else {
		memcpy(item_p->StaInfo.keyMgmtStateInfo.Phase1KeyRx, Phase1Key,
		       10);
...
int
StnMacAddressCopy(vmacApInfo_t * vmac_p, unsigned char *tempmac)
{
	int i = 0;
	vmacApInfo_t *vmacSta_p;
	...
	for (i = 0; i < 32; i++) {
		memcpy((tempmac + i * 6),
		       vmacSta_p->StaCtl->ExtStaInfoDb[i].StaInfo.Addr, 6);
...
extern extStaDb_Status_e
extStaDb_UpdateAgingTime(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p)
{
	extStaDb_Status_e result;
	ExtStaInfoItem_t *item_p = NULL;
	UINT32 idx;
	...
	/*------------------------*/
	item_p->StaInfo.TimeStamp =
		vmacSta_p->StaCtl->aging_time_in_minutes * 15;
...
extStaDb_Status_e
extStaDb_GetStaInfo_noWait(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
...
extern int
set_sta_aging_time(vmacApInfo_t * vmac_p, int minutes)
{
	vmacApInfo_t *vmacSta_p;
	...
	if (vmac_p->master)
		...
		vmacSta_p = vmac_p;
	vmacSta_p->StaCtl->aging_time_in_minutes =
		minutes * 60 / AGING_TIMER_VALUE_IN_SECONDS;
	return minutes;
}
...
void
extStaDb_AGStnsCnt(vmacApInfo_t * vmac_p)
{
	int j;
	ListItem *search;
	ExtStaInfoItem_t *search1;
...
#ifdef IEEE80211H
extern extStaDb_Status_e
extStaDb_GetMeasurementInfo(vmacApInfo_t * vmac_p,
			    IEEEtypes_MacAddr_t * Addr_p,
...
extern extStaDb_Status_e
extStaDb_SetMeasurementInfo(vmacApInfo_t * vmac_p,
			    IEEEtypes_MacAddr_t * Addr_p,
			    extStaDb_measurement_info_t * measureInfo_p)
{
	extStaDb_Status_e result;
...
#ifdef INTOLERANT40
void
extStaDb_SendBeaconReqMeasureReqAction(vmacApInfo_t * vmac_p)
{
	extern BOOLEAN macMgmtMlme_SendBeaconReqMeasureReqAction(struct
								 net_device
								 *dev,
								 IEEEtypes_MacAddr_t
								 * Addr);
	...
	while (search) {
		...
		if ((search1->StaInfo.AP == FALSE) &&
		    (search1->StaInfo.State == ASSOCIATED) &&
		    (search1->StaInfo.ClientMode == NONLY_MODE)) {
			macMgmtMlme_SendBeaconReqMeasureReqAction(vmac_p->dev,
								  (IEEEtypes_MacAddr_t
								   *) search1->
								  StaInfo.Addr);
		}
...
WL_STATUS
ethStaDb_Init(vmacApInfo_t * vmacSta_p, UINT16 MaxStns)
{
	UINT32 i;
	...
	if (vmacSta_p->EthStaCtl == NULL) {
		vmacSta_p->EthStaCtl =
			(struct ETHSTADB_CTL *)
			wl_kmalloc_autogfp(sizeof(struct ETHSTADB_CTL));
	...
	vmacSta_p->EthStaCtl->aging_time_in_minutes =
		3 * 60 / AGING_TIMER_VALUE_IN_SECONDS;

	/*pre-allocate sta datebase */
	if (vmacSta_p->EthStaCtl->EthStaDb == NULL) {
		vmacSta_p->EthStaCtl->EthStaDb =
			(EthStaItem_t *) wl_kmalloc_autogfp(MaxStns *
							    sizeof
							    (EthStaItem_t));
		if (vmacSta_p->EthStaCtl->EthStaDb == NULL) {
			printk("fail to alloc memory\n");
		...
		memset(vmacSta_p->EthStaCtl->EthStaDb, 0,
		       MaxStns * sizeof(EthStaItem_t));
	...
	for (i = 0; i < MaxStns; i++) {
		...
		memset(&vmacSta_p->EthStaCtl->EthStaDb[i].ethStaInfo.Addr, 0,
		       sizeof(IEEEtypes_MacAddr_t));
		ListPutItem(&vmacSta_p->EthStaCtl->FreeEthStaList,
			    (ListItem *) (vmacSta_p->EthStaCtl->EthStaDb + i));
	}
...
static extStaDb_Status_e
EthStaLocateAddr(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		 EthStaItem_t ** Item_pp, UINT32 * Idx_p)
{
	UINT32 key;
...
extStaDb_Status_e
ethStaDb_AddSta(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		extStaDb_StaInfo_t * StaInfo_p)
{
	extStaDb_Status_e result;
...
extStaDb_Status_e
ethStaDb_DelSta(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p, int option)
{
	extStaDb_Status_e result;
	EthStaItem_t *item_p = NULL;
	UINT32 idx;
		...
		if (search_p && item_p) {
			if (search_p->prv_ht && search_p->nxt_ht) {	/*middle element */
				...
				if (search_p->prv_ht) {	/*this is tail */
					...
					search_p->nxt_ht->prv_ht = NULL;
					vmacSta_p->EthStaCtl->EthStaDb_p[idx] =
						search_p->nxt_ht;
				} else {	/*one and only */
					vmacSta_p->EthStaCtl->EthStaDb_p[idx] =
						NULL;
	...
	ListPutItem(&vmacSta_p->EthStaCtl->FreeEthStaList,
		    ListRmvItem(&vmacSta_p->EthStaCtl->EthStaList,
				(ListItem *) item_p));
	/*-------------------------------------------------------*/
	/* Finished - give back the semaphore and return status. */
	/*-------------------------------------------------------*/
...
eth_StaInfo_t *
ethStaDb_GetStaInfo(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p,
		    int option)
{
	extStaDb_Status_e result;
	EthStaItem_t *item_p = NULL;
	...
	/*------------------------------------------------------------------*/
	if ((result =
	     EthStaLocateAddr(vmacSta_p, Addr_p, &item_p,
			      &idx)) != LOCATE_SUCCESS) {
		if (need_lock)
			SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock,
					       dbflags);
	...
	if (vmac_p->VMacEntry.modeOfService != VMAC_MODE_CLNT_INFRA)
		if ((item_p->ethStaInfo.pStaInfo_t) &&
		    memcmp(&vmac_p->macBssId,
			   &((item_p->ethStaInfo.pStaInfo_t)->Bssid),
			   sizeof(IEEEtypes_MacAddr_t)))
			return NULL;
...
void
ethStaDb_RemoveAllStns(vmacApInfo_t * vmac_p)
{
	...
	while (Curr_p != NULL) {
		Item_p = Curr_p;
		{
			...
			ethStaDb_DelSta(vmac_p, &(Item_p->ethStaInfo.Addr),
					STADB_NO_BLOCK);
			Curr_p = (EthStaItem_t *) (vmacSta_p->EthStaCtl->
						   EthStaList.head);
		}
	}
	SPIN_UNLOCK_IRQRESTORE(&vmacSta_p->StaCtl->dbLock, dbflags);
...
UINT16
ethStaDb_list(vmacApInfo_t * vmac_p)
{
	ListItem *search;
	...
	while (search) {
		search1 = (EthStaItem_t *) search;
		{
			struct extStaDb_StaInfo_t *pStaInfo_t =
				search1->ethStaInfo.pStaInfo_t;
			UINT8 *p = (UINT8 *) search1->ethStaInfo.Addr;

			if (vmac_p->VMacEntry.modeOfService !=
...
extStaDb_Status_e
ethStaDb_RemoveSta(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p)
{
	...
	if ((pEthStaInfo =
	     ethStaDb_GetStaInfo(vmac_p, Addr_p, STADB_NO_BLOCK)) == NULL) {
		/* Station not known, do nothing */
		return LOCATE_FAILURE;
	}
...
extStaDb_Status_e
ethStaDb_RemoveStaPerWlan(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p)
{
	EthStaItem_t *Curr_p, *Item_p;
	eth_StaInfo_t *ethStaInfo_p;
	vmacApInfo_t *vmacSta_p = vmac_p;
	...
	while (Curr_p != NULL) {
		Item_p = Curr_p;
		{
			...
			if (Item_p->ethStaInfo.pStaInfo_t &&
			    !memcmp(Addr_p, Item_p->ethStaInfo.pStaInfo_t->Addr,
				    sizeof(IEEEtypes_MacAddr_t))) {
				ethStaDb_DelSta(vmac_p,
						&(Item_p->ethStaInfo.Addr),
						STADB_NO_BLOCK);
				Curr_p = (EthStaItem_t *) (vmacSta_p->
							   EthStaCtl->
							   EthStaList.head);
			} else
				Curr_p = Curr_p->nxt;
...
void
ethStaDb_ProcessAgeTick(UINT8 * data)
{
	vmacApInfo_t *vmacSta_p = (vmacApInfo_t *) data;
	EthStaItem_t *Curr_p, *Item_p;
	eth_StaInfo_t *ethStaInfo_p;
	...
	while (Curr_p != NULL) {
		...
		if (--Item_p->ethStaInfo.TimeStamp) {
			...
			ethStaDb_DelSta(vmacSta_p, &(Item_p->ethStaInfo.Addr),
					STADB_NO_BLOCK);
...
extern int
set_rptrSta_aging_time(vmacApInfo_t * vmac_p, int minutes)
{
	vmacApInfo_t *vmacSta_p;
...
*/
extern extStaDb_StaInfo_t *
extStaDb_GetStaInfoStn(vmacApInfo_t * vmac_p, UINT8 stnid)
{
	ListItem *search;
...
int
FourAddr_SearchHashEntry(IEEEtypes_MacAddr_t * Addr_p,
...
int
FourAddr_AddHashEntry(MultiAP_4Addr_Entry_t ** currentEntry,
		      ...
		      IEEEtypes_MacAddr_t * Addr4_p)
{
	...
	if ((*currentEntry)->head == 1) {
		...
		memcpy((*currentEntry)->tar, Addr2_p,
		       sizeof(IEEEtypes_MacAddr_t));
		memcpy((*currentEntry)->SA, Addr4_p,
		       sizeof(IEEEtypes_MacAddr_t));
		...
		if (((*currentEntry)->nxtEntry =
		      ...
		      *) (wl_kmalloc(sizeof(MultiAP_4Addr_Entry_t),
				     GFP_ATOMIC))) == NULL) {
			return -1;
		}
...
int
FourAddr_ClearHashEntrySTA(void)
{
	MultiAP_4Addr_Entry_t *entry;
	MultiAP_4Addr_Entry_t *delEntry;
	int i;
...
extern extStaDb_StaInfo_t *
extStaDb_GetVapStaInfoStn(vmacApInfo_t * vmacSta_p, UINT8 stnid)
{
	ListItem *search;
	ExtStaInfoItem_t *Entry;
	extStaDb_StaInfo_t *StaInfo_p = NULL;
