...
static void
assocSrv_SndAssocCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 assocResult)
{
	IEEEtypes_AssocCfrm_t AssocCfrm;

	AssocCfrm.Result = assocResult;
	if (mlmeApiSndNotification
	    (vStaInfo_p, (UINT8 *) & AssocCfrm,
	     MlmeAssoc_Cnfm) == MLME_FAILURE) {
...
**************************************************************************/
static void
assocSrv_SndReAssocCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 reAssocResult)
{
	IEEEtypes_ReassocCfrm_t ReAssocCfrm;

	ReAssocCfrm.Result = reAssocResult;
	mlmeApiSndNotification(vStaInfo_p, (UINT8 *) & ReAssocCfrm,
			       MlmeReAssoc_Cnfm);
}
...
**************************************************************************/
static void
assocSrv_SndDisAssocCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 disAssocResult)
{
	IEEEtypes_DisassocCfrm_t DisAssocCfrm;

	DisAssocCfrm.Result = disAssocResult;
	mlmeApiSndNotification(vStaInfo_p, (UINT8 *) & DisAssocCfrm,
			       MlmeDisAssoc_Cnfm);
}
...
**************************************************************************/
static void
assocSrv_SndDisAssocInd(vmacStaInfo_t * vStaInfo_p,
			UINT16 disAssocResult, UINT8 * disAssocPeerAddr)
{
	IEEEtypes_DisassocInd_t DisAssocInd;

	DisAssocInd.Reason = disAssocResult;
	memcpy(&DisAssocInd.PeerStaAddr, disAssocPeerAddr,
	       sizeof(IEEEtypes_MacAddr_t));
	mlmeApiSndNotification(vStaInfo_p, (UINT8 *) & DisAssocInd,
			       MlmeDisAssoc_Ind);
}
...
**************************************************************************/
extern SINT32
assocSrv_AssocActTimeOut(UINT8 * data)
{
...
**************************************************************************/
extern SINT32
assocSrv_DisAssocActTimeOut(vmacStaInfo_t * vStaInfo_p, UINT8 * peerAddr)
{
	/* Notify SME of Time Out */
...
**************************************************************************/
extern SINT32
assocSrv_ReAssocActTimeOut(vmacStaInfo_t * vStaInfo_p,
...
**************************************************************************/
static void
assocSrv_ClearAssocInfo(vmacStaInfo_t * vStaInfo_p)
{
...
extern UINT16 Build_IE_191(vmacApInfo_t * vmacSta_p, UINT8 * IE_p);
...
**************************************************************************/
extern SINT32
assocSrv_AssocCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_AssocCmd_t * AssocCmd_p)
{
	...
	if ((mgtFrame_p =
		...
		assocSrv_SndAssocCnfm(vStaInfo_p,
	...
	if (*(mib->mib_STAMode) == CLIENT_MODE_B) {
		syncSrv_AddAttrib(mgtFrame_p,
				  SUPPORTED_RATES,
				  &vStaInfo_p->bOpRateSet[0], MAX_B_DATA_RATES);
	} else {
		UINT8 supportedRateLen = 0;

		supportedRateLen =
			util_ListLen(&vStaInfo_p->bssDescProfile_p->
				     DataRates[0], IEEEtypes_MAX_DATA_RATES_G);
	...
	if (domainChannelValid(vStaInfo_p->JoinChannel, FREQ_BAND_5GHZ)) {
		isSpectrumMgmt =
			(mgtFrame_p->Body.AssocRqst.CapInfo.
			 SpectrumMgmt) ? TRUE : FALSE;
		if (isSpectrumMgmt) {
			ChannelList =
				wl_kmalloc(IEEE_80211_MAX_NUMBER_OF_CHANNELS,
					   GFP_ATOMIC);
			if (ChannelList == NULL) {
				return MLME_FAILURE;
			}
			pwrcap = wl_kmalloc(sizeof
			...
			pInfo = wl_kmalloc(sizeof(DomainCountryInfo),
					   GFP_ATOMIC);
			if (pInfo == NULL) {
				wl_kfree(pwrcap);
				wl_kfree(ChannelList);
			...
			for (i = 0; i < pInfo->AChannelLen; i++) {
				if (vStaInfo_p->JoinChannel >=
				    pInfo->DomainEntryA[i].FirstChannelNo &&
				    vStaInfo_p->JoinChannel <
				    pInfo->DomainEntryA[i].FirstChannelNo +
				    pInfo->DomainEntryA[i].NoofChannel) {
					maxPwr = pInfo->DomainEntryA[i].
						MaxTransmitPw;
					break;
				}
			}
			...
			if (domainGetInfo(ChannelList)) {
				...
				supp = (UINT8 *) & supp_chan->
					SupportedChannel[0];
				for (i = 0, j = 0;
				     i < IEEE_80211_MAX_NUMBER_OF_CHANNELS;
				     i++) {
					if (ChannelList[i] != 0 &&
					    domainChannelValid(ChannelList[i],
							       FREQ_BAND_5GHZ))
					{
						*supp = ChannelList[i];
						supp++;
	...
	if (IsHTmode(*(mib->mib_ApMode))) {
		if (vStaInfo_p->bssDescProfile_p->HTElement.Len) {
			...
			vStaInfo_p->bssDescProfile_p->HTElement.
				HTCapabilitiesInfo.AdvCoding = 1;

			if ((*(mibSta->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_8K))
				vStaInfo_p->bssDescProfile_p->HTElement.
					HTCapabilitiesInfo.MaxAMSDUSize = 1;
			else
				vStaInfo_p->bssDescProfile_p->HTElement.
					HTCapabilitiesInfo.MaxAMSDUSize = 0;

			syncSrv_AddAttrib(mgtFrame_p,
					  vStaInfo_p->bssDescProfile_p->
					  HTElement.ElementId,
					  (UINT8 *) & vStaInfo_p->
					  bssDescProfile_p->HTElement.
					  HTCapabilitiesInfo,
					  vStaInfo_p->bssDescProfile_p->
					  HTElement.Len);
			{
				extern UINT16 AddAddHT_IE(vmacApInfo_t *
							  vmacSta_p,
							  IEEEtypes_Add_HT_Element_t
							  * pNextElement);
				UINT16 length =
					AddAddHT_IE(priv_p->vmacSta_p, &add_ht);
	...
	if (vmacSta_p->RsnIESetByHost) {
		IEEEtypes_RSN_IE_WPA2_t *pRsn =
			(IEEEtypes_RSN_IE_WPA2_t *) vmacSta_p->RsnIE;

		syncSrv_AddAttrib(mgtFrame_p, pRsn->ElemId, &pRsn->Ver[0],
				  pRsn->Len);
	} else
#endif
	if (vStaInfo_p->bssDescProfile_p->Wpa2Element.Len) {
		syncSrv_AddAttrib(mgtFrame_p,
				  vStaInfo_p->bssDescProfile_p->Wpa2Element.
				  ElemId,
				  (UINT8 *) & vStaInfo_p->bssDescProfile_p->
				  Wpa2Element.Ver[0],
				  vStaInfo_p->bssDescProfile_p->Wpa2Element.
				  Len);
	}
	...
	if (extendedRateLen) {
		syncSrv_AddAttrib(mgtFrame_p,
				  EXT_SUPPORTED_RATES,
				  &vStaInfo_p->bssDescProfile_p->
				  DataRates[MLME_SUPPORT_RATE_IE_MAX],
				  extendedRateLen);
	}
#ifdef WPA_STA
	...
	if (vStaInfo_p->bssDescProfile_p->vendorIENum &&
	    vStaInfo_p->bssDescProfile_p->vendorTotalLen) {
		addedVendorLen = 0;
		for (i = 0; i < vStaInfo_p->bssDescProfile_p->vendorIENum; i++) {
			ieHdr_p =
				(IEEEtypes_InfoElementHdr_t *) & vStaInfo_p->
				bssDescProfile_p->vendorBuf[addedVendorLen];
			if (ieHdr_p->Len) {
...
(memcmp
 (&
  ((IEEEtypes_Generic_HT_Element_t *) & vStaInfo_p->bssDescProfile_p->
   vendorBuf[addedVendorLen])->OUI, B_COMP_OUI, 3)
 && ((IEEEtypes_Generic_HT_Element_t *) & vStaInfo_p->bssDescProfile_p->
     vendorBuf[addedVendorLen])->OUIType != 51)
					) {
	...
	if (*(mib->mib_ApMode) & AP_MODE_11AC) {	/* add IE191  && IE192 to Assoc REQ */
		...
		if (vStaInfo_p->bssDescProfile_p->VHTOp.len) {
			if (vStaInfo_p->bssDescProfile_p->VHTOp.ch_width == 2) {
				...
				if (1 ==
				    (ret =
				     isSupport160MhzByCenterFreq(priv_p,
				...
				if (vStaInfo_p->bssDescProfile_p->VHTOp.
					...
					if (diff == 8) {
						...
						printk("%s 160Mhz: center frequency of the 160 MHz channel = %d\n", __FUNCTION__, vStaInfo_p->bssDescProfile_p->VHTOp.center_freq1);
						HZ160_support = 1;
					} else if (diff > 8) {
#ifdef SOC_W906X
...
#ifdef SOC_W906X
			Build_IE_191(vmacSta_p, (UINT8 *) & ptr);
#else
			/*Check against own configured bw */
			if ((vStaInfo_p->bssDescProfile_p->VHTCap.cap.
			     SupportedChannelWidthSet == 1)
			    || (vStaInfo_p->bssDescProfile_p->VHTCap.cap.
			...
			ptr.SupportedTxMcsSet =
				mib->StationConfig->SupportedTxVhtMcsSet;
#endif
			if ((*(mibSta->pMib_11nAggrMode) &
			     WL_MODE_AMSDU_TX_MASK) == WL_MODE_AMSDU_TX_8K)
				maxamsdu = 1;
			else if ((*(mibSta->pMib_11nAggrMode) &
			...
			syncSrv_AddAttrib(mgtFrame_p,
					  191, (UINT8 *) attribData, 12);
			{
				UINT16 Build_IE_192(vmacApInfo_t * vmacSta_p,
						    UINT8 * IE_p);
				attribData = (UINT8(*)[]) & vhtOpt_ptr.ch_width;
				vhtOpt_ptr.ch_width = 0;
				Build_IE_192(priv_p->vmacSta_p,
					     (UINT8 *) & vhtOpt_ptr);
				syncSrv_AddAttrib(mgtFrame_p, 192,
						  (UINT8 *) attribData, 5);
	...
	if (mibSta->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_STA) {
		...
		if (mibSta->multi_ap_ver == 2) {
			IEEEtypes_MultiAP_Version_t *version =
				(IEEEtypes_MultiAP_Version_t *) multi_ap->
				variable;
		...
		syncSrv_AddAttrib(mgtFrame_p, PROPRIETARY_IE,
				  (UINT8 *) attribData, (size - 2));
		wl_kfree(multi_ap);
	}
	...
	if (wlpptrSta->mboAssocRequestIeLen > 0) {
		UINT8 *pos =
			(UINT8 *) & mgtFrame_p->Body +
			mgtFrame_p->Hdr.FrmBodyLen;
		memcpy(pos, wlpptrSta->mboAssocRequestIe,
		       wlpptrSta->mboAssocRequestIeLen);
	...
	if (extendedCap_needAdd) {
		...
		if (pExtCap) {
			...
			syncSrv_AddAttrib(mgtFrame_p, EXT_CAP_IE,
	...
	mlmeApiStartTimer(vStaInfo_p,
			  ...
			  &assocSrv_AssocActTimeOut, ASSOC_TIME);
...
**************************************************************************/
extern SINT32
assocSrv_ReAssocCmd(vmacStaInfo_t * vStaInfo_p,
		    IEEEtypes_ReassocCmd_t * ReassocCmd_p)
{
	...
	mgtFrame_p->Hdr.FrmBodyLen += sizeof(IEEEtypes_CapInfo_t);
	/* Add Listen Interval */
	mgtFrame_p->Body.ReassocRqst.ListenInterval =
		ReassocCmd_p->ListenInterval;
	mgtFrame_p->Hdr.FrmBodyLen += sizeof(IEEEtypes_ListenInterval_t);
	/* Add AP Address */
	memcpy(&mgtFrame_p->Body.ReassocRqst.CurrentApAddr,
	       vStaInfo_p->macMgmtMlme_ThisStaData.BssId,
	       sizeof(IEEEtypes_MacAddr_t));
	...
	supportedRateLen =
		util_ListLen(&vStaInfo_p->bssDescProfile_p->DataRates[0],
...
#ifdef WPA_STA
	if (vStaInfo_p->staSecurityMibs.mib_PrivacyTable_p->RSNEnabled) {
		((KeyData_t *) vStaInfo_p->keyMgmtInfoSta_p->pKeyData)->
			RSNDataTrafficEnabled = 0;
		syncSrv_AddAttrib(mgtFrame_p, RSN_IE, "\x00\x10\x18\x01\x00", 3);	/*add dummy params for interop issues */
		syncSrv_AddAttrib(mgtFrame_p, RSN_IE,
				  &vStaInfo_p->thisStaRsnIE_p->OuiType[0],
...
**************************************************************************/
extern SINT32
assocSrv_DisAssocCmd(vmacStaInfo_t * vStaInfo_p,
		     IEEEtypes_DisassocCmd_t * DisassocCmd_p)
{
	...
	mlmeApiFreePeerStationStaInfoAndAid(&(DisassocCmd_p->PeerStaAddr),
					    vmacEntry_p);
	return MLME_SUCCESS;
}
...
scan_obss(UINT8 * data)
{
...

	{
		...
		if ((tx_empty == FALSE) || (stapriv->bgscan_period == 0)) {
			...
			TimerFireIn(&vStaInfo_p->obss_Timer_retry, 1,
				    &scan_obss, (unsigned char *)vStaInfo_p,
				    vStaInfo_p->obss_scan_interval);
			return 0;
		}
	}
	...
	if (stapriv->master) {
		struct wlprivate *wlMPrvPtr =
			NETDEV_PRIV_P(struct wlprivate, stapriv->master);
	...
	if ((*(vmacSta_p->Mib802dot11->mib_STAMode) == CLIENT_MODE_AUTO) ||
		...
		for (i = 0; i < IEEEtypes_MAX_CHANNELS; i++) {
			if ((mainChnlList[i] > 0) &&
			    (mainChnlList[i] == vStaInfo_p->JoinChannel)) {
				// Find the working channel
				idx_current = i;
				break;
			}
		...
		if (i < IEEEtypes_MAX_CHANNELS) {
			...
			end_chnl_id =
				(i <=
				 IEEEtypes_MAX_CHANNELS - 1 - 2) ? (i +
								    2)
				: (IEEEtypes_MAX_CHANNELS - 1);
		...
		if (i < IEEEtypes_MAX_CHANNELS_A) {
			...
			bgn_chnl_id =
				(i >=
				 2) ? (i + IEEEtypes_MAX_CHANNELS -
				       2) : IEEEtypes_MAX_CHANNELS;
			end_chnl_id =
				(i <=
				 IEEEtypes_MAX_CHANNELS_A - 1 - 2) ? (i +
								      IEEEtypes_MAX_CHANNELS
								      +
								      2)
				: ((IEEEtypes_MAX_CHANNELS +
				    IEEEtypes_MAX_CHANNELS_A) - 1);
			//for (i=bgn_chnl_id ; i<=end_chnl_id ; i++) {
			for (i = bgn_chnl_id; i <= end_chnl_id; i += 2) {
				if (i == idx_current) {	// Skip current channel
					continue;
	...
	if ((vmacEntry_p =
	     sme_GetParentVMacEntry(((vmacApInfo_t *) stapriv->vmacSta_p)->
...
**************************************************************************/
extern SINT32
assocSrv_RecvAssocRsp(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
#ifdef MULTI_AP_SUPPORT
...
#endif /* STA_QOS */
#ifdef SOC_W8964
	extern void PciWriteMacReg(struct net_device *netdev, UINT32 offset,
				   UINT32 val);
#endif
	vmacEntry_t *vmacEntry_p;
	vmacEntry_p = (vmacEntry_t *) vStaInfo_p->vMacEntry_p;
	...
	PciWriteMacReg(mainNetdev_p[vmacEntry_p->phyHwMacIndx], 0x11a0,
		       MgmtMsg_p->Body.AssocRsp.AId & 0x3f);
#endif
...
#ifdef SOC_W906X
	{
		struct wlprivate *wlpptr =
			NETDEV_PRIV_P(struct wlprivate,
				      mainNetdev_p[vmacEntry_p->phyHwMacIndx]);

		*(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->config.
			  staAid) = MgmtMsg_p->Body.AssocRsp.AId & 0x7ff;
	}
#endif
	...
	if (mibSta->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_STA) {
		...
		while ((attrib_p =
			syncSrv_ParseAttribWithinFrame(MgmtMsg_p, attrib_p,
		       ...
		       NULL) {
			MultiAp_elem_p =
				(IEEEtypes_MultiAP_Element_t *) attrib_p;
			if (!memcmp(MultiAp_elem_p->OUI, MultiAp_OUI, 3) &&
			    (MultiAp_elem_p->OUIType == MultiAP_OUI_type) &&
			    (MultiAp_elem_p->attributes.Attribute == 0x06)) {
				vStaInfo_p->isConnectbBSS =
					(MultiAp_elem_p->attributes.BackBSS ==
					 1) ? TRUE : FALSE;

				if (MultiAp_elem_p->Len >=
				    MAP_R1_IE_LEN +
				    sizeof(IEEEtypes_MultiAP_Version_t)) {
					UINT16 version_len = 0;
					IEEEtypes_MultiAP_Version_t *version =
						(IEEEtypes_MultiAP_Version_t *)
						MultiAp_elem_p->variable;
	...
	if (vStaInfo_p->staSecurityMibs.mib_PrivacyTable_p->RSNEnabled) {
		...
		CounterMeasureInit_Sta(&vStaInfo_p->keyMgmtInfoSta_p->
				       sta_MIC_Error, TRUE);
	} else {
		CounterMeasureInit_Sta(&vStaInfo_p->keyMgmtInfoSta_p->
				       sta_MIC_Error, FALSE);
	...
	memcpy(vStaInfo_p->macMgmtMlme_ThisStaData.BssId,
	       MgmtMsg_p->Hdr.SrcAddr, sizeof(IEEEtypes_MacAddr_t));
...

	{
		...
		p_ds_param_elm =
			...
			syncSrv_ParseAttribWithinFrame(MgmtMsg_p,
						       (UINT8 *) & MgmtMsg_p->
						       Body, DS_PARAM_SET);
		p_obss_param_elm =
			(IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t *)
		...
		if (p_obss_param_elm != NULL) {
			if (stapriv->bgscan_period != 0) {
				...
				vStaInfo_p->obss_scan_interval =
					DEF_OBSS_SCAN_PERIOD;
			}
			printk("=>%s(), File scan_obss(), %d, %d, %p\n",
			       __func__, vStaInfo_p->obss_scan_interval,
			       stapriv->bgscan_period, stapriv);
...
**************************************************************************/
extern SINT32
assocSrv_RecvReAssocRsp(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
#ifdef STA_QOS
	MhsmEvent_t TsMsg;
#endif
	...
	if (mlmeApiGetPeerStationStaInfoAndAid(&(MgmtMsg_p->Hdr.SrcAddr),
		...
		if (mlmeApiCreatePeerStationInfoForWBMode
		    (&(MgmtMsg_p->Hdr.SrcAddr), PeerSupportedRates_p,
		     PeerExtSupportedRates_p,
		     vmacEntry_p->phyHwMacIndx) == TRUE) {

			mlmeApiGetPeerStationStaInfoAndAid(&
							   (MgmtMsg_p->Hdr.
							    SrcAddr),
							   &wbStaInfo_p,
							   &local_urAid);
...
**************************************************************************/
extern SINT32
assocSrv_RecvDisAssocMsg(vmacStaInfo_t * vStaInfo_p,
			 dot11MgtFrame_t * MgmtMsg_p)
{
	...
	/*Free the AssocTable data structure that has been currently assigned to this */
	/*peer station to which the WB was associated/joined */
	mlmeApiFreePeerStationStaInfoAndAid(&(MgmtMsg_p->Hdr.SrcAddr),
					    vmacEntry_p);
...
**************************************************************************/
extern SINT32
assocSrv_Reset(vmacStaInfo_t * vStaInfo_p)
{
	mlmeApiStopTimer(vStaInfo_p, (UINT8 *) & vStaInfo_p->assocTimer);
	/* Init the Association state machines */
	AssocSrvStaCtor(&vStaInfo_p->assocsrv);
	mhsm_initialize(&vStaInfo_p->assocsrv.super,
			&vStaInfo_p->assocsrv.sTop);
	return MLME_SUCCESS;
}
