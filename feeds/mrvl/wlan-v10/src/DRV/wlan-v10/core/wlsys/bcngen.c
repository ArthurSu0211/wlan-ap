...
#ifdef SOC_W906X
void Init_Mbssid_IE(vmacApInfo_t * vmacSta_p,
		    IEEEtypes_Mbssid_Element_t * element);
...
inline UINT16
ErpBufUpdate(UINT8 * pBcnBuf, IEEEtypes_StartCmd_t * StartCmd,
	     UINT16 MsgSubType,
...
void
bcngen_UpdateBeaconErpInfo(vmacApInfo_t * vmacSta_p, BOOLEAN SetFlag)
{
	static int erpval;
	int val = 0;
 ...
 *****************************************************************************/
inline UINT16
bcngen_UpdateQBSSLoad(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf)
{
 ...
 *****************************************************************************/
inline UINT16
bcngen_AppendEDCAParamSet(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf)
{
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
	...
	pEDCA_param_set->QoS_info.EDCA_param_set_update_cnt =
...
#endif
	//Update EDCA for BE
	pEDCA_param_set->AC_BE.ACI_AIFSN.AIFSN =
		mib_QAPEDCATable[0].QAPEDCATblAIFSN;
	...
	pEDCA_param_set->AC_BE.TXOP_lim =
		ENDIAN_SWAP16(mib_QAPEDCATable[0].QAPEDCATblTXOPLimit);
	//Update EDCA for BK
	pEDCA_param_set->AC_BK.ACI_AIFSN.AIFSN =
		mib_QAPEDCATable[1].QAPEDCATblAIFSN;
	...
	pEDCA_param_set->AC_BK.TXOP_lim =
		ENDIAN_SWAP16(mib_QAPEDCATable[1].QAPEDCATblTXOPLimit);
	//Update EDCA for VI
	pEDCA_param_set->AC_VI.ACI_AIFSN.AIFSN =
		mib_QAPEDCATable[2].QAPEDCATblAIFSN;
	...
	pEDCA_param_set->AC_VI.TXOP_lim =
		ENDIAN_SWAP16(mib_QAPEDCATable[2].QAPEDCATblTXOPLimit);
	//Update EDCA for VO
	pEDCA_param_set->AC_VO.ACI_AIFSN.AIFSN =
		mib_QAPEDCATable[3].QAPEDCATblAIFSN;
	...
	pEDCA_param_set->AC_VO.TXOP_lim =
		ENDIAN_SWAP16(mib_QAPEDCATable[3].QAPEDCATblTXOPLimit);
...
InitThisStaRsnIE(vmacApInfo_t * vmacSta_p)
{
	...
	MIB_RSNCONFIGWPA2_AUTH_SUITES *mib_RSNConfigWPA2AuthSuites_p =
		vmacSta_p->Mib802dot11->WPA2AuthSuites;
	MIB_RSNCONFIGWPA2_UNICAST_CIPHERS *mib_RSNConfigWPA2UnicastCiphers_p =
		vmacSta_p->Mib802dot11->WPA2UnicastCiphers;
	MIB_RSNCONFIGWPA2_UNICAST_CIPHERS *mib_RSNConfigWPA2UnicastCiphers2_p =
		vmacSta_p->Mib802dot11->WPA2UnicastCiphers2;
	IEEEtypes_RSN_IE_t *thisStaRsnIE_p =
		vmacSta_p->Mib802dot11->thisStaRsnIE;
	IEEEtypes_RSN_IE_WPA2_t *thisStaRsnIEWPA2_p =
		vmacSta_p->Mib802dot11->thisStaRsnIEWPA2;
	IEEEtypes_RSN_IE_WPA2MixedMode_t *thisStaRsnIEWPA2MixedMode_p =
		vmacSta_p->Mib802dot11->thisStaRsnIEWPA2MixedMode;
	...
	memcpy(thisStaRsnIE_p->GrpKeyCipher, mib_RSNConfig_p->MulticastCipher,
	       4);
	thisStaRsnIE_p->PwsKeyCnt[0] = 0x01;
	thisStaRsnIE_p->PwsKeyCnt[1] = 0x0;
	memcpy(thisStaRsnIE_p->PwsKeyCipherList,
	       mib_RSNConfigUnicastCiphers_p->UnicastCipher, 4);
	thisStaRsnIE_p->AuthKeyCnt[0] = 0x01;
	thisStaRsnIE_p->AuthKeyCnt[1] = 0x0;
	memcpy(thisStaRsnIE_p->AuthKeyList,
	       mib_RSNConfigAuthSuites_p->AuthSuites, 4);
	...
	memcpy(thisStaRsnIEWPA2_p->GrpKeyCipher,
	       mib_RSNConfigWPA2_p->MulticastCipher, 4);
	thisStaRsnIEWPA2_p->PwsKeyCnt[0] = 0x01;
	thisStaRsnIEWPA2_p->PwsKeyCnt[1] = 0x0;
	memcpy(thisStaRsnIEWPA2_p->PwsKeyCipherList,
	       mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher, 4);
	thisStaRsnIEWPA2_p->AuthKeyCnt[0] = 0x01;
	thisStaRsnIEWPA2_p->AuthKeyCnt[1] = 0x0;
	memcpy(thisStaRsnIEWPA2_p->AuthKeyList,
	...
	memcpy(thisStaRsnIEWPA2MixedMode_p->GrpKeyCipher,
	       mib_RSNConfigWPA2_p->MulticastCipher, 4);
	thisStaRsnIEWPA2MixedMode_p->PwsKeyCnt[0] = 0x02;
	thisStaRsnIEWPA2MixedMode_p->PwsKeyCnt[1] = 0x0;
	memcpy(thisStaRsnIEWPA2MixedMode_p->PwsKeyCipherList,
	       mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher, 4);
	...
	if (mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher[3] !=
		 ...
		 */
		memcpy(thisStaRsnIEWPA2MixedMode_p->PwsKeyCipherList2,
		       mib_RSNConfigWPA2UnicastCiphers2_p->UnicastCipher, 4);
		thisStaRsnIEWPA2MixedMode_p->AuthKeyCnt[0] = 0x01;
		thisStaRsnIEWPA2MixedMode_p->AuthKeyCnt[1] = 0x0;
		memcpy(thisStaRsnIEWPA2MixedMode_p->AuthKeyList,
		       mib_RSNConfigWPA2AuthSuites_p->AuthSuites, 4);
		if (mib_RSNConfigWPA2_p->WPA2PreAuthEnabled)
		 ...
		 */
		IEEEtypes_RSN_IE_WPA2MixedMode_singlepwcipher_t
			*StaMixSingleIE_p =
			(IEEEtypes_RSN_IE_WPA2MixedMode_singlepwcipher_t *)
			thisStaRsnIEWPA2MixedMode_p;
		...
		memcpy(StaMixSingleIE_p->AuthKeyList,
		       mib_RSNConfigWPA2AuthSuites_p->AuthSuites, 4);
	...
	if (vmacSta_p->RsnIESetByHost == 1) {
		if (vmacSta_p->RsnIE[0] == RSN_IEWPA2) {
			memcpy(thisStaRsnIEWPA2_p, vmacSta_p->RsnIE,
			       sizeof(IEEEtypes_RSN_IE_WPA2_t));
			memcpy(thisStaRsnIEWPA2MixedMode_p, vmacSta_p->RsnIE,
			       sizeof(IEEEtypes_RSN_IE_WPA2_t));
		}
		if (vmacSta_p->WpaIE[0] == RSN_IE) {
			memcpy(thisStaRsnIE_p, vmacSta_p->WpaIE,
			       sizeof(IEEEtypes_RSN_IE_t));
		}
	} else if (vmacSta_p->RsnIESetByHost == 2) {
		memset((UINT8 *) thisStaRsnIEWPA2_p, 0,
		       sizeof(IEEEtypes_RSN_IE_WPA2_t));
		memset((UINT8 *) thisStaRsnIEWPA2MixedMode_p, 0,
		       sizeof(IEEEtypes_RSN_IE_WPA2MixedMode_t));
...
#ifdef UR_WPA
void
InitThisStaRsnIEUr(vmacApInfo_t * vmacSta_p)
{
	MIB_RSNCONFIG *mib_RSNConfig_p = vmacSta_p->Mib802dot11->RSNConfig;
	MIB_RSNCONFIG_UNICAST_CIPHERS *mib_RSNConfigUnicastCiphers_p =
		vmacSta_p->Mib802dot11->UnicastCiphers;
	...
	memcpy(thisStaRsnIEUr.GrpKeyCipher, mib_RSNConfig_p->MulticastCipher,
	       4);
	thisStaRsnIEUr.PwsKeyCnt[0] = 0x01;
	thisStaRsnIEUr.PwsKeyCnt[1] = 0x0;
	memcpy(thisStaRsnIEUr.PwsKeyCipherList,
	       mib_RSNConfigUnicastCiphers_p->UnicastCipher, 4);
	thisStaRsnIEUr.AuthKeyCnt[0] = 0x01;
	thisStaRsnIEUr.AuthKeyCnt[1] = 0x0;
	memcpy(thisStaRsnIEUr.AuthKeyList,
	       mib_RSNConfigAuthSuites_p->AuthSuites, 4);
...
UINT16
AddRSN_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_RSN_IE_t * pNextElement)
{
	IEEEtypes_RSN_IE_t *thisStaRsnIE_p =
		vmacSta_p->Mib802dot11->thisStaRsnIE;
	...
	if ((mib_urMode == 0) && (mib_wbMode == 0))	//Ap
		...
		memcpy(pNextElement, &thisStaRsnIEUr,
		       sizeof(IEEEtypes_RSN_IE_t));
#else
...
UINT16
AddRSN_IE_TO(IEEEtypes_RSN_IE_t * thisStaRsnIE_p,
...
#ifdef AP_WPA2
UINT16
AddRSN_IEWPA2(vmacApInfo_t * vmacSta_p, IEEEtypes_RSN_IE_WPA2_t * pNextElement)
{
	IEEEtypes_RSN_IE_WPA2_t *thisStaRsnIEWPA2_p =
		vmacSta_p->Mib802dot11->thisStaRsnIEWPA2;
...
UINT16
AddRSN_IEWPA2_TO(IEEEtypes_RSN_IE_WPA2_t * thisStaRsnIEWPA2_p,
...
UINT16
AddRSN_IEWPA2MixedMode(vmacApInfo_t * vmacSta_p,
		       IEEEtypes_RSN_IE_WPA2MixedMode_t * pNextElement)
{
	IEEEtypes_RSN_IE_WPA2MixedMode_t *thisStaRsnIEWPA2MixedMode_p =
		vmacSta_p->Mib802dot11->thisStaRsnIEWPA2MixedMode;

	memcpy(pNextElement, thisStaRsnIEWPA2MixedMode_p,
	       thisStaRsnIEWPA2MixedMode_p->Len + 2);
...
#ifdef COUNTRY_INFO_SUPPORT
void
InitThisCountry_IE(vmacApInfo_t * vmacSta_p)
{
	...
	if (*(mib->mib_ApMode) == AP_MODE_A_ONLY ||
	    *(mib->mib_ApMode) == AP_MODE_AandG) {
		...
		memcpy(thisStaCountryIE.DomainEntry, DomainInfo->DomainEntryA,
		       DomainInfo->AChannelLen);
	} else {
		thisStaCountryIE.Len = DomainInfo->GChannelLen + 3;  /** include 3 byte of country code here **/
		memcpy(thisStaCountryIE.DomainEntry, DomainInfo->DomainEntryG,
...
void
InitThisCountry_IE2(UINT8 mib_ApMode)
{
...
#endif
UINT16
AddCountry_IE(IEEEtypes_COUNTRY_IE_t * pNextElement)
{
...
#ifdef SOC_W8964
static void
InitPowerConstraint_IE(vmacApInfo_t * vmacSta_p, UINT8 channel)
{
#ifndef IEEE80211_DH
	PowerConstraintIE.value =
		(SINT8) mib_SpectrumMagament_p->mitigationRequirement;
...
static UINT16
AddPowerConstraint_IE(UINT8 * pNextElement)
{
...
void
bcngen_AddChannelSwithcAnnouncement_IE(vmacApInfo_t * vmacSta_p,
				       ...
				       * pChannelSwitchAnnouncementIE)
{
	...
	ChannelSwitchAnnouncementIE.Channel =
		pChannelSwitchAnnouncementIE->Channel;
...
void
bcngen_RemoveChannelSwithcAnnouncement_IE(vmacApInfo_t * vmacSta_p)
{
	ChannelSwitchCtrl.isActivated = FALSE;
	ChannelSwitchCtrl.targetChannel = 0;

	ChannelSwitchAnnouncementIE.Len = 0;
	memset(&ChannelSwitchAnnouncementIE, 0,
	       sizeof(ChannelSwitchAnnouncementIE));
...
static UINT16
AddChannelSwithcAnnouncement_IE(UINT8 * pNextElement)
{
...
void
bcngen_AddQuiet_IE(vmacApInfo_t * vmacSta_p,
...
void
bcngen_RemoveQuiet_IE(vmacApInfo_t * vmacSta_p)
{
	QuietIE.Len = 0;
...
UINT16
AddQuiet_IE(UINT8 * pNextElement)
{
	if (QuietIE.Len) {
...
static UINT16
AddTPCRep_IE(UINT8 * pNextElement)
{
...
#ifdef FLEX_TIME
void
UpdateBeaconInterval(UINT8 mode)
{
 ...
 *****************************************************************************/
void
bcngen_UpdateBeaconBuffer(vmacApInfo_t * vmacSta_p,
			  IEEEtypes_StartCmd_t * StartCmd)
{
	...
	memcpy(&BcnBuffer_p->Hdr.DestAddr, &BcastAddr,
	       sizeof(IEEEtypes_MacAddr_t));
	memcpy(&BcnBuffer_p->Hdr.SrcAddr, &vmacSta_p->macBssId,
	       sizeof(IEEEtypes_MacAddr_t));
	memcpy(&BcnBuffer_p->Hdr.BssId, &vmacSta_p->macBssId,
	       sizeof(IEEEtypes_MacAddr_t));

	if (*(mib->mib_ApMode) == AP_MODE_AandG) {
#ifdef FLEX_TIME
		BcnIntervalLocation_p =
			(UINT8 *) & BcnBuffer_p->Body.Bcn.BcnInterval;
#endif
	...
	*SuppRatesPtr = *SuppRateSet_p;
	...
	if (*(UINT8 *) & StartCmd->PhyParamSet == FH_PARAM_SET) {
		...
		NextElementPtr =
			NextElementPtr + sizeof(IEEEtypes_FhParamSet_t);
		...
		NextElementPtr =
			NextElementPtr + sizeof(IEEEtypes_DsParamSet_t);
	}
#ifdef AP_WPA2
	...
	if (mib_PrivacyTable_p->RSNEnabled) {
		...
		NextElementPtr +=
			AddRSN_IE(vmacSta_p,
				  (IEEEtypes_RSN_IE_t *) NextElementPtr);
	} else {
		// ReSet capability info.privacy bit for Beacon and probe rsp.
		if ((mib_StaCfg_p->PrivOption &&
		     mib_PrivacyTable_p->PrivInvoked))
	...
	NextElementPtr +=
		AddCountry_IE((IEEEtypes_COUNTRY_IE_t *) NextElementPtr);

#endif

#ifdef IEEE80211H
	if (mib_StaCfg_p->SpectrumManagementRequired &&
		...
		BcnCSACount =
			&((IEEEtypes_ChannelSwitchAnnouncementElement_t *)
			  NextElementPtr)->Count;
		NextElementPtr +=
			AddChannelSwithcAnnouncement_IE(NextElementPtr);
		NextElementPtr += AddQuiet_IE(NextElementPtr);

	}
#endif /* IEEE80211H */

	totalLen = NextElementPtr - (UINT8 *) BcnBuffer_p -
	...
	if (*(mib->mib_ApMode) != AP_MODE_B_ONLY &&
	    ...
	    *(mib->mib_ApMode) != AP_MODE_AandG) {
		byteCnt =
			ErpBufUpdate(NextElementPtr, StartCmd, IEEE_MSG_BEACON,
				     ExtSuppRateSet_p);
		totalLen += byteCnt;
		NextElementPtr += byteCnt;
	}
	...
	if (*(mib->QoSOptImpl)) {
		...
		NextElementPtr +=
			bcngen_UpdateQBSSLoad(vmacSta_p, NextElementPtr);
#ifdef QOS_WSM_FEATURE
		NextElementPtr +=
			Qos_UpdateWSMQosCapElem(vmacSta_p, NextElementPtr);
		...
		if (EDCA_Beacon_Counter)
			NextElementPtr +=
				bcngen_AppendEDCAParamSet(vmacSta_p,
							  NextElementPtr);
		else
			NextElementPtr +=
				Qos_UpdateQosCapElem(vmacSta_p, NextElementPtr);
 ...
 *****************************************************************************/
void
bcngen_EnableTbttFreeIntr(void)
{
	return;
}

void
bcngen_EnableBcnFreeIntr(void)
{
	return;
}
 ...
 *****************************************************************************/
void
bcngen_DisableTbttFreeIntr(void)
{
	return;
}

void
bcngen_DisableBcnFreeIntr(void)
{
	return;
}
 ...
 *****************************************************************************/
void
bcngen_UpdateBitInTim(UINT16 Aid, BOOLEAN Set)
{
	...
	if (Set) {
		...
		TrafficMap[ByteNum] =
			TrafficMap[ByteNum] & ((1 << BitNum) ^ 0xff);
	}
	N1 = 0;
	N2 = 0;
 ...
 *****************************************************************************/
void
bcngen_UpdateBitInTim2(UINT16 Aid, BOOLEAN Set)
{
	...
	if (Set) {
		...
		if ((ByteNum < LowByte) || (ByteNum > HighByte))
			return;
		TrafficMap2[ByteNum] =
			TrafficMap2[ByteNum] & ((1 << BitNum) ^ 0xff);
	...
	memcpy((UINT8 *) TimPtr2, (UINT8 *) & Tim2,
	       sizeof(IEEEtypes_InfoElementHdr_t) + Tim2.Len);
	/* Enable Beacon Free interrupt */
 ...
 */
static void
InitHT_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_HT_Element_t * elment)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	...
	if (((*(vmacSta_p->Mib802dot11->pMib_11nAggrMode) &
	      WL_MODE_AMSDU_TX_8K)) &&
	...
	default:
		...
		elment->SupportedMCSset[2] =
			mib_StaCfg->SupportedTxHtMCScfg.SupportedTxHtMCSset_2;
#ifdef SOC_W906X
		elment->SupportedMCSset[3] =
			mib_StaCfg->SupportedTxHtMCScfg.SupportedTxHtMCSset_3;
#endif
		break;
	}
	...
	elment->MacHTParamInfo =
		*(vmacSta_p->Mib802dot11->
		  mib_ampdu_factor) |
		((*(vmacSta_p->Mib802dot11->mib_ampdu_density) << 2));
#if defined ( INTOLERANT40) || defined (COEXIST_20_40_SUPPORT)

	elment->HTCapabilitiesInfo.FortyMIntolerant =
...
static void
Init_Generic_HT_IE2(vmacApInfo_t * vmacSta_p,
		    IEEEtypes_Generic_HT_Element_t2 * elment)
{
	...
	elment->MacHTParamInfo =
		*(vmacSta_p->Mib802dot11->
		  mib_ampdu_factor) | (*(vmacSta_p->Mib802dot11->
					 mib_ampdu_density) << 2);
...
UINT16
Add_Generic_HT_IE2(vmacApInfo_t * vmacSta_p,
		   IEEEtypes_Generic_HT_Element_t2 * pNextElement)
{
	if ((*(vmacSta_p->Mib802dot11->mib_ApMode) & 0x4) &&
	    vmacSta_p->Mib802dot11->StationConfig->WSMQoSOptImpl) {
...
#endif
UINT16
AddHT_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_HT_Element_t * pNextElement)
{
	if ((IsHTmode(*(vmacSta_p->Mib802dot11->mib_ApMode))) &&
	    vmacSta_p->Mib802dot11->StationConfig->WSMQoSOptImpl) {
...
static void
InitAddHT_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_Add_HT_Element_t * elment)
{
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = vmacSta_p->Mib802dot11->PhyDSSSTable;
...
UINT16
AddAddHT_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_Add_HT_Element_t * pNextElement)
{
	if (IsHTmode(*(vmacSta_p->Mib802dot11->mib_ApMode)) &&
	    vmacSta_p->Mib802dot11->StationConfig->WSMQoSOptImpl) {
...
#if 0
static void
Init_Generic_AddHT_IE(vmacApInfo_t * vmacSta_p,
		      IEEEtypes_Generic_Add_HT_Element_t * elment)
{
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = vmacSta_p->Mib802dot11->PhyDSSSTable;
...
UINT16
Add_Generic_AddHT_IE(vmacApInfo_t * vmacSta_p,
		     IEEEtypes_Generic_Add_HT_Element_t * pNextElement)
{
	return 0;
...
#endif //0
static void
Init_Generic_AddHT_IE2(vmacApInfo_t * vmacSta_p,
		       IEEEtypes_Generic_Add_HT_Element_t2 * elment)
{
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = vmacSta_p->Mib802dot11->PhyDSSSTable;
...
UINT16
Add_Generic_AddHT_IE2(vmacApInfo_t * vmacSta_p,
		      IEEEtypes_Generic_Add_HT_Element_t2 * pNextElement)
{
	if ((*(vmacSta_p->Mib802dot11->mib_ApMode) & 0x4) &&
	    vmacSta_p->Mib802dot11->StationConfig->WSMQoSOptImpl) {
...
static void
InitM_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_M_Element_t * element)
{
	memcpy((UINT8 *) element, (UINT8 *) & M_COMP_ID_IE,
	       sizeof(IEEEtypes_M_Element_t));
}

UINT16
AddM_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_HT_Element_t * pNextElement)
{
	if ((*(vmacSta_p->Mib802dot11->mib_ApMode) & 0x4)) {
...
IEEEtypes_M_Rptr_Element_t M_COMP_RPTR_ID_IE =
	{ PROPRIETARY_IE, 38, {0, 0x40, 0x96}
, 0x27, 0x00, 0x10, {0}
};

static void
InitM_Rptr_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_M_Rptr_Element_t * element)
{
	memcpy(M_COMP_RPTR_ID_IE.RptrDeviceType,
	       vmacSta_p->Mib802dot11->mib_RptrDeviceType,
	       strlen(vmacSta_p->Mib802dot11->mib_RptrDeviceType));
	memcpy((UINT8 *) element, (UINT8 *) & M_COMP_RPTR_ID_IE,
	       sizeof(IEEEtypes_M_Rptr_Element_t));
}

UINT16
AddM_Rptr_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_HT_Element_t * pNextElement)
{
...
#ifdef SOC_W906X
void
update_nontxd_bssid_profile_ssid(vmacApInfo_t * vmacSta_p, void *pssidIE)
{
	//MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	IEEEtypes_SsIdElement_t *ssid_p = &vmacSta_p->NonTxBssidProf.ssid;

	memcpy((void *)ssid_p, (void *)pssidIE,
	       sizeof(IEEEtypes_SsIdElement_t));

}

void
update_nontxd_bssid_profile_cap(vmacApInfo_t * vmacSta_p, UINT16 CapInfo)
{
	IEEEtypes_NonTransmitted_BSSID_Cap_t *pCap =
		&vmacSta_p->NonTxBssidProf.ntBssidCap;
...
void
update_nontxd_bssid_profile_bssidIdx(vmacApInfo_t * vmacSta_p)
{
	struct wlprivate *wlpptr =
		NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
	...
	wlp_primbss =
		NETDEV_PRIV_P(struct wlprivate, wlp_master->vdev[primbss]);
	...
	if (memcmp(pRefBssid, pmac, 5)) {
		printk("Error: Abnormal mac address between ref BSSID:%02x%02x%02x%02x%02x%02x and dev mac:%02x%02x%02x%02x%02x%02x\n", pRefBssid[0], pRefBssid[1], pRefBssid[2], pRefBssid[3], pRefBssid[4], pRefBssid[5], pmac[0], pmac[1], pmac[2], pmac[3], pmac[4], pmac[5]);
	}
...
void
init_NonTx_bssid_cap(vmacApInfo_t * vmacSta_p, void *element)
{
	IEEEtypes_NonTransmitted_BSSID_Cap_t *pCap =
		&vmacSta_p->NonTxBssidProf.ntBssidCap;
...
void
init_nontxd_bssid_ssid(vmacApInfo_t * vmacSta_p, void *pssidIE)
{
	IEEEtypes_SsIdElement_t *ssid_p = &vmacSta_p->NonTxBssidProf.ssid;
...
void
init_nontxd_bssid_profile_bssidIdx(vmacApInfo_t * vmacSta_p, void *pmbssidIdx)
{
	IEEEtypes_mbssid_idx_t *pidx = &vmacSta_p->NonTxBssidProf.mbssidIdx;
...
u8
Add_NonTxProfile_IEs(vmacApInfo_t * vmacSta_p, u8 * pnext)
{
	u8 total_len = 0;
	IEEEtypes_NonTransmitted_BSSID_Cap_t *pcap;
...
u8
Get_MaxBssid_Indicator(U32 bss_num)
{
	// Note: 2^0 == 1
...
void
Init_Mbssid_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_Mbssid_Element_t * element)
{
	u8 ie_len = 0;
	u8 *pnext;
...
UINT8
Add_Mbssid_IE(vmacApInfo_t * vmacSta_p,
...
UINT16
Get_MultiAP_IE_Size(vmacApInfo_t * vmacSta_p)
{
	...
	if ((vmacSta_p->Mib802dot11->multi_ap_vid) &&
	    (vmacSta_p->Mib802dot11->
...
void
Init_MultiAP_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_MultiAP_Element_t * element,
		UINT8 protocol_type)
{
	...
	if (protocol_type == WL_WLAN_TYPE_AP) {
		MultiAP_IE->attributes.BackBSS =
			((multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS) >> 6);
		MultiAP_IE->attributes.FrontBSS =
			((multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS) >> 5);
		MultiAP_IE->attributes.TearDown =
			((multi_ap_attr & MAP_ATTRIBUTE_TEARDOWN) >> 4);
		//if (MultiAP_IE->attributes.BackBSS) { /* TBD: if check bBSS? need specificatoin clarification */
		/* Encode bit 3 and bit 4 for R2, Table 4. */
		MultiAP_IE->attributes.R1bSTAdisAllowed =
			((multi_ap_attr & MAP_ATTRIBUTE_R1BSTA_DISALLOWED) >>
			 3);
		MultiAP_IE->attributes.R2bSTAdisAllowed =
	...
	if (multi_ap_ver == 2) {
		IEEEtypes_MultiAP_Version_t *version =
			(IEEEtypes_MultiAP_Version_t *) MultiAP_IE->variable;
	...
	if ((multi_ap_vid) && (multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS)) {
		IEEEtypes_MultiAP_Traffic_t *traffic =
			(IEEEtypes_MultiAP_Traffic_t *) (MultiAP_IE->variable +
							 version_len);
...
UINT16
Add_MultiAP_IE(vmacApInfo_t * vmacSta_p,
	       IEEEtypes_InfoElementHdr_t * pNextElement, UINT8 protocol_type)
{
	Init_MultiAP_IE(vmacSta_p, (IEEEtypes_MultiAP_Element_t *) pNextElement,
			protocol_type);
...
getRegulatoryClass(vmacApInfo_t * vmacSta_p)
{
	...
	if (domainCode == DOMAIN_CODE_FCC) {
		...
		case 6:
		case 7:
			if (vmacSta_p->Mib802dot11->PhyDSSSTable->Chanflag.
...
//#ifdef INTOLERANT40
static void
InitChanReport_IE(vmacApInfo_t * vmacSta_p,
		  IEEEtypes_ChannelReportEL_t * elment)
{
	UINT8 ch_list[IEEE_80211_MAX_NUMBER_OF_CHANNELS];
	...
	domainGetInfo(ch_list);
	if (elment->RegClass != 32 &&
	    elment->RegClass != 33 && elment->RegClass != 12)
		memcpy(elment->ChanList, &ch_list[14],
		       IEEE_80211_MAX_NUMBER_OF_CHANNELS - 14);
	else
		memcpy(elment->ChanList, &ch_list[0], 14);
	elment->Len = 1 + strlen(&elment->ChanList[0]);
}

UINT16
AddChanReport_IE(vmacApInfo_t * vmacSta_p,
		 IEEEtypes_ChannelReportEL_t * pNextElement)
{
	InitChanReport_IE(vmacSta_p,
			  (IEEEtypes_ChannelReportEL_t *) pNextElement);
	return (pNextElement->Len + 2);
}
...
static void
InitExtended_Cap_IE(vmacApInfo_t * vmacSta_p,
		    IEEEtypes_Extended_Cap_Element_t * elment)
{
	...
	elment->ExtCap.BSSTransition =
		*(vmacSta_p->Mib802dot11->mib_btm_enabled);
#endif
	...
	if ((*(vmacSta_p->Mib802dot11->mib_ApMode) &
	     (AP_MODE_B_ONLY | AP_MODE_G_ONLY)) == 0) {
...
UINT16
AddExtended_Cap_IE(vmacApInfo_t * vmacSta_p,
		   IEEEtypes_Extended_Cap_Element_t * pNextElement)
{
	InitExtended_Cap_IE(vmacSta_p,
			    (IEEEtypes_Extended_Cap_Element_t *) pNextElement);
	return (pNextElement->Len + 2);
}

static void
Init20_40_Coexist_IE(vmacApInfo_t * vmacSta_p,
		     IEEEtypes_20_40_BSS_COEXIST_Element_t * elment)
{
...
UINT16
Add20_40_Coexist_IE(vmacApInfo_t * vmacSta_p,
		    IEEEtypes_20_40_BSS_COEXIST_Element_t * pNextElement)
{
	Init20_40_Coexist_IE(vmacSta_p,
...
static void
InitOverlap_BSS_Scan_Parameters_IE(vmacApInfo_t * vmacSta_p,
				   ...
				   * elment)
{
	...
	memset(elment, 0,
	       sizeof(IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t));
...
UINT16
AddOverlap_BSS_Scan_Parameters_IE(vmacApInfo_t * vmacSta_p,
				  IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t
				  * pNextElement)
{
	InitOverlap_BSS_Scan_Parameters_IE(vmacSta_p,
					   (IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t
					    *) pNextElement);
	return (pNextElement->Len + 2);
}
...
static void
Init20_40Interant_Channel_Report_IE(vmacApInfo_t * vmacSta_p,
				    ...
				    * elment)
{
	...
	memset(elment, 0,
	       sizeof(IEEEtypes_20_40_INTOLERANT_CHANNEL_REPORT_Element_t));
...
UINT16
Add20_40Interant_Channel_Report_IE(vmacApInfo_t * vmacSta_p,
				   IEEEtypes_20_40_INTOLERANT_CHANNEL_REPORT_Element_t
				   * pNextElement)
{
	Init20_40Interant_Channel_Report_IE(vmacSta_p,
					    (IEEEtypes_20_40_INTOLERANT_CHANNEL_REPORT_Element_t
					     *) pNextElement);
	return (pNextElement->Len + 2);
}
...
#ifdef MRVL_WPS2
UINT16
Build_AssocResp_WSCIE(vmacApInfo_t * vmacSta_p,
		      AssocResp_WSCIE_t * pNextElement)
{
	...
	pNextElement->ResponseType.ID =
		(UINT16) SHORT_SWAP((UINT16) WSC_RESP_TYPE_ATTRB);
	pNextElement->ResponseType.Len = SHORT_SWAP(0x0001);
	pNextElement->ResponseType.ResponseType = 0x03;

	pNextElement->VendorExtn.ID =
		(UINT16) SHORT_SWAP((UINT16) WSC_VENDOR_EXTN_ATTRB);
...
#ifdef IEEE80211K
static void
InitRRM_Cap_IE(vmacApInfo_t * vmacSta_p,
	       IEEEtypes_RM_Enable_Capabilities_Element_t * elment)
{
	memset(elment, 0, sizeof(IEEEtypes_RM_Enable_Capabilities_Element_t));
	elment->eid = RRM_CAP_IE;
#ifdef WLS_FTM_SUPPORT
	if (*(vmacSta_p->Mib802dot11->wls_ftm_enable) == 1) {
		extern void wlsFTM_AddRMEnabledCap_IE(struct net_device *netdev,
...
UINT16
AddRRM_Cap_IE(vmacApInfo_t * vmacSta_p,
...
UINT32
GetCenterFreq(UINT32 ch, UINT32 bw)
{
...
static UINT8
count_max_nss_antenna(vmacApInfo_t * vmacSta_p, UINT8 * ant_bitmap,
		      UINT8 * pNumOfAntenna)
{
	UINT8 max_nss;
	struct wlprivate *wlpptr =
...
UINT16
Build_IE_191(vmacApInfo_t * vmacSta_p, UINT8 * IE_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	...
	if (wlpptr->devid == SC5) {
		...
		max_nss =
			count_max_nss_antenna(vmacSta_p,
					      vmacSta_p->Mib802dot11->
					      mib_rxAntenna, &NumOfAntenna);
	...
	vht_cap &= ~(1 << 20);	//MUBeamformeeCapable
#endif
	memcpy((UINT8 *) & ptr->cap, &vht_cap,
	       sizeof(IEEEtypes_VHT_Cap_Info_t));
#ifdef SOC_W906X
	max_nss =
		count_max_nss_antenna(vmacSta_p,
				      vmacSta_p->Mib802dot11->mib_rxAntBitmap,
				      &NumOfAntenna);
	...
	max_nss =
		count_max_nss_antenna(vmacSta_p,
				      vmacSta_p->Mib802dot11->mib_txAntenna,
				      &NumOfAntenna);
...
#else
	NumOfAntenna =
		countNumOnes((*(vmacSta_p->Mib802dot11->mib_rxAntBitmap) &
			      0xf));
	...
	NumOfAntenna =
		countNumOnes((*(vmacSta_p->Mib802dot11->mib_txAntenna) & 0xf));
	if (NumOfAntenna == 1) {
	...
	ptr->SupportedRxMcsSet =
		SupportedRxVhtMcsSet | SupportedRxVhtMcsSetMask;
	ptr->SupportedTxMcsSet =
		mib_StaCfg->SupportedTxVhtMcsSet | SupportedTxVhtMcsSetMask;
#ifdef NEW_DP

	if ((*(mib->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK) ==
	    WL_MODE_AMSDU_TX_8K)
		maxamsdu = 1;
	else if ((*(mib->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK) ==
...
UINT16
Build_IE_192(vmacApInfo_t * vmacSta_p, UINT8 * IE_p)
{
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;
	...
	if ((PhyDSSSTable->Chanflag.ChnlWidth == CH_40_MHz_WIDTH) ||
		...
		ch_width = 0;
	} else if ((PhyDSSSTable->Chanflag.ChnlWidth == CH_160_MHz_WIDTH) ||
	...
	if (ch_width == 0) {
		...
		if (ie192_version == 2) {
			if (ch_width == 1) {	//80MHz
				center_freq0 =
					GetCenterFreq(PhyDSSSTable->CurrChan,
			...
			if (ch_width == 3) {	/* 80MHz+80MHz */
				center_freq0 =
					GetCenterFreq(PhyDSSSTable->CurrChan,
						      CH_80_MHz_WIDTH);
				center_freq1 =
					GetCenterFreq(PhyDSSSTable->SecChan,
						      CH_80_MHz_WIDTH);
			}
	...
	if (0 ==
	    isSupport160MhzByCenterFreq(priv_p, VHT_EXTENDED_NSS_BW_CAPABLE,
					center_freq0, center_freq1, 0)) {
...
UINT16
Build_IE_Color_Change_Ann_IE(vmacApInfo_t * vmacSta_p, UINT8 * IE_p)
{
	IEEEtypes_Color_Change_Ann_Element_t *ptr =
		(IEEEtypes_Color_Change_Ann_Element_t *) IE_p;
...
UINT16
Build_IE_UoraParamSet_IE(vmacApInfo_t * vmacSta_p, UINT8 * IE_p)
{
...
static u8 ppet16[MAX_NSS][MAX_RU_NUM] = {
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_NONE},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_NONE},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_NONE},
...
static u8 ppet8[MAX_NSS][MAX_RU_NUM] = {
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_NONE},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_BPSK, PPE_CONSTELLATION_BPSK,
	 PPE_CONSTELLATION_BPSK},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_BPSK, PPE_CONSTELLATION_BPSK,
	 PPE_CONSTELLATION_BPSK},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_BPSK, PPE_CONSTELLATION_BPSK,
	 PPE_CONSTELLATION_BPSK},
...
static u8 ppet8_aggressive[MAX_NSS][MAX_RU_NUM] = {
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_NONE},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_256QAM},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_1024QAM, PPE_CONSTELLATION_256QAM},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_256QAM, PPE_CONSTELLATION_256QAM},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_256QAM, PPE_CONSTELLATION_256QAM},
	{PPE_CONSTELLATION_NONE, PPE_CONSTELLATION_NONE,
	 PPE_CONSTELLATION_256QAM, PPE_CONSTELLATION_256QAM},
...
UINT16
Build_IE_HE_CAP(vmacApInfo_t * vmacSta_p, UINT8 * IE_p)
{
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	MIB_STA_CFG *mib_StaCfg;
	...
	if ((mib->PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_5GHZ)) {
		if ((mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_160_MHz_WIDTH)
		    ...
		    ((mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_80_MHz_WIDTH)
			 ...
			 RADIO_MODE_80p80) &&
		     (mib->PhyDSSSTable->SecChan != 0))) {
			he_cap_p->phy_cap.channel_width_set =
	...
	rx_nss = count_max_nss_antenna(vmacSta_p,
				       vmacSta_p->Mib802dot11->mib_rxAntBitmap,
				       &NumOfRxAntenna);
	tx_nss = count_max_nss_antenna(vmacSta_p,
				       vmacSta_p->Mib802dot11->mib_txAntenna,
				       &NumOfTxAntenna);
	...
	if ((NumOfTxAntenna == 0) || (*vmacSta_p->Mib802dot11->he_mu_bf == 0))
		...
		he_cap_p->phy_cap.sounding_dimension_le_80mhz =
			NumOfTxAntenna - 1;

	/*Rx HE-MCS Map <= 80 MHz */
	SupportedRxHeMcsSetMask = SupportedRxHeMcsSetMask << (rx_nss * 2);
	he_cap_p->rx_he_mcs_80m.max_mcs_set =
		mib_StaCfg->
		supoorted_rx_he_80m_mcs_set | SupportedRxHeMcsSetMask;
	/*Tx HE-MCS Map <= 80MHz */
	SupportedTxHeMcsSetMask = SupportedTxHeMcsSetMask << (tx_nss * 2);
	he_cap_p->tx_he_mcs_80m.max_mcs_set =
		mib_StaCfg->
		supoorted_tx_he_80m_mcs_set | SupportedTxHeMcsSetMask;
	...
	if ((NumOfTxAntenna == 0) || (*vmacSta_p->Mib802dot11->he_mu_bf == 0))
		...
		he_cap_p->phy_cap.sounding_dimension_gt_80mhz =
			NumOfTxAntenna - 1;
	...
	if (he_cap_p->phy_cap.ppe_threshold_present == 1) {
		...
		ppe_thresholds =
			(ppe_threshold_info_t *) ((u8 *) he_cap_p + len);
		...
		for (i = 0, k = 0; i < ppe_nss; i++) {
			for (j = 0; j < MAX_RU_NUM; j++) {
				if (ruid_mask & (1 << j)) {
					if (vmacSta_p->Mib802dot11->HEConfig->
					    pe_type == MIB_PE_TYPE_DEFAULT) {
						ppet_present[k++] =
							ppet16[i][j];
						ppet_present[k++] = ppet8[i][j];
					} else {
						ppet_present[k++] =
							ppet16_aggressive[i][j];
						ppet_present[k++] =
							ppet8_aggressive[i][j];
					}
				}
			}
		}
		...
		if (wlpptr->devid == SC5) {
			...
			if ((mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_160_MHz_WIDTH) || ((mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_80_MHz_WIDTH) && (mib->PhyDSSSTable->Chanflag.radiomode == RADIO_MODE_80p80)))	//BW 160 and 80+80
			{
				if ((NumOfRxAntenna / 2) > 2) {
					he_cap_p->phy_cap.
						beamformee_sts_gt_80mhz = 3;
				}
			} else	//BW <= 80
			{
				if (NumOfRxAntenna > 4) {
					he_cap_p->phy_cap.
						beamformee_sts_le_80mhz = 3;
				}
			}
		}
	}
	...
	he_cap_p->hdr.Len =
		len - sizeof(IEEEtypes_ElementId_t) - sizeof(IEEEtypes_Len_t);
...
UINT8
isMbssidConfigured(struct net_device * netdev)
{
...
UINT16
Build_IE_HE_OP(vmacApInfo_t * vmacSta_p, UINT8 * IE_p, UINT8 vhtopie_present)
{
	...
	heop_iep->hdr.Len =
		sizeof(HE_Operation_IE_t) - sizeof(IEEEtypes_ElementId_t) -
		sizeof(IEEEtypes_Len_t) - sizeof(heop_iep->heop_ext);
	...
	printk("%s(), set txop_duration_rts_threshold: %x\n", __func__,
	       heop_iep->he_op_param.txop_duration_rts_threshold);
	...
	if (heop_iep->he_op_param.vht_op_info_present) {
		...
		vht_opp =
			(vht_operation_info_t *) ((u8 *) & heop_iep->
						  basic_mcs_nss_set +
						  sizeof(heop_iep->
							 basic_mcs_nss_set));
...
inline UINT16
Build_IE_MU_EDCA(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf)
{
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
...
#ifdef WMM_PS_SUPPORT
	pIe->QoS_info.EDCA_param_set_update_cnt =
		vmacSta_p->VMacEntry.edca_param_set_update_cnt;
	...
	memcpy((void *)pIe->ac,
	       (void *)vmacSta_p->VMacEntry.mib_QAP_MUEDCA_Table,
	       sizeof(pIe->ac));
...
inline UINT16
Build_IE_SRP(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf)
{
