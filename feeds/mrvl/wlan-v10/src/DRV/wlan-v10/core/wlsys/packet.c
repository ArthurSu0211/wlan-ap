...
UINT32 dbg_BAredr_id = 0xFFFF;	//sta id for logging
UINT32 dbg_BAredr_cardindex = 0;	//wdev interface for logging

//Log for incoming seqno for history keeping.
...
//Log
UINT32 dbg_BAredr_log[DBG_BAREORDER_LOG_MASK + 1];
UINT32 dbg_BAredr_log_cnt = 0;

UINT32 dbg_BAredr_OOR_cont = 0;	//Continous out of range count. Reset when SeqNo not OoR

UINT32 dbg_BAredr_tag[7] =
	{ DBG_BAREORDER_TAG_TMO, DBG_BAREORDER_TAG_SN, DBG_BAREORDER_TAG_OOR,
DBG_BAREORDER_TAG_OOR_RPT, DBG_BAREORDER_TAG_STORE, DBG_BAREORDER_TAG_WEND,
DBG_BAREORDER_TAG_BAR };
UINT8 dbg_BAredr_log_en_mask =
	(DBG_BAREORDER_TMO_EN_BIT | DBG_BAREORDER_OOR_EN_BIT |
	 DBG_BAREORDER_WEND_EN_BIT);
UINT8 dbg_BAredr_hist_log[7] = { 36, 0, 36, 0, 0, 36, 0 };
#endif
...
#else
		if (wlxmit(netdev, skb, IEEE_TYPE_DATA, pStaInfo, 0, FALSE, 0)) {
#endif
...
enableAmpduTx(vmacApInfo_t * vmacSta_p, UINT8 * macaddr, UINT8 tid)
{
	...
	if ((tid == 7) && (*(mib->disable_aggr_for_vo) && (tid == 6)))
		return;
	...
	if (!IS_GROUP((UINT8 *) & (eh.ether_dhost))) {
...
#endif
		{
			...
			wlpptr->wlpd_p->drv_stats_val.
				txq_ac_stats[AccCategoryQ[tid]].drop_skb++;
			wl_free_skb(skb);
			return NULL;
		}
		if (*(mib->QoSOptImpl)) {
...
bad:
	if (skb != NULL) {
		wlpptr->netDevStats.tx_dropped++;
		wlpptr->wlpd_p->drv_stats_val.txq_ac_stats[AccCategoryQ[tid]].
			drop_skb++;
		wl_free_skb(skb);
	}
	return NULL;
		      ...
		      UINT16 SeqNo, UINT8 machdrLen, UINT8 secMode, UINT8 * pn)
{
	...
	{
		pStaInfo = extStaDb_GetStaInfo(vmacSta_p,
					       (IEEEtypes_MacAddr_t *) & skb->
					       data[6], STADB_UPDATE_AGINGTIME);
	}
...
#ifdef DEBUG_BAREORDER
void
BA_DebugLog(UINT8 tid, UINT16 winStartB, UINT16 SeqNo, UINT32 cardIndex_Id,
	    UINT32 tag, UINT8 info)
{
	if (((dbg_BAredr_cardindex << 16) | dbg_BAredr_id) == cardIndex_Id) {
		UINT32 historyNum = 0;
		UINT32 log = 0;

		//Update log
		if (tag == DBG_BAREORDER_TAG_OOR) {
			dbg_BAredr_OOR_cont++;
			if (dbg_BAredr_OOR_cont > 1) {
				tag = DBG_BAREORDER_TAG_OOR_RPT;
			}
			log = (tid << 28) | ((info & 0xF) << 24) | (winStartB <<
								    12) |
				(SeqNo);
		} else if (tag == DBG_BAREORDER_TAG_TMO) {
			dbg_BAredr_OOR_cont = 0;
			log = (tid << 28) | ((info & 0xFF) << 20) | (winStartB << 8) | (SeqNo & 0xFF);	//Seqno=drpCnt, other=bufCnt
		} else {
			dbg_BAredr_OOR_cont = 0;
			log = (tid << 28) | ((info & 0xF) << 24) | (winStartB <<
								    12) |
		...
		historyNum = dbg_BAredr_hist_log[((tag >> 28) - 1)];

		//Enable log
		if ((dbg_BAredr_log_en_mask & (tag & 0xFF)) &&
		    (dbg_BAredr_log_cnt < (DBG_BAREORDER_LOG_MASK - 4))) {
			UINT32 i, index;

			dbg_BAredr_log[dbg_BAredr_log_cnt++ &
				       DBG_BAREORDER_LOG_MASK] =
				tag & 0xFFFFFF00;
			dbg_BAredr_log[dbg_BAredr_log_cnt++ &
				       DBG_BAREORDER_LOG_MASK] = log;

			index = dbg_BAredr_SN_cnt;	//get current seqno history index

			//get previous seqno history log
			for (i = 0; i < historyNum; i++) {
				if (dbg_BAredr_log_cnt >
				    (DBG_BAREORDER_LOG_MASK - 4)) {
					break;
				}

				index = (index - 1) & DBG_BAREORDER_SN_MASK;
				//skip history log without data
				if ((dbg_BAredr_SN[index] == 0) ||
				    (dbg_BAredr_SN[index] == 0xdeaddead)) {
					break;
				}

				//if history log shows this pkt was stored
				if ((dbg_BAredr_SN[index] & 0x0F000000) ==
				    0x09000000) {
					UINT8 loop = 0;
					dbg_BAredr_log[dbg_BAredr_log_cnt++ &
						       DBG_BAREORDER_LOG_MASK] =
					...
					//Only save first and last of pkt store evt, skip all consecutive store in between. 
					while ((i < (historyNum - 1)) &&
					       ((dbg_BAredr_SN
						 [(index -
						   1) & DBG_BAREORDER_SN_MASK] &
						 0x0F000000) == 0x09000000)) {
						index = (index -
							 1) &
		...
		dbg_BAredr_log[dbg_BAredr_log_cnt & DBG_BAREORDER_LOG_MASK] =
			0xdeadbeef;
	}
}
#endif
...
BA_TimerProcess(UINT8 * data)
{
	...
	if ((wlpptr = NETDEV_PRIV_P(struct wlprivate, ro_p->dev)) == NULL) {
		printk("[%s], Invalid dev: %s\n", __func__, ro_p->dev->name);
		goto exit;
	}
	if (wlpptr->wlpd_p == NULL) {
		printk("[%s], Null priv_data: %s\n", __func__, ro_p->dev->name);
		goto exit;
	}
	if ((vmacSta_p = wlpptr->vmacSta_p) == NULL) {
		printk("[%s], Invalid vmacSta, dev: %s\n", __func__,
		       ro_p->dev->name);
		goto exit;
	}
	if ((mib = wlpptr->vmacSta_p->Mib802dot11) == NULL) {
		printk("[%s], Invalid mib, dev: %s\n", __func__,
		       ro_p->dev->name);
		goto exit;
	}
	wlexcept_p = &wlpptr->wlpd_p->except_cnt;

	if (ro_p->pStaInfo == NULL) {
		printk("[%s], Invalid StaInfo, dev: %s\n", __func__,
		       ro_p->dev->name);
		goto exit;
	}
	//printk("%s, [%s], %p, %p, %p, %p\n", __func__, ro_p->dev->name,
	//  wlpptr, vmacSta_p, mib, ro_p->pStaInfo);
		...
		if (BufCnt != 0) {
			if (baRxInfo->ba[tid].winStartB <= leastSeqNo) {
				...
				pStaInfo->rxBaStats[tid].BA_RodrTMODropCnt +=
					dropCnt;
				pStaInfo->rxBaStats[tid].BA_RodrLastDropReason =
					BA_RODR_DROP_TMO;
			} else {
				dropCnt =
					((BA_MAX_SEQ_NUM + 1) -
...
#ifdef DEBUG_BAREORDER
			//dropCnt replace SeqNo info 
			BA_DebugLog(tid, baRxInfo->ba[tid].winStartB, dropCnt,
				    (UINT32) (cardindex << 16 | id),
				    DBG_BAREORDER_TAG_TMO, BufCnt);
#endif
			...
			DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 12,
					 baRxInfo->ba[tid].winStartB,
					 leastSeqNo)
	     ...
	     extStaDb_StaInfo_t * pStaInfo, UINT8 tid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	UINT16 stnid = (pStaInfo) ? pStaInfo->StnId : sta_num;
	int cnt = 0;
...
#endif
		baRxInfo->AmsduQ[bufIndex].seqNo = SeqNo;
...
#ifdef DEBUG_BAREORDER
			if (dbgSeqNo != baRxInfo->AmsduQ[index].seqNo) {
				printk("Flushing skipped, expect:%d, actual:%d\n", dbgSeqNo, baRxInfo->AmsduQ[index].seqNo);
			}
			dbgSeqNo = (dbgSeqNo + 1) & BA_MAX_SEQ_NUM;
#endif
		...
		UINT32 * passPNcnt, UINT8 tid)
{
	...
	{
		vmacSta_p->BA_RodrFlushDropCnt += winDelta;
		pStaInfo->rxBaStats[tid].BA_RodrFlushDropCnt += winDelta;
		...
		//if(baRxInfo->AmsduQ[index].state != 0)      //Not needed, just dequeue and flush
		{
			...
			if (winDelta > winSizeB) {
				vmacSta_p->BA_RodrFlushDropCnt +=
					(winSizeB - 1);
				pStaInfo->rxBaStats[tid].BA_RodrFlushDropCnt += (winSizeB - 1);	//minus 1 for this 1 pkt passed to host

				vmacSta_p->BA_RodrWinEndJumpCnt++;
				pStaInfo->rxBaStats[tid].BA_RodrWinEndJumpCnt++;
			} else {
				vmacSta_p->BA_RodrFlushDropCnt +=
					(winDelta - 1);
				pStaInfo->rxBaStats[tid].BA_RodrFlushDropCnt += (winDelta - 1);	//minus 1 for this 1 pkt passed to host
			}
	...
	if (advanceDelta < winDelta)	// there may be available buffers before New winStartB
	{
		...
		vmacSta_p->BA_RodrFlushDropCnt += (advanceDelta - 1);
				...
				if (!isBcastPkt &&
					...
					if (winStartB == SeqNo) {
...
#ifdef DEBUG_BAREORDER
						BA_DebugLog(tid, winStartB,
							    SeqNo,
							    (UINT32) (cardindex
								      << 16 |
								      id),
							    DBG_BAREORDER_TAG_SN,
							    0);
						...
						if (BA_CheckAmsduDup
							...
							DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 0, baRxInfo->ba[tid].winStartB, SeqNo)
								SPIN_UNLOCK
								(&baRxInfo->
								 ba[tid].
								 BAreodrLock);
						...
						if (baRxInfo->ba[tid].
							...
							DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 1, baRxInfo->ba[tid].winStartB, SeqNo)
						} else {
							if (storedBufCnt == 1) {
								baRxInfo->
									ba[tid].
							...
							DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 2, baRxInfo->ba[tid].winStartB, SeqNo)
						}
					...
					} else	//(winStartB != SeqNo)
					{
						...
						if (BA_CheckAmsduDup
							...
							DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 7, baRxInfo->ba[tid].winStartB, SeqNo)
						...
						if (storedBufCnt == 0) {
							...
							DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 8, baRxInfo->ba[tid].winStartB, SeqNo)
								SPIN_UNLOCK
								(&baRxInfo->
								 ba[tid].
								 BAreodrLock);
...
#ifdef DEBUG_BAREORDER
					BA_DebugLog(tid, winStartB, SeqNo,
						    (UINT32) (cardindex << 16 |
							      id),
						    DBG_BAREORDER_TAG_BAR, 0);
