...
void rpkt_reuse_free_resource(struct sk_buff_head *pqueue, u32 * plast_qlen,
			      u32 threshold);
...
extern struct sk_buff *wlCfhUlToSkb(struct net_device *netdev,
				    wlrxdesc_t * cfh_ul, int rxQid);
extern wltxdesc_t *wlSkbToCfhDl(struct net_device *netdev, struct sk_buff *skb,
				wltxdesc_t * txcfg, int qid, int type);
extern wlrxdesc_t *wlGetCfhUl(struct net_device *netdev, int qid,
			      wlrxdesc_t * cfh_tl_temp);
...
extern void mwl_hex_dump_to_sysfs(const void *buf, size_t len,
				  char *sysfs_buff);
extern void wlTest(struct net_device *netdev);
extern void InitCFHDL(struct net_device *netdev, struct bqm_args *pbqm_args,
		      wltxdesc_t * cfg, struct sk_buff *skb, void *pStaInfo,
		      BOOLEAN eap, UINT8 nullpkt);
extern void InitCFHDLMgmt(struct net_device *netdev, struct bqm_args *pbqm_args,
...
extern void wl_show_stat(struct net_device *netdev, int option, int level,
			 char *sysfs_buff);
extern int wl_show_stat_cmd(struct net_device *netdev, char *info_item,
			    char *info_level, char *sysfs_buff);
extern void wl_show_smac_stat(struct net_device *netdev,
			      SMAC_STATUS_st * pSMACStatus, char *sysfs_buff);
extern void wl_pkttype_stat(struct pkttype_info *wlpkt_type,
			    IEEEtypes_FrameCtl_t * fc);
extern void wl_get_datpkt_prot(struct pkttype_info *wlpkt_type, U16 llc_type,
			       struct iphdr *iph);
extern void wlTxSkbTest_1(struct net_device *netdev, int pktcnt, int pktsize,
			  int txqid, int frameType);
...
extern wlrxdesc_t *wlProcessMsdu(struct net_device *netdev, wlrxdesc_t * cfh_ul,
				 u32 * msduNo, u32 qid);
...
isRQFull(wl_qpair_rq_t * rq)
{
	if (((rq->wrinx + 1) % rq->qsize) == rq->rdinx) {
		WLDBG_INFO(DBG_LEVEL_5,
			   "QINDEX:, (wrinx, rdinx)(%d, %d) qisze = %d RQ full \n",
...
isRQEmpty(wl_qpair_rq_t * rq)
{
	if (rq->wrinx == rq->rdinx) {
		WLDBG_INFO(DBG_LEVEL_5,
			   "QINDEX:, (rdinx, wrinx)=(%d, %d) RQ empty\n",
			   rq->rdinx, rq->wrinx);
		return TRUE;
	} else {
		return FALSE;
...
isSQFull(wl_qpair_sq_t * sq)
{
	if (((sq->wrinx + 1) % sq->qsize) == sq->rdinx) {
		WLDBG_INFO(DBG_LEVEL_5,
			   "QINDEX:, (rdinx, wrinx)(%d, %d) qisze = %d SQ full \n",
...
isSQEmpty(wl_qpair_sq_t * sq)
{
	if (sq->wrinx == sq->rdinx) {
		WLDBG_INFO(DBG_LEVEL_5,
			   "QINDEX:, (rdinx, wrinx)(%d, %d) qisze = %d SQ empty \n",
			   sq->wrinx, sq->rdinx, sq->qsize);
		return TRUE;
	}
