...
extern int wlFwSetCoexConfig(struct net_device *netdev, u8 * enable,
			     u8 * gpioLevelDetect, u8 * gpioEdgeTrigger,
			     u32 * gpioReqPin, u32 * gpioGrantPin,
...
extern int wlFwSetTxPower(struct net_device *netdev, UINT8 flag,
			  UINT32 powerLevel);
...
#ifdef SOC_W906X
extern int wlFwSetChannel(struct net_device *, u_int8_t, u_int8_t, CHNL_FLAGS,
			  u_int8_t);
#else
...
int wlFwGetAddrValue(struct net_device *netdev, UINT32 addr, UINT32 len,
...
int wlFwSendFipsTest(struct net_device *netdev, UINT32 encdec, UINT32 alg,
		     DataEntry_t * pKey, DataEntry_t * pNounce,
		     DataEntry_t * pAAD, DataEntry_t * pData,
		     DataEntry_t * pOutput);
...
#ifdef MRVL_WSC
extern int wlFwSetWscIE(struct net_device *netdev, u_int16_t ieType,
			WSC_COMB_IE_t * pWscIE);
#endif
#ifdef WTP_SUPPORT
...
#endif
#ifdef MRVL_WAPI
extern int wlFwSetWapiIE(struct net_device *netdev, UINT16 ieType,
			 WAPI_COMB_IE_t * pAPPIE);
#endif
extern int wlFwSetIEs(struct net_device *netdev);
int wlFwApplyChannelSettings(struct net_device *netdev);
#ifdef SOC_W906X
int wlchannelSet(struct net_device *netdev, int channel, int Channel2,
		 CHNL_FLAGS chanflag, u_int8_t initRateTable);
#else
int wlchannelSet(struct net_device *netdev, int channel, CHNL_FLAGS chanflag,
...
BOOLEAN wlFwGetHwStatsForWlStats(struct net_device *netdev,
...
int wlFwSetWpaWpa2PWK_STA(struct net_device *netdev,
			  extStaDb_StaInfo_t * StaInfo_p);
int wlFwSetWpaGroupK_rx(struct net_device *netdev, wlreq_key * wk);
int wlFwSetNewStn(struct net_device *dev, u_int8_t * staaddr, u_int16_t assocId,
...
int wlFwSetWpaGroupK_rx(struct net_device *netdev, wlreq_key * wk);
int wlFwSetWpaWpa2PWK(struct net_device *netdev,
		      extStaDb_StaInfo_t * StaInfo_p);

int wlFwSetNewStn(struct net_device *dev, u_int8_t * staaddr, u_int16_t assocId,
		  u_int16_t stnId, u_int16_t action, PeerInfo_t * pPeerInfo,
		  UINT8 Qosinfo, UINT8 isQosSta, UINT8 wds);
int wlFwSetEdcaParam(struct net_device *netdev, u_int8_t Indx, u_int32_t CWmin,
		     u_int32_t CWmax, u_int8_t AIFSN, u_int16_t TXOPLimit);
int wlFwAcMaxTolerableDelay(struct net_device *netdev, u_int8_t action,
			    u_int8_t ac, u_int32_t * maxdelay);

extern int wlFwSetWep(struct net_device *netdev, u_int8_t * staaddr);
extern int wlFwUpdateDestroyBAStream(struct net_device *dev, u_int32_t ba_type,
				     u_int32_t direction, u_int8_t stream,
				     u_int8_t tid, u_int8_t * Macaddr,
				     u_int16_t staid);
#ifdef SOC_W906X
extern int wlFwCreateBAStream(struct net_device *dev, u_int32_t BarThrs,
			      u_int32_t WindowSize, u_int8_t * Macaddr,
			      u_int8_t DialogToken, u_int8_t Tid,
			      u_int32_t ba_type, u_int32_t direction,
			      u_int8_t ParamInfo, u_int8_t * SrcMacaddr,
			      UINT16 seqNo, UINT32 vhtrxfactor, UINT32,
			      u_int16_t staid);
#else
extern int wlFwCreateBAStream(struct net_device *dev, u_int32_t BarThrs,
			      u_int32_t WindowSize, u_int8_t * Macaddr,
...
extern int wlFwSetWpaWpa2PWK(struct net_device *netdev,
			     extStaDb_StaInfo_t * StaInfo_p);
...
extern int wlRegRF(struct net_device *netdev, UINT8 flag, UINT32 reg,
		   UINT32 * val);
extern int wlRegBB(struct net_device *netdev, UINT8 flag, UINT32 reg,
		   UINT32 * val);
extern int wlRegCAU(struct net_device *netdev, UINT8 flag, UINT32 reg,
		    UINT32 * val);
extern int wlFwGetBeacon(struct net_device *netdev, UINT8 * pBcn,
			 UINT16 * pLen);
...
extern int wlFwGetRateTable(struct net_device *netdev, UINT8 * addr,
			    UINT8 * pRateInfo, UINT32 size, UINT8 type, UINT16);
extern int wlFwSetRateTable(struct net_device *netdev, UINT32 action,
			    UINT8 * addr, UINT16 staid, UINT32 rateinfo);
#else
extern int wlFwGetRateTable(struct net_device *netdev, UINT8 * addr,
			    UINT8 * pRateInfo, UINT32 size, UINT8 type);
extern int wlFwSetRateTable(struct net_device *netdev, UINT32 action,
			    UINT8 * addr, UINT32 rateinfo);
#endif

#ifdef CLIENT_SUPPORT
extern int wlFwApplyClientSettings(struct net_device *netdev);
extern int wlFwSetBssForClientMode(struct net_device *netdev,
				   wlfacilitate_e facility);
#endif
extern int wlFwSetHwSpecs(struct net_device *netdev);
extern int wlFwGetWatchdogbitmap(struct net_device *dev, u_int8_t * bitmap);
extern int wlFwGetSeqNoBAStream(struct net_device *, u_int8_t *, uint8_t,
				uint16_t *);
extern int wlFwCheckBAStream(struct net_device *, u_int32_t, u_int32_t,
			     u_int8_t *, u_int8_t, u_int8_t, u_int32_t, int32_t,
			     u_int8_t);
#ifdef RXPATHOPT
...
extern int wlFwGetConsecTxFailAddr(struct net_device *netdev,
...
#ifdef SOC_W906X
extern int wlFwSetVHTOpMode(struct net_device *netdev, UINT16 staid,
			    UINT8 vht_NewRxChannelWidth, UINT8 vht_NewRxNss);
#else
extern int wlFwSetVHTOpMode(struct net_device *netdev,
			    IEEEtypes_MacAddr_t * staaddr,
			    UINT8 vht_NewRxChannelWidth, UINT8 vht_NewRxNss);
#endif
extern int wlFwSetBWSignalType(struct net_device *netdev, UINT32 mode,
			       UINT8 val);

#ifdef AIRTIME_FAIRNESS
extern int wlFwAtfEnable(struct net_device *netdev, u8 enable);
extern int wlFwSetAtfCfg(struct net_device *netdev, UINT8 param, UINT16 value);
extern int wlFwGetAtfCfg(struct net_device *netdev, UINT16 * atf_param);
extern int wlFwAtfDebugEnable(struct net_device *netdev, u8 debug_feature,
			      u8 enable);
extern int wlFwSetAtfDma(struct net_device *netdev, u_int32_t base,
			 u_int32_t size);
extern int wlFwAtfTransfertDone(struct net_device *netdev);
...
#endif
extern int wlFwNewDP_Cmd(struct net_device *netdev, UINT8 ch, UINT8 width,
			 UINT8 rates, UINT8 rate_type, UINT8 rate_bw,
			 UINT8 rate_gi, UINT8 rate_ss);
extern int wlFwNewDP_RateDrop(struct net_device *netdev, UINT32 enabled,
			      UINT32 rate_index, UINT32 staidx);
extern int wlFwNewDP_OffChannel_Start(struct net_device *netdev);
extern int wlFwNewDP_queue_OffChan_req(struct net_device *netdev,
				       DOT11_OFFCHAN_REQ_t * pOffChan);
extern int wlFwNewDP_handle_OffChan_event(struct net_device *netdev);
extern int wlFwNewDP_config_prom(struct net_device *netdev,
				 PROM_CNF_t * PromCnf);
extern int wlFwNewDP_sensorD_init(struct net_device *netdev,
				  sensord_init_t * sensordinit, UINT8 action);
extern int wlFwNewDP_sensorD_cmd(struct net_device *netdev);
extern int wlFwNewDP_set_rx_mcast(struct net_device *netdev,
				  rx_mcast_t * RxMcast);
extern int wlFwNewDP_wifiarb_post_req_intr(struct net_device *netdev);
extern int wlFwNewDP_sensord_set_blanking(struct net_device *netdev,
					  u8 * blankingmask);
extern int wlFwNewDP_bfmr_config(struct net_device *netdev,
				 bfmr_config_t * BFMRconfig, UINT8 action);
extern int wlFwNewDP_bfmr_sbf_open(struct net_device *netdev,
				   wlcfg_sbf_open_t * WlcfgsbfOpen);
extern int wlFwNewDP_bfmr_sbf_close(struct net_device *netdev,
...
extern int wlFwGetPowerPerRate(struct net_device *netdev, UINT32 RatePower,
			       UINT16 * dBm, UINT8 * ant);
#else
extern int wlFwGetPowerPerRate(struct net_device *netdev, UINT32 RatePower,
			       UINT8 * trpcid, UINT16 * dBm, UINT16 * ant);
#endif
extern int wlFwRadioStatusNotification(struct net_device *netdev,
				       UINT32 action);
extern int wlFwSetTxContinuous(struct net_device *netdev, UINT8 mode,
			       UINT32 rateinfo);
...
extern int wlFwNewDP_eeprom(struct net_device *netdev, UINT32 offset,
			    UINT8 * data, UINT32 len, UINT16 action);
extern int wlFwNewDP_EEPROM_access(struct net_device *netdev, UINT32 action);
extern int wlFwNewDP_Set_Offchanpwr(struct net_device *netdev, SINT8 pwr,
				    UINT8 bitmap, UINT8 channel);
extern int wlFwNewDP_NDPA_UseTA(struct net_device *netdev, UINT32 enable);
#ifdef SOC_W906X
extern int wlFwSetMUSet(struct net_device *netdev, UINT8 Option, UINT8 GID,
			UINT8 Setindex, UINT16 * Stn);
extern int wlFwSetMBSSIDSet(struct net_device *netdev, UINT8 Option,
			    UINT8 groupid, UINT8 primary, UINT32 members);
#else
extern int wlFwSetMUSet(struct net_device *netdev, UINT8 Option, UINT8 GID,
			UINT8 Setindex, UINT16 Stn1, UINT16 Stn2, UINT16 Stn3);
#endif
extern int wlFwSetMUDma(struct net_device *netdev, u_int32_t base,
			u_int32_t size);
...
int wlFwGet_Device_Region_Code(struct net_device *netdev,
			       UINT32 * EEPROM_Region_Code);
int wlFwGet_Device_PwrTbl(struct net_device *netdev,
...
extern int wlFwGetQBSSLoad(struct net_device *netdev, UINT8 * ch_util,
			   UINT16 * sta_cnt);
#endif //AP_STEERING_SUPPORT

#if defined(EEPROM_REGION_PWRTABLE_SUPPORT)
int wlFwGet_EEPROM_Region_Code(struct net_device *netdev,
			       UINT32 * EEPROM_Region_Code);
int wlFwGet_EEPROM_PwrTbl(struct net_device *netdev,
			  channel_power_tbl_t * EEPROM_CH_PwrTbl,
			  UINT8 * region_code, UINT8 * number_of_channels,
			  UINT32 channel_index);
#endif
int wlFwGetCoreDump(struct net_device *netdev, coredump_cmd_t * core_dump,
		    char *buff);
int wlFwGetCoreSniff(struct net_device *netdev, coredump_cmd_t * core_dump,
		     char *buff);
...
extern int wlFwMuUserPosition(struct net_device *netdev, u16 action, u8 gid,
			      u8 usr_pos);

#ifdef IEEE80211K
extern int wlFwSetBcnChannelUtil(struct net_device *netdev, UINT8 ch_tril);
extern int wlFwSetQuiet(struct net_device *netdev, UINT8 enable, UINT8 period,
			UINT16 duration, UINT16 offset, UINT16 offset1,
			UINT8 txStop_en);
#endif /* IEEE80211K */
#ifdef SOC_W906X
...
int wlFwSendFrame(struct net_device *netdev, UINT16 staIdx, UINT8 reportId,
		  UINT8 tid, UINT32 rateInfo, UINT8 machdrLen,
		  UINT16 payloadLen, UINT8 * pMacHdr, UINT8 * pData);
int wlFwGetCoreDumpAddrValue(struct net_device *netdev, UINT32 addr, UINT32 len,
			     UINT32 * val, UINT16 set);
...
UINT32 wlFwSetMib(struct net_device *netdev,
		  UINT32 action, UINT32 mibIdx, UINT32 * pValue, UINT32 * pNum);
UINT32 wlFwSentTriggerFrameCmd(struct net_device *netdev, UINT8 action,
			       UINT8 type, UINT32 rateInfo, UINT32 period,
			       UINT32 padNum, void *pData);

extern int wlFw_SetSR(struct net_device *netdev, UINT8 enable,
...
extern int wlFwSetNoAck(struct net_device *netdev, UINT8 Enable,
			UINT8 be_enable, UINT8 bk_enable, UINT8 vi_enable,
			UINT8 vo_enable);
...
extern int wlFwGetStaCBParam(struct net_device *netdev,
			     HostCmd_STA_CB_PARAMS_SYNC * psta_cb_param);
extern int wlFwSetStaCBParam(struct net_device *netdev,
			     HostCmd_STA_CB_PARAMS_SYNC * psta_cb_param);
#endif //CB_SUPPORT

#ifdef SOC_W906X
extern int wlFwSetOFDMASet(struct net_device *netdev, UINT8 enable,
			   UINT8 sta_count, UINT16 * Stn);
extern int wlFwSetULMUSet(struct net_device *netdev, UINT8 Action,
