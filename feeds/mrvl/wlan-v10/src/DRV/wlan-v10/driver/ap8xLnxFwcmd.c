			...
			if (!mib->RSNConfigWPA2->WPA2OnlyEnabled) {
				if (vmacSta_p->Mib802dot11->thisStaRsnIE->
					...
					AddRSN_IE(vmacSta_p,
						  (IEEEtypes_RSN_IE_t *) &
						  tmp_RsnIE);
					memcpy(&pCmd->StartCmd.RsnIE, &tmp_RsnIE, sizeof(IEEEtypes_RSN_IE_t) - 4);	// minus Reserved field.
#ifdef SOC_W906X
					macMgmtMlme_UpdateProbeRspBasicIes
						(vmacSta_p,
						 (UINT8 *) & pCmd->StartCmd.
...
wlsendCommand(struct net_device *netdev)
{
	...
	writel(MACREG_H2ARIC_BIT_DOOR_BELL,
	       wlpptr->ioBase1 + reg_h2a_int_events);
#else
...
#define HAL_ACTSUBCH_BOTH   2
	...
	int wlFwNewDP_queue_OffChan_req(struct net_device *netdev,
		...
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
#ifdef SOC_W906X
		offchReqHead = readl(wlpptr->ioBase1 + reg_offchreq_head);
		offchReqTail = readl(wlpptr->ioBase1 + reg_offchreq_tail);
#else
		offchReqHead = readl(wlpptr->ioBase1 + MACREG_REG_OffchReqHead);
		offchReqTail = readl(wlpptr->ioBase1 + MACREG_REG_OffchReqTail);
#endif
		...
		if ((offchReqHead + 1) == MAX_OFF_CHAN_REQ)
			...
			offchReqHead++;
#ifdef SOC_W906X
		writel(offchReqHead, wlpptr->ioBase1 + reg_offchreq_head);
#else
		writel(offchReqHead, wlpptr->ioBase1 + MACREG_REG_OffchReqHead);
#endif
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
...
OFFCHAN_PROCESS_FAIL:
	...
	int wlFwNewDP_config_prom(struct net_device *netdev,
		...
		if (op_mode != SMAC_OPMODE_NORMAL) {
			...
			*(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->
			...
			regval = readl(wlpptr->ioBase1 + BBRX_CFG);
			if ((regval & 0x01) == 0)
				writel((regval | 0x01),
				       wlpptr->ioBase1 + BBRX_CFG);
		} else {
			*(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->
				  config.opMode) = op_mode;
	...
	UINT32 wlFwSetDbg(struct net_device * netdev,
			  UINT32 action, UINT32 dbgIdx, UINT32 * pValue,
			  UINT32 * pNum) {
		struct wlprivate *wlpptr =
			NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_DBG_CFG *pCmd =
			(HostCmd_FW_DBG_CFG *) & wlpptr->pCmdBuf[0];
		int retval = FAIL, i;
		unsigned long flags;
		...
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_DBG_CFG));

		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_DBG_CFG);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_DBG_CFG));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->action = ENDIAN_SWAP16(action);
		pCmd->dbgIdx = ENDIAN_SWAP16(dbgIdx);
		pCmd->num = ENDIAN_SWAP16(*pNum);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd,
				sizeof(HostCmd_FW_DBG_CFG));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_DBG_CFG);
...
#include "ap8xLnxSwMimo.h"
	...
	int wlFwSendFrame(struct net_device *netdev, UINT16 staIdx,
		...
		payloadLen =
			(UINT16) min((u_int32_t) payloadLen,
				     (u_int32_t) (MAX_TX_FRAME_LEN -
						  machdrLen));
