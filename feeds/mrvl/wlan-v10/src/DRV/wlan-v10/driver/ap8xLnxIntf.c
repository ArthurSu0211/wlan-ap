...
unsigned int mmdu_mgmt_enable = FALSE;
module_param(mmdu_mgmt_enable, uint, S_IRUGO);
MODULE_PARM_DESC(mmdu_mgmt_enable, "Use mmdu mgmt");

unsigned int mmdu_data_enable = FALSE;
module_param(mmdu_data_enable, uint, S_IRUGO);
MODULE_PARM_DESC(mmdu_data_enable, "Use mmdu data");
...
/** external functions **/
extern void *ap8xLnxStat_vap_init(struct kobject *dev_kobj);
extern int ap8xLnxStat_vap_exit(struct kobject *dev_kobj);
extern void *ap8xLnxStat_clients_init(struct kobject *dev_kobj, UINT8 mode);
extern int ap8xLnxStat_sysfs_init(struct kobject *dev_kobj);
extern int ap8xLnxStat_sysfs_exit(struct kobject *dev_kobj);
...
#endif
{
	...
	iounmap(wlpptr->ioBase1);
	iounmap(wlpptr->ioBase0);
	release_mem_region(pci_resource_start(pdev, wlpptr->nextBarNum),
			   pci_resource_len(pdev, wlpptr->nextBarNum));
	release_mem_region(pci_resource_start(pdev, 0),
			   pci_resource_len(pdev, 0));
	pci_disable_device(pdev);
...
//A390/A385 platform supports only 16 interrupts => Use polling function instead
	...
	if (wlPrepareFwFile(netdev) == FAIL) {
		...
		if ((major >= 25) ||
			...
			mmdu_mgmt_enable = mmdu_data_enable =
				!(wlpptr->smacStatusAddr->
				  smacCap & SMAC_CAP_MMDU_NOT_SUPPORT);
		} else {
			mmdu_mgmt_enable = mmdu_data_enable = FALSE;
		}
		dev_info(wlpptr->wlpd_p->dev,
			 "%s MMDU support\n",
...
/* FW version foramt is changed, below version need remap, temporary disbaled */
	...
	ap8xLnxStat_sysfs_init(&netdev->dev.kobj);
#ifdef AP8X_DUMP
...
wlDeinit(struct net_device *netdev)
{
	...
	ap8xLnxStat_sysfs_exit(&netdev->dev.kobj);
#ifdef AP8X_DUMP
...
wlPowerResetFw(struct net_device *netdev)
{
	...
	printk("Power Resetting ...\n");
	rc_pdev =
		pci_get_domain_bus_and_slot(pci_domain_nr(priv->pPciDev->bus),
					    0, 0);
	pci_reset_function(rc_pdev);
	pci_restore_state(priv->pPciDev);
#else
...
wlopen_mbss(struct net_device *netdev)
{
...
#endif
	ap8xLnxStat_clients_init(&netdev->dev.kobj, 1);
	...
	if (wlpptr->wlpd_p->pdfsApMain &&
	    DfsEnabledChannel(netdev,
			      &wlpptr->wlpd_p->pdfsApMain->dfsApDesc.
			      currChanInfo)) {
		MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
		MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;

		/* Send a channel switch event */
		dfs_sme_channel_switch(netdev, PhyDSSSTable->CurrChan, 0, 0);

		/* Switch on quiet mode radar detection */
		macMgmtMlme_StartRadarDetection(netdev, DFS_QUIET_MODE);
	}
#endif
...
wlstop_mbss(struct net_device *netdev)
{
	...
	if (netdev->flags & IFF_RUNNING) {
		...
		if (wlpptr->wlpd_p->pdfsApMain != NULL &&
		    wlpptr->wlpd_p->bCACTimerFired == TRUE) {
			DfsApDesc *dfsDesc_p =
				&wlpptr->wlpd_p->pdfsApMain->dfsApDesc;
			...
			wlpptr->wlpd_p->bCACTimerFired = FALSE;
			// Switch off radar detection
			macMgmtMlme_StopRadarDetection(netdev, DFS_NORMAL_MODE);
			dfsDesc_p->currState = DFS_STATE_IDLE;
		}
#endif
#ifdef SOC_W906X
		wlOffChannelStop(netdev);
...
wlInit_mbss(struct wlprivate *wlp, unsigned char *macAddr)
{
	...
	ap8xLnxStat_vap_init(&dev->dev.kobj);
...
wlDeinit_mbss(struct net_device *netdev)
{
	...
	WLDBG_ENTER(DBG_LEVEL_2);
	ap8xLnxStat_vap_exit(&netdev->dev.kobj);
#ifdef SOC_W906X
	if (wlpptr->wlpd_p->downloadSuccessful == TRUE)
#endif /* SOC_W906X */
	{
	      ...
	      unsigned char *ApRootmacAddr_p)
{
	...
	ap8xLnxStat_clients_init(&wlpptr->netDev->dev.kobj, 2);
