...
extern const u_int32_t buf_pool_size[SC5_BMQ_NUM];
void
	   ...
	   UINT8 scale_mapping)
{
	...
	memcpy(&smacStatus, wlpptr->smacStatusAddr, sizeof(SMAC_STATUS_st));

	if (delta_time == 0) {
		return smacStatus.slotTickCnt;
	} else {
		if (scale_mapping == 0) {
			scale_mapping = 100;
		}
		idleTime = (smacStatus.slotTickCnt - slotTickCnt) * slotTime;
		r_load = (delta_time >
			  idleTime) ? ((delta_time -
	...
	if (vmacSta_p->InfUpFlag && ch_load_info->started &&
	    *(mib->mib_autochannel) != 0) {
		...
		if (ch_idx == IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A) {
			...
			tmp_nf = wl_util_get_nf(vmacSta_p->dev,
						&priv->wlpd_p->NF_path, NULL);
			if (tmp_nf != 0) {
	       ...
	       SINT16 * s_value)
{
	...
	for (i = 0; i < MAX_RF_ANT_NUM; i++) {
		if (nf_value[i] >= 2048) {
			nf_value_signed[i] = -((4096 - nf_value[i]) >> 4);
		} else {
