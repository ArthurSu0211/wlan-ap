...
// ================================================================
static void wl_rx_info_aux_sync(rx_info_aux_t * prx_info_aux,
				rx_info_ppdu_t * prxinfo);
...
static void
wlrxinfo_sync_rxinfoaux_b(rx_info_aux_t * prx_info_aux,
			  rx_info_ppdu_t * prxinfo)
{
...
static void
wlrxinfo_sync_rxinfoaux_a_g(rx_info_aux_t * prx_info_aux,
			    rx_info_ppdu_t * prxinfo)
{
...
static void
wlrxinfo_sync_rxinfoaux_n(rx_info_aux_t * prx_info_aux,
			  rx_info_ppdu_t * prxinfo)
{
	...
	pht_sig2->ht_sig2 =
		prxinfo->bbrx_info.htsig2_vhtsiga2_hesiga2 & 0xffffff;
...
static void
wlrxinfo_sync_rxinfoaux_ac(rx_info_aux_t * prx_info_aux,
			   rx_info_ppdu_t * prxinfo, BOOLEAN is_su)
{
...
static void
wlrxinfo_sync_rxinfoaux_he(rx_info_aux_t * prx_info_aux,
			   rx_info_ppdu_t * prxinfo, BOOLEAN is_su)
{
	...
	//
	phe_siga1->he_sig_a1 =
		(prxinfo->bbrx_info.hesiga1 << 24) | prxinfo->bbrx_info.
...
static void
wl_set_radiotap_he_su_info(ieee80211_radiotap_he_info * he_info,
			   rx_info_aux_t * prx_info_aux)
{
	HE_SIG_A1 *phe_siga1 = &prx_info_aux->he_siga1;
	HE_SIG_A2 *phe_siga2 = &prx_info_aux->he_siga2;
...
static void
wl_set_radiotap_he_suext_info(ieee80211_radiotap_he_info * he_info,
			      rx_info_aux_t * prx_info_aux)
{
	//HE_SIG_A1 *phe_siga1 = &prx_info_aux->he_siga1;
	//HE_SIG_A2 *phe_siga2 = &prx_info_aux->he_siga2;
...
// Not tested yet
static void
wl_set_radiotap_he_mu_info(ieee80211_radiotap_he_info * he_info,
			   rx_info_aux_t * prx_info_aux)
{
	HE_SIG_A1 *phe_siga1 = &prx_info_aux->he_siga1;
	HE_SIG_A2 *phe_siga2 = &prx_info_aux->he_siga2;
...
static void
wl_set_radiotap_he_tb_info(ieee80211_radiotap_he_info * he_info,
			   rx_info_aux_t * prx_info_aux)
{
	//HE_SIG_A1 *phe_siga1 = &prx_info_aux->he_siga1;
	//HE_SIG_A2 *phe_siga2 = &prx_info_aux->he_siga2;
...
static void
wl_set_radiotap_vht_info(ieee80211_radiotap_vht_info * vht_info,
			 rx_info_aux_t * prx_info_aux)
{
	VHT_SIG_A1 *pvht_siga1 = &prx_info_aux->vht_siga1;
	VHT_SIG_A2 *pvht_siga2 = &prx_info_aux->vht_siga2;
...
static void
wl_set_radiotap_lsig(struct ieee80211_radiotap_lsig *plsig,
		     rx_info_aux_t * prx_info_aux)
{
	...
	if ((prx_info_aux->rx_mode == BBRX_RM_B) ||
	    (prx_info_aux->rx_mode == BBRX_RM_A_G)) {
	...
	plsig->data2 =
		(((length << 4) & IEEE80211_RADIOTAP_LSIG_DATA2_LENGTH) |
		 (datarate & IEEE80211_RADIOTAP_LSIG_DATA2_RATE));
	return;
}
...
*/
void
wlrxinfo_2_radiotap(rx_info_aux_t * prx_info_aux, generic_buf * pbuf)
{
	struct ieee80211_radiotap_header *pradhdr =
		(struct ieee80211_radiotap_header *)pbuf->bufpt;
	U8 *pwkbuf;
	U32 *ppres_bit;
	const U32 bit_dbmsig = 1 << IEEE80211_RADIOTAP_DBM_ANTSIGNAL;
	const U32 bit_ant = 1 << IEEE80211_RADIOTAP_ANTENNA;
	const U32 bit_ext =
		(1 << IEEE80211_RADIOTAP_RADIOTAP_NAMESPACE) | (1 <<
	...
	// NSS#
	for (nssid = 0, ppres_bit = &pradhdr->it_present; nssid < nss;
	     nssid++, ppres_bit++) {
		if (nssid > 0) {
			// << Antenna info >>
			// dbm antenna signal
	...
	*pwkbuf = 1;
	...
	if (prx_info_aux->rx_mode == BBRX_RM_AC) {
		// Add the VHT Info
		ieee80211_radiotap_vht_info *vht_info =
			(ieee80211_radiotap_vht_info *) pwkbuf;
		...
		ieee80211_radiotap_he_info *he_info =
			(ieee80211_radiotap_he_info *) pwkbuf;
	...
	// << L-SIG Info >>
	{
		wl_set_radiotap_lsig((struct ieee80211_radiotap_lsig *)pwkbuf,
				     prx_info_aux);
...
static void
wlrxinfo_2_rxhisto(DRV_RATE_HIST * pdrv_rx_hist, rx_info_aux_t * prx_info_aux)
{
	dbRateInfo_t *prate_info = &prx_info_aux->rate_info;
	...
	case BBRX_RM_GREEN_FIELD:
		...
		break;
	case BBRX_RM_N:
		pdrv_rx_hist->HtRates[prate_info->Bandwidth][prate_info->
							     ShortGI]
			[prate_info->RateIDMCS % QS_NUM_SUPPORTED_MCS]++;
		//wlrxinfo_2_n_rxhisto(pdrv_rx_hist, prxinfo, msdu_cnt, name);
		break;
	case BBRX_RM_AC:
		pdrv_rx_hist->VHtRates[prx_info_aux->nss -
				       1][prate_info->Bandwidth][prate_info->
								 ShortGI]
			[prate_info->RateIDMCS & 0xf]++;
	...
	case BBRX_RM_HE_TRIG_BASED:
		// HE rate
		pdrv_rx_hist->HERates[prx_info_aux->nss -
				      1][prate_info->Bandwidth][prate_info->
								ShortGI]
			[prate_info->RateIDMCS & 0xf]++;
...
const u32 R10000xNbpsc[HE_MCS_MAX] =
	{ 5000, 10000, 15000, 20000, 30000, 40000, 45000, 50000, 60000, 66667,
75000, 83333 };
const u32 R10000xNbpsc_ofdma[10] =
...
static u32
get_HE_preamble_10x(U16 nss_1, int gi, int ofdma)
{
	// gi_ltf:
	//              3 => ltf_type=4, gi=3.2
	//              2 => ltf_type=2, gi=1.6
	// su: 8+8+4+4+8+4+ Nltf*(3.2*LTF_type+ GI)
	const u32 preamble32us[8] =
	...
	const u32 preambe32usofdma[8] =
		{ 920, 1080, 1400, 1400, 1720, 1720, 2040, 2040 };
	const u32 preambe16usofdma[8] =
...
*/
static void
wl_rx_info_aux_sync(rx_info_aux_t * prx_info_aux, rx_info_ppdu_t * prxinfo)
{
	...
	wl_set_rssi_info(&prx_info_aux->rssi_info,
			 (RxSidebandInfo_t *) prxinfo);
...
*/
static U32
wl_airtime_calc(rxppdu_airtime_t * prxppdu_airtm, U32 pktlen)
{
...
// 
void static
wl_proc_rx_airtime(extStaDb_StaInfo_t * StaInfo_p,
		   wlrxdesc_t * pCurCfhul, struct sk_buff *pRxSkBuff)
{
	...
	if (prxppdu_airtm->rx_info_aux.rxTs != pCurCfhul->hdr.timestamp) {
		...
		memcpy(&prxppdu_airtm->rx_info_aux, &StaInfo_p->rx_info_aux,
...
*/
void
wlrxinfo_notify_new_msdu(struct net_device *netdev, wlrxdesc_t * pCurCfhul,
			 u8 qid, struct sk_buff *pmsdu)
{
	...
	memcpy(&wlpd_p->rssi_path_info, &prx_info_aux->rssi_info,
	...
	if (frame_ctlp->Type == IEEE_TYPE_DATA) {
		...
		if ((prx_info_aux->StaInfo_p == NULL) || (memcmp(((extStaDb_StaInfo_t *) prx_info_aux->StaInfo_p)->Addr, (IEEEtypes_MacAddr_t *) & pmsdu->data[6], sizeof(IEEEtypes_MacAddr_t)))) {	// different mac address => MU
			...
			prx_info_aux->StaInfo_p =
				(void *)extStaDb_GetStaInfo(vmacSta_p,
							    (IEEEtypes_MacAddr_t
							     *) & pmsdu->
							    data[6],
		...
		if (StaInfo_p != NULL) {
			...
			memcpy(&StaInfo_p->rx_info_aux, prx_info_aux,
			       sizeof(rx_info_aux_t));
			memcpy(&StaInfo_p->RSSI_path, &prx_info_aux->rssi_info,
			       sizeof(RssiPathInfo_t));
...
void
wlrxinfo_qproc(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	for (qid = pbqm_args->racntq_index;
		...
		while (wrinx != wlqm->sq.rdinx) {
			...
			wl_rx_info_aux_sync(&wlpd_p->
					    rxinfo_aux_poll[wlqm->sq.rdinx],
					    &wlpd_p->
					    acntRxInfoQueBaseAddr_v[wlqm->sq.
								    rdinx]
				);
			// Update the rx_index
