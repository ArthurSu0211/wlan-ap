...
#endif
			 curtime = xxGetTimeStamp(); diff = curtime - twt_stick; if (pktlen > wfa_twt_rx_mon_length) {	//long frame

			 //reset short Frame counting cycle
			 twt_stickS = 0; if (wfa_mon_twt_rxtx == 1)	//already in Rx mode.
			 goto exit; switch (twt_pktcntL) {
case 0:
twt_pktcntL++; twt_stick = curtime; break; case 1:
...
#if 0
			 //set PFW TWT Rx mode
			 writel(1, (wlpptr->ioBase0 + wfa_twt_rxing_flag_addr));
			 printk("Set iTWT 5.60.1 Rx Mode:%u\n", *pfwFlag);
			 *pfwFlag = 1;
#else
			 memset(cfg, 0, sizeof(cfg));
			 cfg[0] =
			 (TWT_WFA_RXMODE | TWT_WFA_CODEWORD << 16 |
			  ((++itwtswcnt) << 24));
			 wlFwGetAddrValue(((wlpptr->master) ? wlpptr->
					   master : wlpptr->netDev),
					  wfa_twt_rxing_flag_addr, 4, cfg, 1);
			 printk("Set iTWT 5.60.1 Rx Mode\n");
#endif
			 twt_pktcntL = 0; twt_stick = 0;}
			 else {
			 twt_pktcntL = 1;	//restart another counting cycle
			 twt_stick = curtime;}
			 break;}
			 ...
			 twt_pktcntL = 0; twt_stick = 0; twt_stickS = 0;}
			 else {
			 twt_stickS = 0; goto exit;	//no count this in
			 }
			 }

			 if (wfa_mon_twt_rxtx == 0) {	//not count short when fw is in Tx mode 
			 twt_stickS = 0; goto exit;}
			 else {	//FW in RX
			 ...
			 memset(cfg, 0, sizeof(cfg));
			 cfg[0] =
			 (TWT_WFA_NONRXMODE | TWT_WFA_CODEWORD << 16 |
			  ((++itwtswcnt) << 24));
			 if (!wlFwGetAddrValue
			     (((wlpptr->master) ? wlpptr->master : wlpptr->
			       netDev), wfa_twt_rxing_flag_addr, 4, cfg, 1)) {
			 ...
			 writel(0, (wlpptr->ioBase0 + wfa_twt_rxing_flag_addr));
...
//#endif
			 ...
			 WLDBG_DATA(DBG_LEVEL_3,
				    ...
				    wlpptr->wlpd_p->bpreresetdone); return;}

			 local_irq_save(flags);
			 RxQs = wlpptr->RxQId;
			 wlpptr->RxQId = 0;
			 local_irq_restore(flags);
			 for_each_set_bit(qid, (unsigned long *)&RxQs,
					  SC5_RXQ_NUM) {
			 struct wldesc_data * wlqm = &wlpptr->wlpd_p->descData[qid]; u32 wrinx; if (((1 << qid) & SC5_RXQ_MASK) == 0) {	// Not enabled
			 continue;}
			 wlqm->sq.wrinx = wrinx =
			 ...
			 cfh_ul_amsdu = wlProcessErrCfhul(netdev, &msduNo);
			 if (msduNo == 0 && cfh_ul_amsdu == NULL)
			 ...
			 wlrxdesc_t * pCurCfhul; u8 LMFbit;	//b2:last, b1:mid, b0:1st amsdu
			 seq = (*(u_int32_t *) cfh_ul) & 0xffff0000;
			 qos = cfh_ul->qos; if (cfh_ul_amsdu) {
			 pCurCfhul = &cfh_ul_amsdu[idx];
			 pRxSkBuff =
			 wlCfhUlToSkb(netdev, &cfh_ul_amsdu[idx++], qid);
#ifdef MULTI_AP_SUPPORT
			 if (pCurCfhul && (pCurCfhul->fpkt == 1)) {
			 if (pRxSkBuff &&
			     (pRxSkBuff->
			      protocol & WL_WLAN_TYPE_RX_FAST_DATA)) {
			 //cfhul_template = pRxSkBuff->data + pCurCfhul->cfh_offset;

			 mac_hdr =
			 (IEEEtypes_fullHdr_t *) (pRxSkBuff->data +
						  pCurCfhul->cfh_offset +
						  SMAC_MAC_HDR_OFFSET);}
			 }
#endif
			 }
			 ...
			 {
			 pCurCfhul = cfh_ul;
			 pRxSkBuff = wlCfhUlToSkb(netdev, cfh_ul, qid);}
			 if (pRxSkBuff == NULL) {
			 ...
			 WLDBG_ERROR(DBG_LEVEL_0, "pCurCfhul is NULL!\n");
			 continue;}
#ifdef TP_PROFILE
			 if (frame_ctlp->Type == IEEE_TYPE_DATA) {
			 if (wl_tp_profile_test(11, pRxSkBuff, netdev)) {
			 wl_free_skb(pRxSkBuff); continue;}
			 }
#endif
			 if ((frame_ctlp->Type != IEEE_TYPE_CONTROL) &&
			     wl_proc_mic_defrag(netdev, wlexcept_p, pCurCfhul,
						&pRxSkBuff)) {
			 continue;}
			 ...
			 LMFbit =
			 (pCurCfhul->
			  lpkt << 2) | ((~(pCurCfhul->lpkt | pCurCfhul->fpkt) &
			 ...
			 nullskb = TRUE; wlexcept_p->msdu_err++; continue;}
			 // rx_info processing
			 ...
			 iTWT_monitor_Rx_start(wlpptr, pRxSkBuff->len);}
#endif
			 //add stnid
			 if (qid == 0) {
			 if ((frame_ctlp->FromDs == 0) &&
			     (frame_ctlp->ToDs == 0)) {
			 wl_free_skb(pRxSkBuff); pRxSkBuff = NULL;
			 wlexcept_p->msdu_err++; continue;}
			 else
			 if ((frame_ctlp->FromDs == 0) &&
			     (frame_ctlp->ToDs == 1)) {
			 ...
			 wl_free_skb(pRxSkBuff);
			 pRxSkBuff = NULL; wlexcept_p->msdu_err++; continue;}
			 stnid = StaInfo_p->StnId;}
			 else
			 if ((frame_ctlp->FromDs == 1) &&
			     (frame_ctlp->ToDs == 0)) {
			 ...
			 if ((frame_ctlp->FromDs == 1) &&
			     (frame_ctlp->ToDs == 1)) {
			 stnid = rxring_Ctrl_STAfromDS;
			 pRxSkBuff->protocol |= WL_WLAN_TYPE_WDS;
#ifdef MULTI_AP_SUPPORT
			 {
			 extStaDb_StaInfo_t * pStaInfo = NULL; if (mac_hdr) {
			 pStaInfo =
			 extStaDb_GetStaInfo(vmacSta_p,
					     (IEEEtypes_MacAddr_t *) & mac_hdr->
					     Addr2, STADB_SKIP_MATCH_VAP);
			 if (pStaInfo)
...
#ifdef ENABLE_MONIF
			 if (((wlpptr->vdev[wlpptr->wlpd_p->MonIfIndex] != NULL)
			      && (wlpptr->vdev[wlpptr->wlpd_p->MonIfIndex]->
				  flags & IFF_RUNNING))) {
			 //memset((void *)&rx_info, 0 ,sizeof(rx_info));
			 if (pRxSkBuff != NULL) {
			 //*(generic_buf**)pRxSkBuff->cb = pgbuf;
			 //monif_handle_recv(wlpptr, &rx_info, pRxSkBuff, 1);
			 monif_handle_recv(wlpptr,
					   &wlpd_p->rxinfo_aux_poll[pCurCfhul->
								    hdr.
								    rxInfoIndex],
					   pRxSkBuff, 1);}
			 }
			 ...
			 (struct ieee80211_frame *)pRxSkBuff->data;
			 struct IEEEtypes_Frame_t * wlanMsg_p =
			 (IEEEtypes_Frame_t *) ((UINT8 *) pRxSkBuff->data - 2);
			 wlanMsg_p->Hdr.FrmBodyLen = pRxSkBuff->len;
			 if (wh->FrmCtl.Type == IEEE_TYPE_DATA) {
			 if (wlpptr->offchan_state == OFFCHAN_IDLE) {
			 extStaDb_StaInfo_t * pStaInfo = NULL;
			 vmacApInfo_t * vmactem_p = NULL;
			 /* found vap from addr1 */
			 vmactem_p = vmacGetMBssByAddr(vmacSta_p, wh->addr1);
			 if (vmactem_p &&
			     (vmactem_p->dev->flags & IFF_RUNNING)) {
			 pStaInfo =
...
#ifdef CB_SUPPORT
			 struct wlprivate * wlpptr =
			 NETDEV_PRIV_P(struct wlprivate, vmactem_p->dev);
			 if (wlpptr->is_resp_mgmt == TRUE)
#endif //CB_SUPPORT
			 {
			 if (!vmactem_p->deauth_block) {
			 vmactem_p->deauth_block = 1;
			 macMgmtMlme_SendDeauthenticateMsg(vmactem_p,
							   (IEEEtypes_MacAddr_t
							    *) (wh->addr2), 0,
							   IEEEtypes_REASON_CLASS3_NONASSOC,
							   TRUE);
			 TimerFireIn(&vmactem_p->deauth_block_timer, 1,
				     &reset_deauth_block_timer,
				     (unsigned char *)vmactem_p, 1);}
			 }
			 }
			 }
			 }
			 ...
			 MSAN_unassocsta_recv_proc(vmacSta_p, wlanMsg_p,
						   pCurCfhul->hdr.rssi);}
			 ...
			 MSAN_neighbor_bcnproc(netdev, wlanMsg_p,
					       pRxSkBuff->len,
					       &wlpd_p->rssi_path_info,
					       SCAN_BY_OFFCHAN);
#ifdef MULTI_AP_SUPPORT
			 else
			 ...
			 work_done +=
			 ieee80211_input(netdev, pRxSkBuff, pCurCfhul->hdr.rssi,
					 &wlpd_p->rssi_path_info, pCurCfhul,
					 (idx == 0 ? TRUE : FALSE), stnid,
					 LMFbit);}
...
} while (msduNo != idx); drop:
			 if (nullskb) {
			 nullskb = FALSE;
			 //break;
			 }
			 wlSQIndexGet(&(wlqm->sq));}	// end of while loop
			 /* update write pointer */
			 wlUpdateRdPtr(netdev, qid, SC5_SQ, wlqm->sq.rdinx, false);}	// end of RxQ loop

			 trace_wlRecv_end(netdev);
			 trace_func_delta_end(vmacSta_p); return;}

			 void wl_show_recv_info(struct net_device *netdev,
						char *sysfs_buff) {
			 struct wlprivate * wlpptr =
			 NETDEV_PRIV_P(struct wlprivate, netdev);
			 RxTimeCntStat * RxInfo =
			 &(wlpptr->vmacSta_p->Rx_StatInfo); UINT8 i;
			 ...
			 void reset_deauth_block_timer(UINT8 * data) {
