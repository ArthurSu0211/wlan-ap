...
mwl_config_set_appie(struct net_device *netdev, struct wlreq_set_appie *appie)
{
	...
	case WL_OPTIE_ASSOC_INCL_RSN:
		rsn_ie = FindIEWithinIEs(appie->appBuf, appie->appBufLen,
					 RSN_IEWPA2, NULL);
		wpa_ie = FindIEWithinIEs(appie->appBuf, appie->appBufLen,
					 RSN_IE, WPA_OUI);
#ifdef MRVL_80211R
		...
		memset(vmacSta_p->WpaIE, 0,
		       sizeof(IEEEtypes_RSN_IE_WPAMixedMode_t));
		...
		RsnBIPcap((IEEEtypes_RSN_IE_WPA2_t *) vmacSta_p->RsnIE,
	...
	case WL_APPIE_FRAMETYPE_BEACON:
		...
		if (appie->appBufLen > 8) {
			...
			memcpy(&APWSCIE.beaconIE.WSCData[0], appie->appBuf,
			       (appie->appBufLen >
				WSC_BEACON_IE_MAX_LENGTH) ?
			       WSC_BEACON_IE_MAX_LENGTH : appie->appBufLen);
			memcpy(&vmacSta_p->thisbeaconIEs, &APWSCIE.beaconIE,
			       sizeof(WSC_BeaconIEs_t));
			wps_ie = FindIEWithinIEs(vmacSta_p->thisbeaconIEs.
						 WSCData,
		...
		if (vmacSta_p->thisbeaconIEs.Len > 0) {
			pMBO = FindIEWithinIEs(vmacSta_p->thisbeaconIEs.WSCData,
					       vmacSta_p->thisbeaconIEs.Len,
					       PROPRIETARY_IE, MBO_OUI);
			if (pMBO)
		...
		if (vmacSta_p->thisbeaconIEs.Len > 0) {
			...
			if (pExtCap) {
				...
				*(mib1->mib_btm_enabled) =
					pExtCap->ExtCap.BSSTransition;
#endif /* AP_STEERING_SUPPORT */
#ifdef MBO_SUPPORT
				mib1->mib_mbo_wnm =
					pExtCap->ExtCap.WNMNotification;
				mib1->Interworking =
					pExtCap->ExtCap.Interworking;
#endif /* MBO_SUPPORT */

				/* remove duplicated extended capabilities */
				memmove(&(pExtCap->ElementId),
					vmacSta_p->thisbeaconIEs.WSCData +
					extCapLen,
					vmacSta_p->thisbeaconIEs.Len -
					extCapLen - (&(pExtCap->ElementId) -
						     vmacSta_p->thisbeaconIEs.
						     WSCData));
	...
	case WL_APPIE_FRAMETYPE_PROBE_RESP:
		...
		if (appie->appBufLen > 8) {
			...
			memcpy(&APWSCIE.probeRespIE.WSCData[0], appie->appBuf,
			       (appie->appBufLen >
				WSC_PROBERESP_IE_MAX_LENGTH) ?
			       WSC_PROBERESP_IE_MAX_LENGTH : appie->appBufLen);
			memcpy(&vmacSta_p->thisprobeRespIEs,
			       &APWSCIE.probeRespIE,
			       sizeof(WSC_ProbeRespIEs_t));
			wps_ie = FindIEWithinIEs(vmacSta_p->thisprobeRespIEs.
						 WSCData,
						 vmacSta_p->thisprobeRespIEs.
						 Len, PROPRIETARY_IE, WPS_OUI);
			if (wps_ie)
				vmacSta_p->WPSOn = 1;
			if (wlFwSetIEs(netdev))
				WLDBG_EXIT_INFO(DBG_LEVEL_1,
		...
		if (vmacSta_p->thisprobeRespIEs.Len > 0) {
			pExtCap =
				FindIEWithinIEs(vmacSta_p->thisprobeRespIEs.
						WSCData,
			...
			if (pExtCap) {
				...
				/* remove duplicated extended capabilities */
				memmove(&(pExtCap->ElementId),
					vmacSta_p->thisprobeRespIEs.WSCData +
					extCapLen,
					vmacSta_p->thisprobeRespIEs.Len -
					extCapLen - (&(pExtCap->ElementId) -
						     vmacSta_p->
						     thisprobeRespIEs.WSCData));
				vmacSta_p->thisprobeRespIEs.Len -= extCapLen;
			}
#ifdef MBO_SUPPORT
			if (mib1->mib_mbo_enabled) {
				UINT8 changed = 0;
				pMBO = FindIEWithinIEs(vmacSta_p->
						       thisprobeRespIEs.WSCData,
						       vmacSta_p->
						       thisprobeRespIEs.Len,
						       PROPRIETARY_IE, MBO_OUI);

				if ((pMBO) && (pMBO->Len >= 0x0a) &&
				    (*(pMBO->variable + 3) == 0x04)) {
					if (!mib1->mib_mbo_assoc_disallow) {
						mib1->mib_mbo_assoc_disallow =
							1;
						changed = 1;
					}
				} else {
...
#ifdef MULTI_AP_SUPPORT
		MultiAP_IE_p =
			FindIEWithinIEs(appie->appBuf, appie->appBufLen,
					PROPRIETARY_IE, MAP_OUI);
	...
	if (APWSCIE.beaconIE.Len > 0) {
		pExtCap =
			FindIEWithinIEs(APWSCIE.beaconIE.WSCData,
					APWSCIE.beaconIE.Len, EXT_CAP_IE, NULL);
		...
		if (pExtCap) {
			...
			/* remove duplicated extended capabilities */
			memmove(&(pExtCap->ElementId),
				APWSCIE.beaconIE.WSCData + extCapLen,
				APWSCIE.beaconIE.Len - extCapLen -
				(&(pExtCap->ElementId) -
				 APWSCIE.beaconIE.WSCData));
			APWSCIE.beaconIE.Len -= extCapLen;
		}
	}
	...
	case WL_MLME_ASSOC:
		{
			...
			memcpy(&PeerInfo, mlme->PeerInfo,
			       sizeof(mlme->PeerInfo));
			macMgmtMlme_set_sta_associated(vmacSta_p,
						       &mlme->im_macaddr,
						       mlme->Aid,
						       (PeerInfo_t *) &
						       PeerInfo, mlme->QosInfo,
						       mlme->isQosSta,
						       mlme->rsnSta,
						       (UINT8 *) & mlme->rsnIE);
			break;
...
#define GetParentStaBSSID(x) NULL
	...
	if (wk->ik_keyix == WL_KEYIX_NONE) {
		...
		if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA) {
			...
			UINT32 keyInfo =
				ENCR_KEY_FLAG_PTK | ENCR_KEY_FLAG_STA_MODE;
			...
			if (!pStaSecurityMibs->mib_RSNConfigWPA2_p->
				...
				if (pStaSecurityMibs->
					...
					getTkipStaKeyMaterial(pStaInfo,
							      &tkipParam);
		...
		if (wk->ik_type == WL_CIPHER_TKIP) {
			...
			if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP) {
				...
				memcpy(mib1->mib_MrvlRSN_GrpKey->TxMICKey,
				       &wk->ik_keydata[TK_SIZE], 8);
				memcpy(mib1->mib_MrvlRSN_GrpKey->RxMICKey,
				       &wk->ik_keydata[TK_SIZE + 8], 8);
				...
				memcpy(param.KeyMaterial,
				       mib1->mib_MrvlRSN_GrpKey->EncryptKey,
				       TK_SIZE);
				memcpy(param.RxMicKey,
				       mib1->mib_MrvlRSN_GrpKey->RxMICKey,
				       MIC_KEY_LENGTH);
				memcpy(param.TxMicKey,
				       mib1->mib_MrvlRSN_GrpKey->TxMICKey,
				       MIC_KEY_LENGTH);

				param.Tsc.low =
					mib1->mib_MrvlRSN_GrpKey->g_IV16;
				param.Tsc.high =
					mib1->mib_MrvlRSN_GrpKey->g_IV32;

				wlFwSetSecurityKey(netdev, ACT_SET,
						   KEY_TYPE_ID_TKIP,
			...
			if ((wk->ik_type == WL_CIPHER_GCMP) ||
			    (wk->ik_type == WL_CIPHER_GCMP_256))
				keyType = KEY_TYPE_ID_GCMP;

			memcpy(param.KeyMaterial, wk->ik_keydata,
			       wk->ik_keylen);
...
#endif
			return 0;
		} else if ((wk->ik_type == WL_CIPHER_WEP40) ||
			   (wk->ik_type == WL_CIPHER_WEP104)) {
			if (wk->ik_keylen == 5 || wk->ik_keylen == 13) {
				mwl_config_set_wepkey(netdev, wk->ik_keydata,
						      wk->ik_keylen,
						      MWL_WEP_ENCODE_RESTRICTED,
						      wk->ik_keyix);
		...
		if (wk->ik_type == WL_CIPHER_IGTK ||
			...
			if (vmacSta_p->VMacEntry.modeOfService ==
				...
				if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p,
								     ...
								     *)
								    GetParentStaBSSID
								    ((vmacEntry_p)->phyHwMacIndx), STADB_UPDATE_AGINGTIME)) == NULL)
					return -EOPNOTSUPP;
		  ...
		  UINT16 * ret_len)
{
	...
	extStaDb_StaInfo_t *pStaInfo =
		extStaDb_GetStaInfo(vmacSta_p,
				    (IEEEtypes_MacAddr_t *) & IEReq->macAddr,
...
int
mwl_config_set_wepkey(struct net_device *netdev, UINT8 * data, int key_len,
		      UINT8 encode, UINT16 key_index)
{
	...
	if (encode & MWL_WEP_ENCODE_DISABLED) {
		...
		if (key_len > 0)	//set open/restracted mode at [1] len=1
		{
			...
			if (key_len == 5) {
				wep_type = 1;
				mib->WepDefaultKeys[key_index].WepType =
					wep_type;
			...
			memset(mib->WepDefaultKeys[key_index].
			       WepDefaultKeyValue, 0, 13);
			memset(tmpWEPKey, 0, sizeof(tmpWEPKey));
			memcpy(tmpWEPKey, data, key_len);
			memcpy(mib->WepDefaultKeys[key_index].
			       WepDefaultKeyValue, tmpWEPKey, key_len);
			if (WL_FUN_SetWEPKey
			    ((void *)priv, key_index, wep_type, tmpWEPKey)) {
				PRINT1(IOCTL,
				       "mwl_drv_set_wepkey: length = %d index = %d type = %d\n",
				       key_len, key_index, wep_type);
				PRINT1(IOCTL,
				       "wep key = %x %x %x %x %x %x %x %x %x %x %x %x %x \n",
				       mib->WepDefaultKeys[key_index].
