...
wlmon_ext_trigger_init(struct wlprivate *wlpptr)
{
	...
	case 1:
		...
		val = readl(cp_gpio_base + 0x01C);
		val &= 0xFFFF0FFF;
		writel(val, cp_gpio_base + 0x01C);
		// set CP_GPIO[59].Output = 0
		val = readl(cp_gpio_base + 0x140);
		val &= ~(1 << 27);
		writel(val, cp_gpio_base + 0x140);
		// set CP_GPIO[59].OE = 0 (active low)
		val = readl(cp_gpio_base + 0x144);
		val &= ~(1 << 27);
		writel(val, cp_gpio_base + 0x144);
		break;
	case 2:
...
wlmon_ext_trigger_assert(struct net_device *netdev)
{
	...
	case 2:
		...
		break;
	case 3:
		writel(2, wlpptr->ioBase1 + 0x13F84);
		break;
	default:
		dev_err(wlpptr->wlpd_p->dev,
			"unsupported gpio trigger mode %ul", hm_gpio_trigger);
...
get_mci_errcnt(struct wlprivate * wlpptr)
{
	...
	writel(regval, wlpptr->ioBaseExt + 0x04);
	regval = readl(wlpptr->ioBaseExt + 0x00);
...
wlmon_kthread(void *arg)
{
	...
	for (;;) {
		...
		if (wlpd_p->smon.exceptionEvt_rcvd ||
			...
			if (wlpd_p->smon.exceptionEvt_rcvd) {
				u32 is_sfw =
					(wlpptr->smacStatusAddr->smacSts[7] ==
					 0x07390000);
