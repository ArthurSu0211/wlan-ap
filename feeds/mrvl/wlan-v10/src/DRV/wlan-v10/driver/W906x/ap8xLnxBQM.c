...
const u_int32_t buf_pool_size[SC5_BMQ_NUM] = { 0x1000, 0x3000, 0x6400, BUF_8K };	// 4k, 12k, 25k 8k,
...
wlQueryRdPtr(struct net_device * netdev, int qid, int qoff)
{
	...
	if (qoff == SC5_RQ) {
		...
		regval = readl(wlpptr->ioBase1 + SC5_RQ_RDPTR_REG(qid));
		qsize = wlqm->rq.qsize;
		WLDBG_INFO(DBG_LEVEL_5,
		...
		regval = readl(wlpptr->ioBase1 + SC5_SQ_RDPTR_REG(qid));
		qsize = wlqm->sq.qsize;
		WLDBG_INFO(DBG_LEVEL_5,
...
CheckSMACReady(struct net_device * netdev)
{
	...
	while (loopcnt++ < 1000) {
		regval = readl(parent_wlpptr->ioBase0 + regoff);
	      ...
	      bool is_init)
{
	...
	if (qoff == SC5_RQ) {
		writel(ptval, (wlpptr->ioBase1 + SC5_RQ_RDPTR_REG(qid)));
		WLDBG_INFO(DBG_LEVEL_5, "QINDEX(%d) RQ: w_reg(%p)=%xh\n",
			   (wlpptr->ioBase1 + SC5_RQ_RDPTR_REG(qid)), ptval);
	} else {
		writel(ptval, (wlpptr->ioBase1 + SC5_SQ_RDPTR_REG(qid)));
	      ...
	      bool is_init)
{
	...
	if (qoff == SC5_RQ) {
		writel(ptval, (wlpptr->ioBase1 + SC5_RQ_WRPTR_REG(qid)));
		WLDBG_INFO(DBG_LEVEL_5, "QINDEX(%d),r_reg(ioBase1+%xh) = %xh\n",
			   qid, SC5_RQ_WRPTR_REG(qid), ptval);
	} else {
		writel(ptval, (wlpptr->ioBase1 + SC5_SQ_WRPTR_REG(qid)));
		WLDBG_INFO(DBG_LEVEL_5, "QINDEX(%d),r_reg(ioBase1+%xh) = %xh\n",
...
wlTxQueueInit(struct net_device *netdev, int qid)
{
	...
	writel((u32) wlqm->rq.phys_addr,
	       (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_RQ)));
	WLDBG_INFO(DBG_LEVEL_0,
	...
	// high address
	writel(smac_buf_hi_addr,
	       (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_RQ)) + 4);
	WLDBG_INFO(DBG_LEVEL_0, "TXQ(%d),w_reg(ioBase1+%xh) = %xh\n", qid,
		   (SC5_Q_BASE_ADDR_REG(qid, SC5_RQ) + 4), smac_buf_hi_addr);
	writel(((wlqm->rq.qsize * sizeof(wltxdesc_t)) / 128) << 3,
	       (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_RQ)));
	WLDBG_INFO(DBG_LEVEL_0,
...
wlBmQueueInit(struct net_device *netdev, int qid)
{
	...
	//
	writel(((wlqm->rq.qsize * sizeof(bm_pe_hw_t)) / 128) << 3,
	       (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_RQ)));
	WLDBG_INFO(DBG_LEVEL_0,
...
wlBufReleaseQueueInit(struct net_device *netdev, int qid)
{
	...
	writel(((wlqm->sq.qsize * sizeof(bm_pe_hw_t)) / 128) << 3,
	       (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0,
	...
	writel(SC5_TXDONE_INT_THRESHOLD,
	       (wlpptr->ioBase1 + SC5_Q_THRES_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d),w_reg(ioBase1+%xh) = %xh\n", qid,
		   SC5_Q_THRES_REG(qid, SC5_SQ), 0x1);

	writel(SQ5_MAC_CTRL_TIMEOUT(SC5_TXDONE_INT_TIMEOUT),
	       (wlpptr->ioBase1 + SC5_Q_MAC_CTRL_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d),w_reg(ioBase1+%xh) = %xh\n", qid,
...
wlRAcntQueueInit(struct net_device *netdev, int qid)
{
	...
	writel(((wlqm->sq.qsize * sizeof(rxacnt_rec)) / 128) << 3,
	       (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0,
...
#ifdef ASSERT_MALBUF
		dbg_level = 0x1;
		writel(0xdeadbeef, wlpptr->ioBase1 + PCI_REG_SCRATCH14_REG);
	     ...
	     wltxdesc_t * txcfg, int qid, int type)
{
	...
	if (type == IEEE_TYPE_DATA) {
		...
		if (txcfg->mpdu_flag) {
			U16 ethertype = get_unaligned_le16(datap + 12);
			put_unaligned_le16(ethertype,
					   (U8 *) (&(txcfg->mpdu_ht_a_ctrl)) +
					   6);
			skb_push(skb, 8);
			memcpy(skb->data, &(txcfg->mpdu_ht_a_ctrl), 8);
			txcfg->hdr.length += 8;
		}
	}
	wlqm = &wlpptr->wlpd_p->descData[qid];
		...
		*((struct sk_buff_head **)skb->cb) =
...

		{
			...
			if (txcfg->qid < QUEUE_STAOFFSET) {
				...
				max_tx_pend_cnt = max_tx_pend_cnt_per_q;
		...
		if (type == IEEE_TYPE_DATA)
			wlpptr->wlpd_p->drv_stats_val.
				txq_ac_stats[AccCategoryQ
					     [txcfg->qid % MAX_TID]].drop_cfh++;
...
error_to_send:
	...
	if (type == IEEE_TYPE_DATA)
		wlpptr->wlpd_p->drv_stats_val.
			txq_ac_stats[AccCategoryQ[txcfg->qid % MAX_TID]].
			drop_cfh++;
...
InitCFHDL(struct net_device *netdev, struct bqm_args *pbqm_args,
	  wltxdesc_t * cfg, struct sk_buff *skb, void *StaInfo, BOOLEAN eap,
	  UINT8 nullpkt)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	extStaDb_StaInfo_t *pStaInfo = (extStaDb_StaInfo_t *) StaInfo;
	...
	if (mmdu_data_enable == 0) {
		cfg->mpdu_flag = 0;
	} else {
		// mmdu feature enabled:
		// Set the mpdu_flag == 1 if it's eapol packet
		if (eap == TRUE || nullpkt) {
			cfg->mpdu_flag = 1;
			if (*(vmacSta_p->Mib802dot11->QoSOptImpl)) {
				if (!pStaInfo) {
					/* if sta info is not available but AP is qos enabled */
					is_qos = 1;
				} else if (pStaInfo->IsStaQSTA) {
					/* if sta info is available and sta is qos STA */
					is_qos = 1;
				}
			}
			if (txqid_msg == 1) {
				printk("===> %s(), eap packets => set mpdu_flag = 1, txq=%d\n", __func__, cfg->qid);
			}
		} else {
			cfg->mpdu_flag = 0;
		}
	...
	if (cfg->mpdu_flag == 0) {
		// mmdu_data_enable disabled or this is not an eapol packet => no change
		cfg->snap1 = *(u_int32_t *) & skb->cb[16 + 4];
	} else {
		// "cfg->mpdu_flag!=0" occurs only if mmdu_data_enable == TRUE => give the type
		// Fill in the fc->type so that sfw can identify the packet type
		IEEEtypes_FrameCtl_t fc;
		memset(&fc, 0, sizeof(IEEEtypes_FrameCtl_t));
		fc.ProtocolVersion = 0;
		fc.Type = IEEE_TYPE_DATA;

		if (nullpkt) {
			/* for null data */
			if (is_qos)
				fc.Subtype = QoS_NULL_DATA;
			else
				fc.Subtype = NULL_DATA;
		} else {
			/* for other data */
			if (is_qos)
				fc.Subtype = QoS_DATA;
...
wlQMInit(struct net_device *netdev)
{
	...
	writel(wlpptr->smacconfig.ddrHighAddr, wlpptr->ioBase1 + 0x1a848);	//hack, smac_control_base_nss
	...
	writel(0, wlpptr->ioBase1 + reg_evt_rdptr);
	writel(0, wlpptr->ioBase1 + reg_evt_wrptr);
...
wlGetCfhUl(struct net_device * netdev, int qid, wlrxdesc_t * cfh_ul)
{
	...
	if (cfh_ul->hdrFormat == 1) {
		...
		if ((cfh_ul->mic_err == 0) && (cfh_ul->icv_err == 0)) {
			...
			// Free the payload of QoS Null packet
			{
	...
	if (pmrvl->FrmCtl.Type == IEEE_TYPE_DATA) {
		...
		if ((pmrvl->FrmCtl.Subtype == QoS_DATA) ||
			...
			if (pmrvl->FrmCtl.FromDs && pmrvl->FrmCtl.ToDs) {
				...
				psrc = (u8 *) pmrvl +
					(size_t) (&((mrvl_hdr_t *) 0)->FrmCtl);
				pdst = (u8 *) pHdr +
					(size_t) (&
						  ((IEEEtypes_Promiscuous_WDS_QoSHdr_t *) 0)->FrmCtl);
				memmove(pdst, psrc,
					sizeof
					(IEEEtypes_Promiscuous_WDS_QoSHdr_t) -
			...
			psrc = (u8 *) pmrvl +
				(size_t) (&((mrvl_hdr_t *) 0)->FrmCtl);
			pdst = (u8 *) pHdr +
				(size_t) (&((IEEEtypes_GenHdr_t *) 0)->FrmCtl);
			memmove(pdst, psrc,
		...
		psrc = (u8 *) pmrvl + (size_t) (&((mrvl_hdr_t *) 0)->FrmCtl);
		pdst = (u8 *) pHdr +
			(size_t) (&((IEEEtypes_CtlHdr_t *) 0)->FrmCtl);
		memmove(pdst, psrc,
...
mrvl_hdr_2_80211_hdr_mgmt(wlrxdesc_t * cfh_ul, struct sk_buff * skb_c)
{
	...
	psrc = (u8 *) pmrvl + (size_t) (&((mrvl_hdr_t *) 0)->FrmCtl);
	pdst = (u8 *) pHdr + (size_t) (&((IEEEtypes_GenHdr_t *) 0)->FrmCtl);
	memmove(pdst, psrc,
		sizeof(IEEEtypes_GenHdr_t) -
		sizeof(((IEEEtypes_GenHdr_t *) 0)->FrmBodyLen));
...
wl_show_generic_info(struct net_device *netdev, char *sysfs_buff)
{
	...
	Sysfs_Printk(" \t\t%08x\t%08x\n",
		     readl(wlpptr->ioBase1 + reg_evt_rdptr),
		     readl(wlpptr->ioBase1 + reg_evt_wrptr));
...
wl_show_ba_reorder_stat(struct net_device *netdev, char *sysfs_buff)
{
	...
	Sysfs_Printk(" Pkt drop cnt: Duplicate pkt                   = %8d\n",
		     vmacSta_p->BA_RodrDupDropCnt);
	Sysfs_Printk(" Pkt drop cnt: Out of range seqno, wExt<SN     = %8d\n",
		     vmacSta_p->BA_RodrOoRDropCnt);
	Sysfs_Printk(" Pkt drop cnt: Retry, SN<wStart                = %8d\n",
		     vmacSta_p->BA_RodrRetryDropCnt);
	Sysfs_Printk(" Pkt drop cnt: Enqueue to buf error            = %8d\n",
		     vmacSta_p->BA_RodrAmsduEnQCnt);
	Sysfs_Printk(" Pkt drop cnt: Timeout                         = %8d\n",
		     vmacSta_p->BA_RodrTMODropCnt);
	Sysfs_Printk(" Pkt drop cnt: Missing pkt in flush any        = %8d\n",
		     vmacSta_p->BA_RodrFlushDropCnt);
	Sysfs_Printk(" WinEnd evt jump count, wEnd<SN<=wExt          = %8d\n",
		     vmacSta_p->BA_RodrWinEndJumpCnt);
		...
		for (i = 0; i < entries; i++) {
			if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p,
				...
				if (StaInfo_p->State == ASSOCIATED) {
					...
					Sysfs_Printk("\n");
					Sysfs_Printk
						(" Pkt drop cnt: Out of range seqno, SN>wExt:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     StaInfo_p->
							     rxBaStats[i].
							     BA_RodrOoRDropCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Pkt drop cnt: Retry:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     StaInfo_p->
							     rxBaStats[i].
					...
					Sysfs_Printk("\n");
					Sysfs_Printk
						(" Pkt drop cnt: Enqueue to buf error:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     StaInfo_p->
							     rxBaStats[i].
					...
					Sysfs_Printk("\n");
					Sysfs_Printk
						(" Pkt drop cnt: Missing pkt in flush any:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     StaInfo_p->
							     rxBaStats[i].
					...
					Sysfs_Printk("\n");
					Sysfs_Printk
						(" WinEnd jump count, wEnd<SN<=wExt:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     StaInfo_p->
							     rxBaStats[i].
							     BA_RodrWinEndJumpCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk
						(" The last drop reason:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     StaInfo_p->
							     rxBaStats[i].
							     BA_RodrLastDropReason);
					Sysfs_Printk("\n");
					Sysfs_Printk
						(" Window starting seqno:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     baRxInfo->ba[i].
							     winStartB);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Window ending seqno:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     (baRxInfo->ba[i].
							      winStartB +
...
void
wl_show_txqos_stat(struct net_device *netdev, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	char buf[4][4] = { "BK", "BE", "VI", "VO" };
	u8 i;

	Sysfs_Printk("\n======== [Tx Qos stats] ========\n");
	for (i = 0; i < 4; i++) {
		Sysfs_Printk
			("TxQ[%s]: Qlen %4d    Tx %8d    Drop: Qfull %8d    skb %8d    dfs %8d    if %8d    cfh %8d\n",
			 buf[i], wlpptr->wlpd_p->txQ[i].qlen,
			 wlpd_p->drv_stats_val.txq_ac_stats[i].tx,
			 wlpd_p->drv_stats_val.txq_ac_stats[i].drop_qfl,
			 wlpd_p->drv_stats_val.txq_ac_stats[i].drop_skb,
			 wlpd_p->drv_stats_val.txq_ac_stats[i].drop_dfs,
			 wlpd_p->drv_stats_val.txq_ac_stats[i].drop_iff,
			 wlpd_p->drv_stats_val.txq_ac_stats[i].drop_cfh);
	}
	return;
}
...
wl_show_stat(struct net_device *netdev, int option, int level, char *sysfs_buff)
{
	...
	} else if (option == (int)drvstatsopt_txqos) {
		wl_show_txqos_stat(netdev, sysfs_buff);
	} else {
		 ...
		 char *sysfs_buff)
{
	...
	if ((option >= drvstatsopt_geninfo) && (option < drvstatsopt_end))
		...
		else if (!strcmp(info_item, "txqos"))
			option = drvstatsopt_txqos;
		else
			return -EINVAL;
...
//              pskb = skb_dequeue(&txQTrace);
		...
		InitCFHDL(netdev, pbqm_args, &cfg, pskb, NULL, FALSE, 0);
