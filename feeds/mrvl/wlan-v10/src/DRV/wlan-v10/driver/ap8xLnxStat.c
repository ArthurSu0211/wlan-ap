		     ...
		     struct kobj_attribute *attr, char *buf)
{
	...
	if (!vmacSta_p->InfUpFlag) {
		...
		Sysfs_Printk("\n======== [Auto Channel Select] ========\n");
		Sysfs_Printk
			("channel \tBSS \tminrss \tmaxrss \tNF \tCh load \tUse 2nd-CH\n");
		for (i = 0;
...
void *
ap8xLnxStat_vap_init(struct kobject *dev_kobj)
{
	struct kobject *kobj;
	int err;

	kobj = kobject_create_and_add("vap_info", dev_kobj);
	if (!kobj) {
		pr_err("%s: cannot create vap_info kobject\n", __func__);
		return NULL;
	}

	err = sysfs_create_group(kobj, &VAP_info_group);
	if (err) {
		pr_err("%s: sysfs group failed for tp%d\n", __func__, err);
		return NULL;
	}

	kobj = kobject_create_and_add("stat", dev_kobj);
	if (!kobj) {
		pr_err("%s: cannot create stat kobject\n", __func__);
		return NULL;
	}

	err = sysfs_create_group(kobj, &ap8xLnxStat_vap_stat_group);
	if (err) {
...
int
ap8xLnxStat_vap_exit(struct kobject *dev_kobj)
{
	sysfs_remove_group(dev_kobj, &VAP_info_group);
	sysfs_remove_group(dev_kobj, &ap8xLnxStat_vap_stat_group);
	return 0;
}
...
void *
ap8xLnxStat_clients_init(struct kobject *dev_kobj, UINT8 mode)
{
	int err = 0;
	struct clients_kobject *clients_kobj = NULL;
	...
	if (mode == MODE_STA) {
		...
		kobj = kobject_create_and_add("connection_info", dev_kobj);
		if (!kobj) {
		...
		err = sysfs_create_group(kobj, &ap8xLnxStat_ap_info_group);
		if (err) {
...
static void
ap8xLnxStat_clients_deinit(struct kobject *dev_kobj)
{
	ap8xLnxStat_clients_kobj_list_free(&ap8xLnxStat_clients_vap_kobjs);
...
static int IsLnxStat_sysfs_init = FALSE;

int
ap8xLnxStat_sysfs_init(struct kobject *dev_kobj)
{
	int err;
	struct kobject *tp_kobj;

	tp_kobj = kobject_create_and_add("tp", dev_kobj);
	if (!tp_kobj) {
		pr_err("%s: cannot create qm kobject\n", __func__);
		return -ENOMEM;
	}

	err = sysfs_create_group(tp_kobj, &ap8xLnxStat_tp_group);
	if (err) {
		pr_err("%s: sysfs group failed for tp%d\n", __func__, err);
		return err;
	}

	tp_kobj = kobject_create_and_add("stat", dev_kobj);
	if (!tp_kobj) {
		pr_err("%s: cannot create qm kobject\n", __func__);
		return -ENOMEM;
	}

	err = sysfs_create_group(tp_kobj, &ap8xLnxStat_stat_group);
	if (err) {
		pr_err("%s: sysfs group failed for tp%d\n", __func__, err);
		return err;
	}
	...
	tp_kobj = kobject_create_and_add("hw", dev_kobj);
	if (!tp_kobj) {
		pr_err("%s: cannot create hw kobject\n", __func__);
		return -ENOMEM;
	}

	err = sysfs_create_group(tp_kobj, &ap8xLnxStat_hw_group);
	if (err) {
		pr_err("%s: sysfs group failed for hw%d\n", __func__, err);
		return err;
	}
#ifdef AUTOCHANNEL
	tp_kobj = kobject_create_and_add("acs", dev_kobj);
	if (!tp_kobj) {
		pr_err("%s: cannot create acs kobject\n", __func__);
		return -ENOMEM;
	}

	err = sysfs_create_group(tp_kobj, &ap8xLnxStat_acs_group);
	if (err) {
...
#ifdef MEMORY_USAGE_TRACE
	tp_kobj = kobject_create_and_add("mem", dev_kobj);
	if (!tp_kobj) {
		pr_err("%s: cannot create hw kobject\n", __func__);
		return -ENOMEM;
	}
	err = sysfs_create_group(tp_kobj, &ap8xLnxStat_mem_group);
	if (err) {
		pr_err("%s: sysfs group failed for mem%d\n", __func__, err);
		return err;
	}
#endif /* MEMORY_USAGE_TRACE */
	IsLnxStat_sysfs_init = TRUE;

	return err;
}

int
ap8xLnxStat_sysfs_exit(struct kobject *dev_kobj)
{
	if (IsLnxStat_sysfs_init == TRUE) {
		sysfs_remove_group(dev_kobj, &ap8xLnxStat_tp_group);
#ifdef SYSFS_STADB_INFO
		ap8xLnxStat_clients_deinit(dev_kobj);
#endif /* SYSFS_STADB_INFO */
	}
	return 0;
}
				 ...
				 UINT8 * macAddr, char *buf)
{
	...
	UINT32 currIdx;
	size_t acnt_size = (sizeof(acnt_tx_t) * ACNT_TX_RECORD_MAX);
	UINT32 bw = 0;
	...
	if ((!strcmp(mode, "ax") || !strcmp(mode, "ac")) &&
		...
		if ((acnt_tx_record_dump =
		     (acnt_tx_t *) wl_kmalloc(acnt_size, GFP_KERNEL)) != NULL) {
			UINT32 delta_time = 0;

			currIdx = (wlpptr->wlpd_p->acnt_tx_record_idx == 0) ?
				(ACNT_TX_RECORD_MAX -
				 1) : (wlpptr->wlpd_p->acnt_tx_record_idx - 1);
			memset(acnt_tx_record_dump, 0, acnt_size);
			memcpy(&acnt_tx_record_dump[0], &acnt_tx_record[0],
			       acnt_size);
						...
						if (acnt_cnt_in_5s == 0) {
							...
							tx_sgi_mode =
								(acnt_tx_record_dump
								 ...
								 0x000000c0) >>
								6;
							bw = (acnt_tx_record_dump[currIdx].rateInfo & 0x00000030) >> 4;
							tx_dcm = (acnt_tx_record_dump[currIdx].rateInfo & 0x00000008) >> 3;
							if (tx_dcm == 0 &&
							    tx_sgi_mode == 3) {
