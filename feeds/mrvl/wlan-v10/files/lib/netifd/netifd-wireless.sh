...
_wdev_common_device_config() {
	config_add_string channel hwmode htmode txantenna rxantenna rts optlevel beacon_int greenfield tx_stbc short_gi_20 rifs \
		preamble regioncode agingtime gprotect htprotect ratectl ratemode ratestbc ratedcm ratebandwidth rategi rateltf \
                ratelegacy raten rateac rateax ratenss ratepreamble rategf rateldpc ratebf rateantenna dhenable dmode \
		hdfsmode htpc hcsamode hcsacount hnoptout hcactout intolerant40 beamforming mumimo opmode secondchannel mainchannel \
		mainhtmode monchannel monhtmode antennamode extsubch pwrfraction MCS
}

_wdev_common_iface_config() {
	config_add_string mode ssid encryption 'key:wpakey' key1 key2 key3 key4 hidden dtim_period auth_server auth_port auth_secret \
		acct_server acct_port acct_secret nasid intrabss macclone wmm bkcwminap bkcwmaxap bkaifsnap bktxopblap bktxopglap \
		bkcwminsta bkcwmaxsta bkaifsnsta bktxopblsta bktxopglsta bkacm becwminap becwmaxap beaifsnap betxopblap betxopglap \
		becwminsta becwmaxsta beaifsnsta betxopblsta betxopglsta beacm vicwminap vicwmaxap viaifsnap vitxopblap vitxopglap \
		vicwminsta vicwmaxsta viaifsnsta vitxopblsta vitxopglsta viacm vocwminap vocwmaxap voaifsnap votxopblap votxopglap \
		vocwminsta vocwmaxsta voaifsnsta votxopblsta votxopglsta voacm wdsenable disableassoc wdsport wdsmacaddr wdsmode \
		amsdu ampdutx wpsenable wpsappin bssid fltmode maclist1 maclist2 maclist3 maclist4 maclist5 index bandsteer bandsteerintf backhaulsta extiface \
		wpa_group_rekey
		
	config_add_int ieee80211w maxassoc
