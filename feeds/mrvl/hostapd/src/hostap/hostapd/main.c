...
static int hostapd_driver_init(struct hostapd_iface *iface)
{
	...
	if (hapd->driver->set_bandsteer) {
		if (iface->conf->track_sta_max_num != 0 ||
			conf->no_probe_resp_if_seen_on ||
			conf->no_auth_if_seen_on) {
			hapd->driver->set_bandsteer(hapd->drv_priv, 1);
		} else {
			hapd->driver->set_bandsteer(hapd->drv_priv, 0);
		}
	}

	if (hapd->driver->set_rrm) {
        if (hapd->conf->radio_measurements[0] &
            WLAN_RRM_CAPS_NEIGHBOR_REPORT) {
            hapd->driver->set_rrm(hapd->drv_priv, 1);
        } else {
            hapd->driver->set_rrm(hapd->drv_priv, 0);
        }
    }
