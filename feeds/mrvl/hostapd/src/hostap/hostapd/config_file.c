...
{
	char *delim;
	struct wpabuf *payload;	u16 infoid;
	size_t len;

	struct anqp_element *elem;
	static char *ptmp;
	static char *pcacode=NULL, *pcatype=NULL, *pcalen=NULL, *pcaaddr=NULL;
	int cacode_len=0, catype_len=0, calen_len=0, caaddr_len=0;
	char *pcmdbuf = NULL;
	...
	// ++++++++++++++++++++++++++++++++++++++++++
	if (infoid == 265) {
		struct wpabuf* new_buf;
		char *pcmdbuf = NULL;
		int pcmd_len=0;
		int subid = -1;
		char *delim_i = os_strchr(delim, '-');

		#define ANQP_LCI_ID		0
		#define ANQP_Z_ID		4
		#define ANQP_RULE_ID	6
		#define ANQP_MAC_ID		7

		wpa_printf(MSG_DEBUG, "=> id = 265\n");
		if (delim_i != NULL) {
			*delim_i = '\x00';
			delim_i++;

			if (!os_strcmp(delim, "LCI")) {
				subid = ANQP_LCI_ID;
			} else if (!os_strcmp(delim, "Z")) {
				subid = ANQP_Z_ID;
			} else if (!os_strcmp(delim, "RuleUsage")) {
				subid = ANQP_RULE_ID;
			} else if (!os_strcmp(delim, "CoLocMac")) {
				subid = ANQP_MAC_ID;
			}
			if (subid < 0) {
				wpa_printf(MSG_ERROR, "=> Incorrect conf tag: %s\n", delim);
				return -1;
			}
			if (os_strlen(delim_i) & 1) {
				wpa_printf(MSG_ERROR, "=> Incorrect conf tag: %s, %s in length\n", delim, delim_i);
				return -1;
			}
			...
			switch (subid) {
				...
				case ANQP_Z_ID:
				case ANQP_RULE_ID:
					pcmdbuf = os_zalloc(os_strlen(delim_i) + 4 + 1);
					pcmd_len = os_strlen(delim_i);
					memset(pcmdbuf, 0, sizeof(pcmdbuf));
					sprintf(pcmdbuf, "%02x%02x%s", 
						//lci_len/2, plci);
						subid, pcmd_len/2, delim_i);
					break;
				case ANQP_MAC_ID:
					pcmdbuf = os_zalloc(os_strlen(delim_i) + 6 + 1);
					pcmd_len = os_strlen(delim_i) + 2;
					memset(pcmdbuf, 0, sizeof(pcmdbuf));
					sprintf(pcmdbuf, "%02x%02x00%s", 
						//mac_len/2, pmac);
						subid, pcmd_len/2, delim_i);
					break;
			}
			wpa_printf(MSG_DEBUG, "=> id=%xh, %xh, %s\n", subid, pcmd_len, pcmdbuf);
			//wpa_printf(MSG_ERROR, "=> %s, %s, %s\n", delim, delim_i+1, pcmdbuf);
			delim = pcmdbuf;
			// ======================================================
			// Find & append the chunk
			len = os_strlen(delim);
			if (len & 1)
				return -1;
			...
			dl_list_for_each(elem, &bss->anqp_elem, struct anqp_element, list) {
				if (elem->infoid == infoid) {
					/* Update existing entry */
					// => Append it
					//wpa_printf(MSG_ERROR, "=> Concate buffer (orig, new)=(%d + %d) \n", orig_len, len);
					new_buf = wpabuf_concat(elem->payload, payload);
					//wpabuf_free(payload);	//payload will be free in wpabuf_concat
					elem->payload = new_buf;
	...
	if (infoid == 266) {
		wpa_printf(MSG_DEBUG, "=> id = 266\n");
		// Truncate the "LCI-", "ZInfo-", "ZHeight-"...
		char *delim_i = os_strchr(delim, '-');

		if (delim_i != NULL) {
			*delim_i = '\x00';

			if (!os_strcmp(delim, "CACode")) {
				ptmp = pcacode = os_zalloc(os_strlen(delim_i+1)+1);
			} else if (!os_strcmp(delim, "CAType")) {
				ptmp = pcatype = os_zalloc(os_strlen(delim_i+1)+1);
			} else if (!os_strcmp(delim, "CALength")) {
				ptmp = pcalen = os_zalloc(os_strlen(delim_i+1)+1);				
			} else if (!os_strcmp(delim, "CAAddr")) {
				ptmp = pcaaddr = os_zalloc(os_strlen(delim_i+1)+1);
			} else {
				wpa_printf(MSG_ERROR, "=> Incorrect conf tag: %s\n", delim);
				return -1;
			}
			os_strlcpy(ptmp, delim_i+1, os_strlen(delim_i+1)+1);
			
			wpa_printf(MSG_DEBUG, "=> %s, %s\n", delim, ptmp);
			if ((pcacode == NULL) || (pcatype == NULL) || (pcalen==NULL) || (pcaaddr == NULL)) {
				return 0;
			}
			cacode_len = os_strlen(pcacode);
			catype_len = os_strlen(pcatype);
			calen_len = os_strlen(pcalen);
			caaddr_len = os_strlen(pcaaddr);
			if ((cacode_len & 1) || (catype_len&1) || (calen_len&1) || (caaddr_len&1)) {
				return -1;
		}
			pcmdbuf = os_zalloc(3+cacode_len + catype_len + calen_len + caaddr_len + 1);
			
			memset(pcmdbuf, 0, sizeof(pcmdbuf));
			sprintf(pcmdbuf, "00%04x%s%s%s%s", 
				(cacode_len+catype_len+calen_len+caaddr_len)/2,
				pcacode,
				pcatype,
				pcalen,
				pcaaddr);
			
			wpa_printf(MSG_DEBUG, "=>[final] %s\n", pcmdbuf);
			os_free(pcacode);
			os_free(pcatype);
			os_free(pcalen);
			os_free(pcaaddr);
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
		...
		char *map_buf;
		char *map_pos, *map_r, *sptr;

		map_buf = os_strdup(pos);
		map_pos = map_buf;
		for (map_r = strtok_r(map_pos, " ", &sptr); map_r ; map_r = strtok_r(NULL, " ", &sptr)) {
			if (map_r && os_strcmp(map_r, "TearDown") == 0) {
				bss->multi_ap |= MAP_ATTRIBUTE_TEARDOWN;
			}
			if (map_r && os_strcmp(map_r, "frontBSS") == 0) {
				bss->multi_ap |= MAP_ATTRIBUTE_FRONTHAUL_BSS;
			}
			if (map_r && os_strcmp(map_r, "backBSS") == 0) {
				bss->multi_ap |= MAP_ATTRIBUTE_BACKHAUL_BSS;
			}
		}
		os_free(map_buf);
	} else if (os_strcmp(buf, "ext_iface") == 0) {
		os_strlcpy(bss->ext_iface, pos, sizeof(bss->ext_iface));
