...
static int hostapd_cli_cmd_track_sta_list(struct wpa_ctrl *ctrl, int argc,
					  char *argv[])
{
	char buf[4096];
	os_snprintf(buf, sizeof(buf), "TRACK_STA_LIST");
	return wpa_ctrl_command(ctrl, buf);
}
...
static int hostapd_cli_cmd_set_neighbor_perf(struct wpa_ctrl *ctrl, int argc,
					char *argv[])
{
    char cmd[64];
    int res;
    ...
    if (argc < 2) {
        printf("Invalid set_neighbor command: needs 2 arguments\n");
        return -1;
    }

    res = os_snprintf(cmd, sizeof(cmd), "SET_NR_PREF %s %s",
              argv[0], argv[1]);
    if (os_snprintf_error(sizeof(cmd), res)) {
        printf("Too long SET_NR_PREF command.\n");
        return -1;
    }
    return wpa_ctrl_command(ctrl, cmd);
}
...
static int hostapd_cli_cmd_beacon_request(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	char buf[2000], *tmp;
	int res, i, total;
	...
	if (argc < 1) {
		printf("Invalid 'bcn_request' command - at least one argument (STA addr) is needed\n");
		return -1;
	}

	res = os_snprintf(buf, sizeof(buf), "BCN_REQUEST %s", argv[0]);
	if (os_snprintf_error(sizeof(buf), res))
		return -1;
...
static const struct hostapd_cli_cmd hostapd_cli_commands[] = {
	...
	{ "track_sta_list", hostapd_cli_cmd_track_sta_list, NULL,
	  "= show interface track station list"},
...
#endif /* CONFIG_DPP */
    { "bcn_request", hostapd_cli_cmd_beacon_request, NULL, NULL },
    { "set_nr_pref", hostapd_cli_cmd_set_neighbor_perf, NULL, NULL },
	{ NULL, NULL, NULL, NULL }
};
