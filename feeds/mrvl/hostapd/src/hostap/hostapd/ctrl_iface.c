					 ...
					 const char *cmd)
{
	...
	if (pos) {
        ...
        bss_term_dur[0] = 4; /* Subelement ID, BSS Termination Duration */
		bss_term_dur[1] = 10; /* Length */
        os_memset(&bss_term_dur[2], 2, 8);
        bss_term_timer = atoi(pos);
        WPA_PUT_LE64(&bss_term_dur[2], bss_term_timer);
		end = os_strchr(pos, ',');
		if (end == NULL) {
        ...
        hapd->conf->bss_term_timer = bss_term_timer;
        hapd->conf->bss_term_dur = atoi(end);
	} else {
	    hapd->conf->bss_term_timer = 0;
	    hapd->conf->bss_term_dur = 0;
	}
	...
	if (os_strstr(cmd, " mrvl_nei=1")) {
        nr_buf = wpabuf_alloc(3 + IEEE80211_MAX_MMPDU_SIZE);
        if (nr_buf) {
            ...
            dl_list_for_each(nr, &hapd->nr_db, struct hostapd_neighbor_entry,
                ...
                if (hapd->conf->ssid.ssid_len != nr->ssid.ssid_len ||
                    os_memcmp(hapd->conf->ssid.ssid, nr->ssid.ssid, hapd->conf->ssid.ssid_len) != 0)
                    continue;

                /* Ignore non-preferred channels */
                for (info = sta->non_pref_chan; info; info = info->next) {
                    int i_chan;

                    for (i_chan = 0; i_chan < info->num_channels; i_chan++) {
                        if (info->channels[i_chan] == nr->nr->buf[11]) {  // buf[11] is channel
                            non_pref_chan = 1;
                            break;
                        }
                    }
                ...
                if (len > wpabuf_tailroom(nr_buf))
                    break;

                wpabuf_put_u8(nr_buf,WLAN_EID_NEIGHBOR_REPORT);
                wpabuf_put_u8(nr_buf,wpabuf_len(nr->nr) + 3);
                wpabuf_put_buf(nr_buf, nr->nr);
                wpabuf_put_u8(nr_buf,WNM_NEIGHBOR_BSS_TRANSITION_CANDIDATE);
                wpabuf_put_u8(nr_buf,1);
                if (os_memcmp(hapd->own_addr, wpabuf_head(nr->nr), ETH_ALEN) == 0) {
                    /* For MBO test plan 4.2.5.3, APUT's preference value (if present) is set to 0 */
                    wpabuf_put_u8(nr_buf,0);
                } else {
                    wpabuf_put_u8(nr_buf,nr->preference);
                }
            }
        }
    }
    ...
    if (nr_buf) {
        ret = wnm_send_bss_tm_req(hapd, sta, req_mode, disassoc_timer,
                        valid_int, bss_term_dur, bss_term_timer, url,
                        wpabuf_head(nr_buf), wpabuf_len(nr_buf),
...
set:
	ret = hostapd_neighbor_set_by_cli(hapd, bssid, &ssid, nr, lci, civic,
				   stationary, pref);
...
static int hostapd_ctrl_iface_beacon_request(struct hostapd_data *hapd,
					 const char *cmd)
{
	struct wpabuf *buf;
    struct sta_info *sta;
	...
	if (sta == NULL) {
		wpa_printf(MSG_DEBUG, "Station " MACSTR
			   " not found for Beacon Request message",
			   MAC2STR(addr));
		return -1;
	}

	buf = wpabuf_alloc(5 + 200);
	if (!buf) {
		return -ENOBUFS;
	}

	pos = os_strstr(cmd, " request_mode=");
	if (pos) {
		pos += 14;
        request_mode = atoi(pos);
    }

	pos = os_strstr(cmd, " op_class=");
	if (pos) {
		pos += 10;
        wpabuf_put_u8(buf, atoi(pos));
	} else {
		wpa_printf(MSG_DEBUG, "Not found op_class");
        ret = -1;
        goto err_ret;
    }

	pos = os_strstr(cmd, " channel_num=");
	if (pos) {
		pos += 13;
        channel_num = atoi(pos);
        wpabuf_put_u8(buf, channel_num);
	} else {
		wpa_printf(MSG_DEBUG, "Not found channel_num");
        ret = -1;
        goto err_ret;
    }

	pos = os_strstr(cmd, " random_interval=");
	if (pos) {
		pos += 17;
        wpabuf_put_le16(buf, atoi(pos));
	} else {
		wpa_printf(MSG_DEBUG, "Not found random_interval");
        ret = -1;
        goto err_ret;
    }

	pos = os_strstr(cmd, " measure_dur=");
	if (pos) {
		pos += 13;
        wpabuf_put_le16(buf, atoi(pos));
	} else {
		wpa_printf(MSG_DEBUG, "Not found measure_dur");
        ret = -1;
        goto err_ret;
    }

	pos = os_strstr(cmd, " measure_mode=");
	if (pos) {
		pos += 14;
        wpabuf_put_u8(buf, atoi(pos));
	} else {
		wpa_printf(MSG_DEBUG, "Not found measure_mode");
        ret = -1;
        goto err_ret;
    }

    /* Put BSSID */
	pos = os_strstr(cmd, " bssid=");
	if (pos) {
		pos += 7;
        hwaddr_aton(pos, bssid);
    }
	...
	pos = os_strstr(cmd, " report_info=");
	if (pos) {
        wpabuf_put_u8(buf, 1);  // Subelement ID for Beacon Reporting Information
        wpabuf_put_u8(buf, 2);
		pos += 13;
        wpabuf_put_le16(buf, atoi(pos));
	}
	...
	pos = os_strstr(cmd, " report_detail=");
	if (pos) {
        wpabuf_put_u8(buf, 2);  // Subelement ID for Reporting Detail
        wpabuf_put_u8(buf, 1);
		pos += 15;
        wpabuf_put_u8(buf, atoi(pos));
	}
	...
	pos = os_strstr(cmd, " channel_num=255:");
	if (pos) {
        char channel_list[64];
        struct wpabuf *cl_buf;
        const char *pos_1, *next_list;

		pos += 17;
        cl_buf = wpabuf_alloc(64);
        for(;;) {
            next_list = os_strstr(pos, ":");
            end = os_strstr(pos, " ");
            os_memset(channel_list, 0, sizeof(channel_list));
            if (next_list && (next_list < end)) {
                os_memcpy(channel_list, pos, (next_list - pos));
                pos = next_list + 1;
            } else {
                os_memcpy(channel_list, pos, (end - pos));
                pos = end + 1;
            }
            pos_1 = channel_list;
            os_memset(wpabuf_mhead(cl_buf), 0, wpabuf_len(cl_buf));
            cl_buf->used = 0;
            for(;;) {
                wpabuf_put_u8(cl_buf, atoi(pos_1));
                pos_1 = os_strstr(pos_1, ",");
                if (!pos_1) {
                    break;
                }
                pos_1 ++;
            }
            if (wpabuf_len(cl_buf)) {
                wpabuf_put_u8(buf, WLAN_EID_AP_CHANNEL_REPORT);
                wpabuf_put_u8(buf, wpabuf_len(cl_buf));
                wpabuf_put_buf(buf, cl_buf);
            }
            if (pos > end) {
                break;
            }
	...
	pos = os_strstr(cmd, " request_info=");
	if (pos) {
        struct wpabuf *cl_buf;
        ...
        if (wpabuf_len(cl_buf)) {
            wpabuf_put_u8(buf, WLAN_EID_REQUEST);
            wpabuf_put_u8(buf, wpabuf_len(cl_buf));
            wpabuf_put_buf(buf, cl_buf);
        }
        wpabuf_free(cl_buf);
	}
    ret = hostapd_send_bcn_req(hapd, addr, buf, request_mode);
//	ret = hostapd_send_beacon_req(hapd, addr, request_mode, buf);
					      ...
					      socklen_t fromlen)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (hostapd_dpp_pkex_remove(hapd, buf + 16) < 0)
			reply_len = -1;
#endif /* CONFIG_DPP */
    } else if (os_strncmp(buf, "BCN_REQUEST ", 12) == 0) {
        if (hostapd_ctrl_iface_beacon_request(hapd, buf + 12))
            reply_len = -1;
	} else {
					      ...
					      void *sock_ctx)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		eloop_terminate();
	} else if (os_strncmp(buf, "DOWN_VAP ", 9) == 0) {
		if (hostapd_global_ctrl_iface_updown_vap(interfaces, 0, buf + 9) < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "UP_VAP ", 7) == 0) {
		if (hostapd_global_ctrl_iface_updown_vap(interfaces, 1, buf + 7) < 0)
			reply_len = -1;
	} else {
