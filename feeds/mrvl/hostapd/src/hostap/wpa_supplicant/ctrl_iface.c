...
#define MAX_ANQP_INFO_ID 100
	...
	bss = wpa_bss_get_bssid(wpa_s, dst_addr);
	if (bss) {
		ext_capab = wpa_bss_get_ie(bss, WLAN_EID_EXT_CAPAB );
		if(ext_capab) {			
			ext_capab_len = ext_capab[1];
			wpa_hexdump(MSG_DEBUG,"bss ext capas IE", ext_capab, ext_capab_len + 2);
		}
	}
#endif
	pos = dst + used;
	...
	printf("%s(), bssid=" MACSTR "\n", __func__, MAC2STR(bssid));
	os_memcpy(wpa_s->gas_address3, bssid, ETH_ALEN);
	pos += used;
	if (*pos == ' ')
		pos++;
	}
...
#ifdef CONFIG_WLS_PF
			if((ext_capab == NULL) || (ext_capab_len < 4)) {
#endif
			id[num_id] = atoi(pos);
#ifdef CONFIG_WLS_PF
			} else {
				tempid = atoi(pos);
				if(tempid == 265) {
					if((ext_capab[5] & 0x80) && (ext_capab[3] & 0x80))
						id[num_id] = tempid;
					else
						wpa_printf(MSG_ERROR,
							   "This BSS doesn't support Geosptial location information");
				} else if(tempid == 266) {
					if((ext_capab[5] & 0x80) && (ext_capab[3] & 0x40))
						id[num_id] = tempid;
					else
						wpa_printf(MSG_ERROR,
							   "This BSS doesn't support Civic location information");						
				} else {
					id[num_id] = tempid;
				}				
			}
	...
	pos = os_strstr(dst + used, "timeout=");
	if (pos) {
        timeout = atoi(pos + 8);
	}
