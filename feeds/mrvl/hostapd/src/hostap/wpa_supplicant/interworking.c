				      ...
				      struct wpa_bss *bss)
{
	...
	res = gas_query_req(wpa_s->gas, bss->bssid, bss->freq, buf,
			    interworking_anqp_resp_cb, wpa_s, 0);
	if (res < 0) {
...
int anqp_send_req(struct wpa_supplicant *wpa_s, const u8 *dst,
		  ...
		  u32 mbo_subtypes, unsigned int timeout)
{
	struct wpabuf *buf;
	...
	res = gas_query_req(wpa_s->gas, dst, freq, buf, anqp_resp_cb, wpa_s, timeout);
	if (res < 0) {
		wpa_msg(wpa_s, MSG_DEBUG, "ANQP: Failed to send Query Request");
		wpabuf_free(buf);
		ret = -1;
		     ...
		     const struct wpabuf *query)
{
	...
	res = gas_query_req(wpa_s->gas, dst, freq, buf, gas_resp_cb, wpa_s, 0);
	if (res < 0) {
