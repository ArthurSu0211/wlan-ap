				...
				enum offchannel_send_action_result result)
{
	...
	if (result == OFFCHANNEL_SEND_ACTION_SUCCESS ||
		...
		if (result == OFFCHANNEL_SEND_ACTION_NO_ACK) {
			...
			eloop_register_timeout(query->timeout, 0,
					       gas_query_timeout, gas, query);
		}
...
static int gas_query_new_dialog_token(struct gas_query *gas, const u8 *dst)
{
	...
	for (dialog_token = 1; dialog_token < 256; dialog_token++) {
 ...
 * @timeout: timeout for send GAS Query and receive GAS response
		  ...
		  void *ctx, unsigned int timeout)
{
    ...
    if (timeout) {
        ...
        query->timeout= GAS_QUERY_TIMEOUT_PERIOD;
    }
