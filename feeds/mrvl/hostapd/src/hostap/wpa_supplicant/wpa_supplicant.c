...
static int hostapd_updown_vap(struct wpa_supplicant *wpa_s, int up)
{
	const char *cmd = NULL;
	char buf[256];
	size_t len = sizeof(buf);
	...
	if (asprintf(&cmd, "%s %s", up? "UP_VAP":"DOWN_VAP", wpa_s->ifname) < 0)
		return -1;
	ret = wpa_ctrl_request(wpa_s->hostapd, cmd, os_strlen(cmd), buf, &len, NULL);
	free(cmd);
	if (ret < 0) {
		wpa_printf(MSG_ERROR, "\nFailed to down hostapd VAP interfaces\n");
		return -1;
	}
	return 0;
}
			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		if (wpa_s->wps_success)
			system("touch /tmp/virtual_push_button");
...
//	if (bss)
//		params.multi_ap =
//				((bss->map_bss_mode & MAP_ATTRIBUTE_BACKHAUL_BSS) ||
//				 (bss->map_bss_mode & MAP_ATTRIBUTE_FRONTHAUL_BSS)) ? 1 : 0;
//#endif /* CONFIG_MULTI_AP */
...
//				((bss->map_bss_mode & MAP_ATTRIBUTE_BACKHAUL_BSS) ||
//				 (bss->map_bss_mode & MAP_ATTRIBUTE_FRONTHAUL_BSS)) ? 1 : 0;
	ssid->eap.multi_ap = wpa_drv_get_mapmode(wpa_s);
#endif /* CONFIG_MULTI_AP */
				     ...
				     struct wpa_interface *iface)
{
	...
	if (iface->hostapd_ctrl) {
		...
		if (hostapd_updown_vap(wpa_s, 0) < 0)
			return -1;
	}
