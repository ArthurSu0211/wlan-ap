...
struct wpa_driver_ops {
	...
	/**
	 * set_bandisteer - Notify driver of bandsteer setting
	 * @priv: Private driver interface data
	 * @enable: 0 on disable, 1 on enable
	 * Returns 0 on success, -1 on failure
	 */
	int (*set_bandsteer)(void *priv, int enable);

	/**
	 * send_mgmt - In bandsteering implement, some packets
	 * such as probe request and auth will be seen on hostapd
	 * but send back to driver.
	 * @priv: Private driver interface data
	 * @data: IEEE 802.11 management frame with IEEE 802.11 header
	 * @data_len: Size of the management frame
	 * @noack: Do not wait for this frame to be acked (disable retries)
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
    int (*send_mgmt)(void *priv, const u8 *data, size_t data_len,
			 int noack, unsigned int freq, const u16 *csa_offs,
			 size_t csa_offs_len);
	...
	/**
	 * set_rrm - Notify driver of hostapd/wpa_supplicant enable/disable rrm
	 * @priv: Private driver interface data
	 * @enable: 0 on disable, 1 on enable
	 * Returns 0 on success, -1 on failure
	 */
	int (*set_rrm)(void *priv, u8 enable);

	/**
	 * updown_vap - set vap interface down/up when sta state change
	 * 		- sta started but not connect, set vap interface down
	 * 		- sta connected, set vap interface up
	 * @priv: Private driver interface data
	 * @up: 0 on down, 1 on up
	 * @sta_ifname: sta interface name
	 * Returns 0 on success, -1 on failure
	 */
	int (*updown_vap)(void *priv, int up, const char *sta_ifname);

	/**
	 * get_mapmode - Notify driver of MAP mode
	 * @priv: Private driver interface data
	 * Returns the mode of Multi AP on success, 0 on failure
	 */
	u8 (*get_mapmode)(void *priv);
};
...
extern const struct wpa_driver_ops wpa_driver_marvell_ops; /* driver_marvell.c */
