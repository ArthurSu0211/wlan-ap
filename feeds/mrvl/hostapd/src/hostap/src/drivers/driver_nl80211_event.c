...
static void mwl_nl80211_new_sta_event(struct wpa_driver_nl80211_data *drv,
					u8 *data, size_t len)
{
	int ret;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR_IE];
	ret = nla_parse(tb, MAX_MWL_VENDOR_ATTR_IE, (struct nlattr *)data, len, NULL);
	if (ret)
		return;
	if (!tb[MWL_VENDOR_ATTR_IE_MAC] ||
		!tb[MWL_VENDOR_ATTR_IE_DATA] ||
		!tb[MWL_VENDOR_ATTR_IE_LEN] ||
		!tb[MWL_VENDOR_ATTR_IE_REASSOC])
		return;
	hostapd_logger(drv->ctx, nla_data(tb[MWL_VENDOR_ATTR_IE_MAC]), HOSTAPD_MODULE_IEEE80211,
		       HOSTAPD_LEVEL_INFO, "associated");
	drv_event_assoc(drv->ctx, nla_data(tb[MWL_VENDOR_ATTR_IE_MAC]),
			nla_data(tb[MWL_VENDOR_ATTR_IE_DATA]),
			nla_get_u16(tb[MWL_VENDOR_ATTR_IE_LEN]),
			nla_get_u8(tb[MWL_VENDOR_ATTR_IE_REASSOC]));
	return;
}
static void mwl_nl80211_del_sta_event(struct wpa_driver_nl80211_data *drv,
					u8 *data, size_t len)
{
	int ret;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
	ret = nla_parse(tb, MAX_MWL_VENDOR_ATTR, (struct nlattr *)data, len, NULL);
	if (ret)
		return;
	if (!tb[MWL_VENDOR_ATTR_MAC])
		return;
	hostapd_logger(drv->ctx, nla_data(tb[MWL_VENDOR_ATTR_MAC]), HOSTAPD_MODULE_IEEE80211,
		       HOSTAPD_LEVEL_INFO, "disassociated");
	drv_event_disassoc(drv->ctx, nla_data(tb[MWL_VENDOR_ATTR_MAC]));
	return;
}
static void mwl_nl80211_rx_mgmt_event(struct wpa_driver_nl80211_data *drv,
					u8 *data, size_t len)
{
	int ret;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
	...
	if (!tb[MWL_VENDOR_ATTR_MGMT])
		return;
	mgmt = (const struct ieee80211_mgmt *) nla_data(tb[MWL_VENDOR_ATTR_MGMT]);
	if (tb[MWL_VENDOR_ATTR_RSSI])
		rssi = nla_get_u8(tb[MWL_VENDOR_ATTR_RSSI]);
	fc = le_to_host16(mgmt->frame_control);
	...
	event.rx_mgmt.frame_len = nla_len(tb[MWL_VENDOR_ATTR_MGMT]);
	event.rx_mgmt.ssi_signal = rssi;
	wpa_supplicant_event(drv->ctx, EVENT_RX_MGMT, &event);
	return;
}
#ifdef CONFIG_WPS
static void mwl_nl80211_rx_wps_event(struct wpa_driver_nl80211_data *drv,
					u8 *data, size_t len)
{
	int ret;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
	...
	if (!tb[MWL_VENDOR_ATTR_MGMT])
		return;
	mgmt = (const struct ieee80211_mgmt *) nla_data(tb[MWL_VENDOR_ATTR_MGMT]);
	if (nla_len(tb[MWL_VENDOR_ATTR_MGMT]) < IEEE80211_HDRLEN + sizeof(mgmt->u.probe_req))
		return;
	fc = le_to_host16(mgmt->frame_control);
	if (WLAN_FC_GET_TYPE(fc) != WLAN_FC_TYPE_MGMT ||
			WLAN_FC_GET_STYPE(fc) != WLAN_FC_STYPE_PROBE_REQ)
	...
	event.rx_probe_req.ie_len =
		nla_len(tb[MWL_VENDOR_ATTR_MGMT]) - IEEE80211_HDRLEN;
...
#ifdef CONFIG_ACS
static void mwl_nl80211_rx_acs_completed_event(struct wpa_driver_nl80211_data *drv,
					u8 *data, size_t len)
{
	int ret;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
	...
	if (ret)
		return;
	if (!tb[MWL_VENDOR_ATTR_CHANNEL])
		return;

	channel = nla_get_u8(tb[MWL_VENDOR_ATTR_CHANNEL]);
	os_memset(&event, 0, sizeof(event));
	event.acs_selected_channels.pri_channel = channel;
	wpa_printf(MSG_INFO,
		   "nl80211: ACS Results: PCH: %d SCH: %d BW: %d VHT0: %d VHT1: %d HW_MODE: %d",
		   event.acs_selected_channels.pri_channel,
		   event.acs_selected_channels.sec_channel,
...
#ifdef CONFIG_AP
static void mwl_nl80211_neighbor_list_event(struct wpa_driver_nl80211_data *drv,
					u8 *data, size_t len)
{
	int ret;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
	const struct mwl_nr *nei_elem;
	int is_add = 0;
	union wpa_event_data event;
    ...
    wpa_printf(MSG_INFO,"hostapd_neighbor_set enter");
	...
	nei_elem = (const struct mwl_nr *) nla_data(tb[MWL_VENDOR_ATTR_MGMT]);
	if (tb[MWL_VENDOR_ATTR_RSSI])
		is_add = nla_get_u8(tb[MWL_VENDOR_ATTR_RSSI]);
	...
	if (is_add) {
		...
		wpa_printf(MSG_INFO,"own_addr %02x:%02x:%02x:%02x:%02x:%02x",
							hapd->own_addr[0],hapd->own_addr[1],hapd->own_addr[2],hapd->own_addr[3],hapd->own_addr[4],hapd->own_addr[5]);

		wpabuf_put_data(nr, nei_elem->bssid, ETH_ALEN);
		wpabuf_put_le32(nr, nei_elem->bssid_info);
		wpabuf_put_u8(nr, nei_elem->reg_class);
		wpabuf_put_u8(nr, nei_elem->chan);
		wpabuf_put_u8(nr, nei_elem->phy_type);
		...
		ret = hostapd_neighbor_set(hapd, nei_elem->bssid, &ssid, nr, hapd->iconf->lci,
				     hapd->iconf->civic, hapd->iconf->stationary_ap);

		wpa_printf(MSG_INFO,"hostapd_neighbor_set ret:%d", ret);

		wpabuf_free(nr);
	} else {
		ret = hostapd_neighbor_remove(hapd, nei_elem->bssid, &ssid);
		wpa_printf(MSG_INFO,"remove neighbor ret:%d", ret);
	}
	return;
}
#endif /* CONFIG_AP */
...
{
	switch (subcmd) {
	case MWL_VENDOR_EVENT_TEST:
		wpa_hexdump(MSG_DEBUG, "nl80211: MRVL test event", data, len);
		break;
#ifdef CONFIG_DRIVER_NL80211_MRVL
	case MWL_VENDOR_EVENT_ASSOC:
		mwl_nl80211_new_sta_event(drv, data, len);
		break;
	case MWL_VENDOR_EVENT_DISASSOC:
		mwl_nl80211_del_sta_event(drv, data, len);
		break;
	case MWL_VENDOR_EVENT_PROBE_REQ:
	case MWL_VENDOR_EVENT_AUTH:
		mwl_nl80211_rx_mgmt_event(drv, data, len);
#ifdef CONFIG_WPS
	case MWL_VENDOR_EVENT_WPS_REQ:
		mwl_nl80211_rx_wps_event(drv, data, len);
#endif
		break;
#ifdef CONFIG_ACS
	case MWL_VENDOR_EVENT_ACS_COMPLETED:
		mwl_nl80211_rx_acs_completed_event(drv, data, len);
		break;
#endif
#ifdef CONFIG_AP
	case MWL_VENDOR_EVENT_NEIGHBOR_LIST:
		mwl_nl80211_neighbor_list_event(drv, data, len);
		break;
#endif /* CONFIG_AP */
#endif /* CONFIG_DRIVER_NL80211_MRVL */
	default:
