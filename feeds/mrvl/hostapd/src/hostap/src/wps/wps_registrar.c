...
int wps_update_context(struct wps_data *wps,
			   struct wps_context *wps_ctx)
{
	os_memcpy(wps_ctx->ssid, wps->wps->wps_ext->ssid,
		  wps->wps->wps_ext->ssid_len);
	wps_ctx->ssid_len = wps->wps->wps_ext->ssid_len;

	wps_ctx->auth_types = wps->wps->wps_ext->auth_types;
	wps->auth_type &= wps->wps->wps_ext->auth_types;
	if (wps->auth_type == 0) {
		wpa_printf(MSG_DEBUG, "WPS: No match in supported "
			   "authentication types (own 0x%x Enrollee 0x%x)",
			   wps->wps->wps_ext->auth_types, wps->auth_type);
#ifdef WPS_WORKAROUNDS
		/*
	...
	wps_ctx->encr_types = wps->wps->wps_ext->encr_types;
	wps_ctx->encr_types_rsn = wps->wps->wps_ext->encr_types_rsn;
	wps_ctx->encr_types_wpa = wps->wps->wps_ext->encr_types_wpa;

	wps->encr_type &= wps_ctx->encr_types;
	if (wps->encr_type == 0) {
		wpa_printf(MSG_DEBUG, "WPS: No match in supported "
			   "encryption types (own 0x%x Enrollee 0x%x)",
			   wps->wps->wps_ext->encr_types, wps->encr_type);
#ifdef WPS_WORKAROUNDS
		/*
		 * Some deployed implementations seem to advertise incorrect
		 * information in this attribute. For example, Linksys WRT350N
		...
		wpa_printf(MSG_DEBUG, "WPS: Workaround - assume Enrollee "
			   ...
			   "correctly");
		wps->encr_type = wps->wps->wps_ext->encr_types;
#else /* WPS_WORKAROUNDS */
		return -1;
#endif /* WPS_WORKAROUNDS */
	}

	wps_ctx->psk_set = wps->wps->wps_ext->psk_set;
	if (wps_ctx->psk_set)
		os_memcpy(wps_ctx->psk, wps->wps->wps_ext->psk,
			  sizeof(wps->wps->wps_ext->psk));

	wps_ctx->network_key_len = wps->wps->wps_ext->network_key_len;
	if (wps_ctx->network_key_len)
		os_memcpy(wps_ctx->network_key, wps->wps->wps_ext->network_key,
			  wps_ctx->network_key_len);
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wps_ctx = os_zalloc(sizeof(*wps_ctx));
	if (wps_ctx == NULL)
		return -1;

	os_memcpy(wps_ctx, wps->wps, sizeof(*wps_ctx));

#ifdef CONFIG_MULTI_AP
	/* the credential data should be filled with bBSS's config */
	if ((wps->wps->dev.multi_ap != MAP_ATTRIBUTE_DISABLE) &&
	    !(wps->wps->dev.multi_ap & MAP_ATTRIBUTE_BACKHAUL_BSS) &&
	    (wps->wps->wps_ext != NULL)) {
		wps_update_context(wps, wps_ctx);
	}
#endif /* CONFIG_MULTI_AP */

	os_memset(&wps->cred, 0, sizeof(wps->cred));

	os_memcpy(wps->cred.ssid, wps_ctx->ssid, wps_ctx->ssid_len);
	wps->cred.ssid_len = wps_ctx->ssid_len;

	/* Select the best authentication and encryption type */
	wpa_printf(MSG_DEBUG,
		   "WPS: Own auth types 0x%x - masked Enrollee auth types 0x%x",
		   wps_ctx->auth_types, wps->auth_type);
	...
	wpa_printf(MSG_DEBUG,
		   ...
		   wps_ctx->encr_types, wps_ctx->encr_types_rsn,
		   wps_ctx->encr_types_wpa, wps->encr_type);
	if (wps_ctx->ap && wps->auth_type == WPS_AUTH_WPA2PSK)
		wps->encr_type &= wps_ctx->encr_types_rsn;
	else if (wps_ctx->ap && wps->auth_type == WPS_AUTH_WPAPSK)
		wps->encr_type &= wps_ctx->encr_types_wpa;
	if (wps->auth_type == WPS_AUTH_WPA2PSK ||
	    wps->auth_type == WPS_AUTH_WPAPSK) {
	...
	if (wps_ctx->wps_state == WPS_STATE_NOT_CONFIGURED && wps_ctx->ap &&
	    !wps_ctx->registrar->disable_auto_conf) {
		u8 r[16];
	...
	} else if (!wps_ctx->registrar->force_per_enrollee_psk &&
		   wps->use_psk_key && wps_ctx->psk_set) {
		char hex[65];
		wpa_printf(MSG_DEBUG, "WPS: Use PSK format for Network Key");
		wpa_snprintf_hex(hex, sizeof(hex), wps_ctx->psk, 32);
		os_memcpy(wps->cred.key, hex, 32 * 2);
		wps->cred.key_len = 32 * 2;
	} else if (!wps_ctx->registrar->force_per_enrollee_psk &&
		   wps_ctx->network_key) {
		os_memcpy(wps->cred.key, wps_ctx->network_key,
			  wps_ctx->network_key_len);
		wps->cred.key_len = wps_ctx->network_key_len;
