...
#ifdef CONFIG_MULTI_AP
/**
 * struct wps_context_ext - Long term WPS external context data (ex. bBSS's)
 * used to build credential data for M8
 */
...
#ifdef CONFIG_MULTI_AP
	/**
	 * wps_ext - WPS extend context (ex. bBSS context) used
	 * to build credential override for M8 (only used at AP)
	 *
	 * If %NULL, credential in M8 will be generated based on the
	 * current network configuration.
	 */
	struct wps_context_ext *wps_ext;
#endif /* CONFIG_MULTI_AP */
};
