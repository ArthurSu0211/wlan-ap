...
static void handle_auth(struct hostapd_data *hapd,
			...
			struct hostapd_frame_info *fi, int from_queue);
			...
			int from_queue)
{
	...
	if ((auth_alg != WLAN_AUTH_SAE) && (auth_alg != WLAN_AUTH_FT)) {
		goto mrvl_bandsteer;
	}
#endif
...
mrvl_bandsteer:
...
#ifdef MRVL_BANDSTEER
	if ((auth_alg != WLAN_AUTH_SAE) && (auth_alg != WLAN_AUTH_FT)) {
    	// forward authenticate to driver
    	if (hostapd_drv_send_mgmt(hapd, mgmt, len, 0) >= 0)
    		return;
	}
#endif
	...
	if (auth_alg == WLAN_AUTH_SAE && !from_queue &&
		 ...
		 * SAE exchange. */
		auth_sae_queue(hapd, mgmt, len, 40);
		return;
	}
#endif /* CONFIG_SAE */
...
#ifdef CONFIG_MRVL_OWE
		    && !(hapd->conf->wpa_key_mgmt & WPA_KEY_MGMT_OWE)
#endif /* CONFIG_MRVL_OWE */
#ifdef CONFIG_MBO
		    && !hapd->conf->mbo_enabled
#endif /* CONFIG_MBO */
		    ) {
			/*
...
#ifdef CONFIG_MRVL_OWE
		if ((hapd->conf->wpa_key_mgmt & WPA_KEY_MGMT_OWE) != 0)
			sta_remove = 0;
#endif /* CONFIG_MRVL_OWE */
#ifdef CONFIG_MBO
...
#ifdef CONFIG_MRVL_OWE
	if (hapd->conf->wpa_key_mgmt & WPA_KEY_MGMT_OWE) {
		/* OWE mode, set the station as authenticated */
		if(sta == NULL)
			sta = ap_sta_add(hapd, mgmt->sa);
		if(sta != NULL)
			sta->flags |= WLAN_STA_AUTH;
		    ...
		    struct hostapd_frame_info *fi)
{
	...
	case WLAN_FC_STYPE_AUTH:
		...
		handle_auth(hapd, mgmt, len, fi, 0);
		ret = 1;
		break;
