...
//	if (hapd->conf->multi_ap)
	{
		struct wpabuf *a = hostapd_build_multi_ap_ie(hapd->conf->multi_ap);
//		add_buf(&beacon, a);
//		add_buf(&proberesp, a);
		if (hostapd_set_generic_elem(hapd, a->buf, a->used)) {
			wpa_printf(MSG_ERROR, "Failed to configure MAP IE for "
				   "the kernel driver.");
		}
		wpabuf_free(a);
	}
...
int hostapd_drv_send_mgmt(struct hostapd_data *hapd,
			  const void *msg, size_t len, int noack)
{
	if (!hapd->driver || !hapd->driver->send_mgmt || !hapd->drv_priv)
		return 0;
	return hapd->driver->send_mgmt(hapd->drv_priv, msg, len, noack, 0,
				       NULL, 0);
}
...
#ifdef CONFIG_MULTI_AP
struct wpabuf * hostapd_build_multi_ap_ie(u8 map_attribute)
{
	struct wpabuf *ie;
	u8 *ie_len, *attr_len;
	...
	ie_len = wpabuf_put(ie, 1);
	wpabuf_put_be32(ie, MAP_IE_VENDOR_TYPE);

	wpabuf_put_u8(ie, WFA_ELEM_MULTI_AP_ATTRIBUTE);
	wpabuf_put_u8(ie, 1);
	wpabuf_put_u8(ie, map_attribute);

	WPA_PUT_BE16(attr_len, (u8 *) wpabuf_put(ie, 0) - attr_len - 2);
	*ie_len = wpabuf_len(ie) - 2;
