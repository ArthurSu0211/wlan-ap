		  ...
		  const char *ifname)
{
	...
	for (i = 0; i < interfaces->count; i++) {
		...
		*/
		...
		/* If the sta has been added in 5G, whether need to check the age time */
		if (hapd) {
			sta_track_expire(iface, 0);
			if (sta_track_get(iface, addr))
				return hapd;
		}
	}
		      ...
		      struct hostapd_frame_info *fi)
{
	...
	if (hapd->iconf->track_sta_max_num && ssi_signal <= 80)
		sta_track_add(hapd->iface, mgmt->sa, ssi_signal);
	ie_len = len - IEEE80211_HDRLEN;

#ifndef MRVL_BANDSTEER
	...
	// forward probe request to driver
