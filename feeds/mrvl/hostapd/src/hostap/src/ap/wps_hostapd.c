...
#ifdef CONFIG_MULTI_AP
struct wps_context_ext *
hostapd_init_wps_ext(struct hostapd_bss_config *conf)
{
	struct wps_context_ext *wps_ext = NULL;

	if (conf == NULL)
		return NULL;

	wps_ext = os_zalloc(sizeof(*wps_ext));
	if (wps_ext == NULL)
		return NULL;

	wps_ext->ssid_len = conf->ssid.ssid_len;
	os_memcpy(wps_ext->ssid, conf->ssid.ssid, wps_ext->ssid_len);

	if (conf->wpa & WPA_PROTO_RSN) {
		if (conf->wpa_key_mgmt & WPA_KEY_MGMT_PSK)
			wps_ext->auth_types |= WPS_AUTH_WPA2PSK;
		if (conf->wpa_key_mgmt & WPA_KEY_MGMT_IEEE8021X)
			wps_ext->auth_types |= WPS_AUTH_WPA2;
		if (conf->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP)) {
			wps_ext->encr_types |= WPS_ENCR_AES;
			wps_ext->encr_types_rsn |= WPS_ENCR_AES;
		} else if (conf->rsn_pairwise & WPA_CIPHER_TKIP) {
			wps_ext->encr_types |= WPS_ENCR_TKIP;
			wps_ext->encr_types_rsn |= WPS_ENCR_TKIP;
		}
	}

	if (conf->wpa & WPA_PROTO_WPA) {
		if (conf->wpa_key_mgmt & WPA_KEY_MGMT_PSK)
			wps_ext->auth_types |= WPS_AUTH_WPAPSK;
		if (conf->wpa_key_mgmt & WPA_KEY_MGMT_IEEE8021X)
			wps_ext->auth_types |= WPS_AUTH_WPA;
		if (conf->wpa_pairwise & WPA_CIPHER_CCMP) {
			wps_ext->encr_types |= WPS_ENCR_AES;
			wps_ext->encr_types_wpa |= WPS_ENCR_AES;
		}
		if (conf->wpa_pairwise & WPA_CIPHER_TKIP) {
			wps_ext->encr_types |= WPS_ENCR_TKIP;
			wps_ext->encr_types_wpa |= WPS_ENCR_TKIP;
		}
	}

	if (conf->ssid.security_policy == SECURITY_PLAINTEXT) {
		wps_ext->encr_types |= WPS_ENCR_NONE;
		wps_ext->auth_types |= WPS_AUTH_OPEN;
	}

	if (conf->ssid.wpa_psk_file) {
		/* Use per-device PSKs */
	} else if (conf->ssid.wpa_passphrase) {
		wps_ext->network_key = (u8 *) os_strdup(conf->ssid.wpa_passphrase);
		wps_ext->network_key_len = os_strlen(conf->ssid.wpa_passphrase);
	} else if (conf->ssid.wpa_psk) {
		wps_ext->network_key = os_malloc(2 * PMK_LEN + 1);
		if (wps_ext->network_key == NULL)
			goto fail;
		wpa_snprintf_hex((char *) wps_ext->network_key, 2 * PMK_LEN + 1,
				 conf->ssid.wpa_psk->psk, PMK_LEN);
		wps_ext->network_key_len = 2 * PMK_LEN;
	} else if (conf->ssid.wep.keys_set && conf->ssid.wep.key[0]) {
		wps_ext->network_key = os_malloc(conf->ssid.wep.len[0]);
		if (wps_ext->network_key == NULL)
			goto fail;
		os_memcpy(wps_ext->network_key, conf->ssid.wep.key[0],
			  conf->ssid.wep.len[0]);
		wps_ext->network_key_len = conf->ssid.wep.len[0];
	}

	if (conf->ssid.wpa_psk) {
		os_memcpy(wps_ext->psk, conf->ssid.wpa_psk->psk, PMK_LEN);
		wps_ext->psk_set = 1;
	}

	wps_ext->ap_auth_type = wps_ext->auth_types;
	wps_ext->ap_encr_type = wps_ext->encr_types;
...
#ifdef CONFIG_MULTI_AP
	wps->dev.multi_ap = hapd->conf->multi_ap ;
#endif /* CONFIG_MULTI_AP */

	hostapd_register_probereq_cb(hapd, hostapd_wps_probe_req_rx, hapd);

#ifdef CONFIG_MULTI_AP
	if ((hapd->conf->multi_ap != MAP_ATTRIBUTE_DISABLE) &&
	    !(hapd->conf->multi_ap & MAP_ATTRIBUTE_BACKHAUL_BSS) &&
	    os_strlen(hapd->conf->ext_iface)) {
		...
		for (i = 0; i < interfaces->count; i++) {
			...
			for (j = 0; j < iface->num_bss; j++) {
				struct hostapd_data *hapd_data = iface->bss[j];

				/* filer out self */
				if (hapd == hapd_data)
					continue;

				if ((os_strcmp(hapd->conf->ext_iface,
					hapd_data->conf->iface) == 0) &&
				    (hapd_data->conf->multi_ap &
					MAP_ATTRIBUTE_BACKHAUL_BSS)) {
					wps->wps_ext =
						hostapd_init_wps_ext(
							hapd_data->conf);
					if (wps->wps_ext)
						wps->wps_state =
