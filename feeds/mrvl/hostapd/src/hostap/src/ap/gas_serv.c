...
static void anqp_add_neighbor_report(struct hostapd_data *hapd, struct wpabuf *buf)
{
    ...
    if ((hapd->conf->radio_measurements[0] &
        ...
        dl_list_for_each(nr, &hapd->nr_db, struct hostapd_neighbor_entry,
            ...
            /* For MBO spec 4.3.3 , shall include the Element ID and the Length fields */
            wpabuf_put_u8(nr_buf, WLAN_EID_NEIGHBOR_REPORT);
            wpabuf_put_u8(nr_buf, len - 2);
            wpabuf_put_buf(nr_buf, nr->nr);
        }
	}
    wpabuf_put_le16(buf, ANQP_NEIGHBOR_REPORT);
    wpabuf_put_le16(buf, wpabuf_len(nr_buf));
    wpabuf_put_buf(buf, nr_buf);
    wpabuf_free(nr_buf);
}
