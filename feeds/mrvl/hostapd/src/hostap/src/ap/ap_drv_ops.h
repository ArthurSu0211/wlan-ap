...
static inline int hostapd_drv_updown_vap(struct hostapd_data *hapd,
				   int up, const char *ifname)
{
	if (!hapd->driver || !hapd->driver->updown_vap || !hapd->drv_priv)
		return 0;
	return hapd->driver->updown_vap(hapd->drv_priv, up, ifname);
}
#endif /* AP_DRV_OPS */
