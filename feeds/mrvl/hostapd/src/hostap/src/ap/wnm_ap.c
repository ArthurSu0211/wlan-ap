					       ...
					       size_t len)
{
	...
	*mbo_pos++ = 0;
    ...
    while((pos < end) && (*pos == WLAN_EID_NEIGHBOR_REPORT)) {
        struct hostapd_neighbor_entry *nr;
        u8 pref = 255;
        u8 nr_len = *(pos+1);

        nr = hostapd_neighbor_get(hapd, pos+2, NULL);
        pos = pos + 2 + nr_len;
        if (!nr) {
            continue;
        }
        if (wpabuf_len(nr->nr) > 0xff) {
            wpa_printf(MSG_DEBUG,
                   "NR entry for " MACSTR " exceeds 0xFF bytes",
                   MAC2STR(nr->bssid));
            continue;
        }
        if ((wpabuf_len(nr->nr) + 5) > wpabuf_tailroom(nr_buf))
            break;
        ...
        if (os_memcmp(hapd->own_addr, wpabuf_head(nr->nr), ETH_ALEN) == 0) {
            /* For MBO test plan 4.2.5.1, If configurable, value for own BSS is 255, 
               for AP1 is 254, for AP2 is 253 */
            wpabuf_put_u8(nr_buf, 255);
            own_bss_in_query = 1;
        } else {
    ...
    if (wpabuf_len(nr_buf) == 0) {
        ...
        /* Query has candidate list, but not include own BSS */
        if (own_bss_in_query == 0) {
            struct hostapd_neighbor_entry *nr;
  ...
  send_req:
	...
	ret = wnm_send_bss_tm_req(hapd, sta, req_mode, disassoc_timer,
                    valid_int, NULL, 0, NULL, wpabuf_head(nr_buf), wpabuf_len(nr_buf),
...
static void bss_updown_handle_timer(void *eloop_ctx, void *timeout_ctx)
{
	struct hostapd_data *hapd = eloop_ctx;
	struct sta_info *sta = timeout_ctx;

    /* ifconfig up vap */
    hostapd_global_ctrl_iface_updown_vap(hapd->iface->interfaces, 1, hapd->conf->iface);
}

static void bss_terminate_handle_timer(void *eloop_ctx, void *timeout_ctx)
{
	struct hostapd_data *hapd = eloop_ctx;
	struct sta_info *sta = timeout_ctx;
    ...
    /* Disassociate STA */
    ap_handle_timer(eloop_ctx, timeout_ctx);

    /* ifconfig down vap */
    hostapd_global_ctrl_iface_updown_vap(hapd->iface->interfaces, 0, hapd->conf->iface);
    /* Set a duration timer to up vap */
	eloop_cancel_timeout(bss_updown_handle_timer, hapd, sta);
	eloop_register_timeout(hapd->conf->bss_term_dur * 60,
			       0,
			       bss_updown_handle_timer, hapd, sta);
}

static void set_bss_terminate_timer(struct hostapd_data *hapd, struct sta_info *sta,
			       int bss_term_timer)
{
	/*
	 * Prevent STA from reconnecting using cached PMKSA to force
	...
	wpa_printf(MSG_DEBUG, "BSS Termination timer for " MACSTR
		   " set to %d ms", MAC2STR(sta->addr), bss_term_timer);

	sta->timeout_next = STA_DISASSOC_FROM_CLI;
	if (!bss_term_timer) {
        	/* Set 200ms to disassoc STA */
        	bss_term_timer = 200;
	} else {
    		/* For MBO test plan 4.2.5.4, disassociation frame should be
    		 * sent to the STA and stops sending Beacon frames
    		 * within BSS Termination TSF period */
    		if (bss_term_timer > 500)
    			bss_term_timer -= 500;
	}

	eloop_cancel_timeout(bss_terminate_handle_timer, hapd, sta);
	eloop_register_timeout(bss_term_timer / 1000,
			       bss_term_timer % 1000 * 1000,
			       bss_terminate_handle_timer, hapd, sta);
}

int wnm_send_bss_tm_req(struct hostapd_data *hapd, struct sta_info *sta,
			u8 req_mode, int disassoc_timer, u8 valid_int,
			const u8 *bss_term_dur, int bss_term_timer, const char *url,
			...
			const u8 *mbo_attrs, size_t mbo_len)
{
	...
	wpa_printf(MSG_DEBUG, "WNM: Send BSS Transition Management Request to "
		   MACSTR " req_mode=0x%x disassoc_timer=%d bss_term_timer=%d valid_int=0x%x",
		   MAC2STR(sta->addr), req_mode, disassoc_timer, bss_term_timer, valid_int);
	...
	if ((req_mode & WNM_BSS_TM_REQ_BSS_TERMINATION_INCLUDED) &&
	    bss_term_dur) {
		/* send disassociation and stop beacon frame after time-out */
		set_bss_terminate_timer(hapd, sta, bss_term_timer);
	}
