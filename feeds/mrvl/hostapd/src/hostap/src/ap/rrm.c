...
int hostapd_send_bcn_req(struct hostapd_data *hapd, const u8 *addr, struct wpabuf *body,
                                    u8 request_mode)
{
    struct wpabuf *buf;
    struct sta_info *sta = ap_get_sta(hapd, addr);
    ...
    /* Measurement request (5) + Measurement element with Beacon Request (200) */
    buf = wpabuf_alloc(300);
    if (!buf)
        return -1;
    ...
    wpabuf_put_u8(buf, WLAN_EID_MEASURE_REQUEST);
    wpabuf_put_u8(buf, 3 + wpabuf_len(body));
     ...
     * reserved.
     */
    wpabuf_put_u8(buf, request_mode);
    wpabuf_put_u8(buf, MEASURE_TYPE_BEACON);
    wpabuf_put_buf(buf, body);

    wpa_hexdump_buf(MSG_DEBUG, "Beacon Request", buf);
