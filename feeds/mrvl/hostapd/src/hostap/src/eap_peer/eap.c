...
/**
 * eap_get_config_multi_ap - Get MultiAP IE required or not
 *                           from the network configuration
 * @sm: Pointer to EAP state machine allocated with eap_peer_sm_init()
 * Returns: 1 if MultiAP IE is required, 0 if not
 */
u8 eap_get_config_multi_ap(struct eap_sm *sm)
{
	struct eap_peer_config *config = eap_get_config(sm);
	if (config == NULL)
		return NULL;
	return config->multi_ap;
}
