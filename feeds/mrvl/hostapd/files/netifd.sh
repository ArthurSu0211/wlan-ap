	...
	case "$auth_type" in
		...
		eap)
			json_get_vars \
				...
				auth_servers auth_secrets auth_ports \
			...
			[ -z "$auth_port" ] || append bss_conf "auth_server_port=$auth_port" "$N"
...
wpa_supplicant_add_network() {
	...
	[ -n "$bssid" ] && { 
		if [ "$bssid" != "FF:FF:FF:FF:FF:FF" -a "$bssid" != "ff:ff:ff:ff:ff:ff" ]; then
			append network_data "bssid=$bssid" "$N$T"
		fi
	}
...
start_hostapd_supplicant() {
	pid_supplicant=`pidof wpa_supplicant`
	if [ "$pid_supplicant" = "" ]; then
		if [ -f /var/run/wpa_supplicant-wdev0sta0.conf ]; then
			sta_ifname="wdev0sta0"
		else
			if [ -f /var/run/wpa_supplicant-wdev1sta0.conf ]; then
				sta_ifname="wdev1sta0"
			fi
		fi
		if [ -n "$sta_ifname" ]; then
			if [ -d /sys/class/net/${sta_ifname}/brport ]; then
				bridge=br-lan
			fi
			/usr/sbin/wpa_supplicant -B \
				${bridge:+-b $bridge} \
				-P "/var/run/wpa_supplicant-${sta_ifname}.pid" \
				-D nl80211 \
				-i "$sta_ifname" \
				-c "/var/run/wpa_supplicant-${sta_ifname}.conf" \
				-C "/var/run/wpa_supplicant"
			ret="$?"
			[ "$ret" != 0 ] && wireless_setup_vif_failed WPA_SUPPLICANT_FAILED
		fi
	fi
	pid_hostapd=`pidof hostapd`
	if [ "$pid_hostapd" = "" ]; then
		if [ "$(find /var/run/ -name hostapd-wdev*.conf)" != "" ]; then
			sleep 1
			local hostapd_ctrl="/var/run/hostapd/global"
			/usr/sbin/hostapd -P /var/run/hostapd.pid -g $hostapd_ctrl -B `ls /var/run/hostapd-wdev*.conf`
			ret="$?"
			[ "$ret" != 0 ] && {
