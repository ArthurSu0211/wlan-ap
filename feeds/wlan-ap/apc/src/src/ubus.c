...
void apc_update_state()
{
	...
	if ( apc_ifa->state == APC_IS_DR) {
		snprintf(state.mode, sizeof(state.mode), "DR");
		snprintf(state.dr_addr, sizeof(state.dr_addr),
			 "%s", inet_ntoa(dr_addr));
		snprintf(state.bdr_addr, sizeof(state.bdr_addr),
			 "%s", inet_ntoa(bdr_addr));
	}
	else if (apc_ifa->state == APC_IS_BACKUP) {
		snprintf(state.mode, sizeof(state.mode), "BDR");
		snprintf(state.dr_addr, sizeof(state.dr_addr),
			 "%s", inet_ntoa(dr_addr));
		snprintf(state.bdr_addr, sizeof(state.bdr_addr),
			 "%s", inet_ntoa(bdr_addr));
	}
	else if (apc_ifa->state == APC_IS_DROTHER) {
		snprintf(state.mode, sizeof(state.mode), "SR");
		snprintf(state.dr_addr, sizeof(state.dr_addr),
			 "%s", inet_ntoa(dr_addr));
		snprintf(state.bdr_addr, sizeof(state.bdr_addr),
			 "%s", inet_ntoa(bdr_addr));
